...
When(/^I set both local and network block heights to a static, equal number$/, async function () {
...
When(/^I set the node subscription status to (subscribing|subscribed)$/, async function (state) {
  const subscriptionState = state === 'subscribed' ? { status: 'subscribed' } : {};
...
When(/^I reconnect local and network block heights to the node$/, async function () {
  await this.client.executeAsync((done) => {
    daedalus.api.ada.setNetworkBlockHeight(null)
