...
- A checked 87-channel manifest accounts for all production privileged IPC, including dApp connection management, catalog status/state channels, the main-to-trusted-renderer wallet executor, and exact hardware transaction/message signing. Every privileged channel uses exact trusted main WebContents/frame/document authentication and correlated caller-targeted responses; the sole raw guest gateway is separately audited and exact-guest authenticated. Production guest rollout remains blocked by the empty release catalog, hardware activation, collateral, audit, and release gates.
- A production-bundled hostile-renderer harness now drives the real Electron policies against every incoming privileged IPC channel, wrong senders and subframes, lifecycle revocation, popup/download/permission/device/certificate attempts, destination binding, transport removal, DevTools denial, packaged launcher-policy variants, and fresh-session storage cleanup. It validates the actual guest renderer PID with the same `/proc` invariants as the runtime sandbox canary. Final Nix `.deb`, `.rpm`, and `.pkg.tar.zst` artifacts include the harness.
...
3. Guest is proven OS-sandboxed in every supported packaged Linux `.deb`, `.rpm`, and `.pkg.tar.zst` platform row.
