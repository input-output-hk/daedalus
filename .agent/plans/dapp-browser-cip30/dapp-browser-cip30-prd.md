...
- A checked 83-channel manifest accounts for all production privileged IPC, including the catalog status/state channels and main-to-trusted-renderer wallet executor. Every privileged channel uses exact trusted main WebContents/frame/document authentication and correlated caller-targeted responses; the sole raw guest gateway is separately audited and exact-guest authenticated. Production guest rollout remains blocked by the empty release catalog, hardware, batch, collateral, review, and release gates.
