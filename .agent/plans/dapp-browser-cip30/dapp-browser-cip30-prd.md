...
- Task ordering is contract-first and evidence-driven: historical `task-005` retains the cancelled portable spike; `task-005-a` froze the system-package, probe, matrix, and fail-closed contracts; `task-108` and `task-109` produced flag-free `.deb` and `.rpm` launchers; task-005-b completed installed-artifact certification on 2026-08-23; and task-103 is now dependency-unblocked for remaining development/legacy bypass removal, runtime enforcement, and the canary.
- **Task-005-b certification (2026-08-23):** the exact installed `.deb` passed on Ubuntu 24.04/26.04 and Debian 12/13; the exact installed `.rpm` passed on Fedora 43; Ubuntu 22.04 produced its expected wallet-only refusal. Every passing exact renderer had no-new-privileges `1`, seccomp mode `2` with an active filter, zero effective capabilities, separate PID/user namespaces, exact package identity, and required AppArmor/SELinux state. AppArmor, SELinux, helper-mode, and userns denials failed closed without an unsandboxed retry, and restoration returned to passing results. The normalized privacy-safe index is `scripts/linux-chromium-sandbox-probe/evidence/task-005-b/index.json`. Production guests remain disabled behind task-103 and all later gates.
