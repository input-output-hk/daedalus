...
- Task-406 completed on 2026-08-27: a revisioned immutable bundled catalog now separates renderer-safe local presentation from main-resolved authoritative network URLs and accepts only opaque catalog IDs over authenticated trusted IPC. Main repeats route/policy checks, tracks guest open/close state, and prunes removed or identity-changed catalog grants at startup. The exact wallet page uses one MobX store for route-selected Shelley-wallet/node readiness and renders only local icons and polished en-US/ja-JP names, descriptions, controls, and a prominent compatibility-tested-not-audited-or-endorsed disclaimer. The preferred switch independently hides/refuses catalog launch without enabling Diagnostics. Production revision 1 deliberately contains no external entry until task-806 supplies release-approved URL/resource/compatibility evidence. Focused Jest passed 41 tests; TypeScript, i18n, focused lint/style checks, Storybook and production bundles passed; browser verification covered English/Japanese ready, not-ready, and unavailable states without horizontal overflow.
...
- Task-504 completed on 2026-08-27: negotiated CIP-95 `api.signTx` now reuses the task-503 exact context/review/consent/password/witness path with `governance-transaction-signing` authority, DRep key-hash witness requirements, and canonical DeprecatedCertificate code 3. Conway certificate effects retain key/script/target identities, making DRep registration/deregistration/update and voter identity explicit in the existing trusted review. `api.cip95.signData` reuses task-502 after deriving the authenticated DRep credential from key state; raw DRep IDs and matching type-6 addresses produce identical independently verified COSE, while mismatched, unnegotiated, script, unsupported, declined, and stale cases fail canonically. Focused Jest passed 91 tests; TypeScript, i18n, focused non-decoder lint/format, Storybook build, and English/Japanese visual checks passed. The pre-existing transaction decoder lint baseline remains unchanged.
- Task-505 completed on 2026-08-27: `api.submitTx` now receives a separate exact full-envelope review through the global consent coordinator, marks the approved operation as submission-authorized, and forwards only the frozen original CBOR to the pinned wallet-scoped durable submission endpoint. Main accepts only fixed attempted/reconciled statuses and releases the locally derived transaction ID after exact returned-hash equality; decline, backend failure, malformed bytes, and mismatched results fail canonically. A single in-process per-wallet FIFO coordinator now serializes dApp submission with native software sends and the hardware select-sign-submit lease, while different wallets remain independent; reset/error paths release idempotently. Post-authorization route loss no longer skips backend invocation, and existing coordinator coverage proves stale delivery suppression without cancel/repeat. Focused Jest passed 56 tests; TypeScript, i18n, focused lint/format, and renderer production build passed.
...
- Task-601 completed on 2026-08-27: a vendor-neutral exact hardware model now consumes task-304 context, digest-bound required proof rows, original task-303 envelopes, trusted ownership paths, and task-306 VKey primitives without accepting coin-selection reconstruction or vendor payloads as authority. Normal/collateral and other proof kinds join exact owned credentials; references never request signers; owned outputs require exact encoded network/header/credential equality; expected required/pre-existing/requested/missing/unexpected witness sets remain explicit. Partial no-path returns canonical `a0` without device interaction, and every current locked Ledger 7/Ledger 8/Trezor task-006 row remains pre-device rejected. Returned hardware VKeys must exactly match the requested set and pass body-hash/key-hash/Ed25519 verification. Focused Jest passed 31 tests including the locked 14-row matrix and service regression; TypeScript and focused lint/format passed. No device row or certification was promoted.
- Task-602 completed on 2026-08-27: a common Ledger 8 adapter now maps task-601's authenticated exact model to the full frozen ordinary, Plutus, Conway, CIP-95, output, certificate, voting, collateral, and set-tag request inventory. Before device interaction it independently reconstructs canonical transaction-body CBOR from the generated Ledger request and requires byte equality with the immutable original, rejecting noncanonical variants, proposal procedures, mixed set tagging, unbound paths, and unsupported forms. The main-owned hardware service reuses task-600 cancellation, requires the exact device-returned body hash and witness-path set, derives each public key from the device, and verifies key hashes, Ed25519 signatures, completeness, duplicates, pre-existing keys, and extras before releasing canonical fresh VKeys. The locked task-006 source inventory and cases were revalidated; focused Jest passed 24 tests, TypeScript, focused lint/format, and the production main build passed. Physical/emulator certification and product enablement remain unset/disabled for task-607, while task-606 retains legacy UI/broker cutover.
- Task-603 completed on 2026-08-28: a main-owned exact Trezor Connect 9.7.2 adapter now maps task-006's frozen ordinary, Plutus, supported Conway certificate, output, collateral, and set-tag inventory from the task-601 model. External Shelley/Byron addresses, owned address parameters, pool owners, and IPv4/IPv6 relays use the exact Trezor wire forms. Before device interaction, Daedalus reconstructs canonical transaction-body CBOR entirely from the generated request and requires byte equality with the immutable original; governance voting, proposals, treasury, donation, unsupported DRep/committee certificates, mixed set tags, and unbound paths fail closed. The service requires the returned exact body hash, rejects bootstrap witnesses and unexpected auxiliary or chain-code proof material, binds returned Shelley public keys to the requested hashes, verifies every Ed25519 signature and witness-set completeness, and releases only a fresh VKey delta. The locked task-006 source inventory was revalidated; focused Jest passed 28 tests, TypeScript and focused ESLint/Prettier passed, and static/mock proof remains product-disabled pending tasks 606/607 and physical certification.
...
- Task-608 completed on 2026-08-30: base payment/stake and CIP-95 raw/matching-type-6 DRep data signing now route only through the task-604 exact Ledger/Trezor adapters over one authenticated privileged channel. Exact vendor, model, app or firmware, matrix revision, physical-certification, certified-extension, and packaged-activation evidence is checked before negotiation and immediately before device invocation; disabled, malformed, unsupported, cancelled, and stale cases fail closed with no software, passphrase, backend-signing, or vendor-COSE fallback. The task-607 certified-row registry and packaged activation remain empty. Focused Jest passed 94 tests; TypeScript, main/renderer production builds, and focused lint passed.
- Task-607 completed with explicitly scoped certification on 2026-08-31: existing task-608 public-routing and task-705 ordered-queue mocks remain the cross-vendor integration evidence, while a permanent production-Ledger-8 runner physically certified the attached Nano X with Cardano app 7.3.0. Reviewer-approved privacy-safe records cover payment, stake, direct and matching type-6 DRep messages, one exact transaction, and one ordered batch with a canonical empty-witness item; physical refusal at every batch position, cancellation without staged-result release, disconnect/reconnect recovery, returned body hashes, witness keys/signatures, and locally reconstructed COSE all passed. Product activation remains disabled. By explicit operator decision, remaining Ledger models, app-major-8, and representable Trezor rows are deferred to non-blocking tail task-906.
...
- Task-702 completed on 2026-08-30: the pure caller-ordered overlay now retains item-scoped conflict flags and ledger-outcome effects without rejecting signable conflicts or creating an aggregate. Normal and collateral inputs identify the first earlier claimant; references remain dependency-only. Valid items claim normal inputs and produce ordinary outputs, while invalid items claim collateral and produce only a collateral return at its ledger output index. Each item independently retains semantic, spent, referenced, and produced effects. Focused CIP-103 Jest passed 12 tests; TypeScript and focused lint passed.
- Task-703 completed on 2026-08-30: a strict clone-safe batch review contract now derives separate sign and submit consent from the authenticated ordered overlay, preserving per-item effects, current/pending dependencies, conflict flags, exact body/envelope identities, and maximum collateral without aggregate totals. The trusted dialog fails unsupported items before host/device confirmation, keeps one ordered accessible review while the guest is hidden, and provides one-password software, ordered hardware, attempt-all submission, and interruption-recovery guidance in polished en-US and ja-JP. Focused Jest passed 22 tests; TypeScript, i18n validation, Storybook build, and focused lint passed; browser verification covered all 18 supported theme/locale combinations and a narrow blocked-submit state.
- Task-704 completed on 2026-08-30: a dormant main-owned CIP-103 software batch signer now binds one approved ordered review, one authenticated backend context, and one transient password to exactly one witness-only backend call for task-708's later atomic extension cutover. It verifies every aligned body hash and VKey signature against the immutable reviewed envelope, enforces optional per-item CIP-95 required key hashes, stages only in call-local memory, and returns the frozen ordered witness array only after total success. Missing, extra, reordered, malformed, mismatched, late-failing, or review-mismatched results release nothing; ProofGeneration and DeprecatedCertificate remain distinct. Focused Jest passed 42 tests; TypeScript and focused lint passed.
- Task-705 completed on 2026-08-30: the dormant main-owned CIP-103 signer now includes a reviewed caller-ordered hardware queue for task-708's atomic cutover. It identity/context-checks every exact preparation before the first device call, stages canonical `a0` for a partial-sign item with no requested hardware key without confirmation, invokes ready Ledger/Trezor items strictly in order, independently verifies every returned VKey signature, and keeps all results in call-local memory until total success. Abort invokes device cancellation once, suppresses in-flight late completion, and stops later items; deterministic rejection at every batch position releases nothing. Existing hardware generation invalidation remains the device-level stale guard, while task-607 retains final physical certification. Focused and hardware regression Jest passed 54 tests; TypeScript and focused lint passed.
- Task-707 completed on 2026-08-30: a dormant main-owned CIP-103 attempt-all runner now binds one approved submit review to the exact caller-ordered CBOR/hash vector for task-708's atomic cutover. Every authorized item is attempted sequentially without guest-route input; earlier failures never stop later items, and successful responses must retain the reviewed transaction hash. All success returns frozen aligned hashes. Any failure rejects with the aligned hash/TxSendError array itself, never an Error or resolved settled wrapper; malformed, mismatched, rejected, expired, and thrown responses become fixed error entries. The existing preload already reconstructs that frozen rejection contract directly. Exact retries rely on cardano-wallet pending reconciliation, no Daedalus journal or signing persistence exists, and process exit naturally leaves later calls unattempted. Focused broker/consent/preload/schema/executor/submission Jest passed 61 tests; TypeScript and focused lint passed.
- Task-800 completed on 2026-08-30: pinned differential tests now prove the three supported Conway ledger encodings retain their source body hashes and normalize to identical semantic effects. Deterministic parser mutation coverage enforces exact body identity for every accepted byte mutation and rejects duplicate keys, invalid tags, excessive nesting, every truncation, and trailing bytes. Commitment mutations cover auxiliary data, redeemers, datums, Plutus scripts, reference scripts, language views, and existing witnesses; exhaustive generated VKey-witness and CIP-8 COSE_Sign1/COSE_Key byte mutations all fail verification. No parser bug was discovered, so no regression source fixture was added. Focused differential/fuzz and exact-CBOR/transaction/witness/CIP-8 Jest suites passed 98 tests; TypeScript compilation and explicit focused ESLint passed.
...
- Task-408 completed on 2026-08-27: a single authenticated trusted-renderer management channel now projects main-owned saved grants into localized connection settings without making the renderer authoritative. Close, disconnect, and forget have distinct documented behavior; disconnect, forget, disclosure revocation, repair, and wallet pruning cancel pending consent and revoke matching live capabilities before durable mutation while preserving already-authorized submission execution semantics. CIP-95 governance-key disclosure is independently visible/revocable, CIP-104 remains terminal-disabled with only defensive legacy-scope removal, and corruption exposes one fail-closed empty-store repair path. Successful authoritative wallet inventory refresh prunes absent IDs, while backend-confirmed wallet deletion triggers immediate cleanup. Focused service/IPC/store/component and 84-channel privileged-manifest tests, TypeScript and Storybook builds, i18n validation, and English/Japanese narrow-layout browser checks passed.
- Task-409 completed on 2026-08-27: a dedicated main-owned versioned collateral-preference repository now atomically persists only wallet/network/target/input identity with fail-closed repair, while a route-authenticated service derives ready/not-ready/preparing/in-use/will-be-spent/stale projections from fresh exact empty-transaction wallet context and adopts existing payment-key pure-ADA candidates deterministically. One authenticated trusted IPC channel feeds a projection-only `CollateralStore`; the wallet dApp page presents all eight specified states, explicit preparation through the normal confirmed Send route, metadata-only clearing, and polished en-US/ja-JP 5 ADA convention copy. `charged` is represented but final accepted-invalid/rollback reconciliation and exact spend-warning execution remain task-507 scope. Focused Jest passed 48 tests, TypeScript compilation, i18n validation, focused lint/format, and Storybook build passed; browser checks covered all states in English and Japanese without horizontal overflow.
- Task-410 completed on 2026-08-28: frozen contract and Dispatcher fixtures now cover every base and extension read, exact serialization, pagination, terminal-disabled CIP-104 namespace omission, and deprecated `getCollateral` null/error/no-side-effect/deprecation behavior. Existing catalog, Diagnostics, grant/revocation, collateral, and localized copy coverage was consolidated; Diagnostics Storybook now includes ready, not-ready, and unavailable states. New Cucumber scenarios prove active-wallet route replacement and persisted read-grant reuse after repository restart. Focused Jest passed 72 tests, Cucumber passed 2 scenarios/9 steps, TypeScript compilation, Storybook build, i18n validation, focused lint/format, and English/Japanese browser checks passed. Production signing and CIP-104 remain disabled.
- Task-507 completed on 2026-08-28: explicit 5 ADA collateral preparation now uses the existing confirmed software-wallet Send flow and records only the exact submitted transaction before main adopts its confirmed output. `CollateralService` remains the sole persisted authority and reconciles exact UTxO, pending, and transaction-history evidence into ready/preparing/in-use/will-be-spent/charged/stale outcomes while ordinary coin selection remains unchanged. Exact dApp transaction reviews and ordinary hardware selections warn without blocking a spend. Shared Conway validation fails closed on foreign collateral returns, missing returned tokens, excessive collateral inputs, and insufficient declared collateral charge. Focused Jest passed 73 tests and TypeScript compilation passed.
- Task-508 completed on 2026-08-28: three Cucumber user journeys now close software connect/read/sign/submit with separate approvals, negotiated CIP-95 raw/type-6 DRep signing, and confirmation-gated collateral preparation through the normal Send path. Broker conformance now accepts every durable nonterminal single-submit status and proves fixed public errors omit request and credential material. Existing frozen contracts, CIP-8, consent, collateral, component, Storybook, i18n, and task-208 backend write-ahead fault/property coverage were revalidated. Focused Jest passed 81 tests, Cucumber passed 3 scenarios/16 steps, the cardano-wallet durable submission store passed 4 examples plus 100 properties, TypeScript compilation, i18n validation, focused lint, and Storybook build passed; English transaction review and Japanese DRep/collateral views had no horizontal overflow.
- Task-708 completed on 2026-08-31: one atomic production cutover now advertises and negotiates CIP-103 only with executable `signTxs` and `submitTxs`. The authenticated broker binds one coherent task-304/701 context and task-703 review to all-or-nothing software/hardware witness verification or attempt-all durable submission, preserves direct normative mixed-array rejection, and rejects 51-item requests before wallet work. Normative preload fixtures, same-request children, conflicts, partial signing, every failure position, an actual process exit between submissions, retry reconciliation, software and mocked Ledger/Trezor Cucumber journeys, and localized/theme/focus/device-progress/recovery Storybook/Jest states close conformance without adding a store or journal. Focused Jest passed 80 tests, Cucumber passed 3 scenarios/11 steps, TypeScript compilation, i18n validation, focused lint/format, main/renderer/Storybook builds, and English/Japanese light/dark browser checks passed.
- Task-803 completed on 2026-08-31: the shared main/renderer observability boundary now omits sensitive structured fields by default and redacts arbitrary URLs, long hex payloads, and Cardano addresses from log and analytics text before emission. Crash callbacks retain only a validated error name/code, so corruption and recovery payloads cannot enter crash handling. Grant loading and atomic temporary writes enforce mode `0600`; no transaction store or non-authoritative cache was added. Automated spies cover logs, analytics, crash errors, corruption, file permissions, canonical-origin-only persistence, and retention removal. Focused Jest passed 27 tests; TypeScript compilation, focused lint/format, and main/renderer production builds passed.
...
- Task-902 completed on 2026-09-09 under the user's explicit bounded-evidence decision. The sole preferred entry, Liqwid mainnet, retains its exact revision-2 entry and three resource origins and passed the task-901 Ledger Flex deposit/withdrawal flows using base CIP-30 with no negotiated extensions. Existing certified Ledger and fail-closed hardware coverage communicates unsupported behavior; Trezor and unreproducible ecosystem clients remain explicitly unavailable. Existing automated collateral preparation, confirmation-gated adoption, spend-warning, and reconciliation evidence was accepted instead of another live partner collateral run. No catalog, origin, resource, protocol, package, hardware-row, source, or global-security-boundary delta was introduced. Focused Jest passed 54 tests and focused Cucumber passed 7 scenarios/30 steps. Task-903-a/task-906 retain final-package and broader physical gates.

## Problem Statement

The connector now has authenticated read, software signing, product-disabled exact hardware transaction and data signing, single and ordered CIP-103 batch submission, preferred-catalog, Diagnostics launch, collateral execution/recovery paths, completed internal/external security review, and a recorded task-807 release candidate, but Daedalus still lacks hardware product activation and a release-approved preferred entry.
...
- Support Arch Linux and representative Arch-based distributions such as Omarchy through a native, sandbox-certified system package.
...
- No Linux portable self-extracting `.bin`, AppImage, Flatpak, Snap, or other non-native portable package. Supported Linux release channels are `.deb`, `.rpm`, and the task-111 native pacman-compatible `.pkg.tar.zst`.
...
tasks 600-608 remain product-disabled; task-608 provides the public hardware
data-signing/capability-evidence bridge, while task-607 records the reviewed
production-Ledger-8 Nano X/app-7 physical scope. Tail task-906 owns the deferred
Ledger model/app-major-8 and Trezor matrix without blocking current development.
...
- A dedicated self-contained dApp preload synchronously exposes only `window.cardano.daedalus`, removes WebRTC/data-channel, WebTransport, and direct-socket APIs from the hostile page world, validates frozen gateway envelopes, and reconstructs typed public rejections without privileged globals. The main broker authenticates the exact guest top frame/origin/document/route and uses main-owned grants, live sessions, registry capability checks, correlated trusted consent, and one authenticated trusted-renderer wallet executor. Base reads, software signing/data signing/submission, CIP-95, CIP-103 ordered signing/submission, and gated CIP-142 are implemented; hardware transaction and data signing remain behind certified-row and packaged-activation gates.
- Each guest session installs a loopback-only fixed CONNECT proxy before its `BrowserWindow` exists. The proxy resolves each HTTPS/WSS tunnel independently, rejects every non-public IPv4/IPv6 result including IPv4-mapped forms, and dials only the validated numeric address with no direct fallback. URL origin allowlisting remains a separate required check, proxy setup failure rejects launch, and teardown closes tunnels before clearing the nonpersistent session. Electron 41 provides no pre-connect peer-address hook, so Chromium Private Network Access is defense in depth rather than the connection-binding proof. Preferred-catalog launch remains disabled; Diagnostics launch is enabled only in the Windows production package after its native sandbox gate passes.
- A checked 87-channel manifest accounts for all production privileged IPC, including dApp connection management, catalog status/state channels, the main-to-trusted-renderer wallet executor, and exact hardware transaction/message signing. Every privileged channel uses exact trusted main WebContents/frame/document authentication and correlated caller-targeted responses; the sole raw guest gateway is separately audited and exact-guest authenticated. Hardware activation and the empty preferred catalog remain separately gated.
- A production-bundled hostile-renderer harness now drives the real Electron policies against every incoming privileged IPC channel, wrong senders and subframes, lifecycle revocation, popup/download/permission/device/certificate attempts, destination binding, transport removal, DevTools denial, packaged launcher-policy variants, and fresh-session storage cleanup. It validates the actual guest renderer PID against Linux `/proc` invariants or Windows native OS-sandbox metrics. Final Nix `.deb`, `.rpm`, `.pkg.tar.zst`, and Windows NSIS artifacts include the harness.
...
- Main rejects sandbox-disabling argv/environment state and requires supported production-package identity plus a hidden, nonpersistent, exact-PID renderer canary before dApp launch becomes available. Linux launch paths no longer pass `--disable-setuid-sandbox` or `--no-sandbox`; Windows additionally requires the exact protected x64 NSIS layout and native OS-sandbox evidence.
- Linux releases now publish only native `.deb`, `.rpm`, and `.pkg.tar.zst` packages. Package-manager upgrades replace files under `/opt/daedalus/<cluster>` without owning `${XDG_DATA_HOME:-$HOME/.local/share}/Daedalus`; legacy `.bin` users receive an ordinary migration announcement rather than executable automatic-update bytes.
...
- The exact wallet-scoped dApp route now mounts a localized preferred-catalog page backed by `DappStore`; it uses only bundled local branding, exposes synced eligible-wallet and browser-open state, and hides/refuses preferred launch under its independent packaged switch. Production revision 1 has no external entry pending task-806 evidence. Diagnostics now stages a validated arbitrary URL only in main memory, navigates with the selected eligible wallet ID alone, and launches under the independent Diagnostics switch only after the matching main-owned route lease commits.
...
- A localized full-size transaction approval surface renders every task-303 effect in order, exact body and outer-envelope identities, commitment/witness status, and maximum collateral risk. Unknown, incomplete, or unresolved semantics are visibly non-approvable; tasks 503/505 remain responsible for constructing and executing sign/submit consent requests.
- The wallet dApp page includes a localized preferred-collateral panel backed by a projection-only `CollateralStore`. Main atomically owns the versioned preference record, derives current candidate and pending-use readiness from authenticated wallet context, reconciles accepted-invalid and rollback outcomes from exact transaction history, and clears metadata without a transaction. Explicit 5 ADA preparation uses the normal confirmed software Send route; exact dApp reviews and ordinary hardware selections warn before spending the soft preference without blocking submission.
...
- Existing Daedalus normal-payment and collateral-preparation hardware UI flows now retain exact constructed bytes and authenticated context through trusted review, exact Ledger/Trezor signing, witness merge, durable submission, and history reconciliation. Delegation and voting retain their separate legacy flows.
...
- Both installed libraries expose message signing. Exact Ledger/Trezor transaction and CIP-8 adapters plus the public base/CIP-95 hardware data-signing bridge exist. Task-607 physically certifies only the production-Ledger-8 Nano X/app-7 row; all other model/app/firmware rows remain fail-closed and are deferred to task-906. Every hardware connector row remains product-disabled until a later reviewed packaged release configuration activates an exactly certified row.
...
- [x] Add connection/key-disclosure revocation and wallet deletion cleanup.
...
- [x] Add ordered batch review with per-item conflict flags.
...
- **Accepted strategy:** ship Linux exclusively as native system packages: **`.deb`**, **`.rpm`**, and pacman-compatible **`.pkg.tar.zst`**. Evidence and the authoritative matrix: [research/06-linux-system-package-decision.md](./research/06-linux-system-package-decision.md). Portable feasibility negative evidence: [research/05-linux-chromium-sandbox-packaging.md](./research/05-linux-chromium-sandbox-packaging.md).
- **Rejected for Linux shipping:** portable self-extracting `.bin` to `$HOME/.daedalus/<cluster>`, AppImage, Flatpak, Snap, and other non-native portable channels.
- Install each cluster to `/opt/daedalus/<cluster>`, where `<cluster>` is the build-time installer cluster slug, so privileged package hooks can establish Chromium sandbox prerequisites and host policy can bind the exact Electron executable path:
  ...
  - task-111 certifies only Arch Linux installed from the 2026.09.01 ISO and Omarchy 4.0.2 installed from its 2026.08.31 image, using exact host/kernel identity and user namespaces with a mode-`0755` helper.
- Authoritative successor x86_64 matrix revision `task-111-matrix-2026-09-02` retains Ubuntu 24.04.x/26.04.x, Debian 12.x/13.x, and Fedora 43 from `task-108-matrix-2026-08-18`, and adds only `arch-2026.09.01` and `omarchy-4.0.2`. Ubuntu 22.04.x, later Arch/Omarchy snapshots, and every other omitted row are wallet-only.
- Ubuntu may use independently proven SUID or userns containment, Fedora 43 requires SELinux, and Debian requires no package policy asset by default. The exact Arch/Omarchy rows use only userns containment and no package policy asset. A listed-row setup failure refuses dApp authority rather than weakening containment; omitted rows may remain wallet-only without remote guest launch or unapproved host-policy changes.
- Tasks 108/109/111 own the root-owned identity manifest at `/opt/daedalus/<cluster>/share/daedalus-sandbox-identity.json`; the probe binds exact package hashes and task-reviewed policy identity to live exact-path files and independently observed renderer/host policy evidence.
- Tasks 108/109/111 produce flag-free `.deb`/`.rpm`/`.pkg.tar.zst` launchers. Task-103 also removed the remaining development and legacy `--no-sandbox` and `--disable-setuid-sandbox` defaults.
...
- Keep Daedalus wallet functionality available where practical, but hide and reject dApp guest launch when OS sandbox proof is unavailable. Ubuntu 22.04, later or mismatched Arch/Omarchy snapshots, omitted hosts, legacy `.bin`, and Nix-store production launches are wallet-only.
...
- **Task-110 Linux release migration (2026-08-23):** new Linux releases initially exposed only distinct `.deb` and `.rpm` artifacts; Hydra, Buildkite, and release manifests no longer expose a portable installer. Task-111 subsequently added only the native `.pkg.tar.zst` channel. Linux package URLs remain excluded from executable `softwareUpdate` payloads. Existing portable users migrate through the host package manager while preserving `${XDG_DATA_HOME:-$HOME/.local/share}/Daedalus`.
...
- Linux policy remains unchanged and continues to require its stronger platform-specific `/proc`, namespace, seccomp, capability, helper, and package-policy evidence.
...
The main-owned authoritative grant repository persists the canonical exact origin, wallet ID, network genesis, scopes, and catalog entry or Diagnostics launch kind needed to enforce and revoke a grant. A full Diagnostics URL is never persisted anywhere; only the canonical origin may persist in the grant repository. Grants remain until explicit disconnect/forget, wallet deletion or pruning, catalog identity removal/change, or fail-closed corruption repair; there is no separate time-based copy or cache.

Authoritative wallet history and pending-submission data follow the existing cardano-wallet retention rules. Main-owned collateral preference records remain until explicit clearing, wallet pruning, or reconciliation replaces their state. These authoritative stores may retain data Daedalus/cardano-wallet already persist today, use restrictive permissions, and never enter observability channels. Task-803 introduces no additional Daedalus-side transaction persistence.
...
- `nix/internal/x86_64-linux.nix`: sandbox-safe `.deb`/`.rpm`/`.pkg.tar.zst` launchers and package outputs.
- Native Linux packaging (independently proven SUID or userns route, mandatory Ubuntu AppArmor/Fedora 43 SELinux, exact Arch/Omarchy userns-only identity, desktop entries, update path).
- Linux release tooling: distinct `.deb`/`.rpm`/`.pkg.tar.zst` artifacts, package-manager upgrades, and non-destructive migration from retired portable `.bin` installations.
...
- Add base and DRep CIP-8 hardware adapters and route them publicly with exact certified-row capability evidence behind a packaged default-disabled activation value.
- Add cancellation and late-result suppression.
- Complete the task-705 mocked/emulator batch queue before task-607 certifies the scoped production-Ledger-8 Nano X/app-7 physical single/batch cases; defer the remaining model/app/firmware matrix to non-blocking tail task-906.
...
- Task-708 atomically enabled CIP-103 registry availability, provider `supportedExtensions`, advertisement, negotiation, namespace construction, `signTxs`, and `submitTxs` after implementation, certification, and conformance gates passed.
...
- Complete extension conformance, interoperability, and remaining physical hardware evidence before review; task-803 completed the privacy gate.
...
- Installed Linux `.deb`, `.rpm`, and `.pkg.tar.zst` packages prove OS sandboxing on the exact guest renderer.
...
3. Guest is proven OS-sandboxed in every supported packaged Linux `.deb`, `.rpm`, and `.pkg.tar.zst` row and in the protected Windows x64 NSIS installation.
...
Task-805 closed internal gate 12 and task-806 closed external gate 13 on 2026-08-31 after remediation and independent re-review found no open critical/high finding. Task-111 closed the native Arch/Omarchy package and adversarial-certification scope on 2026-09-02. This does not close physical hardware gate 11 or downstream gates 14-16; task-807 still owns current dependency/CVE review, exact release-candidate source/package/catalog identities, all remaining platform/package evidence, the retained V6 migration evidence gap, and the frozen Trezor runtime-tree identity.
...
- The sole current preferred dApp, Liqwid mainnet, retains its task-902-verified exact resource allowlist; every future catalog entry or resource change still requires task-903-a change control and fresh compatibility/security evidence.
