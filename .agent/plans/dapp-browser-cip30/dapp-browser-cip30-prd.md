...
- Planning revision 1.7.42 adds task-111 for native Arch Linux packaging and snapshot-specific Arch/Omarchy certification. Task-111 and task-807 now require exact installed Arch and representative Omarchy artifacts to pass the same per-platform packaged adversarial matrix before rollout; no support is inferred across rolling-release snapshots or other Arch derivatives.
...
- **Accepted strategy:** ship Linux exclusively as native system packages: **`.deb`**, **`.rpm`**, and, after task-111 certification, pacman-compatible **`.pkg.tar.zst`**. Evidence for the original `.deb`/`.rpm` decision: [research/06-linux-system-package-decision.md](./research/06-linux-system-package-decision.md). Portable feasibility negative evidence: [research/05-linux-chromium-sandbox-packaging.md](./research/05-linux-chromium-sandbox-packaging.md).
...
- Install each cluster to `/opt/daedalus/<cluster>`, where `<cluster>` is the build-time installer cluster slug, so privileged package hooks can establish Chromium sandbox prerequisites and host policy can bind the exact Electron executable path:
  ...
  - task-111 must freeze and certify the exact Arch Linux and representative Omarchy snapshots, package identity, kernel/userns or SUID-helper route, and any active host policy before enabling dApp launch.
- Authoritative successor x86_64 matrix revision `task-108-matrix-2026-08-18` supports Ubuntu 24.04.x and 26.04.x LTS with row-specific semantic AppArmor checks, Debian 12.x and 13.x without a package policy asset, and Fedora 43 under the task-109 SELinux contract. Task-111 must issue a reviewed successor matrix revision before any Arch Linux or Omarchy snapshot becomes supported. Ubuntu 22.04.x, uncertified Arch/Omarchy snapshots, and every other omitted row are wallet-only.
- A supported row may pass through either independently proven SUID or userns containment. Ubuntu requires AppArmor, Fedora 43 requires SELinux, and Debian requires no package policy asset by default; task-111 must record the exact active policy for each Arch/Omarchy snapshot. A listed-row setup failure refuses package configuration; omitted rows may install wallet-only without remote guest launch or unapproved host-policy changes.
...
- Keep Daedalus wallet functionality available where practical, but hide and reject dApp guest launch when OS sandbox proof is unavailable. Ubuntu 22.04, Arch/Omarchy snapshots pending task-111 certification, omitted hosts, legacy `.bin`, and Nix-store production launches are wallet-only.
...
- Task ordering is contract-first and evidence-driven: historical `task-005` retains the cancelled portable spike; `task-005-a` froze the original system-package, probe, matrix, and fail-closed contracts; `task-108` and `task-109` produced flag-free `.deb` and `.rpm` launchers; task-005-b completed their installed-artifact certification; task-103 completed runtime enforcement; and task-111 now owns the native Arch package plus exact Arch/Omarchy package, sandbox, lifecycle, and adversarial certification.
...
Task-805 closed internal gate 12 and task-806 closed external gate 13 on 2026-08-31 after remediation and independent re-review found no open critical/high finding. This does not close physical hardware gate 11 or downstream gates 14-16; task-111 owns native Arch/Omarchy package and adversarial certification, and task-807 still owns current dependency/CVE review, exact release-candidate source/package/catalog identities, all remaining platform/package evidence including task-111, the retained V6 migration evidence gap, and the frozen Trezor runtime-tree identity.
