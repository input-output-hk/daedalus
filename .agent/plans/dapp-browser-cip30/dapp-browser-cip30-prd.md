...
The connector now has authenticated read, signing, submission, preferred-catalog, Diagnostics launch, and collateral execution/recovery paths, but Daedalus still has no release-approved preferred entry, hardware signing, or CIP-103 execution.
...
- A checked 84-channel manifest accounts for all production privileged IPC, including dApp connection management, catalog status/state channels, and the main-to-trusted-renderer wallet executor. Every privileged channel uses exact trusted main WebContents/frame/document authentication and correlated caller-targeted responses; the sole raw guest gateway is separately audited and exact-guest authenticated. Production guest rollout remains blocked by the empty release catalog, hardware, batch, collateral, review, and release gates.
...
- Existing Daedalus hardware UI flows still reconstruct reduced transactions; task-602 adds a separate exact Ledger adapter over task-601 data, and task-606 owns the atomic legacy UI/broker cutover.
...
- Both installed libraries expose message signing. Ledger exact transaction adaptation is implemented but not product-connected pending tasks 606/607; Daedalus still has no hardware CIP-8 bridge.
