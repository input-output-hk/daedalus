...
The connector now has authenticated read, signing, submission, preferred-catalog, Diagnostics launch, and collateral preference/readiness UX paths, but Daedalus still has no release-approved preferred entry, hardware signing, CIP-103 execution, or final collateral execution/recovery integration.
...
- The wallet dApp page now includes a localized preferred-collateral panel backed by a projection-only `CollateralStore`. Main atomically owns the versioned preference record, derives current candidate and pending-use readiness from authenticated wallet context, and clears metadata without a transaction. Preparation opens the normal confirmed Send route and never signs or submits automatically; task-507 retains accepted-invalid/rollback reconciliation and exact transaction spend-warning execution.
