...
data-signing/capability-evidence bridge, while task-607 records final reviewed
...
- Both installed libraries expose message signing. Exact Ledger/Trezor transaction and CIP-8 adapters plus the public base/CIP-95 hardware data-signing bridge exist. Every hardware connector row remains product-disabled until task-607 certifies the final single/batch service commit and a later reviewed packaged release configuration activates that exact row.
