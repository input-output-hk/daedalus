...
- Shared chain-storage contracts now live in `mithril-bootstrap.types.ts`: `ChainStorageConfig` carries `customPath`, `defaultPath`, `availableSpaceBytes`, `requiredSpaceBytes`, and optional recovery metadata; `ChainStorageValidation` still carries `isValid`, `path`, and optional `resolvedPath/space metrics/reason/message` with canonical reasons (`not-writable`, `insufficient-space`, `inside-state-dir`, `path-not-found`, `unknown`).
...
- `ChainStorageManager.setDirectory()` is now a pure repoint for empty/default chain storage: it validates the requested target and recreates `{stateDir}/chain` as a symlink/junction without migrating existing chain contents into the new location.
...
- `setDirectory(null)` now routes to `resetToDefault()`: removes any chain symlink, recreates the default `{stateDir}/chain` directory, and does not migrate data back from previously selected custom locations.
- Runtime chain-storage state now comes only from the live `{stateDir}/chain` entry point; the legacy `chain-storage-config.json` cleanup path has been removed.
- `MithrilBootstrapService.cancel()` must no-op when no bootstrap is active so idle cancel requests cannot clean snapshot artifacts from the live managed chain directory.
...
- Post-download Mithril progress now clears `filesDownloaded/filesTotal/elapsedSeconds/remainingSeconds` when network transfer ends, then emits `unpacking` followed by `finalizing`; `_convertSnapshot()` also uses `converting` if that path is re-enabled.
- The visible Mithril flow is now fixed at `preparing -> downloading -> finalizing`, with verification kept as an error stage only and post-download unpacking/cleanup mapped into the visible finalizing step.
- Preferred follow-up is explicit backend instrumentation of `_installSnapshot()` and later cleanup/handoff so finalizing emits real local restore telemetry; do not synthesize verification percentages while status is still `downloading`.
- `handleDiskSpace` must check for an existing accepted Mithril decision before re-emitting `decision` during empty-chain polling; otherwise accepted bootstraps can jump back to the decision/storage UI while final local restore is still running.
- Post-download service/detail copy now uses `unpacking` rather than `installing` where the main process still distinguishes snapshot materialization work.
- Mithril status transport no longer includes raw `currentStep`; renderer components derive localized progress headings and step labels directly from `status`.
- Destination-size polling against the chain target remains a fallback-only option if direct instrumentation proves infeasible; it is not the preferred plan because same-device moves and cleanup phases do not yield stable user-facing progress.
