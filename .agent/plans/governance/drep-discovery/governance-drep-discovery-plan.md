...
| DRep query shape | Bulk `--all-dreps` once per latest-state refresh. Slice-1 reads voting power from `drep-state --all-dreps --include-stake` and the current epoch from `query tip --output-json`; the separate `drep-stake-distribution` and `gov-state` queries are deferred to the slices that need them (as shipped, slice-5 cohort ranking derives top-35 from the Phase-2 `drep-stake-distribution` enrichment of the two-phase load — see `research/slice-5-findings.md` F-1; proposal vote positions need `gov-state` in slice-4). Per-DRep CLI invocations are forbidden for directory/detail refresh. |
...
| High Value badge | Deferred to anchor-1 (task-172). Slice-5 ships Primary / Threshold / Non-metadata only. |
| DRep category badges | Each DRep card and the detail view render exactly one informational category badge (High value / Primary / Threshold / Non-metadata) with a tooltip explaining the rule that placed the DRep there. Badges are visual annotation only; they never reorder the cohort or override the randomized default. The category is grounded in actual default-cohort membership (anchor-1, task-172): no category tooltip may assert default-Recommended membership or verified metadata for an entry reached through show-all, search, favorites, or a detail deep-link, or for any entry while `isCohortActive` is false. |
...
| Cucumber / e2e coverage | **Deliberate non-goal for v1.** The slice model has no terminal test phase: every slice lands its own Jest and Storybook coverage in-slice, and task-125 covers the full browse -> evaluate -> select -> delegate journey by manual release verification on a synced node across both wallet types, including on-device hardware-wallet confirmation. Measured when this decision was taken: 48 `.feature` files exist under `tests/`, none of them reference governance, DRep, or voting, and the task tracker contains no e2e reference. No task row is minted for it. |
...
- From `cv-2` (task-175) confirmation renders the full pre-anchor [shared-design-tokens](./designs/shared-design-tokens.md) §7 identity block rather than the DRep ID string alone: CIP-129 primary, CIP-105 secondary when derivable, the signed-payload line carrying the `vote.id` credential hex, and the `(Source: On-chain)` label. Abstain and No Confidence keep their sentinel labels and render no identity block.
- `anchor-2` (task-154) swaps the verified `givenName` into that block and extends the label to `Source: On-chain · Name: Verified off-chain content`.
...
> **Note on sequencing.** This plan is sequenced as vertical capability slices, not horizontal layers. The companion [task tracker](governance-drep-discovery-plan-tasks.json) is the source of truth: 75 tasks across 14 phases (`slice-1`..`slice-8`, `cv-1`, `cv-2`, `anchor-1`, `anchor-2`, `standing`, `ux-refinement`).
...
- Formatting authority is `nix fmt` (treefmt, the flake formatter). `yarn prettier:check` runs prettier 2.1.2 and is not one of the flake checks in [perSystem/checks.nix](../../../perSystem/checks.nix), so it is advisory only: a disagreement between the two gates is not accumulated formatting debt. A final `nix fmt` after the last code commit is the release-readiness formatting step.
...
| Unverified metadata misleads delegation | Render anchor-derived content only after hash verification and label it as verified off-chain content. Hash verification proves only that the registrant authored the blob — which an impersonator satisfies exactly — so it is not on its own a mitigation for a claimed identity. anchor-2 (task-157) therefore renders CIP-119 `references` split by `@type`, with Identity entries in their own sub-section under a caption stating the identity is the DRep's own claim and is not verified by Daedalus. |
...
| Latent `voting_and_delegating` literal blocks all VOTING_AND_DELEGATING resolution | cv-1 (task-127) fixes the underlying string while preserving the export name and adds a unit test asserting the constant equals the wire literal. |
| Vote target leaks into logs or analytics | The slice-1 sanitization floor (task-109 + task-110) widens `filterLogData` and reduces the `Casted governance vote` payload to vote-kind only, with a Jest spy regression test in task-111. That floor is call-site-only — there is no sanitizing log transport — so it does not reach call sites that never invoke `filterLogData`. After cv-1 (task-128) widened the wallet wire types, `AdaApi::getWallets` and `AdaApi::getWallet` log whole wallet payloads carrying `delegation.*.voting` on every poll. cv-1 (task-170) wraps those two call boundaries and adds a `getWallets` case to the sanitization suite. |
...
| Anchor 1 MB response cap can reject valid CIP-119 metadata that inlines a base64 `imageObject` | Resolved to the second branch: anchor-2 (task-157) defers the CIP-119 `image` field, with the deferral recorded in [drep-discovery-design.md](./designs/drep-discovery-design.md), so inline-image DReps degrade to the default avatar with an explicit source label. The ~1 MB cap is unchanged and is not raised for the image case in this release. |
