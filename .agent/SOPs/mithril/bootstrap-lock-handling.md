...
## Purpose
The lock file acts as a safety mechanism. If Daedalus crashes or is killed during a Mithril bootstrap, the partial data in `stateDir/chain` is not valid. The lock file ensures that on the next launch, this partial data is wiped before any sync begins.
...
- It calls `wipeChainAndSnapshots()` (which clears the lock).
- It re-prompts the user for a Mithril decision.

## Common Pitfalls

### ⚠️ Lock file causes repeated wipes
**Problem:** If the lock file is not cleared when a user switches to "Sync from genesis" after a Mithril failure, every subsequent launch will see the lock, assume an incomplete bootstrap, and wipe the (now partially synced) genesis data.  
