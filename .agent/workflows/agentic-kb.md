...
- `sync docs` refreshes the allowlisted docs corpus from the current checkout, removes stale indexed rows for allowlisted docs that are no longer discovered, and now reports docs `processed_count` as stored markdown chunk rows while `source_paths` stay unique per source file.
