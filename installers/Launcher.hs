  ...
  , unwords $ map (\x ->  batchCmdNewline ++ "-n " ++ x) nodeArgs
    ...
    where
      ...
      nodeArgs = [
        ...
        "--keyfile", quote (runtimePath <> "Secrets-" <> version <> "-rc" <> (pathSeparator : "secret.key")),
        "--logs-prefix", quote (runtimePath <> "Logs"),
        "--db-path", quote (runtimePath <> "DB-" <> version <> "-rc"),
        "--wallet-db-path", quote (runtimePath <> "Wallet-" <> version <> "-rc"),
