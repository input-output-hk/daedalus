{ target, cardanoWalletPkgs, runCommandCC, cardano-wallet, cardano-node, cardano-shell, cardano-cli, cardano-address, lib, shelley-test-cluster ? null }:
...
in runCommandCC "daedalus-cardano-bridge" {
  ...
  ${lib.optionalString (shelley-test-cluster != null) "cp -f ${shelley-test-cluster}/bin/shelley-test-cluster* ."}
