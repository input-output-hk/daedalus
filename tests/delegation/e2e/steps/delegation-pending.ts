...
let walletsTickers = [];
Given(
  /^the wallets have the following pending delegations:$/,
  async function (delegationScenariosTable) {
    ...
    const walletsTickersInfo = await this.client.executeAsync(
      (delegationScenarios, done) => {
        ...
        for (let index = 0; index < delegationScenarios.length; index++) {
          const delegationQueue =
            delegationScenarios[index].DELEGATION_SCENARIO.split(' > ');
          ...
          const tickers = delegationQueue.map((delegationInfo, index) => {
            ...
            const stakePool =
              status === 'delegating' ? stakePools[index] : null;
...
Then(
  /^the ADA logo should be displayed as follows:$/,
  async function (visibilityTable) {
...
Then(
  /^the tooltips should be displayed as follows:$/,
  async function (tooltipsTable) {
...
Then(
  /^the action links should be displayed as follows:$/,
