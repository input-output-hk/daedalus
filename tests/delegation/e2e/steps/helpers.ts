...
export const delegationCentreStakingHelper = {
  stakingButtonVisible: async (client: Record<string, any>) =>
    client.waitForVisible(STAKING_BUTTON_SIDEBAR),
  clickStakingButton: async (client: Record<string, any>) =>
    client.click(STAKING_BUTTON_SIDEBAR),
  delegationCenterVisible: async (client: Record<string, any>) =>
...
export const getCardanoEpochData = async function (
  epoch: 'current' | 'next',
  param: string
) {
  const headerIndex = epoch === 'current' ? 1 : 2;
  return await this.waitAndGetText(
    `(//div[@class="DelegationCenterHeader_heading"])[${headerIndex}]//following-sibling::div//div[text()="${param}"]//following-sibling::div[@class="DelegationCenterHeader_fieldValue"]`
  );
};
...
export const getNextEpoch = async function () {
  const headerText = await this.waitAndGetText(
    `(//div[@class="DelegationCenterHeader_heading"])[2]`
  );
