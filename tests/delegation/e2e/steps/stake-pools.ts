...
const STAKE_POOL_TAB_BUTTON =
  '.stake-pools.NavButton_component.NavButton_normal';
const STAKE_POOL_PAGE = '.StakePools_component';
const STAKE_POOLS_LIST_SELECTOR =
  '.StakePoolsList_component .ThumbPool_component:nth-child(1)';
const STAKE_POOLS_SEARCH_SELECTOR =
  '.StakePoolsSearch_component .StakePoolsSearch_searchInput.SimpleFormField_root input';
const SEARCH_RESULTS_LABEL_SELECTOR = '.StakePools_component h2 span';
const STAKE_POOL_SLUG_SELECTOR =
  '.StakePoolsList_component .ThumbPool_component:nth-child(3) .ThumbPool_ticker';
const STAKE_POOL_TOOLTIP_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible';
const STAKE_POOL_TOOLTIP_DESCRIPTION_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_description';
const STAKE_POOL_TOOLTIP_TICKER_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_ticker';
const STAKE_POOL_TOOLTIP_HOMEPAGE_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_homepage';
const STAKE_POOL_TOOLTIP_RANKING_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_ranking span';
const STAKE_POOL_TOOLTIP_PROFIT_MARGIN_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_profitMargin span';
const STAKE_POOL_TOOLTIP_COST_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_cost span';
const STAKE_POOL_TOOLTIP_PERFORMANCE_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_performance span';
const STAKE_POOL_TOOLTIP_NAME_SELECTOR =
  '.TooltipPool_component.TooltipPool_isVisible .TooltipPool_name';
const STAKE_POOL_TOOLTIP_BUTTON_SELECTOR =
  '//button[text()="Delegate to this pool"]';
const DELEGATE_WALLET_SELECTOR =
  '.DelegationSteps_delegationSteps.DelegationStepsIntroDialog_delegationStepsIntroDialogWrapper';
const DIALOG_CONTINUE_SELECTOR = '//button[text()="Continue"]';
const DIALOG_CONFIRM_SELECTOR =
  '.DelegationSteps_delegationSteps .DialogCloseButton_component';
const DELEGATION_WALLET_FIRST_STEP_SELECTOR =
  '.DelegationSteps_delegationSteps.DelegationStepsChooseWalletDialog_delegationStepsChooseWalletDialogWrapper';
const DELEGATION_WALLET_SECOND_STEP_SELECTOR =
  '.DelegationSteps_delegationSteps.DelegationStepsChooseStakePoolDialog_delegationStepsChooseStakePoolDialogWrapper';
const DELEGATION_WALLET_DROPDOWN_SELECTOR =
  '.DelegationSteps_delegationSteps.DelegationStepsChooseWalletDialog_delegationStepsChooseWalletDialogWrapper .DelegationStepsChooseWalletDialog_walletSelect';
...
Given(
  /^I have a wallet "([^"]*)" delegated to stake pool with rank "([^"]*)"$/,
  async function (walletName, stakePoolRank) {
...
Then(
  /^I should see stake pool tooltip with order number "([^"]*)"/,
  async function (rankNumber) {
    const stakePoolRankingText = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_RANKING_SELECTOR
    );
    const stakePoolRanking = Array.isArray(stakePoolRankingText)
      ? stakePoolRankingText[0]
      : stakePoolRankingText;
...
When(
  /^I enter staking pool spending password "([^"]*)" and click "confirm" button$/,
...
When(/^I click on delegated stake pool/, async function () {
  await this.waitAndClick(
    '.StakePoolsList_component.stakePoolsDelegatingList .ThumbPool_component:nth-child(1)'
  );
});
Then(
  /^Stake pool with rank "([^"]*)" tooltip shows correct data$/,
  async function (stakePoolRank) {
    const stakePool = await getStakePoolByRanking(this.client, stakePoolRank);
    await this.client.waitForVisible(STAKE_POOL_TOOLTIP_SELECTOR);
    const stakePoolDescription = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_DESCRIPTION_SELECTOR
    );
    const stakePoolTicker = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_TICKER_SELECTOR
    );
    const stakePoolHomepage = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_HOMEPAGE_SELECTOR
    );
    const stakePoolPerformanceText = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_PERFORMANCE_SELECTOR
    );
    const stakePoolCost = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_COST_SELECTOR
    );
    const stakePoolName = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_NAME_SELECTOR
    );
    const stakePoolProfitMargin = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_PROFIT_MARGIN_SELECTOR
    );
    const stakePoolRanking = await this.waitAndGetText(
      STAKE_POOL_TOOLTIP_RANKING_SELECTOR
    );
    const cost = new BigNumber(stakePool.cost.c);
    const formattedCost = formattedWalletAmount(cost, true, false);
    const stakePoolPerformance = Array.isArray(stakePoolPerformanceText)
      ? stakePoolPerformanceText[0]
      : stakePoolPerformanceText;
...
Then(
  ...
  async function (stakePoolRank) {
    ...
    expect(selectedStakePoolLabel.toString().split('.')[0]).to.oneOf([
      messageWithPendingDelegation,
      messageWithNoPendingDelegation,
    ]);
  }
);
Then(/^I should see label: "([^"]*)"$/, async function (message) {
  const delegatedStakePoolLabel = await this.waitAndGetText(
    STAKE_POOLS_DELEGATING_LABEL
  );
...
Then(/^I should see stake pools ordered by rank$/, async function () {
  ...
  const orderCorrect = result.every(
    ({ ranking }, i) => parseInt(ranking, 10) === i + 1
  );
...
Then(/^I should see the following loading message:$/, async function (message) {
  ...
  const loadingMsgOnScreen = await this.waitAndGetText(
    LOADING_MESSAGE_SELECTOR
  );
