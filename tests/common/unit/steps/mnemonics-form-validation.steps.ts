...
} from '../../../../source/renderer/app/utils/validations';
...
Then(
  'the result should be the return value of the mnemonics validator',
  async function () {
    expect(this.context.result).to.equal(STUB_VALIDATOR_RETURN_VALUE);
  }
);
Then(
  'the mnemonics validator should have been called with the provided words',
  async function () {
    expect(
      this.context.validator.calledWithExactly(this.context.providedWords)
    ).to.equal(true);
  }
);
