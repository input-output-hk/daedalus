...
Given('I generate {int} word mnemonic', function(
  numberOfWords
) {
...
When('I generate 18-word scrambled mnemonic', function() {
  this.context.scrambledMnemonic = scrambleMnemonics({ passphrase: this.context.spendingPassword, scrambledInput: this.context.mnemonic })
});
...
When('I unscramble mnemonics', function() {
  ...
  const { passphrase, scrambledInput } = getScrambledInput(this.context.paperWalletCertificateRecoveryPhrase);
  // Unscramble 18-word wallet certificate mnemonic to 12-word initial mnemonic
  this.context.unscrambledMnemonic = unscrambleMnemonics({ passphrase, scrambledInput });
});
