...
When(
  /^I enter spending password in "Transfer ada" wizard step 2 dialog:$/,
...
When(
  /^I click continue button on "Transfer ada" wizard step 2 dialog$/,
  function () {
    return this.waitAndClick('.TransferFundsStep2Dialog_dialog .confirmButton');
  }
);
When(
  /^I see "Transfer ada" wizard step 2 transfer funds button disabled$/,
  async function () {
    const isEnabled = await this.client.isEnabled(
      '.TransferFundsStep2Dialog_dialog .confirmButton'
    );
...
When(/^I see initial wallets balance$/, async function () {
  ...
  const shelleyWalletName = await this.waitAndGetText(
    '.SidebarWalletsMenu_wallets button:nth-child(1) .SidebarWalletMenuItem_title'
  );
  const byronWalletName = await this.waitAndGetText(
    '.SidebarWalletsMenu_wallets button:nth-child(2) .SidebarWalletMenuItem_title'
  );
  // Set initial values for further use
  const shelleyFixedWalletAmount = await getFixedAmountByName.call(
    this,
    shelleyWalletName
  );
  const byronFixedWalletAmount = await getFixedAmountByName.call(
    this,
    byronWalletName
  );
...
Then(
  /^"Transfer ada" wizard step 2 dialog continue button should be disabled$/,
  async function () {
...
Then(/^I should see "Transfer ada" wizard step 2 dialog$/, async function () {
  ...
  const transferFee = await this.waitAndGetText(
    '.TransferFundsStep2Dialog_dialog .Dialog_content div:nth-child(3) .TransferFundsStep2Dialog_amount'
  );
...
Then(
  ...
  {
    timeout: 60000,
  }, // Transferring funds sometimes last more than "Default" test timeout
  function () {
...
Then(
  /^I should see increased shelley wallet byron and 0 ADA in Daedalus Byron wallet$/,
  async function () {
    const shelleySelector =
      '.SidebarWalletsMenu_wallets button:nth-child(1) .SidebarWalletMenuItem_info';
    const byronSelector =
      '.SidebarWalletsMenu_wallets button:nth-child(2) .SidebarWalletMenuItem_info';
    const transferSumWithoutFees = this.shelleyWalletAmount.plus(
      this.byronWalletAmount
    );
    const transferSumWithFees = transferSumWithoutFees.minus(this.transferFee);
    const initialShelleyFormattedAmount = formattedWalletAmount(
      this.shelleyWalletAmount,
      true,
      false
    );
    const initialByronFormattedAmount = formattedWalletAmount(
      this.byronWalletAmount,
      true,
      false
    );
    const expectedShelleyAmount = formattedWalletAmount(
      transferSumWithFees,
      true,
      false
    );
...
Then(
  ...
  async function (data) {
    const errorSelector =
      '.TransferFundsStep2Dialog_dialog .TransferFundsStep2Dialog_error';
...
Then(
  /^"Transfer ada" wizard step 2 dialog continue button should not be disabled anymore$/,
  async function () {
    const isEnabled = await this.client.isEnabled(
      '.TransferFundsStep2Dialog_dialog .confirmButton'
    );
