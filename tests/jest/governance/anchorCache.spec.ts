...
describe('anchorCache', () => {
  ...
  it('evicts the oldest entries once the entry bound is exceeded', () => {
    ...
    // Above the loop's range, so the sentinel cannot collide with a file the
    // loop already wrote: entries are immutable, and a colliding write would
    // be refused rather than overwriting.
    const newHash = hashOf(ANCHOR_CACHE_MAX_ENTRIES + 10);
