...
const names = ['Laura', 'Layla', 'Leia', 'Lily', 'Linda', 'Louise', 'Lorraine', 'Lucille', 'Luz', 'Lyn'];
...
(async () => {
    ...
    const shuffledMnemonics = (0, lodash_1.shuffle)(mnemonics_1.shelleyMnemonics);
    ...
    try {
        if (IS_HTTPS) {
            ...
            await Promise.all((0, lodash_1.sampleSize)(shuffledMnemonics, WALLET_COUNT).map((mnemonic, index) => {
                const name = shuffledNames[index];
                const payload = generateImportPayload(mnemonic, name);
                return axios_1.default.post(`http://localhost:${API_PORT}/v2/wallets`, payload);
            }));
        }
    }
    catch (e) {
...
//# sourceMappingURL=shelley-wallet-importer.js.map