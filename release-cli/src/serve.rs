...
pub async fn serve(
    ...
    let env = installer_dir
        ...
        .unwrap_or("mainnet")
        .to_string();
    let newsfeed_tmp = generate_local_newsfeed(&installer_dir, &hashes, &urls, &env, &base)?;
    println!(
        "  newsfeed : {base}/newsfeed/newsfeed_{env}.json"
    );
    println!(
        "  nf-verify: {base}/newsfeed-verification/{env}/<timestamp>.txt"
    );
...
/// Generate `newsfeed_<env>.json` and its SHA-256 verification file in a temp
/// directory, returning the temp dir path.
///
/// Directory layout:
/// ```
/// <tmpdir>/
...
fn generate_local_newsfeed(
    ...
    // Timestamp: current time rounded down to the nearest hour, in milliseconds.
    let now_ms = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_millis() as u64;
    let timestamp_ms = (now_ms / 3_600_000) * 3_600_000;
    ...
    let newsfeed_bytes = serde_json::to_vec_pretty(&newsfeed)?;

    // SHA-256 of the newsfeed JSON (no trailing newline in the .txt file).
    let sha256_hex = hex::encode(Sha256::digest(&newsfeed_bytes));

    // Write to temp dir.
    let tmp = std::env::temp_dir().join(format!("drt-serve-{timestamp_ms}"));
    ...
    std::fs::write(&newsfeed_path, &newsfeed_bytes)
        .with_context(|| format!("writing {}", newsfeed_path.display()))?;

    let verify_path = verify_dir.join(format!("{timestamp_ms}.txt"));
