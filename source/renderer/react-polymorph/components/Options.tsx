...
type Props = {
  ...
  setMouseIsOverOptions?: (arg0: boolean) => void;
  skin?: ComponentType<any>;
  targetRef?: ElementRef<any>;
  theme: Record<string, any> | null | undefined;
  // if passed by user, it will take precedence over this.props.context.theme
  themeId: string;
...
class OptionsBase extends Component<Props, State> {
  // declare ref types
  optionsElement: Element<any> | null | undefined; // TODO: Does this get used? Don't think so.
  ...
  getOptionProps = ({
    ...
    return {
      ...
      onClick: (
        option: Record<string, any> | null | undefined,
        event: React.SyntheticEvent // the user's custom onClick event handler is composed with
      ) =>
        // the internal functionality of Options (this.handleClickOnOption)
        composeFunctions(onClick, handleClickOnOption)(option, event),
      onMouseEnter: (
        index: number,
        event: React.MouseEvent // user's custom onMouseEnter is composed with this.setHighlightedOptionIndex
      ) =>
  ...
  // ========= PRIVATE HELPERS =========
  _handleSelectionOnKeyDown = (event: React.KeyboardEvent) => {
    const options = this.getFilteredOptions();
