...
class HeaderBase extends Component<Props, State> {
  ...
  _assembleInlineStyles = ({ center, lowerCase, left, right, upperCase }) => {
    const inlineStyles = {};
    const textAlign = pickBy({
      center,
      left,
      right,
    });
    const textTransform = pickBy({
      lowerCase,
      upperCase,
    });
  ...
  _assembleHeaderTheme = (styleProps: Record<string, any>) => {
  ...
  _getActiveClasses = (styleProps: Record<string, any>) => {
