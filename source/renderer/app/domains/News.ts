...
class NewsCollection {
  ...
  all: Array<News> = [];
  @observable
  update: News | null | undefined = null;

  constructor(data: Array<News>) {
    // Filter news by platform and versions
    const filteredNews = filter(data, newsItem => {
      const availableTargetVersionRange = get(newsItem, ['target', 'daedalusVersion'], '');
      const targetPlatforms = get(newsItem, ['target', 'platforms']);
      return (!availableTargetVersionRange || availableTargetVersionRange && semver.satisfies(version, availableTargetVersionRange, {
        includePrerelease: true
      })) && (platform === 'browser' || includes(targetPlatforms, platform)) && newsItem.type !== NewsTypes.UPDATE && newsItem.id && newsItem.title && newsItem.content && newsItem.action.label && newsItem.date;
    });
  ...
  get incident(): News | null | undefined {
    const incidents = filter(this.all, item => item.type === NewsTypes.INCIDENT);
    const lastIncidentIndex = incidents.length > 0 ? incidents.length - 1 : null;
  ...
  get announcements(): NewsTypesStateType {
    const announcements = filter(this.all, item => item.type === NewsTypes.ANNOUNCEMENT && !item.read);
  ...
  get infos(): NewsTypesStateType {
    const infos = filter(this.all, item => item.type === NewsTypes.INFO && !item.read);
