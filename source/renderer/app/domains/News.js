...
class NewsCollection {
  ...
  @computed get incident(): ?MethodStateReturnType {
    const incidents = filter(this.all, item =>
      item.type === NewsTypes.INCIDENT
    );
  ...
  @computed get alerts(): MethodStateReturnType {
    const alerts = filter(this.all, item =>
      item.type === NewsTypes.ALERT
    );
  ...
  @computed get announcements(): MethodStateReturnType {
    const announcements = filter(this.all, item =>
      item.type === NewsTypes.ANNOUNCEMENT &&
      !item.read
    );
  ...
  @computed get infos(): MethodStateReturnType {
    const infos = filter(this.all, item =>
      item.type === NewsTypes.INFO &&
      !item.read
    );
