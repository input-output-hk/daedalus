...
const messages = defineMessages({
  connecting: {
    ...
    description: '"Connect your device and enter your PIN to unlock it" device state'
  },
  connecting_failed: {
  ...
  exporting_public_key: {
    ...
    description: '"Confirm exporting your public key on your device" device state'
  },
  ...
  verifying_transaction: {
    ...
    defaultMessage: '!!!Confirm the transaction using the "{walletName}" device',
    description: '"Confirm the transaction using the IOHK Trezor 1 device" device state'
  },
  ...
  trezor_bridge_failure: {
    ...
    description: '"Trezor Bridge not installed! {instructionsLink}" device state'
  },
  ...
  wrong_cardano_app_version_link_url: {
    ...
    defaultMessage: '!!!https://support.ledger.com/hc/en-us/articles/360020095874-Cardano-ADA-',
    description: 'URL for the "Firmware Update"'
  },
  verifying_address: {
...
class HardwareWalletStatus extends Component<Props, State> {
  ...
  UNSAFE_componentWillReceiveProps(nextProps: Props) {
    if (nextProps.hwDeviceStatus !== this.props.hwDeviceStatus) {
      // Set with delay
      if (nextProps.hwDeviceStatus === HwDeviceStatuses.CONNECTING && this.props.hwDeviceStatus === HwDeviceStatuses.LAUNCHING_CARDANO_APP) {
        setTimeout(() => {
  ...
  render() {
    ...
    const isLoading = hwDeviceStatus === HwDeviceStatuses.CONNECTING || hwDeviceStatus === HwDeviceStatuses.LAUNCHING_CARDANO_APP || hwDeviceStatus === HwDeviceStatuses.EXPORTING_PUBLIC_KEY || hwDeviceStatus === HwDeviceStatuses.VERIFYING_TRANSACTION || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS_CONFIRMATION;
    const isReady = hwDeviceStatus === HwDeviceStatuses.READY || hwDeviceStatus === HwDeviceStatuses.VERIFYING_TRANSACTION_SUCCEEDED || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS_SUCCEEDED;
    const hasErrored = hwDeviceStatus === HwDeviceStatuses.EXPORTING_PUBLIC_KEY_FAILED || hwDeviceStatus === HwDeviceStatuses.CONNECTING_FAILED || hwDeviceStatus === HwDeviceStatuses.TREZOR_BRIDGE_FAILURE || hwDeviceStatus === HwDeviceStatuses.WRONG_FIRMWARE || hwDeviceStatus === HwDeviceStatuses.WRONG_CARDANO_APP_VERSION || hwDeviceStatus === HwDeviceStatuses.UNSUPPORTED_DEVICE || hwDeviceStatus === HwDeviceStatuses.VERIFYING_TRANSACTION_FAILED || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS_FAILED || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS_ABORTED;
    const hasPassphraseLabel = hwDeviceStatus === HwDeviceStatuses.EXPORTING_PUBLIC_KEY || hwDeviceStatus === HwDeviceStatuses.VERIFYING_TRANSACTION || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS;
    const componentClasses = classnames([styles.component, isReady ? styles.isReady : null, hasErrored ? styles.isError : null]);
    const hasInstructionsLink = hwDeviceStatus === HwDeviceStatuses.TREZOR_BRIDGE_FAILURE || hwDeviceStatus === HwDeviceStatuses.WRONG_CARDANO_APP_VERSION || hwDeviceStatus === HwDeviceStatuses.WRONG_FIRMWARE;
    let instructionsLink;
    let label;

    if (hasInstructionsLink && onExternalLinkClick) {
      // @TODO - add Ledger firmware update support article links
      instructionsLink = <Link className={styles.externalLink} onClick={event => onExternalLinkClick(intl.formatMessage(messages[`${hwDeviceStatus}_link_url`]), event)} label={intl.formatMessage(messages[`${hwDeviceStatus}_link_label`])} skin={LinkSkin} />;
    } else if (walletName && (hwDeviceStatus === HwDeviceStatuses.CONNECTING || hwDeviceStatus === HwDeviceStatuses.VERIFYING_TRANSACTION || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS || hwDeviceStatus === HwDeviceStatuses.VERIFYING_ADDRESS_CONFIRMATION)) {
      const message = hwDeviceStatus === HwDeviceStatuses.CONNECTING ? `${hwDeviceStatus}_known` : hwDeviceStatus;
      label = <FormattedMessage {...messages[message]} values={{
        walletName
      }} />;
    } else {
        ...
        <div className={componentClasses}>
          <div className={styles.messageWrapper}>
            <div className={styles.message}>
              {hasInstructionsLink && instructionsLink ? <FormattedMessage {...messages[hwDeviceStatus]} values={{
              instructionsLink
            }} /> : label}
            </div>
            {hasPassphraseLabel && isTrezor && <div className={styles.passphraseLabel}>
          ...
          {hasErrored && <SVGInline svg={clearIcon} className={styles.clearIcon} />}
        </div>
        {(hwDeviceStatus === HwDeviceStatuses.EXPORTING_PUBLIC_KEY_FAILED || hwDeviceStatus === HwDeviceStatuses.CONNECTING_FAILED) && <div className={styles.errorText}>
