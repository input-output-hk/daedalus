...
import { SMASH_SERVERS_LIST, SMASH_SERVER_TYPES, SMASH_URL_VALIDATOR } from "../../../config/stakingConfig";
...
const messages = defineMessages({
  description: {
    ...
    defaultMessage: '!!!The {link} is an off-chain metadata server that enables the fast loading of stake pool details. Stake pools are also curated and each server has a different curation policy.',
  ...
  invalidUrl: {
    ...
    description: 'invalidUrl for the "Smash Custom Server" selection on the Stake Pools settings page.'
  },
  invalidUrlPrefix: {
    id: 'settings.stakePools.smashUrl.input.invalidUrlPrefix',
    defaultMessage: '!!!The URL should start with "https://"',
    description: 'invalidUrlPrefix for the "Smash Custom Server" selection on the Stake Pools settings page.'
  },
  invalidUrlParameter: {
    id: 'settings.stakePools.smashUrl.input.invalidUrlParameter',
    defaultMessage: '!!!Only "https://" protocol and hostname (e.g. domain.com) are allowed',
    description: 'invalidUrlParameter for the "Smash Custom Server" selection on the Stake Pools settings page.'
  }
});
type Props = {
...
class StakePoolsSettings extends Component<Props, State> {
  ...
  static getDerivedStateFromProps({
    ...
    const successfullyUpdated = wasLoading && !isLoading && !smashServerUrlError;
    return {
      successfullyUpdated,
  ...
  handleErrorMessage = (value: string) => {
    ...
    if (!/^https:\/\//i.test(value)) errorMessage = messages.invalidUrlPrefix;else if (search || pathname && pathname.slice(1)) errorMessage = messages.invalidUrlParameter;
  ...
  renderSmashTypeDropdown = () => {
    ...
    const smashServerTypes = isSelfnode ? omit(SMASH_SERVER_TYPES, SMASH_SERVERS_LIST[SMASH_SERVER_TYPES.IOHK].name) : SMASH_SERVER_TYPES;
    const smashSelectOptions = map(smashServerTypes, value => ({
      label: this.smashSelectMessages[value] || value,
      value
    }));
    const selectedValue = !isSyncing && smashServerType ? this.smashSelectMessages[smashServerType] || smashServerType : '-';

    if (isSyncing) {
      return <div className={styles.disabledSelect}>
    ...
    return <Select label={<div>
            {intl.formatMessage(messages.smashSelectLabel)}
            {successfullyUpdated && <span className={styles.savingResultLabel}>
                {intl.formatMessage(messages.changesSaved)}
              </span>}
          </div>} value={smashServerType} options={smashSelectOptions} onChange={this.handleOnSelectSmashServerType} className={styles.select} optionHeight={50} selectionRenderer={({
      label
  ...
  renderSmashCustomServerInput = () => {
    ...
    const smashServerType = getSmashServerIdFromUrl(editingSmashServerUrl || '');
    const errorMessage = smashServerUrlError ? intl.formatMessage(smashServerUrlError) : null;
    const smashServerUrlStyles = classnames([styles.smashServerUrl, isSyncing ? styles.syncing : null]);

    if (smashServerType !== SMASH_SERVER_TYPES.CUSTOM) {
      return null;
    }

    return <InlineEditingInput className={smashServerUrlStyles} label={intl.formatMessage(messages.smashURLInputLabel)} value={editingSmashServerUrl || ''} placeholder={intl.formatMessage(messages.smashUrlInputPlaceholder)} onSubmit={this.handleSubmit} isValid={this.handleIsValid} valueErrorMessage={this.handleErrorMessage} errorMessage={errorMessage} readOnly={isLoading} isLoading={isLoading} successfullyUpdated={false} />;
  };
  renderBottomContent = () => {
    const {
      onOpenExternalLink,
    ...
    if (isSyncing) {
      return <div className={styles.optionDescription}>
          <FormattedHTMLMessage {...globalMessages.featureUnavailableWhileSyncing} values={{
          syncPercentage
        }} />
        </div>;
    }
    ...
    if (smashServerType === SMASH_SERVER_TYPES.IOHK) {
      return <div className={styles.optionDescription}>
          ...
          <p>
            <FormattedMessage {...messages.descriptionIOHKContent2} values={{
            link: <Link onClick={() => onOpenExternalLink(intl.formatMessage(messages.descriptionIOHKLinkUrl))} label={intl.formatMessage(messages.descriptionIOHKLinkLabel)} />
          }} />
          </p>
        </div>;
    }
  ...
  render() {
    ...
    return <div className={styles.component}>
        <div className={styles.description}>
          ...
          link: <Link className={styles.link} onClick={() => onOpenExternalLink(intl.formatMessage(messages.descriptionLinkUrl))} label={intl.formatMessage(messages.descriptionLinkLabel)} />
        }} />
        </div>
