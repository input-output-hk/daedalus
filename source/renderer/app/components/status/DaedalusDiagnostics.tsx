...
import { formattedNumber, formattedCpuModel, formattedSize } from "../../utils/formatters";
...
const FINAL_CARDANO_NODE_STATES = [CardanoNodeStates.RUNNING, CardanoNodeStates.UPDATED, CardanoNodeStates.CRASHED, CardanoNodeStates.ERRORED, CardanoNodeStates.UNRECOVERABLE];
...
class DaedalusDiagnostics extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (cardanoNodeState !== prevCardanoNodeState && includes(FINAL_CARDANO_NODE_STATES, cardanoNodeState)) {
      this.setState({
        isNodeRestarting: false
      }); // eslint-disable-line
    }
  ...
  getSectionRow = (messageId: string, content?: Node) => {
    return <div className={styles.layoutRow}>
  ...
  getRow = (messageId: string, value: Node | boolean) => {
    ...
    const classNameHeader = classNames([styles[messageId], styles.layoutHeader]);
    const classNameRow = classNames([styles.layoutRow, messageId]);

    if (typeof value === 'boolean') {
      content = value ? intl.formatMessage(messages.statusOn) : intl.formatMessage(messages.statusOff);
      className = value && messageId !== 'systemTimeIgnored' || !value && messageId === 'systemTimeIgnored' ? classNames([className, styles.green]) : classNames([className, styles.red]);
    }

    return <div className={classNameRow}>
  ...
  render() {
    ...
    const availableDiskSpace = formattedSize(availableDiskSpaceInOriginalFormat);
    const {
      daedalusVersion,
      daedalusBuildNumber,
      daedalusProcessID,
    ...
    const unknownDiskSpaceSupportUrl = intl.formatMessage(messages.unknownDiskSpaceSupportUrl);
    const cardanoNetworkValue = intl.formatMessage(globalMessages[`network_${cardanoNetwork}`]);
    const localTimeDifferenceClasses = isCheckingSystemTime ? classNames([styles.layoutData, styles.localTimeDifference]) : classNames([styles.layoutData, styles.localTimeDifference, !isNTPServiceReachable || localTimeDifference && Math.abs(localTimeDifference) > ALLOWED_TIME_DIFFERENCE ? styles.red : styles.green]);
    const {
      getSectionRow,
      getRow
    } = this;
        ...
        <div className={styles.tables}>
          <div className={styles.table}>
            <div>
              ...
              {getRow('ram', ram)}
              {getRow('availableDiskSpace', availableDiskSpace || <Link onClick={() => onOpenExternalLink(unknownDiskSpaceSupportUrl)} label={intl.formatMessage(messages.unknownDiskSpace)} skin={LinkSkin} />)}
            </div>
            <div>
              ...
              {getRow('blankScreenFix', isBlankScreenFixActive ? intl.formatMessage(messages.statusOn) : intl.formatMessage(messages.statusOff))}
              {getRow('stateDirectoryPath', <Fragment>
                  <button className={styles.stateDirectoryOpenBtn} onClick={() => onOpenStateDirectory(daedalusStateDirectoryPath)}>
                    {intl.formatMessage(messages.stateDirectoryPathOpenBtn)}
                  </button>
                  <CopyToClipboard text={daedalusStateDirectoryPath} onCopy={onCopyStateDirectoryPath}>
                    <div className={styles.stateDirectoryPath}>
                      <PopOver maxWidth={400} content={<div className={styles.tooltipLabelWrapper}>
            ...
            {isConnected && nodeConnectionError ? <div>
          ...
          <div className={styles.table}>
            <div>
              ...
              {getRow('syncPercentage', `${formattedNumber(syncPercentage, 2)}%`)}
              {getRow('lastNetworkBlock', <Fragment>
                  <span>{intl.formatMessage(messages.epoch)}:</span>{' '}
                  {networkTip && networkTip.epoch ? formattedNumber(networkTip.epoch) : <SVGInline svg={sandClockIcon} className={styles.networkTipSandClock} />}
                  <span>{intl.formatMessage(messages.slot)}:</span>{' '}
                  {networkTip && networkTip.slot ? formattedNumber(networkTip.slot) : <SVGInline svg={sandClockIcon} className={styles.networkTipSandClock} />}
                </Fragment>)}
              {getRow('lastSynchronizedBlock', <Fragment>
                  <span>{intl.formatMessage(messages.epoch)}:</span>{' '}
                  {localTip && localTip.epoch ? formattedNumber(localTip.epoch) : <SVGInline svg={sandClockIcon} className={styles.networkTipSandClock} />}
                  <span>{intl.formatMessage(messages.slot)}:</span>{' '}
                  {localTip && localTip.slot ? formattedNumber(localTip.slot) : <SVGInline svg={sandClockIcon} className={styles.networkTipSandClock} />}
                </Fragment>)}
              <div className={styles.layoutRow}>
                <div className={styles.layoutHeader}>
                ...
                <div className={localTimeDifferenceClasses}>
                  {<button onClick={() => this.checkTime()} disabled={isForceCheckingSystemTime || !isNodeResponding}>
                      {isForceCheckingSystemTime ? intl.formatMessage(messages.localTimeDifferenceChecking) : intl.formatMessage(messages.localTimeDifferenceCheckTime)}
                    </button>}
                  {isCheckingSystemTime ? <span className={localTimeDifferenceClasses}>
                      <SVGInline svg={sandClockIcon} className={styles.networkTipSandClock} />
                    </span> : <span className={localTimeDifferenceClasses}>
                      {isNTPServiceReachable ? `${formattedNumber(localTimeDifference || 0)} μs` : intl.formatMessage(messages.serviceUnreachable)}
                    </span>}
                </div>
              </div>
              ...
              {<div className={styles.layoutRow}>
                  ...
                  <div className={styles.layoutData}>
                    {isCheckingSystemTime ? intl.formatMessage(messages.statusOn) : intl.formatMessage(messages.statusOff)}
                  </div>
                </div>}
            </div>
            <div>
              {getSectionRow('cardanoNodeStatus', <button className={styles.cardanoNodeStatusBtn} onClick={() => this.restartNode()} disabled={!includes(FINAL_CARDANO_NODE_STATES, cardanoNodeState)}>
                  {isNodeRestarting ? intl.formatMessage(messages.cardanoNodeStatusRestarting) : intl.formatMessage(messages.cardanoNodeStatusRestart)}
                </button>)}
              {getRow('cardanoNodeState', upperFirst(cardanoNodeState != null ? intl.formatMessage(this.getLocalisationForCardanoNodeState()) : 'unknown'))}
