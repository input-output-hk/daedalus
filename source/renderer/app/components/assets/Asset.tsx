...
const messages = defineMessages({
  ...
  settingsCogPopOver: {
    ...
    defaultMessage: '!!!You can configure the number of decimal places for this native token.',
...
class Asset extends Component<Props, State> {
  ...
  get isSettingsPopOverVisible() {
    ...
    if (assetSettingsDialogWasOpened === false && anyAssetWasHovered === false) {
      return true;
    }
  ...
  renderPillContent() {
    ...
    const contentStyles = classnames([styles.pill, small ? styles.small : null, hasError ? styles.error : null]);
    ...
    return <div className={contentStyles}>
        ...
        {name && <div className={styles.metadataName}>
            {metadataNameChars ? ellipsis(name, metadataNameChars) : name}
          </div>}
        {hasWarning && <div className={styles.warningIconWrapper}>
  ...
  renderPillPopOverContainer = () => {
    ...
    const popOverContent = <AssetContent asset={asset} onCopyAssetParam={onCopyAssetParam} assetSettingsDialogWasOpened={assetSettingsDialogWasOpened} anyAssetWasHovered={anyAssetWasHovered} className={styles.popOverContent} highlightFingerprint />;
    const {
      isPillPopOverVisible
    } = this.state;
    return <div className={styles.popOverContainer} onMouseEnter={this.handleShowPillPopOver} onMouseLeave={this.handleHidePillPopOver}>
        <PopOver themeVariables={{
        '--rp-pop-over-bg-color': 'var(--theme-widgets-asset-token-background-color)',
        ...
        '--rp-pop-over-box-shadow': '0 5px 20px 0 var(--theme-widgets-asset-token-box-shadow)'
      }} contentClassName={styles.popOver} content={popOverContent} visible={isPillPopOverVisible} appendTo="parent" maxWidth={376} allowHTML interactive>
          {pillContent}
        </PopOver>
      </div>;
  };
  renderSettingsContent = () => {
    ...
    return <button className={styles.settingsButton} onClick={onClickSettingsBind}>
        <PopOver className={styles.test} content={intl.formatMessage(messages.settingsCogPopOver)} visible={isSettingsPopOverVisible}>
          <SVGInline onMouseEnter={handleSettingsMouseEnter} onMouseLeave={handleSettingsMouseLeave} className={styles.settingsIcon} svg={settingsIcon} />
        </PopOver>
  ...
  render() {
    ...
    const content = hidePopOver ? this.renderPillContent() : this.renderPillPopOverContainer();
