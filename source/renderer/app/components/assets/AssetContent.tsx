...
const AssetContent = observer((props: Props) => {
  ...
  const handleCopyParam = (newParamCopied: string, param: string, fullValue: string) => {
    const {
      onCopyAssetParam
    } = props;
  ...
  const renderAssetParam = (assetId: string, param: string, value: string) => {
    ...
    const iconClassnames = classnames([styles.copyIcon, paramCopied === assetId ? styles.copiedIcon : null]);

    const onCopy = () => {
      handleCopyParam(assetId, param, value);
    };

    return <CopyToClipboard text={value} onCopy={onCopy}>
        <div className={styles.assetParam}>
          <div className={styles.value}>
            {value}
            <SVGInline svg={icon} className={iconClassnames} />
          </div>
          {assetId === 'assetName' && <div className={styles.assetASCIIName}>
  ...
  const componentStyles = classnames([styles.component, className, highlightFingerprint ? styles.highlightFingerprint : null, hasError ? styles.error : null]);
  return <div className={componentStyles}>
      {highlightFingerprint && <div className={styles.fingerprint}>
          {renderAssetParam('fingerprint', intl.formatMessage(messages.fingerprintAssetParam), fingerprint)}
        </div>}
      <dl>
        {!highlightFingerprint && <>
        ...
        {ticker && <>
            ...
            <dd>
              {renderAssetParam('ticker', intl.formatMessage(messages.tickerAssetParam), ticker)}
            </dd>
          </>}
        {name && <>
            <dt>{intl.formatMessage(messages.nameAssetParam)}</dt>
            <dd>
              {renderAssetParam('name', intl.formatMessage(messages.nameAssetParam), name)}
            </dd>
          </>}
        {description && <>
            <dt>{intl.formatMessage(messages.descriptionAssetParam)}</dt>
            <dd>
              {renderAssetParam('description', intl.formatMessage(messages.descriptionAssetParam), description)}
            </dd>
          </>}
        <dt>{intl.formatMessage(messages.policyIdAssetParam)}</dt>
        <dd>
          {renderAssetParam('policyId', intl.formatMessage(messages.policyIdAssetParam), policyId)}
        </dd>
        <dt>{intl.formatMessage(messages.assetNameAssetParam)}</dt>
        <dd>
          {assetName ? renderAssetParam('assetName', intl.formatMessage(messages.assetNameAssetParam), assetName) : <span className={styles.blankValue}>
