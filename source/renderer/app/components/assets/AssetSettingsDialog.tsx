...
import { DEFAULT_DECIMAL_PRECISION, MAX_DECIMAL_PRECISION } from "../../config/assetsConfig";
const messages = defineMessages({
  ...
  description: {
    ...
    defaultMessage: '!!!Updates made here will be applied in other wallets containing this token too.',
...
class AssetSettingsDialog extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    this.state = {
      decimals: hasSavedDecimals ? savedDecimals : recommendedDecimals || DEFAULT_DECIMAL_PRECISION
    };
  }
  ...
  optionRenderer = (props: {
    ...
    if (typeof recommendedDecimals === 'number' && recommendedDecimals === value) {
  ...
  render() {
    ...
    const actions = [{
      ...
      primary: true,
      disabled: hasSavedDecimals && decimals === savedDecimals || !hasSavedDecimals && decimals === DEFAULT_DECIMAL_PRECISION,
      onClick: () => onSubmit(asset, decimals)
    }];
    const hasWarning = hasRecommendedDecimals && savedDecimals !== recommendedDecimals;
    let warningPopOverMessage;

    if (hasWarning) {
      warningPopOverMessage = hasSavedDecimals ? messages.warningPopOverNotUsing : messages.warningPopOverAvailable;
    }

    return <Dialog className={styles.component} title={intl.formatMessage(messages.title)} subtitle={<Asset asset={asset} className={styles.assetToken} />} actions={actions} closeOnOverlayClick onClose={onCancel} closeButton={<DialogCloseButton />} className1={styles.dialog}>
        <div>
          ...
          <Select options={options} value={decimals} className={styles.decimalsDropdown} label={<span className={styles.decimalsDropdownLabel}>
            ...
            recommendedDecimals
              ...
              </span>} onChange={this.onSetDecimalPrecision} optionRenderer={this.optionRenderer} selectionRenderer={this.selectionRenderer} />
        </div>
      </Dialog>;
  }
