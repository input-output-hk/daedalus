...
const AssetsTransactionConfirmation = observer((props: Props) => {
  ...
  const adaAmountStyles = classnames([styles.adaAmount, insufficientAdaAmount ? styles.adaAmountError : null]);
  const adaAmountContent = <div className={adaAmountStyles}>
  ...
  return <div className={componentStyles}>
      {adaError ? <PopOver content={adaError} className={styles.adaErrorPopOver} appendTo="parent">
          {adaAmountContent}
        </PopOver> : adaAmountContent}
      {assets.map((asset, index) => <AssetTransactionConfirmation key={asset.uniqueId} assetNumber={index + 1} isHardwareWallet={false} asset={asset} amount={assetsAmounts[index]} tokenIsMissing={isTokenMissingInWallet(wallet, asset)} insufficientBalance={!!wallet && !tokenHasBalance(asset, assetsAmounts[index])} />)}
    </div>;
});
