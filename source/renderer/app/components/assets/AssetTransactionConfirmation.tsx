...
const messages = defineMessages({
  ...
  unformattedAmountLabel: {
    ...
    description: '"unformattedAmountLabel" item on AssetTransactionConfirmation.'
  },
  unformattedAmountMessageForHardwareWallets: {
    ...
    description: '"unformattedAmountMessageForHardwareWallets" item on AssetTransactionConfirmation.'
  },
  unformattedAmountMessageForSoftwareWallets: {
    ...
    description: '"unformattedAmountMessageForSoftwareWallets" item on AssetTransactionConfirmation.'
  },
  missingToken: {
  ...
  insufficientBalance: {
    ...
    defaultMessage: '!!!Insufficient funds. The balance of the token in this wallet is  {formattedBalance} (Unformatted: {unformattedBalance})',
...
const AssetTransactionConfirmation = observer((props: Props) => {
  ...
  const formattedAmount = formattedTokenWalletAmount(amount, metadata, decimals);
  const unformattedAmount = formattedTokenWalletAmount(amount, null, 0);
  const formattedBalance = formattedTokenWalletAmount(asset.quantity, metadata, decimals);
  const unformattedBalance = formattedTokenWalletAmount(asset.quantity, null, 0);
  ...
  const content = <>
      <div className={styles.assetsContainer}>
        <h3>
          ...
          <Asset asset={asset} onCopyAssetParam={onCopyAssetParam} hasError={hasError} />
        </h3>
      ...
      <div className={styles.assetsContainer}>
        ...
        <div className={styles.unformattedAmountLabel}>
          ...
          <PopOver content={<FormattedHTMLMessage {...messages[isHardwareWallet ? 'unformattedAmountMessageForHardwareWallets' : 'unformattedAmountMessageForSoftwareWallets']} tagName="div" />}>
  ...
  if (tokenIsMissing) {
    return <div className={componentStyles}>
        <PopOver content={intl.formatMessage(messages.missingToken)} appendTo="parent" placement="bottom">
          {content}
        </PopOver>
      </div>;
  }
  ...
  if (insufficientBalance) {
    return <div className={componentStyles}>
        <PopOver content={intl.formatMessage(messages.insufficientBalance, {
        formattedBalance,
        unformattedBalance
      })} appendTo="parent" placement="bottom">
          {content}
        </PopOver>
      </div>;
  }
