...
class AssetAmount extends Component<Props> {
  static contextTypes = {
  ...
  render() {
    ...
    const content = !isLoading ? formattedTokenWalletAmount(amount, metadata, decimals, isShort) : '-';
    return <div className={componentStyles}>
        {decimals ? <PopOver content={<FormattedHTMLMessage {...messages.unformattedAmount} values={{
        amount: formattedTokenWalletAmount(amount, null, 0)
      }} />} visible={decimals ? undefined : false} className={styles.unformattedAmount}>
            {content}
