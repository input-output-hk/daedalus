...
type StepId = 'preparing' | 'downloading' | 'verifying' | 'finalizing';
                ...
                (bytesDownloaded != null || throughputBps != null) && (
                  <span className={styles.meta}>
                    {bytesDownloaded != null && (
                      <>
                        {formattedBytesToSize(bytesDownloaded)}
                        {snapshotSize != null && snapshotSize > 0 && (
                          <> / {formattedBytesToSize(snapshotSize)}</>
                        )}
                      </>
                    )}
                    {bytesDownloaded != null && throughputBps != null && ' • '}
                    {throughputBps != null &&
                      `${formattedBytesToSize(throughputBps)}/s`}
                  </span>
                )}
            </div>
          </div>
