...
const messages = defineMessages({
  ...
  nodeIsStopping: {
    ...
    description: 'Message "Cardano node is stopping" on the status icon tooltip'
  },
  nodeHasStopped: {
    id: 'status.icons.nodeHasStopped',
    defaultMessage: '!!!Cardano node has stopped!',
    description: 'Message "Cardano node has stopped" on the status icon tooltip'
  },
  nodeIsUpdating: {
    id: 'status.icons.nodeIsUpdating',
    defaultMessage: '!!!Cardano node is updating!',
    description: 'Message "Cardano node is updating" on the status icon tooltip'
  },
  nodeHasBeenUpdated: {
    id: 'status.icons.nodeHasBeenUpdated',
    defaultMessage: '!!!Cardano node has been updated!',
    description: 'Message "Cardano node has been updated" on the status icon tooltip'
  },
  nodeHasCrashed: {
    id: 'status.icons.nodeHasCrashed',
    defaultMessage: '!!!Cardano node has crashed!',
    description: 'Message "Cardano node has crashed" on the status icon tooltip'
  },
  nodeHasErrored: {
    id: 'status.icons.nodeHasErrored',
    defaultMessage: '!!!Cardano node has errored!',
    description: 'Message "Cardano node has errored" on the status icon tooltip'
  },
  nodeIsUnrecoverable: {
    id: 'status.icons.nodeIsUnrecoverable',
    defaultMessage: '!!!Cardano node is unrecoverable!',
    description: 'Message "Cardano node is unrecoverable" on the status icon tooltip'
  },
  ...
  isNodeSyncingOff: {
    ...
    description: 'Message "Cardano node is not syncing" on the status icon tooltip'
  },
  isNodeSyncingLoading: {
    id: 'status.icons.isNodeSyncingLoading',
    defaultMessage: '!!!Checking if Cardano node is syncing!',
    description: 'Message "Checking if Cardano node is syncing" on the status icon tooltip'
  }
});
type Props = {
...
export default class StatusIcons extends Component<Props> {
  ...
  getClassName = (paramName: string) => {
    ...
    return classNames([styles.icon, styles[`icon-${status}`], styles[`icon-${paramName}`], isNodeSyncing ? styles.syncing : styles.loading]);
  };
  getTooltipClassname = (paramName: string) => {
    const paramValue = this.props[paramName];
    return classNames([styles.tooltip, typeof paramValue === 'undefined' ? styles.ellipsis : null, this.isDisabled(paramName) ? styles.disabled : null]);
  };
  isDisabled = (paramName: string) => paramName !== 'nodeState' && this.props.nodeState !== CardanoNodeStates.RUNNING;
  getIconWithPopover = (icon: string, paramName: string) => <PopOver themeVariables={{
    '--rp-pop-over-bg-color': 'var(--theme-loading-status-icons-tooltip-color)',
    '--rp-pop-over-border-radius': '5px',
    '--rp-bubble-padding': '6px 12px 7px'
  }} contentClassName={this.getTooltipClassname(paramName)} key={paramName} content={this.getTip(paramName, this.props[paramName])}>
  ...
  render() {
    return <div className={styles.component}>
        {[this.getIconWithPopover(nodeStateIcon, 'nodeState'), this.getIconWithPopover(isNodeRespondingIcon, 'isNodeResponding'), // this.getIconWithPopover(isNodeSubscribedIcon, 'isNodeSubscribed'),
      this.getIconWithPopover(isNodeTimeCorrectIcon, 'isNodeTimeCorrect'), this.getIconWithPopover(isNodeSyncingIcon, 'isNodeSyncing')]}
      </div>;
  }
