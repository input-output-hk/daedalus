...
export default class SyncingConnectingStatus extends Component<Props> {
  ...
  _getConnectingMessage = () => {
    ...
    const isConnectingMessage = connectingMessage === messages.connecting || connectingMessage === messages.reconnecting;
  ...
  render() {
    ...
    const showEllipsis = !isConnected && (isNodeStopped || isTlsCertInvalid && !isNodeStopping);
    const componentStyles = classNames([styles.component, isConnected ? styles.syncing : styles.connecting]);
    const headlineStyles = classNames([styles.headline, showEllipsis ? styles.withoutAnimation : null]);
