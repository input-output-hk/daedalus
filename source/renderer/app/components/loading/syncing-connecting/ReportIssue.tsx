...
const messages = defineMessages({
  ...
  connectivityIssueArticleUrl: {
    ...
    defaultMessage: '!!!https://iohk.zendesk.com/hc/en-us/articles/360010522913',
...
export default class ReportIssue extends Component<Props> {
  ...
  render() {
    ...
    const reportIssueButtonClasses = classNames(['primary', 'reportIssueButton', styles.actionButton]);
    const readArticleButtonClasses = classNames(['primary', 'readArticleButton', styles.actionButton]);
    const downloadLogsButtonClasses = classNames([styles.downloadLogsButton, disableDownloadLogs ? styles.disabled : null]);
    ...
    return <div className={styles.component}>
        ...
        <Button className={readArticleButtonClasses} label={<p>
              <SVGInline svg={externalLinkIcon} className={styles.externalLinkIcon} />
              {intl.formatMessage(messages.readArticleButtonLabel)}
            </p>} onClick={() => onOpenExternalLink(intl.formatMessage(readArticleButtonUrl))} skin={ButtonSkin} />
        <Button className={reportIssueButtonClasses} label={<p>
              <SVGInline svg={externalLinkIcon} className={styles.externalLinkIcon} />
              {intl.formatMessage(messages.reportIssueButtonLabel)}
            </p>} onClick={() => onIssueClick(intl.formatMessage(messages.reportIssueButtonUrl))} skin={ButtonSkin} />
        <br />
        <Link className={downloadLogsButtonClasses} onClick={!disableDownloadLogs ? onDownloadLogs : null} hasIconAfter={false} label={intl.formatMessage(messages.reportIssueDownloadLogsLinkLabel)} skin={LinkSkin} />
      </div>;
  }
