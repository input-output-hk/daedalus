...
class SystemTimeError extends Component<Props> {
  ...
  render() {
    ...
    const supportPortalLink = <Link className={styles.supportPortalLink} onClick={event => onExternalLinkClick(supportPortalLinkUrl, event)} label={intl.formatMessage(messages.supportPortalLink)} skin={LinkSkin} />;
    ...
    return <div className={styles.component}>
        ...
        {isNTPServiceReachable ? <div>
            <p>
              <FormattedMessage {...messages.overlayTextP1} values={{
            timeOffset: <em className="time-off">{timeOffset}</em>
          }} />
            </p>

            <p>
              <FormattedMessage {...messages.overlayTextP2} values={{
            supportPortalLink
          }} />
            </p>

            <Link className={classNames([styles.checkLink, isCheckingSystemTime ? styles.disabled : null])} onClick={() => onCheckTheTimeAgain()} label={intl.formatMessage(messages.onCheckTheTimeAgainLink)} hasIconAfter={false} skin={LinkSkin} />
          </div> : <div>
            <p>
              <FormattedMessage {...messages.ntpUnreachableTextP1} values={{
            timeOffset
          }} />
            </p>

            <p>
              <FormattedMessage {...messages.ntpUnreachableTextP2} values={{
            supportPortalLink,
            allowedTimeDifferenceInSeconds
          }} />
            </p>

            <Link className={styles.checkLink} onClick={() => onContinueWithoutClockSyncCheck()} label={intl.formatMessage(messages.onContinueWithoutClockSyncCheckLink)} hasIconAfter={false} skin={LinkSkin} />
          </div>}
      </div>;
  }
