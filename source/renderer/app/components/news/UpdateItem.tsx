...
class UpdateItem extends Component<Props> {
  ...
  render() {
    ...
    const componentClasses = classNames([styles.component, updateItem.type ? styles[updateItem.type] : null]);
    const title = this.generateTitleWithBadge(updateItem.title);
    return <div className={componentClasses} role="presentation" onClick={onOpenAppUpdate}>
        {title}
        <div className={styles.date}>
          {moment(updateItem.date).format(currentDateFormat)}
        </div>
        {!isUpdatePostponed && <div className={styles.downloadProgress}>
            <span style={{
