...
class NewsFeed extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    this.scrollableDomElement = document.querySelector(SCROLLABLE_DOM_ELEMENT_SELECTOR);
  ...
  componentWillUnmount() {
    ...
    if (this.scrollableDomElement) {
      this.scrollableDomElement.removeEventListener('scroll', this.handleOnScroll);
    }
  }
  ...
  handleWindowClick = (event: MouseEvent) => {
    ...
    if (isNewsFeedOpen && newsFeedElement && clickedElement instanceof Node && !newsFeedElement.contains(clickedElement)) {
  ...
  render() {
    ...
    const componentClasses = classNames([styles.component, isNewsFeedOpen ? styles.show : null, openWithoutTransition ? styles.noTransition : null]);
    const newsFeedHeaderStyles = classNames([styles.newsFeedHeader, hasShadow && !hasUpdateItem ? styles.hasShadow : null]);
    const newsFeedContainerStyles = classNames([styles.newsFeedContainer, !hasUpdateItem ? styles.noUpdateItem : null, hasShadow ? styles.hasShadow : null]);
    const newsFeedListStyles = classNames([styles.newsFeedList, hasUpdateItem ? styles.hasUpdate : null, hasShadow ? styles.hasShadow : null]);
    const newsFeedUpdateStyles = classNames([styles.updateItem, hasShadow ? styles.hasShadow : null]);
    return <div className={componentClasses} ref={this.newsFeedRef}>
        <div className={newsFeedHeaderStyles}>
          <h3 className={styles.newsFeedTitle}>
            {intl.formatMessage(messages.newsFeedTitle)}
            {totalUnreadNewsItems > 0 && <span className={styles.newsFeedBadge}>
        ...
        <div className={newsFeedContainerStyles}>
          {hasUpdateItem && <div className={newsFeedUpdateStyles}>
              {<UpdateItem key={update.id} updateItem={update} isNewsFeedOpen={isNewsFeedOpen} onMarkNewsAsRead={onMarkNewsAsRead} onOpenAlert={onOpenAlert} onProceedNewsAction={onProceedNewsAction} onOpenAppUpdate={onOpenAppUpdate} currentDateFormat={currentDateFormat} downloadProgress={updateDownloadProgress} isUpdatePostponed={isUpdatePostponed} />}
            </div>}
          {items.length > 0 && <div className={newsFeedListStyles}>
              {hasUpdateItem && <hr className={styles.separator} />}
              {items.map(newsItem => <NewsItem key={newsItem.id} hasUpdateItem={hasUpdateItem} newsItem={newsItem} isNewsFeedOpen={isNewsFeedOpen} onMarkNewsAsRead={onMarkNewsAsRead} onOpenAlert={onOpenAlert} onProceedNewsAction={onProceedNewsAction} onOpenExternalLink={onOpenExternalLink} currentDateFormat={currentDateFormat} />)}
            </div>}
          {news && items.length === 0 && !isLoadingNews && <div className={styles.newsFeedEmptyContainer}>
              <p className={styles.newsFeedEmpty}>
                {intl.formatMessage(messages.newsFeedEmpty)}
              </p>
            </div>}
          {(!news || items.length === 0) && isLoadingNews && <div className={styles.newsFeedNoFetchContainer}>
