...
class NewsItem extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (prevProps.isNewsFeedOpen && !this.props.isNewsFeedOpen && newsItemExpanded) {
      this.setState({
        newsItemExpanded: false
      }); // eslint-disable-line
    }
  ...
  generateTitleWithBadge = (title: string, isRead: boolean) => {
    ...
    return <h4 className={styles.newsItemTitle}>
  ...
  render() {
    ...
    const componentClasses = classNames([styles.component, newsItem.type ? styles[newsItem.type] : null, this.state.newsItemExpanded ? styles.expanded : null, newsItem.read ? styles.isRead : null, !hasUpdateItem ? styles.noUpdateItem : null]);
    const {
      url = ''
    } = newsItem.action;
    const title = this.generateTitleWithBadge(newsItem.title, newsItem.read);
    return <div className={componentClasses} role="presentation" onClick={this.newsItemClickHandler.bind(this)}>
        {title}
        ...
        <div className={styles.newsItemContentWrapper}>
          <AnimateHeight duration={expandWithoutTransition ? 0 : 500} height={this.state.newsItemExpanded ? 'auto' : 0}>
            <div className={styles.newsItemContentContainer}>
              <ReactMarkdown escapeHtml={false} source={newsItem.content} disallowedTypes={['image', 'imageReference', 'table', 'definition', 'inlineCode', 'code', 'html', 'virtualHtml']} />
            </div>
            <ButtonLink className={styles.newsItemActionBtn} onClick={e => this.onProceedNewsAction(e)} skin={ButtonSkin} label={newsItem.action.label} linkProps={{
