...
import { defineMessages, intlShape, injectIntl, FormattedHTMLMessage } from "react-intl";
...
const messages = defineMessages({
  ...
  walletsDropdownPlaceholder: {
    ...
    description: '"walletsDropdownPlaceholder" in the dApp transaction request dialog'
  },
  addWalletLabel: {
  ...
  insufficientBalanceErrorMessage: {
    ...
    defaultMessage: '!!!This wallet does not contain the minimum amount of {adaBalanceRequired} which is required for delegation to be available. Please select a wallet with <b>a minimum amount of {adaBalanceRequired}</b>.',
    description: '"Not enough ada" error in the dApp transaction request dialog'
  }
});
type Props = {
  adaAmount: BigNumber;
...
const DappTransactionRequest = observer((props: Props) => {
  const [isAdditionalDataVisible, toggleAdditionalData] = useState<boolean>(false);
  const [isMetadataVisible, toggleMetadata] = useState<boolean>(false);
  const getToggleLabel = useCallback((isVisible: boolean) => isVisible ? intl.formatMessage(globalMessages.hide) : intl.formatMessage(globalMessages.view));
  const {
    adaAmount,
    address,
    additionalData,
  ...
  const hasTokenError = useMemo(() => assets.reduce((result, token, index) => {
    if (!selectedWallet) return false;
    if (result) return true;
    return isTokenMissingInWallet(selectedWallet, token) || !tokenHasBalance(token, assetsAmounts[index]);
  }, false), [assets, selectedWallet]);
  const adaBalanceRequired = adaAmount.plus(transactionFee);
  const walletsDropdownHasError = selectedWallet?.amount.isLessThan(adaBalanceRequired);

  if (walletsDropdownHasError) {
    hasAmountError = true;
  }

  const adaAmountErrorMessage = walletsDropdownHasError ? <FormattedHTMLMessage {...messages.insufficientBalanceErrorMessage} values={{
    adaBalanceRequired: formattedWalletAmount(adaBalanceRequired)
  }} /> : null;
  const walletsDropdownStyles = classnames([styles.walletsDropdown, walletsDropdownHasError || hasTokenError ? styles.error : null, hasAmountError ? styles.amountError : null]);
  const canSubmit = !!selectedWallet && !hasTokenError && !walletsDropdownHasError;
  const actions = [{
  ...
  return <Dialog className={componentStyles} title={intl.formatMessage(messages.title)} subtitle={intl.formatMessage(messages.subtitle, {
    triggeredFrom
  })} actions={actions}>
      <p className={styles.label}>
        {intl.formatMessage(messages.fromWalletLabel)}
      </p>
      {wallets.length ? <WalletsDropdown className={walletsDropdownStyles} getStakePoolById={() => {}} numberOfStakePools={100} onChange={onSelectWallet} placeholder={intl.formatMessage(messages.walletsDropdownPlaceholder)} value={selectedWallet?.id} wallets={wallets} /> : <Select onChange={onAddWallet} placeholder={intl.formatMessage(messages.walletsDropdownPlaceholder)} value="" options={[{
      ...
      <AssetsTransactionConfirmation adaAmount={adaAmount} assets={assets} assetsAmounts={assetsAmounts} wallet={selectedWallet} adaError={adaAmountErrorMessage} />
      <p className={styles.label}>
      ...
      <p className={styles.labelData}>
        ...
        <button className={styles.toggleButton} onClick={() => toggleAdditionalData(!isAdditionalDataVisible)}>
          {getToggleLabel(isAdditionalDataVisible)}
        </button>
      </p>
      {isAdditionalDataVisible && <div className={styles.additionalData}>
          <pre>{additionalData}</pre>
        </div>}
      <p className={styles.labelData}>
        {intl.formatMessage(messages.metaDataLabel)}
        <button className={styles.toggleButton} onClick={() => toggleMetadata(!isMetadataVisible)}>
          {getToggleLabel(isMetadataVisible)}
        </button>
      </p>
      {isMetadataVisible && <div className={styles.metadata}>
          <pre>{metadata}</pre>
        </div>}
    </Dialog>;
});
