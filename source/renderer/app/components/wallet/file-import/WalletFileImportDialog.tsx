...
import { isValidWalletName // isValidSpendingPassword,
...
class WalletFileImportDialog extends Component<Props> {
    ...
    fields: {
      ...
      walletName: {
        ...
        validators: [({
          ...
          return [isValidWalletName(field.value), this.context.intl.formatMessage(globalMessages.invalidWalletName)];
        }]
      },
      spendingPassword: {
        type: 'password',
  ...
  render() {
    ...
    const dialogClasses = classnames([styles.component, 'WalletFileImportDialog']);
    const actions = [{
    ...
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.headline)} actions={actions} closeOnOverlayClick onClose={onClose} closeButton={<DialogCloseButton />}>
        <div className={styles.fileUpload}>
          <FileUploadWidget label={intl.formatMessage(messages.walletFileLabel)} placeholder={intl.formatMessage(messages.walletFileHint)} acceptedFileTypes={['*']} selectedFile={walletFilePath.value} onFileSelected={filePath => {
