...
const messages = defineMessages({
  ...
  description: {
    ...
    defaultMessage: '!!!The {walletName} wallet is not responding. This is caused by a known but rare issue, which is currently being fixed. Please restart the Cardano node by clicking the button below, which should resolve the issue. If the issue persists, or if it happens again, please submit a support request.',
  ...
  render() {
    ...
    return <div className={styles.component}>
        <div className={styles.content}>
          ...
          <Button className={styles.restartNodeButton} label={intl.formatMessage(messages.restartNodeButtonLabel)} onClick={onRestartNode} skin={ButtonSkin} />
          <Link className={styles.submitSupportLink} onClick={() => onOpenExternalLink(intl.formatMessage(messages.submitSupportRequestUrl))} label={intl.formatMessage(messages.submitSupportRequestLabel)} skin={LinkSkin} />
        </div>
      </div>;
  }
