...
const messages = defineMessages({
  ...
  currencyLastFetched: {
    ...
    description: '"Currency - last fetched" label on Wallet summary currency page'
  },
...
class WalletSummaryCurrency extends Component<Props> {
  ...
  render() {
    ...
    const hasCurrency = currencyIsActive && !!currencySelected && (!!currencyRate || currencyIsFetchingRate);
    const {
      decimalDigits
    } = currencySelected || {};
    let currencyWalletAmount;
    if (isRestoreActive || !currencyRate) currencyWalletAmount = '–';else if (hasCurrency && currencyRate) currencyWalletAmount = formattedWalletCurrencyAmount(wallet.amount, currencyRate, decimalDigits);
    const currencyWalletAmountSymbol = currencySelected && currencySelected.code ? currencySelected.code.toUpperCase() : '';
    const fetchedTimeAgo = moment(currencyLastFetched).locale(intl.locale).fromNow();
    const buttonStyles = classnames([styles.currencyLastFetched, currencyIsFetchingRate ? styles.currencyIsFetchingRate : null]);
    return <div className={styles.component}>
        ...
        <button className={buttonStyles} onClick={onCurrencySettingClick}>
          <em>
            {currencyIsFetchingRate ? intl.formatMessage(messages.currencyIsFetchingRate) : intl.formatMessage(messages.currencyLastFetched, {
            fetchedTimeAgo
          })}
          </em>
          <SVGInline svg={currencySettingsIcon} className={styles.currencySettingsIcon} />
        </button>
      </div>;
  }
