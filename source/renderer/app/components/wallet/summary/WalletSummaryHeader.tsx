...
const messages = defineMessages({
  ...
  pendingTransactionsLabel: {
    ...
    description: '"Number of pending transactions" label on Wallet summary header page'
  }
});
type Props = {
  wallet: Wallet;
...
class WalletSummaryHeader extends Component<Props> {
  ...
  render() {
    ...
    const numberOfTransactionsStyles = classnames([styles.numberOfTransactions, isLoadingAllTransactions ? styles.isLoadingNumberOfTransactions : null]);
    const numberOfPendingTransactionsStyles = classnames([styles.numberOfPendingTransactions]);
    const walletNameStyles = classnames([styles.walletName]);
    const walletAmountStyles = classnames([styles.walletAmount]);
    const isRestoreActive = wallet.isRestoring;
    const walletAmount = isRestoreActive ? '-' : formattedWalletAmount(wallet.amount, false);
        ...
        <BorderedBox>
          <div className={styles.walletContent}>
            <div>
              ...
              {!isLoadingTransactions && <div className={styles.transactionsCountWrapper}>
