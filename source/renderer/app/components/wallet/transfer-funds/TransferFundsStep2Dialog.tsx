...
class TransferFundsStep2Dialog extends Component<Props> {
  ...
  handleSubmitOnEnter = (event: KeyboardEvent) => this.form.$('spendingPassword').isValid && submitOnEnter(this.submit, event);
  ...
  render() {
    ...
    const leftovers = leftoversAmount && !leftoversAmount.isZero() ? leftoversAmount.toFormat(DECIMAL_PLACES_IN_ADA) : null;
    const totalAmount = sourceWalletAmount.minus(feesAmount).minus(leftoversAmount);
    const totalToBeReceived = formattedWalletAmount(totalAmount, false);
    const sourceWalletBalance = formattedWalletAmount(sourceWalletAmount, false);
    const spendingPasswordField = this.form.$('spendingPassword');
    const buttonClasses = classnames(['confirmButton', isSubmitting ? styles.submitButtonSpinning : null]);
    const actions = [{
    ...
    return <Dialog className={styles.dialog} title={intl.formatMessage(messages.dialogTitle)} actions={actions} closeOnOverlayClick onClose={onClose} closeButton={<DialogCloseButton />} backButton={<DialogBackButton onBack={onBack} />}>
        ...
        <div className={styles.amountGroupFull}>
          <p className={styles.label}>
            <FormattedMessage {...messages.sourceWalletAmountLabel} values={{
            sourceWalletName: <b key="source">{sourceWalletName}</b>
          }} />
          </p>
        ...
        {leftovers && <div className={styles.amountGroup}>
            <p className={styles.label}>
              {intl.formatMessage(messages.leftoversLabel)}
              <Link className={styles.leftoversLearnMoreLink} onClick={(event: React.MouseEvent<HTMLElement>) => onOpenExternalLink(intl.formatMessage(messages.leftoversLearnMoreUrl, event))} label={intl.formatMessage(messages.leftoversLearnMoreLabel)} skin={LinkSkin} />
            </p>
        ...
        <Input type="password" className={styles.currentPassword} {...spendingPasswordField.bind()} error={spendingPasswordField.error} skin={InputSkin} onKeyPress={this.handleSubmitOnEnter} autoFocus />
