...
export default class TransferFundsStep1Dialog extends Component<Props> {
  ...
  render() {
    ...
    return <Dialog title={intl.formatMessage(messages.dialogTitle)} actions={[{
      className: isSubmitting ? styles.isSubmitting : null,
      label: intl.formatMessage(error ? globalMessages.close : messages.buttonLabel),
      onClick: !isSubmitting ? onClick : () => null,
      primary: true,
      disabled: isSubmitting || !this.props.targetWalletId
    }]} closeOnOverlayClick onClose={onClose} closeButton={<DialogCloseButton />}>
        <p className={styles.label}>
          {intl.formatMessage(messages.sourceWallet)}
        </p>
        <div className={styles.sourceWallet}>
          <ItemDropdownOption label={sourceWallet.name} detail={formattedWalletAmount(sourceWallet.amount)} selected />
        </div>
        <WalletsDropdown label={intl.formatMessage(messages.targetWallet)} wallets={wallets} onChange={onSetSourceWallet} value={targetWalletId} numberOfStakePools={numberOfStakePools} getStakePoolById={getStakePoolById} />
