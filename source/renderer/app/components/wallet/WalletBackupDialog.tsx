...
class WalletBackupDialog extends Component<Props> {
  render() {
    ...
    if (currentStep === WALLET_BACKUP_STEPS.PRIVACY_WARNING) {
      return <WalletBackupPrivacyWarningDialog canPhraseBeShown={canPhraseBeShown} isPrivacyNoticeAccepted={isPrivacyNoticeAccepted} countdownRemaining={countdownRemaining} onAcceptPrivacyNotice={onAcceptPrivacyNotice} onCancelBackup={onCancelBackup} onContinue={onContinue} />;
    }

    if (currentStep === WALLET_BACKUP_STEPS.RECOVERY_PHRASE_DISPLAY) {
      return <WalletRecoveryPhraseDisplayDialog recoveryPhrase={recoveryPhrase} onStartWalletBackup={onStartWalletBackup} onCancelBackup={onCancelBackup} isSubmitting={isSubmitting} />;
    }

    if (currentStep === WALLET_BACKUP_STEPS.RECOVERY_PHRASE_ENTRY) {
      return <WalletRecoveryPhraseEntryDialog isTermOfflineAccepted={isTermOfflineAccepted} enteredPhrase={enteredPhrase} canFinishBackup={canFinishBackup} isTermRecoveryAccepted={isTermRecoveryAccepted} isValid={isValid} isSubmitting={isSubmitting} onAcceptTermOffline={onAcceptTermOffline} onAcceptTermRecovery={onAcceptTermRecovery} onUpdateVerificationPhrase={onUpdateVerificationPhrase} onCancelBackup={onCancelBackup} onFinishBackup={onFinishBackup} onRestartBackup={onRestartBackup} />;
    }
