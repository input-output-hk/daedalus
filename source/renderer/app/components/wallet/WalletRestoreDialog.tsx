...
import { isValidWalletName, isValidSpendingPassword, isValidRepeatPassword, errorOrIncompleteMarker, validateMnemonics } from "../../utils/validations";
import globalMessages from "../../i18n/global-messages";
import LocalizableError from "../../i18n/LocalizableError";
import { FORM_VALIDATION_DEBOUNCE_WAIT } from "../../config/timingConfig";
import styles from "./WalletRestoreDialog.scss";
import { submitOnEnter } from "../../utils/form";
import { WALLET_RESTORE_TYPES, RECOVERY_PHRASE_WORD_COUNT_OPTIONS } from "../../config/walletsConfig";
import { LEGACY_WALLET_RECOVERY_PHRASE_WORD_COUNT, WALLET_RECOVERY_PHRASE_WORD_COUNT, YOROI_WALLET_RECOVERY_PHRASE_WORD_COUNT } from "../../config/cryptoConfig";
...
const messages = defineMessages({
  ...
  recoveryPhraseTypeLabel: {
    ...
    description: 'Label for the recovery phrase type options on the wallet restore dialog.'
  },
  recoveryPhraseTypeOptionWord: {
    id: 'wallet.restore.dialog.recovery.phrase.type.word',
    defaultMessage: '!!! words',
    description: 'Word for the recovery phrase type on the wallet restore dialog.'
  },
  recoveryPhraseType15WordOption: {
    id: 'wallet.restore.dialog.recovery.phrase.type.15word.option',
    defaultMessage: '!!!Rewards wallet',
    description: 'Label for the recovery phrase type 15-word option on the wallet restore dialog.'
  },
  recoveryPhraseType12WordOption: {
    id: 'wallet.restore.dialog.recovery.phrase.type.12word.option',
    defaultMessage: '!!!Balance wallet',
    description: 'Label for the recovery phrase type 12-word option on the wallet restore dialog.'
  },
  ...
  passwordSectionDescription: {
    ...
    defaultMessage: '!!!Keep your wallet secure by setting the spending password',
  ...
  certificateTabTitle: {
    ...
    description: 'Tab title "Daedalus paper wallet" in the wallet restore dialog.'
  },
  yoroiTabTitle: {
  ...
  shieldedRecoveryPhraseInputHint: {
    ...
    defaultMessage: '!!!Enter your {numberOfWords}-word paper wallet recovery phrase',
    description: 'Hint "Enter your 27-word paper wallet recovery phrase." for the recovery phrase input on the wallet restore dialog.'
  },
  shieldedRecoveryPhraseInputPlaceholder: {
    ...
    description: 'Placeholder "Enter word #" for the recovery phrase input on the wallet restore dialog.'
  },
  restorePaperWalletButtonLabel: {
    id: 'wallet.restore.dialog.paper.wallet.button.label',
    defaultMessage: '!!!Restore paper wallet',
    description: 'Label for the "Restore paper wallet" button on the wallet restore dialog.'
  },
  passwordTooltip: {
...
class WalletRestoreDialog extends Component<Props, State> {
    ...
    fields: {
      ...
      recoveryPhrase: {
        ...
        validators: ({
          ...
          const expectedWordCount = RECOVERY_PHRASE_WORD_COUNT_OPTIONS[this.state.walletType];
          return validateMnemonics({
            requiredWords: expectedWordCount,
            providedWords: field.value,
            validator: providedWords => [// TODO: we should also validate paper wallets mnemonics here!
            !this.isCertificate() ? this.props.mnemonicValidator(providedWords, expectedWordCount) : true, this.context.intl.formatMessage(messages.invalidRecoveryPhrase)]
          });
        }
      },
      spendingPassword: {
        ...
        validators: [({
          ...
          return [isValidSpendingPassword(field.value), this.context.intl.formatMessage(globalMessages.invalidSpendingPassword)];
        }]
      },
      repeatPassword: {
        type: 'password',
        ...
        validators: [({
          ...
          return [isValidRepeatPassword(spendingPassword, field.value), this.context.intl.formatMessage(globalMessages.invalidRepeatPassword)];
        }]
      }
    }
  }, {
  ...
  submit = () => {
    this.form.submit({
      ...
      onError: () => handleFormErrors('.SimpleFormField_error', {
        focusElement: true
      })
    });
  };
  ...
  render() {
    ...
    const dialogClasses = classnames([styles.component, styles.dialogWithCertificateRestore, 'WalletRestoreDialog']);
    const walletNameFieldClasses = classnames(['walletName', styles.walletName]);
    const walletNameField = form.$('walletName');
    const recoveryPhraseField = form.$('recoveryPhrase');
    const spendingPasswordField = form.$('spendingPassword');
    const repeatedPasswordField = form.$('repeatPassword');
    const label = this.isCertificate() ? this.context.intl.formatMessage(messages.restorePaperWalletButtonLabel) : this.context.intl.formatMessage(messages.importButtonLabel);
    ...
    const regularTabClasses = classnames(['regularTab', this.isRegular() || this.isLegacy() ? styles.activeButton : '']);
    const certificateTabClasses = classnames(['certificateTab', this.isCertificate() ? styles.activeButton : '']);
    const yoroiTabClasses = classnames(['yoroiTab', this.isYoroi() ? styles.activeButton : '']);
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.title)} actions={actions} closeOnOverlayClick onClose={onCancel} closeButton={<DialogCloseButton />}>
        <div className={styles.restoreTypeChoice}>
          <button className={regularTabClasses} onClick={() => this.onSelectWalletType(WALLET_RESTORE_TYPES.LEGACY, true)}>
            {intl.formatMessage(messages.recoveryPhraseTabTitle)}
          </button>
          <button className={certificateTabClasses} onClick={() => this.onSelectWalletType(WALLET_RESTORE_TYPES.CERTIFICATE, true)}>
            {intl.formatMessage(messages.certificateTabTitle)}
          </button>
          <button className={yoroiTabClasses} onClick={() => this.onSelectWalletType(WALLET_RESTORE_TYPES.YOROI_LEGACY, true)}>
            {intl.formatMessage(messages.yoroiTabTitle)}
          </button>
        </div>

        <Input className={walletNameFieldClasses} onKeyPress={this.handleSubmitOnEnter} {...walletNameField.bind()} error={walletNameField.error} />

        {(this.isRegular() || this.isLegacy()) && <RadioSet label={intl.formatMessage(messages.recoveryPhraseTypeLabel)} items={[{
        ...
        onChange: () => this.onSelectWalletType(WALLET_RESTORE_TYPES.LEGACY)
      }, {
        ...
        selected: !this.isLegacy(),
        onChange: () => this.onSelectWalletType(WALLET_RESTORE_TYPES.REGULAR)
      }]} />}

        {this.isYoroi() && <RadioSet label={intl.formatMessage(messages.recoveryPhraseTypeLabel)} items={[{
        ...
        onChange: () => this.onSelectWalletType(WALLET_RESTORE_TYPES.YOROI_LEGACY)
      }, {
        ...
        onChange: () => this.onSelectWalletType(WALLET_RESTORE_TYPES.YOROI_REGULAR)
      }]} />}

        <Autocomplete {...recoveryPhraseField.bind()} ref={autocomplete => {
        this.recoveryPhraseAutocomplete = autocomplete;
      }} label={!this.isCertificate() ? intl.formatMessage(messages.recoveryPhraseInputLabel) : intl.formatMessage(messages.shieldedRecoveryPhraseInputLabel)} placeholder={!this.isCertificate() ? intl.formatMessage(messages.recoveryPhraseInputHint) : intl.formatMessage(messages.shieldedRecoveryPhraseInputPlaceholder, {
        wordNumber: recoveryPhraseField.value.length + 1
      })} options={suggestedMnemonics} requiredSelections={[RECOVERY_PHRASE_WORD_COUNT_OPTIONS[walletType]]} requiredSelectionsInfo={(required, actual) => intl.formatMessage(globalMessages.knownMnemonicWordCount, {
        actual,
        required
      })} maxSelections={RECOVERY_PHRASE_WORD_COUNT_OPTIONS[walletType]} error={errorOrIncompleteMarker(recoveryPhraseField.error)} maxVisibleOptions={5} noResultsMessage={intl.formatMessage(messages.recoveryPhraseNoResults)} optionHeight={50} />
        ...
        <div className={styles.spendingPasswordWrapper}>
          ...
          <div className={styles.spendingPasswordFields}>
            <div className={styles.spendingPasswordField}>
              <PasswordInput className="spendingPassword" onKeyPress={this.handleSubmitOnEnter} {...spendingPasswordField.bind()} />
              <PopOver content={<FormattedHTMLMessage {...messages.passwordTooltip} />} key="tooltip">
                <SVGInline svg={infoIconInline} className={styles.infoIcon} />
              </PopOver>
            </div>
            <div className={styles.spendingPasswordField}>
              <PasswordInput className="repeatedPassword" onKeyPress={this.handleSubmitOnEnter} {...repeatedPasswordField.bind()} repeatPassword={spendingPasswordField.value} isPasswordRepeat />
            </div>
          </div>
