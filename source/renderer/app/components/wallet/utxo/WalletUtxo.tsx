...
import { BarChart, CartesianGrid, XAxis, YAxis, Tooltip, Label, Bar, ResponsiveContainer } from "recharts";
...
export const messages = defineMessages({
  ...
  findOutMoreLinkUrl: {
    ...
    defaultMessage: '!!!https://iohk.zendesk.com/hc/en-us/articles/360034118013',
  ...
  pendingTransactions: {
    ...
    defaultMessage: '!!!<b>Pending transactions</b> may affect the accuracy of data presented here. <br /> You have <b>{pendingTxnsCount}</b> pending transaction{txnsPlural}.',
...
export default class WalletUtxo extends Component<Props, State> {
  ...
  renderPendingTxns = (pendingTxnsCount: number) => {
    ...
    return <div className={styles.pendingTxnsWrapper}>
        <div>
          <p>
            <FormattedHTMLMessage {...messages.pendingTransactions} values={{
            pendingTxnsCount,
            txnsPlural
          }} />
          </p>
  ...
  render() {
    ...
    const componentStyles = classnames([styles.component, isEmpty ? styles.isEmpty : null]);
    const findOutMoreLinkUrl = intl.formatMessage(messages.findOutMoreLinkUrl);
    const findOutMoreLink = <Link className={styles.externalLink} onClick={event => onExternalLinkClick(findOutMoreLinkUrl, event)} label={intl.formatMessage(messages.findOutMoreLink)} skin={LinkSkin} />;
    const emptyOrLoadingState = isLoadingInitialUtxoData ? <LoadingSpinner /> : <p>{intl.formatMessage(messages.emptyWallet)}</p>;
    return <div className={componentStyles}>
        <BorderedBox className={styles.borderedBox}>
          <div className={styles.container} onMouseEnter={() => this.setState({
          ...
          isHoveringChart: false
            ...
            {!isLoadingInitialUtxoData && !isEmpty ? <Fragment>
                <p>
                ...
                <div className={styles.responsiveContainerWrapper}>
                  <ResponsiveContainer height={280} className={styles.responsiveContainer}>
                    <BarChart data={chartData} barSize={23}>
                      <CartesianGrid className={styles.cartesianGrid} horizontal={false} vertical={false} y={-10} height={255} fill="transparent" />
                      <XAxis dataKey="walletAmount" interval={0} axisLine={false} tickLine={false} tick={(props: TickProps) => <Tick {...props} textAnchor="start" vertical />} className={styles.xAxis} y={0}>
                        <Label value={intl.formatMessage(messages.labelX)} offset={20} position="insideBottomRight" className={styles.xAxisLabel} />
                      </XAxis>
                      <YAxis dataKey="walletUtxosAmount" axisLine={false} tickLine={false} allowDecimals={false} domain={[0, 'dataMax']} tick={(props: TickProps) => <Tick {...props} textAnchor="end" />}>
                        <Label value={intl.formatMessage(messages.labelY)} offset={0} position="insideTopLeft" className={styles.yAxisLabel} />
                      </YAxis>
                      <Tooltip cursor={<Cursor offsetWidth={28} />} isAnimationActive={false} content={<CustomTooltip />} />
                      <Bar dataKey="walletUtxosAmount" className={styles.bar} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
                {this.renderPendingTxns(pendingTxnsCount)}
              </Fragment> : emptyOrLoadingState}
          </div>
        </BorderedBox>
      </div>;
  }
