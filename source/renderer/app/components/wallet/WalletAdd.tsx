...
const messages = defineMessages({
  ...
  createDescription: {
    ...
    description: 'Description for the "Create a new wallet" button on the wallet add dialog.'
  },
  joinLabel: {
  ...
  restoreWithCertificateDescription: {
    ...
    defaultMessage: '!!!Restore a wallet or paper wallet using wallet recovery phrase',
    ...
    return <div className={componentClasses}>
        <div className={styles.buttonsContainer}>
          <div className={styles.firstRow}>
            <BigButtonForDialogs className="createWalletButton" onClick={onCreate} icon={createIcon} label={intl.formatMessage(messages.createLabel)} description={intl.formatMessage(messages.createDescription)} isDisabled={isMaxNumberOfWalletsReached} />
            <BigButtonForDialogs className="connectWalletButton" onClick={onConnect} icon={connectIcon} label={intl.formatMessage(messages.connectLabel)} description={intl.formatMessage(messages.connectDescription)} isDisabled={isMaxNumberOfWalletsReached || !isHardwareWalletSupportEnabled} />
          </div>
          <div className={styles.secondRow}>
            <BigButtonForDialogs className="restoreWalletButton" onClick={onRestore} icon={restoreIcon} label={intl.formatMessage(messages.restoreLabel)} description={intl.formatMessage(messages.restoreWithCertificateDescription)} isDisabled={isMaxNumberOfWalletsReached} />
            <BigButtonForDialogs className="importWalletButton" onClick={onImport} icon={importIcon} label={intl.formatMessage(messages.importLabel)} description={intl.formatMessage(messages.importDescription)} isDisabled={isMaxNumberOfWalletsReached || isProduction && !(isMainnet || isTestnet)} />
          </div>
          {activeNotification ? <div className={styles.notification}>
              <FormattedHTMLMessage {...messages[activeNotification]} values={{
