...
const messages = defineMessages({
  ...
  settingsWarningPopOverAvailable: {
    ...
    defaultMessage: '!!!Recommended configuration for decimal places for this native token is available.',
    description: 'Asset settings recommended pop over content'
  },
  settingsWarningPopOverNotUsing: {
    id: 'assets.warning.notUsing',
    defaultMessage: '!!!You are not using the recommended decimal place configuration for this native token.',
    description: 'Asset settings recommended pop over content'
  }
});
type Props = {
...
const WalletToken = observer((props: Props) => {
  ...
  const favoriteIconStyles = classNames([styles.favoriteIcon, props.isFavorite ? styles.isFavorite : null]);
  const {
    anyAssetWasHovered,
    asset,
  ...
  const assetHeaderContent = useMemo(() => {
    ...
    const hasWarning = typeof recommendedDecimals === 'number' && decimals !== recommendedDecimals;
    return <>
        <Asset asset={asset} onCopyAssetParam={onCopyAssetParam} metadataNameChars={get('name', asset.metadata, 0)} assetSettingsDialogWasOpened={assetSettingsDialogWasOpened} anyAssetWasHovered={anyAssetWasHovered} className={styles.asset} hidePopOver fullFingerprint hasWarning={hasWarning} />
        <AssetAmount amount={asset.quantity} metadata={asset.metadata} decimals={asset.decimals} isLoading={isLoading} className={styles.assetAmount} isShort />
      </>;
  }, [anyAssetWasHovered, asset, assetSettingsDialogWasOpened, isFavorite, isLoading, onCopyAssetParam, onToggleFavorite]);
  useEffect(() => setArrowStyles(classNames(styles.arrow, {
  ...
  const header = useMemo(() => {
    ...
    return <div className={styles.header} onClick={toggleIsExpanded}>
        <button className={favoriteIconStyles} onClick={event => {
        event.persist();
  ...
  const footer = useMemo(() => {
    return <div className={styles.footer}>
        <dl>
          ...
          <dd>
            {' '}
            <AssetAmount amount={asset.quantity} metadata={asset.metadata} decimals={asset.decimals} isLoading={isLoading} className={styles.assetAmount} />
          </dd>
        </dl>
        {buttons}
      </div>;
  ...
  const buttons = useMemo(() => {
    ...
    if (hasWarning) {
      warningPopOverMessage = typeof decimals === 'number' ? messages.settingsWarningPopOverNotUsing : messages.settingsWarningPopOverAvailable;
      settingsButtonLabel = <>
          {settingsButtonLabel}
    ...
    const settingsButton = <Button className={classNames(['flat', styles.button, styles.settingsButton])} label={settingsButtonLabel} onClick={() => onAssetSettings({
      asset
    })} />;
    return <div className={styles.footerButtons}>
        {hasWarning ? <PopOver content={intl.formatMessage(warningPopOverMessage, {
        recommendedDecimals
      })} className={styles.warningIconWrapper}>
            {settingsButton}
          </PopOver> : settingsButton}
        <Button className={classNames(['primary', styles.button, asset.quantity.isZero() ? styles.disabled : null])} onClick={() => onOpenAssetSend(asset)} label={intl.formatMessage(messages.tokenSendButton)} disabled={asset.quantity.isZero()} />
      </div>;
  ...
  return <div className={componentStyles}>
      ...
      <div className={styles.content}>
        <AssetContent asset={asset} onCopyAssetParam={onCopyAssetParam} highlightFingerprint={false} />
        {footer}
      </div>
    </div>;
});
