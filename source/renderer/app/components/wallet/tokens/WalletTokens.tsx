...
const messages = defineMessages({
  ...
  syncingMessage: {
    ...
    defaultMessage: '!!!The balance and transaction history of this wallet is being synced with the blockchain.',
...
const WalletTokens = observer((props: Props) => {
  ...
  const [insertingAssetUniqueId, setInsertingAssetUniqueId] = useState<string | null | undefined>(null);
  ...
  const favoriteTokensList = useMemo(() => assets.filter(({
    uniqueId
  }) => tokenFavorites[uniqueId]), [assets, tokenFavorites, searchValue]);
  ...
  if (isRestoring) {
    return <div className={styles.syncing}>
        <LoadingSpinner big />
  ...
  return <div className={styles.component}>
      {hasTokens && <WalletTokensSearch searchValue={searchValue} onSearch={setSearchValue} />}
      {!!favoriteTokensList.length && <WalletTokensList {...listProps} assets={favoriteTokensList} insertingAssetUniqueId={insertingAssetUniqueId} onToggleFavorite={handleToggleFavorite} removingAssetUniqueId={removingAssetUniqueId} searchValue={searchValue} title={intl.formatMessage(messages.favoritesListTitle)} tokenFavorites={tokenFavorites} />}
      <WalletTokensList {...listProps} assets={assets} onToggleFavorite={handleToggleFavorite} searchValue={searchValue} title={intl.formatMessage(messages.tokensListTitle)} tokenFavorites={tokenFavorites} />
    </div>;
});
