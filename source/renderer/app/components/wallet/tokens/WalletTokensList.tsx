...
const getSortIconClasses = (item: SortBy, sortBy: SortBy, sortDirection: SortDirection) => {
  const isSorted = item === sortBy;
  return classnames([styles.sortIcon, isSorted ? styles.sorted : null, isSorted ? styles[`${sortDirection}Sorting`] : styles.ascSorting]);
};
...
const WalletTokensList = observer((props: Props) => {
  ...
  const hasSearch = !isLoadingAssets && !!searchValue && searchValue.trim().length >= 3;
  ...
  const columnsStyles = classnames([styles.columns, hasSorting ? styles.hasSorting : null]);
  const sortIconClassesToken = useMemo(() => getSortIconClasses('token', sortBy, sortDirection), [sortBy, sortDirection]);
  const sortIconClassesAmount = useMemo(() => getSortIconClasses('quantity', sortBy, sortDirection), [sortBy, sortDirection]);
  ...
  const onSortBy = useCallback((newSortBy: SortBy) => {
  ...
  const onSortByToken = useCallback(() => onSortBy('token'), [sortDirection, sortBy, hasSorting]);
  const onSortByAmount = useCallback(() => onSortBy('quantity'), [sortDirection, sortBy, hasSorting]);
  const hasTokens = assets.length || isLoadingAssets;
  if (!hasTokens) return <WalletNoTokens numberOfAssets={assets.length} isLoadingAssets={isLoadingAssets} onExternalLinkClick={onExternalLinkClick} />;
  let content;

  if (isLoadingAssets) {
    content = <div className={styles.syncingWrapper}>
        <LoadingSpinner big />
      </div>;
  } else if (noResults) {
    content = <p className={styles.noResults}>
        {intl.formatMessage(messages.noResults)}
      </p>;
  } else {
    content = filteredAssets.map(asset => {
      return <WalletToken key={asset.uniqueId} asset={asset} onOpenAssetSend={onOpenAssetSend} onCopyAssetParam={onCopyAssetParam} onAssetSettings={onAssetSettings} anyAssetWasHovered isLoading={isRestoreActive} assetSettingsDialogWasOpened={assetSettingsDialogWasOpened} onToggleFavorite={onToggleFavorite} isFavorite={tokenFavorites[asset.uniqueId]} isInsertingAsset={insertingAssetUniqueId === asset.uniqueId} isRemovingAsset={removingAssetUniqueId === asset.uniqueId} />;
    });
  }
  ...
  return <div className={styles.component}>
      ...
      <BorderedBox>
        ...
        {content}
        {onViewAllButtonClick && <Button className={viewAllButtonStyles} onClick={onViewAllButtonClick} label={intl.formatMessage(messages.viewAllButtonLabel)} />}
      </BorderedBox>
    </div>;
});
