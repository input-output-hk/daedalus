...
class AssetInput extends Component<Props> {
  ...
  generateInputFieldStyle = () => {
    const {
      current: rightContentDom
    } = this.rightContentRef;
    ...
    return {
      paddingRight: `${rightContentDomRect.width + INPUT_FIELD_PADDING_DELTA}px`
    };
  };
  ...
  render() {
    ...
    const sortedAssets = orderBy([asset, ...availableAssets], 'uniqueId', 'asc');
    ...
    return <div key={`receiver_asset_${uniqueId}`} onMouseOver={() => showRemoveAssetButton(uniqueId)} onMouseLeave={() => hideRemoveAssetButton(uniqueId)} onMouseEnter={() => showRemoveAssetButton(uniqueId)} onFocus={() => {// jsx-a11y/mouse-events-have-key-events
    }} className={styles.component}>
        {quantity.isPositive() && <div className={styles.amountTokenTotal}>
            {intl.formatMessage(messages.ofLabel)}
            {` `}
            {formattedTokenWalletAmount(quantity, metadata, decimals)}
          </div>}
        <NumericInput {...assetField.bind()} ref={field => addFocusableField(field)} placeholder={decimals ? `0${currentNumberFormat.decimalSeparator}${'0'.repeat(decimals)}` : '0'} className={styles.assetItem} label={<>
              {`${intl.formatMessage(messages.assetLabel)} #${index + 1}`}
              {removeAssetButtonVisible[uniqueId] && <span className={classNames([styles.removeAssetButton, 'flat'])} onClick={() => removeAssetRow(uniqueId)}>
                  {intl.formatMessage(messages.removeLabel)}
                </span>}
            </>} bigNumberFormat={decimals ? currentNumberFormat : null} decimalPlaces={decimals} numberLocaleOptions={{
        minimumFractionDigits: decimals
      }} onChange={value => {
        assetField.onChange(value);
      }} currency={ticker} value={assetField.value} error={assetField.error} skin={AmountInputSkin} style={inputFieldStyle} onKeyPress={(evt: React.KeyboardEvent<EventTarget>) => {
        if (decimals === 0) {
          const {
            charCode
          } = evt;
      ...
      }} allowSigns={false} autoFocus={autoFocus} />
        <div className={styles.rightContent} ref={this.rightContentRef}>
          {this.hasAssetValue(assetField) && <div className={styles.clearAssetContainer}>
              <PopOver content={intl.formatMessage(messages.clearLabel)} placement="top">
                <button onClick={() => clearAssetFieldValue(assetField)} className={styles.clearAssetButton}>
          ...
          <div className={styles.assetsDropdownWrapper}>
            <AssetsDropdown className={styles.assetsDropdown} {...assetsDropdownField.bind()} assets={sortedAssets} onChange={onChangeAsset} value={uniqueId} hasSearch />
          </div>
        </div>
      </div>;
  }
