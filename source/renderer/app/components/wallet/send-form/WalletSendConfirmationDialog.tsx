...
class WalletSendConfirmationDialog extends Component<Props, State> {
  ...
  renderConfirmationElement = (isHardwareWallet: boolean): React.ReactElement<React.ComponentProps<any>, any> | null | undefined => {
    ...
    if (!isFlight || isFlight && areTermsAccepted) {
      ...
      return isHardwareWallet ? <div className={styles.hardwareWalletStatusWrapper}>
  ...
  render() {
    ...
    const actions = [{
      ...
      disabled: !isHardwareWallet && !passphraseField.isValid || isHardwareWallet && hwDeviceStatus !== HwDeviceStatuses.VERIFYING_TRANSACTION_SUCCEEDED || !areTermsAccepted && isFlight
    }];
    let errorElement = null;

    if (error) {
      const errorHasLink = !!error.values.linkLabel;
      errorElement = errorHasLink ? <FormattedHTMLMessageWithLink message={error} onExternalLinkClick={onExternalLinkClick} /> : intl.formatMessage(error);
    }
    ...
    return <Dialog title={intl.formatMessage(messages.dialogTitle)} subtitle={name} actions={actions} closeOnOverlayClick primaryButtonAutoFocus onClose={!isSubmitting ? onCancel : () => {}} className={styles.dialog} closeButton={<DialogCloseButton />}>
        {shouldShowEmptyWalletWarning(totalAmount, wallet, !!allAvailableTokens?.length && allAvailableTokens.length > 0) && <div className={styles.warning}>
