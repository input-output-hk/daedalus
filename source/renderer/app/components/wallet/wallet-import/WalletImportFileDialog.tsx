...
const messages = defineMessages({
  ...
  secretFileNoWallets: {
    ...
    defaultMessage: '!!!No wallets found. Make sure you have selected a valid `secret.key` file.',
    description: 'No wallets found. Make sure you have selected a valid `secret.key` file.'
  },
  linkLabel: {
...
class WalletImportFileDialog extends Component<Props, State> {
  ...
  isImportFromStateDir = (importFrom: ImportFromOption) => importFrom === ImportFromOptions.STATE_DIR;
  isImportFromSecretFile = (importFrom: ImportFromOption) => importFrom === ImportFromOptions.SECRET_FILE;
  ...
  render() {
    ...
    const noWalletError = intl.formatMessage(messages[`${importFrom}NoWallets`]);

    const onLinkClick = () => onOpenExternalLink(intl.formatMessage(messages.linkUrl));

    const error = exportErrors !== '';
    const inputClasses = classNames([styles.stateFolderInput, error ? styles.error : null]);
    const buttonClasses = classNames(styles.actionButton, [isSubmitting || error || this.isImportFromSecretFile(importFrom) && !exportSourcePath ? styles.disabled : null]);
    return <Dialog className={styles.dialog} closeOnOverlayClick={false} onClose={onClose} onRequestClose={onClose} shouldCloseOnOverlayClick={false} shouldCloseOnEsc={false} ariaHideApp={false} defaultThemeOverrides>
        <div className={styles.component}>
          ...
          <div className={styles.content}>
            ...
            <div className={styles.radioButtons}>
              <RadioSet label={intl.formatMessage(messages.importFromLabel)} items={Object.keys(ImportFromOptions).map((key: string) => {
              const importFromOption: ImportFromOption = ImportFromOptions[key];
              return {
                key: importFromOption,
                label: intl.formatMessage(messages[`${importFromOption}OptionLabel`]),
                selected: importFrom === importFromOption,
                onChange: () => this.onSetImportFromOption(importFromOption)
              };
            })} verticallyAligned />
            </div>
            <div className={styles.stateFolderContainer}>
              ...
              <div className={styles.stateFolderInputWrapper}>
                <Input type="text" className={inputClasses} ref={input => {
                this.importPathInput = input;
              }} skin={InputSkin} value={exportSourcePath || (this.isImportFromStateDir(importFrom) ? defaultExportSourcePath : '')} placeholder={this.isImportFromStateDir(importFrom) ? defaultExportSourcePath : 'secret.key'} readOnly />
                <Button className={styles.selectStateDirectoryButton} onClick={() => onSelectExportSourcePath({
                importFrom
              })} label={<SVGInline svg={penIcon} className={styles.penIcon} />} skin={ButtonSkin} />
              </div>
              {error && <p className={styles.noWalletError}>{noWalletError}</p>}
            </div>
            <div className={styles.action}>
              <Button className={buttonClasses} disabled={isSubmitting || error || this.isImportFromSecretFile(importFrom) && !exportSourcePath} label={buttonLabel} onClick={onContinue} skin={ButtonSkin} />
            </div>
