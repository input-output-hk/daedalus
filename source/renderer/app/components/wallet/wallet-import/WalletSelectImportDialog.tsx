...
const messages = defineMessages({
  ...
  description: {
    ...
    defaultMessage: '!!!These wallets were found in your Daedalus state directory.<p>Please select the wallets you want to import.</p>',
  ...
  maxWalletsReachedTooltip: {
    ...
    defaultMessage: '!!!Daedalus supports up to {maxWalletsCount} wallets. You will need to remove another wallet before you can import this one.',
  ...
  linkUrl: {
    ...
    defaultMessage: '!!!https://iohk.zendesk.com/hc/en-us/articles/900000623463',
...
class WalletSelectImportDialog extends Component<Props> {
  ...
  getWalletStatusIcon = (wallet: ExportedByronWallet, index: number) => {
    ...
    if (wallet.import.status === WalletImportStatuses.UNSTARTED || wallet.import.status === WalletImportStatuses.PENDING || wallet.import.status === WalletImportStatuses.ERRORED) {
      const invalidWalletName = wallet.name === null || !nameValidator(wallet.name);
      const walletNotSelectable = isMaxNumberOfWalletsReached && wallet.import.status !== WalletImportStatuses.PENDING;
      const disabled = invalidWalletName || walletNotSelectable;
      let isOpeningUpward = true;
      const checkboxes = document.getElementsByClassName('SimpleCheckbox_check');

      if (checkboxes.length && wallet.hasName) {
        const topWrapper = document.getElementsByClassName('WalletSelectImportDialog_topWrapper');
        ...
        if (checkboxes[index] && topWrapper.length) {
          ...
          if (walletNotSelectable && spaceForTooltip < 85 || invalidWalletName && spaceForTooltip < 27) {
            isOpeningUpward = false;
          }
        }
      }

      statusIcon = <Checkbox onChange={() => {
        onToggleWalletImportSelection({
          index: wallet.index
        });
      }} checked={wallet.import.status === WalletImportStatuses.PENDING} disabled={disabled} skin={CheckboxSkin} />;

      if (disabled) {
        statusIcon = <PopOver content={invalidWalletName ? this.context.intl.formatMessage(messages.enterWalletNameTooltip) : <FormattedMessage {...messages.maxWalletsReachedTooltip} values={{
          maxWalletsCount: MAX_ADA_WALLETS_COUNT
        }} />} isBounded={walletNotSelectable} placement={isOpeningUpward ? 'top' : 'bottom'}>
            {statusIcon}
          </PopOver>;
      }
    } else if (wallet.import.status === WalletImportStatuses.RUNNING) {
      statusIcon = <LoadingSpinner medium />;
    } else if (wallet.import.status === WalletImportStatuses.COMPLETED || wallet.import.status === WalletImportStatuses.EXISTS) {
      statusIcon = <SVGInline svg={checkmarkImage} className={styles.walletsStatusIconCheckmark} />;
    }

    return statusIcon;
  };
  getInlineEditingSmallInput = (wallet: ExportedByronWallet, validationMessage: string, placeholderMessage: string, nameValidator: (...args: Array<any>) => any, onWalletNameChange: (...args: Array<any>) => any) => {
    return <InlineEditingSmallInput isActive={false} className={styles.walletsInputFieldInner} isDisabled={wallet.import.status === WalletImportStatuses.COMPLETED || wallet.import.status === WalletImportStatuses.EXISTS || wallet.import.status === WalletImportStatuses.RUNNING} inputFieldValue={wallet.name || ''} placeholder={placeholderMessage} isValid={nameValidator} validationErrorMessage={validationMessage} onSubmit={(name: string) => onWalletNameChange({
      index: wallet.index,
      name
    })} maxLength={40} successfullyUpdated />;
  };
  ...
  render() {
    ...
    const buttonLabel = !isSubmitting ? intl.formatMessage(messages.buttonLabel) : <LoadingSpinner />;
    const linkLabel = intl.formatMessage(messages.linkLabel);

    const onLinkClick = () => onOpenExternalLink(intl.formatMessage(messages.linkUrl));

    const walletsWithNames = exportedWallets.filter(({
      hasName
    }: ExportedByronWallet) => hasName);
    const walletsWithoutNames = exportedWallets.filter(({
      hasName
    }: ExportedByronWallet) => !hasName);
    let previousWalletId = '';
    let rowNumber = 1;
    const anyWalletWithoutName = walletsWithoutNames.filter(item => (!item.name || item.name.length < 3) && item.import.status === WalletImportStatuses.PENDING);
    const isDisabled = isSubmitting || anyWalletWithoutName.length || !pendingImportWalletsCount;
    const buttonClasses = classNames(styles.actionButton, [isDisabled ? styles.disabled : null]);
    return <Dialog className={styles.dialog} closeOnOverlayClick={false} onClose={onClose} onRequestClose={onClose} shouldCloseOnOverlayClick={false} shouldCloseOnEsc={false} defaultThemeOverrides>
        <div className={styles.component}>
          <DialogCloseButton className={styles.closeButton} icon={closeCrossThin} onClose={onClose} />
          <div className={styles.content}>
            ...
            <div className={styles.walletsContainer}>
              ...
              const rowClasses = classNames([styles.walletsRow, 'namedWalletsRow']);
              const walletRow = <div className={rowClasses} key={`${wallet.id}-${wallet.index}`}>
                    <div className={styles.walletsCounter}>
                      {!isDuplicate && `${rowNumber}.`}
                    </div>
                    <div className={styles.walletsInputField}>
                      {this.getInlineEditingSmallInput(wallet, intl.formatMessage(globalMessages.invalidWalletName), intl.formatMessage(messages.walletName), nameValidator, onWalletNameChange)}
                    </div>
              ...
              {!!walletsWithoutNames.length && <div className={styles.unamedWalletsTitle}>
                  {!!walletsWithNames.length && <hr className={styles.separatorMiddle} />}
                  <p>{intl.formatMessage(messages.unamedWalletsTitle)}</p>
                </div>}

              {walletsWithoutNames.map((wallet, index) => {
              const isDuplicate = previousWalletId === wallet.id;
              const rowClasses = classNames([styles.walletsRow, 'unnamedWalletsRow']);
              const walletRow = <div className={rowClasses} key={`${wallet.id}-${wallet.index}`}>
                    <div className={styles.walletsCounter}>
                      {!isDuplicate && `${rowNumber}.`}
                    </div>
                    <div className={styles.walletsInputField}>
                      {!wallet.name ? <PopOver content={intl.formatMessage(messages.enterWalletNameTooltip)}>
                          {this.getInlineEditingSmallInput(wallet, intl.formatMessage(globalMessages.invalidWalletName), intl.formatMessage(messages.notFound), nameValidator, onWalletNameChange)}
                        </PopOver> : <>
                          {this.getInlineEditingSmallInput(wallet, intl.formatMessage(globalMessages.invalidWalletName), intl.formatMessage(messages.notFound), nameValidator, onWalletNameChange)}
                        </>}
                    </div>
            ...
            <div className={styles.action}>
              <Button className={buttonClasses} disabled={isDisabled} label={buttonLabel} onClick={onContinue} skin={ButtonSkin} />
              <div>
                <Link className={styles.learnMoreLink} onClick={onLinkClick} label={linkLabel} skin={LinkSkin} />

                <Link className={styles.closeWindowLink} onClick={onClose} label={intl.formatMessage(messages.closeWindow)} skin={LinkSkin} hasIconAfter={false} />
              </div>
            </div>
          </div>
        </div>
