...
const messages = defineMessages({
  ...
  autocompleteNoResults: {
    ...
    description: '"No results" message for the mnemonics autocomplete search results.'
  },
  continueButtonLabel: {
...
class MnemonicsDialog extends Component<Props> {
    ...
    fields: {
      recoveryPhrase: {
        ...
        validators: ({
          ...
          validator: enteredWords => [this.props.onValidateMnemonics(enteredWords), this.context.intl.formatMessage(messages.invalidRecoveryPhrase)]
        })
      }
    }
  }, {
  ...
  render() {
    ...
    return <WalletRestoreDialog stepNumber={1} actions={[{
      primary: true,
      disabled: !canSubmit,
      ...
      onClick: this.submit
        ...
        <div>
          <Autocomplete {...recoveryPhraseField.bind()} label={intl.formatMessage(globalMessages.recoveryPhraseDialogTitle)} placeholder={Array.isArray(expectedWordCount) ? intl.formatMessage(messages.autocompleteMultiLengthPhrase) : intl.formatMessage(messages.autocompletePlaceholder, {
          wordNumber: mnemonics.length + 1
        })} options={validWords} requiredSelections={Array.isArray(expectedWordCount) ? expectedWordCount : [expectedWordCount]} requiredSelectionsInfo={(required, actual) => intl.formatMessage(globalMessages.knownMnemonicWordCount, {
          actual,
          required
        })} maxSelections={maxWordCount} error={errorOrIncompleteMarker(recoveryPhraseField.error)} maxVisibleOptions={5} noResultsMessage={intl.formatMessage(messages.autocompleteNoResults)} skin={AutocompleteSkin} onChange={enteredMnemonics => {
          recoveryPhraseField.set(enteredMnemonics);
          onSetWalletMnemonics(enteredMnemonics);
        }} preselectedOptions={[...mnemonics]} optionHeight={50} />
          <div className="error" />
        </div>
      </WalletRestoreDialog>;
  }
