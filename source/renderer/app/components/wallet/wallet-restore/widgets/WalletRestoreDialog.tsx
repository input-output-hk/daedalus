...
export default class WalletRestoreDialog extends Component<Props> {
  ...
  render() {
    ...
    const title = hasStep ? intl.formatMessage(messages.dialogTitle) : intl.formatMessage(messages.dialogTitleSuccess);
    const currentStep = (stepNumber || 0) + 1;
    const totalSteps = this.filteredSteps.length;
    const subTitle = <FormattedHTMLMessage {...messages.stepsCounter} values={{
      currentStep,
      totalSteps
    }} />;
    return <Dialog className={styles.component} title={title} subtitle={hasStep && subTitle} actions={actions} closeOnOverlayClick={false} onClose={onClose} closeButton={<DialogCloseButton />} backButton={onBack && <DialogBackButton onBack={onBack} />}>
