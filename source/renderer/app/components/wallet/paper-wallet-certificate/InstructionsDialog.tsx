...
import { PAPER_WALLET_RECOVERY_PHRASE_WORD_COUNT, PAPER_WALLET_WRITTEN_WORDS_COUNT, WALLET_RECOVERY_PHRASE_WORD_COUNT } from "../../../config/cryptoConfig";
...
class InstructionsDialog extends Component<Props> {
  ...
  render() {
    ...
    const printButtonClasses = classnames(['printButton', inProgress ? styles.submitButtonSpinning : null]);
    const actions = [{
    ...
    const openNetworkExplorer = () => onOpenExternalLink(getNetworkExplorerUrl(network));

    const cardanoExplorerLink = <Link className={styles.link} onClick={openNetworkExplorer} label={intl.formatMessage(messages.cardanoExplorer)} skin={LinkSkin} />;
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.headline)} actions={actions} closeOnOverlayClick onClose={onClose} closeButton={<DialogCloseButton />}>
        <div className={styles.instructionsContentWrapper}>
          ...
          <div className={styles.instructionsList}>
            ...
            <ul>
              ...
              <li>
                <FormattedMessage {...messages.instructionsListDefinition4} values={{
