...
import { PAPER_WALLET_PRINTED_WORDS_COUNT, PAPER_WALLET_WRITTEN_WORDS_COUNT } from "../../../config/cryptoConfig";
...
class PrintDialog extends Component<Props, State> {
  ...
  render() {
    ...
    const certificatePrintedCheckboxClasses = classnames(['printedCheckbox', styles.checkbox]);
    const certificateReadableCheckboxClasses = classnames(['readableCheckbox', styles.checkbox]);
    const qrScannableCheckboxClasses = classnames(['scannableCheckbox', styles.checkbox]);
    ...
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.headline)} actions={actions} onClose={onClose} closeButton={<DialogCloseButton />}>
        <div className={styles.printContentWrapper}>
          ...
          <div className={styles.content}>
            <Checkbox className={certificatePrintedCheckboxClasses} label={intl.formatMessage(messages.certificatePrintedConfirmationLabel)} onChange={this.onConfirmCorrectPrinting} checked={isPrintedCorrectly} skin={CheckboxSkin} />

            <Checkbox className={certificateReadableCheckboxClasses} label={intl.formatMessage(messages.certificateReadableConfirmationLabel, {
            paperWalletPrintedWordsCount: PAPER_WALLET_PRINTED_WORDS_COUNT
          })} onChange={this.onConfirmReadable} checked={isReadable} skin={CheckboxSkin} />

            <Checkbox className={qrScannableCheckboxClasses} label={intl.formatMessage(messages.qrScannableConfirmationLabel)} onChange={this.onConfirmScannable} checked={isScannable} skin={CheckboxSkin} />
          </div>
        </div>
      </Dialog>;
  }
