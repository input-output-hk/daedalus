...
import { PAPER_WALLET_PRINTED_WORDS_COUNT, PAPER_WALLET_RECOVERY_PHRASE_WORD_COUNT, PAPER_WALLET_WRITTEN_WORDS_COUNT } from "../../../config/cryptoConfig";
...
class VerificationDialog extends Component<Props, State> {
  ...
  render() {
    ...
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.headline)} actions={actions} onClose={onClose} closeButton={<DialogCloseButton />}>
        <div className={styles.verificationContentWrapper}>
          ...
          <div className={styles.content}>
            <Autocomplete className={styles.recoveryPhrase} options={suggestedMnemonics} maxSelections={PAPER_WALLET_RECOVERY_PHRASE_WORD_COUNT} ref={autocomplete => {
            this.recoveryPhraseAutocomplete = autocomplete;
          }} {...recoveryPhraseField.bind()} error={recoveryPhraseField.error} maxVisibleOptions={5} noResultsMessage={intl.formatMessage(messages.recoveryPhraseNoResults)} skin={AutocompleteSkin} optionHeight={50} />

            <Checkbox className={storingUnderstandanceCheckboxClasses} label={intl.formatMessage(messages.storingUnderstandanceLabel)} onChange={this.onStoringConfirmation} checked={storingConfirmed} disabled={!isRecoveryPhraseValid} skin={CheckboxSkin} />

            <Checkbox className={recoveringUnderstandanceCheckboxClasses} label={intl.formatMessage(messages.recoveringUnderstandanceLabel)} onChange={this.onRecoveringConfirmation} checked={recoveringConfirmed} disabled={!isRecoveryPhraseValid} skin={CheckboxSkin} />
          </div>
        </div>
      </Dialog>;
  }
