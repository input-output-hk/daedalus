...
import { formattedAmountToNaturalUnits, formattedAmountToLovelace, formattedWalletAmount } from "../../utils/formatters";
...
class WalletSendForm extends Component<Props, State> {
  ...
  get selectedAssets(): Array<AssetToken> {
    ...
    return map(selectedAssetUniqueIds, uniqueId => allAssets.find(asset => asset.uniqueId === uniqueId));
  }
  ...
  get selectedAssetsAmounts(): Array<string> {
    ...
    return map(selectedAssetUniqueIds, uniqueId => formattedAmountToNaturalUnits(assetFields[uniqueId].value));
  }
  ...
  get availableAssets(): Array<AssetToken> {
    ...
    return filter(allAssets, ({
      uniqueId
    }) => !selectedAssetUniqueIds.includes(uniqueId));
  }
  ...
  handleSubmitOnEnter = (event: KeyboardEvent): void => {
    if (event.target instanceof HTMLInputElement && event.key === 'Enter') this.handleOnSubmit();
  };
  handleOnSubmit = () => {
    if (this.isDisabled()) {
      return;
  ...
  updateFormFields = (resetFormFields: boolean, uniqueId?: string) => {
    ...
    if (resetFormFields) {
      ...
      const {
        assetFields,
        assetsDropdown
      } = this.state.formFields.receiver;
      ...
      this.setState(prevState => ({
        formFields: { ...prevState.formFields,
          receiver: { ...prevState.formFields.receiver,
            assetFields,
            assetsDropdown
          }
        }
  ...
  isAddressFromSameWallet = () => {
    ...
    const receiverField = this.form.$('receiver');
    return this.hasReceiverValue() && isAddressFromSameWallet && receiverField.isValid;
  };
  isDisabled = () => this._isCalculatingTransactionFee || !this.state.isTransactionFeeCalculated || !this.form.isValid;
    ...
    fields: {
      receiver: {
        ...
        validators: [async ({
          ...
          return [isValid, this.context.intl.formatMessage(apiErrorMessages.invalidAddress)];
        }]
      },
      adaAmount: {
        label: this.context.intl.formatMessage(messages.adaAmountLabel),
        placeholder: `0${this.getCurrentNumberFormat().decimalSeparator}${'0'.repeat(this.props.currencyMaxFractionalDigits)}`,
        value: '',
        validators: [async ({
          field
        }) => {
  ...
  isLatestTransactionFeeRequest = (currentFeeCalculationRequestQue: number, prevFeeCalculationRequestQue: number) => currentFeeCalculationRequestQue - prevFeeCalculationRequestQue === 1;
  calculateTransactionFee = async () => {
    const {
      form
    } = this;
    const emptyAssetFieldValue = '0';
    const hasEmptyAssetFields = this.selectedAssetsAmounts.includes(emptyAssetFieldValue);
    ...
    const assets: ApiTokens = filter(this.selectedAssets.map(({
      policyId,
      assetName
    }, index) => {
    ...
    try {
      ...
      if (this._isMounted && this.isLatestTransactionFeeRequest(this.state.feeCalculationRequestQue, prevFeeCalculationRequestQue) && !this.selectedAssetsAmounts.includes(emptyAssetFieldValue)) {
      ...
      if (this._isMounted && this.isLatestTransactionFeeRequest(this.state.feeCalculationRequestQue, prevFeeCalculationRequestQue)) {
        ...
        if (error.id === 'api.errors.utxoTooSmall') {
          ...
          if (minimumAda && !Number.isNaN(Number(minimumAda))) {
            localizableError = selectedAssetUniqueIds.length ? messages.minAdaRequiredWithAssetTooltip : messages.minAdaRequiredWithNoAssetTooltip;
            values = {
              minimumAda
            };
            this.setState({
        ...
        if (errorHasLink) {
          transactionFeeError = <FormattedHTMLMessageWithLink message={localizableError} onExternalLinkClick={this.props.onExternalLinkClick} />;
        } else {
          transactionFeeError = <FormattedHTMLMessage {...localizableError} values={values} />;
        }
  ...
  addAssetFields = (uniqueId: string) => {
    ...
    this.form.$(newAsset).set('placeholder', `0${this.getCurrentNumberFormat().decimalSeparator}${'0'.repeat(this.props.currencyMaxFractionalDigits)}`);
    this.form.$(newAsset).set('validators', [async ({
      field
    }) => {
      const {
        value
      ...
      const amountValue = value.toString();
      const isValidAmount = await this.props.validateAssetAmount(formattedAmountToNaturalUnits(amountValue));
      ...
      const assetValue = new BigNumber(formattedAmountToNaturalUnits(field.value));
      const isValidRange = assetValue.isGreaterThan(0) && assetValue.isLessThanOrEqualTo(asset.quantity);
  ...
  onChangeAsset = async (currentUniqueId: string, newUniqueId: string) => {
    ...
    const {
      selectedAssetUniqueIds: oldSelectedAssetUniqueIds
    } = this.state;
  ...
  renderReceiverRow = (): Node => {
    ...
    const assetsSeparatorCalculatedHeight = selectedAssetUniqueIds.length ? assetsSeparatorBasicHeight * (selectedAssetUniqueIds.length + 1) - 40 * selectedAssetUniqueIds.length : assetsSeparatorBasicHeight;
    const minimumAdaValue = minimumAda.isZero() ? TRANSACTION_MIN_ADA_VALUE : minimumAda.toFormat();
    const addAssetButtonClasses = classNames([styles.addAssetButton, !this.hasAvailableAssets ? styles.disabled : null, 'primary']);
    const receiverFieldClasses = classNames([styles.receiverInput, this.isAddressFromSameWallet() ? styles.sameReceiverInput : null]);
    const minAdaRequiredTooltip = selectedAssetUniqueIds.length ? messages.minAdaRequiredWithAssetTooltip : messages.minAdaRequiredWithNoAssetTooltip;
    ...
    if (this.isAddressFromSameWallet()) {
      ...
      receiverFieldThemeVars = {
        '--rp-input-border-color-errored': 'var(--rp-password-input-warning-score-color)',
    ...
    return <div className={styles.fieldsContainer}>
        <div className={receiverFieldClasses}>
          <Input {...receiverField.bind()} ref={field => {
          this.addFocusableField(field);
        }} className="receiver" error={receiverFieldError} onChange={value => {
          receiverField.onChange(value || '');
          this.setState({
            isResetButtonDisabled: false
          });
        }} onKeyPress={this.handleSubmitOnEnter} themeVariables={receiverFieldThemeVars} />
          {this.hasReceiverValue() && <div className={styles.clearReceiverContainer}>
              <PopOver content={intl.formatMessage(messages.clearLabel)} placement="top">
                <button onClick={() => this.handleOnReset()} className={styles.clearReceiverButton} tabIndex={-1}>
                  <SVGInline svg={closeIcon} className={styles.clearReceiverIcon} />
                </button>
              </PopOver>
            </div>}
        </div>
        {this.hasReceiverValue() && isReceiverAddressValid && <>
          ...
          marginTop: `-${assetsSeparatorCalculatedHeight}px`
            ...
            <div className={styles.assetInput}>
              <Fragment>
                {walletAmount && <div className={styles.amountTokenTotal}>
                ...
                <NumericInput {...adaAmountField.bind()} ref={field => {
              this.addFocusableField(field);
            }} className="adaAmount" value={adaAmountField.value} bigNumberFormat={this.getCurrentNumberFormat()} decimalPlaces={currencyMaxFractionalDigits} numberLocaleOptions={{
              minimumFractionDigits: currencyMaxFractionalDigits
            }} onChange={value => {
              adaAmountField.onChange(value);
            }} currency={globalMessages.adaUnit} error={adaAmountField.error || transactionFeeError} onKeyPress={this.handleSubmitOnEnter} allowSigns={false} autoFocus={this._isAutoFocusEnabled} />
                <div className={styles.minAdaRequired}>
                  ...
                  <PopOver content={intl.formatMessage(minAdaRequiredTooltip, {
                minimumAda: minimumAdaValue
              })} contentClassName={styles.minAdaTooltipContent} key="tooltip">
                    <SVGInline svg={questionMarkIcon} className={styles.infoIcon} />
                  </PopOver>
                </div>
              </Fragment>
              <Fragment>
                {selectedAssetUniqueIds.map((uniqueId: string, index: number) => <AssetInput key={uniqueId} uniqueId={uniqueId} index={index} getAssetByUniqueId={this.getAssetByUniqueId} availableAssets={this.availableAssets} assetFields={assetFields} assetsDropdown={assetsDropdown} addFocusableField={this.addFocusableField} removeAssetButtonVisible={this.state.showRemoveAssetButton} showRemoveAssetButton={this.showRemoveAssetButton} hideRemoveAssetButton={this.hideRemoveAssetButton} currentNumberFormat={this.getCurrentNumberFormat()} removeAssetRow={this.removeAssetRow} handleSubmitOnEnter={this.handleSubmitOnEnter} clearAssetFieldValue={this.clearAssetFieldValue} onChangeAsset={newUniqueId => this.onChangeAsset(uniqueId, newUniqueId)} autoFocus={this._isAutoFocusEnabled} />)}
              </Fragment>
              <Button className={addAssetButtonClasses} label={intl.formatMessage(messages.addAssetButtonLabel)} disabled={!this.hasAvailableAssets} onClick={() => {
  ...
  render() {
    ...
    const calculatingFeesSpinnerButtonClasses = classNames([styles.calculatingFeesSpinnerButton, styles.spinning]);
    return <div className={styles.component}>
        {isRestoreActive ? <div className={styles.syncingTransactionsWrapper}>
            <LoadingSpinner big />
            ...
            <div className={styles.walletSendForm}>
              ...
              <div className={styles.estimatedFeeInput}>
                <ReadOnlyInput label={intl.formatMessage(messages.estimatedFeeLabel)} value={fees && !transactionFeeError ? `${fees} ${intl.formatMessage(globalMessages.adaUnit)}` : `0${this.getCurrentNumberFormat().decimalSeparator}${'0'.repeat(this.props.currencyMaxFractionalDigits)} ${intl.formatMessage(globalMessages.adaUnit)}`} isSet />
                {this._isCalculatingTransactionFee && <div className={styles.calculatingFeesContainer}>
                    <PopOver content={intl.formatMessage(messages.calculatingFeesLabel)}>
                      <button className={calculatingFeesSpinnerButtonClasses} />
                    </PopOver>
                  </div>}
              </div>
              <div className={styles.buttonsContainer}>
                <Button className="flat" label={intl.formatMessage(messages.resetButtonLabel)} disabled={isResetButtonDisabled} onClick={this.handleOnReset} />
                <Button className="primary" label={intl.formatMessage(messages.sendButtonLabel)} disabled={this.isDisabled()} onClick={this.handleOnSubmit} />
              </div>
            </div>
          </BorderedBox>}

        {isDialogOpen(WalletSendAssetsConfirmationDialog) ? <WalletSendConfirmationDialogContainer receiver={receiver} selectedAssets={this.selectedAssets} assetsAmounts={this.selectedAssetsAmounts} amount={adaAmount.toFormat(currencyMaxFractionalDigits)} amountToNaturalUnits={formattedAmountToNaturalUnits} totalAmount={total} transactionFee={fees} hwDeviceStatus={hwDeviceStatus} isHardwareWallet={isHardwareWallet} onExternalLinkClick={onExternalLinkClick} formattedTotalAmount={total.toFormat(currencyMaxFractionalDigits)} /> : null}
      </div>;
  }
