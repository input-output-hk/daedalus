...
const messages = defineMessages({
  ...
  descriptionWithTicker: {
    ...
    defaultMessage: '!!!<p>The stake from your wallet <strong>{walletName}</strong> is currently delegated to the <strong>[{stakePoolTicker}] {stakePoolName}</strong> stake pool.</p><p>Do you want to undelegate your stake and stop earning rewards?</p>',
  ...
  descriptionWithUnknownTicker: {
    ...
    defaultMessage: '!!!<p>The stake from your wallet <strong>{walletName}</strong> is currently delegated to the <strong>{stakePoolTicker}</strong> stake pool.</p><p>Do you want to undelegate your stake and stop earning rewards?</p>',
    description: 'Description of current delegation of wallet in the "Undelegate wallet" dialog.'
  },
...
class UndelegateWalletConfirmationDialog extends Component<Props> {
  ...
  form = new ReactToolboxMobxForm({
    fields: {
      confirmUnsupportChecked: {
        type: 'checkbox',
        label: this.context.intl.formatMessage(messages.confirmUnsupportNotice),
        value: false,
        validators: [({
          field
        }) => {
      ...
      confirmIneligibleChecked: {
        type: 'checkbox',
        label: this.context.intl.formatMessage(messages.confirmIneligibleNotice),
        value: false,
        validators: [({
          field
        }) => {
      ...
      passphrase: {
        ...
        validators: [({
          ...
          const isHardwareWallet = get(this.props.selectedWallet, 'isHardwareWallet');
  ...
  confirmationDisabled = () => {
    ...
    const {
      isValid: unsupportCheckboxIsValid
    } = form.$('confirmUnsupportChecked');
    const {
      isValid: ineligibleCheckboxIsValid
    } = form.$('confirmIneligibleChecked');
    const {
      isValid: passphraseIsValid
    } = form.$('passphrase');
    const isHardwareWallet = get(selectedWallet, 'isHardwareWallet');

    if (isHardwareWallet) {
      return hwDeviceStatus !== HwDeviceStatuses.VERIFYING_TRANSACTION_SUCCEEDED;
    }

    return isSubmitting || !fees || !unsupportCheckboxIsValid || !ineligibleCheckboxIsValid || !passphraseIsValid;
  };
  handleSubmit = () => {
  ...
  handleSubmitOnEnter = (event: KeyboardEvent) => submitOnEnter(this.handleSubmit, event);
  generateErrorElement = () => {
    ...
    const result = errorHasLink ? <FormattedHTMLMessageWithLink message={error} onExternalLinkClick={onExternalLinkClick} /> : this.context.intl.formatMessage(error);
    return result;
  };
  ...
  render() {
    ...
    return <Dialog title={intl.formatMessage(messages.title)} subtitle={walletName} actions={actions} closeOnOverlayClick onClose={!isSubmitting ? onCancel : () => null} className={styles.dialog} closeButton={<DialogCloseButton onClose={!isSubmitting ? onCancel : () => null} />}>
        <div className={styles.description}>
          {stakePoolTicker ? <FormattedHTMLMessage {...messages.descriptionWithTicker} values={{
          walletName,
          stakePoolName,
          stakePoolTicker
        }} /> : <FormattedHTMLMessage {...messages.descriptionWithUnknownTicker} values={{
          walletName,
          stakePoolTicker: intl.formatMessage(messages.unknownStakePoolLabel)
        }} />}
        </div>
        <Checkbox {...unsupportCheckboxField.bind()} error={unsupportCheckboxField.error} />
        <Checkbox {...ineligibleCheckboxField.bind()} error={ineligibleCheckboxField.error} />
        ...
        <div className={styles.feesRow}>
          <div className={styles.feesWrapper}>
            ...
            <p className={styles.feesAmount}>
              {!fees || !fees.fee ? <span className={styles.calculatingFeesLabel}>
        ...
        {isHardwareWallet ? <div className={styles.hardwareWalletStatusWrapper}>
            <HardwareWalletStatus hwDeviceStatus={hwDeviceStatus} walletName={walletName} isTrezor={isTrezor} onExternalLinkClick={onExternalLinkClick} />
          </div> : <Input type="password" {...passphraseField.bind()} error={passphraseField.error} onKeyPress={this.handleSubmitOnEnter} />}
