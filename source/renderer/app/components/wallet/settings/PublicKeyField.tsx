...
const PublicKeyField = (props: Props) => {
  ...
  const fieldStyles = classnames([styles.field, publicKeyHidden || !publicKey ? styles.valueHidden : styles.valueShown, locale === LOCALES.japanese ? styles.withBigToggleButton : null]);
  const hiddenValuePlaceholder = intl.formatMessage(messages.publicKeyShowInstruction);
  const toggleButtonTooltip = intl.formatMessage(globalMessages[publicKeyHidden ? 'reveal' : 'hide']);
  const qrCodeButtonStyles = classnames([styles.imageButton, styles.qrCodeButton, 'flat']);
  const revealHideButtonStyles = classnames([styles.imageButton, publicKeyHidden ? styles.revealButton : styles.hideButton, 'flat']);
  return <div className={styles.component}>
      ...
      <div className={styles.inputBox}>
        <Input className={fieldStyles} type="text" value={publicKeyHidden ? hiddenValuePlaceholder : publicKey} readOnly skin={PublicKeyFieldSkin} tooltip={intl.formatMessage(globalMessages.copy)} valueVisible={!publicKeyHidden} onCopyValue={handleCopyPublicKey} />
        <div className={styles.addons}>
          {!publicKeyHidden && <div className={styles.imageButtonContainer}>
              <PopOver content={intl.formatMessage(messages.showQRCode)}>
                <Button className={qrCodeButtonStyles} onClick={onShowQRCode} label={<SVGInline svg={qrCodeImage} />} />
              </PopOver>
            </div>}
          <PopOver content={toggleButtonTooltip}>
            <Button className={revealHideButtonStyles} label={publicKeyHidden ? <SVGInline svg={revealKeyImage} /> : <SVGInline svg={hideKeyImage} />} onClick={togglePublicKeyVisibility} />
          </PopOver>
        </div>
      </div>
    </div>;
