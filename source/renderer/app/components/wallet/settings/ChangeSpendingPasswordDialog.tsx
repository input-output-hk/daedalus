...
import { isValidSpendingPassword, isValidRepeatPassword } from "../../../utils/validations";
...
class ChangeSpendingPasswordDialog extends Component<Props> {
  ...
  form = new ReactToolboxMobxForm({
    fields: {
      currentPassword: {
        ...
        validators: [({
          ...
          if (this.props.isSpendingPasswordSet) {
            ...
            return [currentPasswordField.value.length > 0, this.context.intl.formatMessage(globalMessages.invalidSpendingPassword)];
          }
      ...
      spendingPassword: {
        type: 'password',
        label: this.context.intl.formatMessage(messages[this.props.isSpendingPasswordSet ? 'newPasswordLabel' : 'spendingPasswordLabel']),
  ...
  render() {
    ...
    const dialogClasses = classnames([styles.dialog, isSpendingPasswordSet ? 'changePasswordDialog' : 'createPasswordDialog']);
    const confirmButtonClasses = classnames(['confirmButton', isSubmitting ? styles.isSubmitting : null]);
    const spendingPasswordClasses = classnames([styles.spendingPasswordField, currentLocale === 'ja-JP' ? styles.jpLangTooltipIcon : '']);
    ...
    const currentPasswordError = canSubmit && error && error.code === 'wrong_encryption_passphrase';
    const actions = [{
    ...
    return <Dialog title={intl.formatMessage(messages[!isSpendingPasswordSet ? 'dialogTitleSetPassword' : 'dialogTitleChangePassword'], {
      walletName
    })} subtitle={walletName} actions={actions} closeOnOverlayClick onClose={!isSubmitting ? onCancel : () => {}} className={dialogClasses} closeButton={<DialogCloseButton onClose={onCancel} />}>
        <div className={styles.spendingPasswordFields}>
          {isSpendingPasswordSet && <div className={spendingPasswordClasses}>
              <Input className={styles.currentPassword} error={currentPasswordField.error || currentPasswordError} {...currentPasswordField.bind()} onKeyPress={this.handleSubmitOnEnter} />
          ...
          <div className={newPasswordClasses}>
            <div className={spendingPasswordClasses}>
              <PasswordInput {...newPasswordField.bind()} onKeyPress={this.handleSubmitOnEnter} />
              {!isSpendingPasswordSet && <PopOver content={<FormattedHTMLMessage {...messages.passwordTooltip} />} key="tooltip">
                  <SVGInline svg={infoIconInline} className={styles.infoIcon} />
                </PopOver>}
            </div>
          </div>
          ...
          <div className={styles.repeatedPassword}>
            <PasswordInput {...repeatedPasswordField.bind()} onKeyPress={this.handleSubmitOnEnter} repeatPassword={newPasswordField.value} isPasswordRepeat />
          </div>
