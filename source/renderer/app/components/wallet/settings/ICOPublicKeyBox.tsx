...
export const messages: Record<string, ReactIntlMessage> = defineMessages({
  ...
  publicKeyDescription: {
    ...
    defaultMessage: "!!!Your wallet's ICO public key enables participation in the initial coin offering presales.",
  ...
  publicKeyShowInstruction: {
    ...
    defaultMessage: '!!!Click the icon on the right to view your ICO public key.',
...
const ICOPublicKeyBox = (props: Props) => {
  ...
  return <>
      <BorderedBox className={styles.walletPublicKeyBox}>
        <PublicKeyField publicKey={publicKey || ''} description={intl.formatMessage(messages.publicKeyDescription)} locale={locale} onCopyPublicKey={onCopyICOPublicKey} onShowQRCode={() => openDialogAction({
        dialog: PublicKeyQRCodeDialog
      })} onOpenWalletKeyDialog={() => openDialogAction({
