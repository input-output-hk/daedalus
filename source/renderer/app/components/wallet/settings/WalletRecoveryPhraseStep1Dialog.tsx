...
export const messages = defineMessages({
  ...
  recoveryPhraseStep1Paragraph1: {
    ...
    defaultMessage: '!!!To verify that you have the correct recovery phrase for this wallet, you can enter it on the following screen.',
...
class WalletRecoveryPhraseStep1Dialog extends Component<Props, State> {
  ...
  render() {
    ...
    return <Dialog className={styles.dialog} title={intl.formatMessage(messages.recoveryPhraseStep1Title)} subtitle={walletName} actions={actions} closeOnOverlayClick onClose={onClose} closeButton={<DialogCloseButton />}>
        <p>{intl.formatMessage(messages.recoveryPhraseStep1Paragraph1)}</p>
        <div className={styles.checkboxContainer}>
          <Checkbox onChange={this.onToggleSafetyAgreement} checked={safetyAgreement} skin={CheckboxSkin} className={styles.checkbox} label={intl.formatMessage(messages.recoveryPhraseStep1Paragraph2)} />
        </div>
      </Dialog>;
  }
