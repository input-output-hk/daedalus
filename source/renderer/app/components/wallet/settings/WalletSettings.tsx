...
import { IS_ICO_PUBLIC_KEY_SHARING_ENABLED, IS_WALLET_PUBLIC_KEY_SHARING_ENABLED, IS_WALLET_UNDELEGATION_ENABLED } from "../../../config/walletsConfig";
...
class WalletSettings extends Component<Props, State> {
  ...
  componentDidUpdate() {
    ...
    if (!isDialogOpen(WalletSettingsActionConfirmationDialog) && !isDialogOpen(ChangeSpendingPasswordDialog) && isFormBlocked) {
      this.unblockForm();
    }
  }
  ...
  renderUndelegateWalletBox = () => {
    ...
    if (isDelegating) {
      ...
      warningMessage = isRestoring || isSyncing ? intl.formatMessage(messages.undelegateWalletDisabledWarning) : intl.formatMessage(messages.undelegateWalletWarning);
    } else {
      headerMessage = intl.formatMessage(messages.delegateWalletHeader);
      warningMessage = isRestoring || isSyncing ? intl.formatMessage(messages.delegateWalletDisabledWarning) : intl.formatMessage(messages.delegateWalletWarning);
    }
    ...
    return <>
        <BorderedBox className={styles.undelegateWalletBox}>
          ...
          <div className={styles.contentBox}>
            ...
            {isDelegating ? <UndelegateWalletButton disabled={isRestoring || isSyncing} onUndelegate={this.onUndelegateWalletClick} /> : <DelegateWalletButton disabled={isRestoring || isSyncing} onDelegate={onDelegateClick} />}
          </div>
        </BorderedBox>
        {isDialogOpen(UndelegateWalletConfirmationDialog) ? undelegateWalletDialogContainer : false}
      </>;
  };
  ...
  render() {
    ...
    const passwordMessage = isSpendingPasswordSet ? intl.formatMessage(messages.passwordLastUpdated, {
      lastUpdated: moment(spendingPasswordUpdateDate).locale(this.context.intl.locale).fromNow()
    }) : intl.formatMessage(messages.passwordNotSet);
    return <div className={styles.root}>
        <BorderedBox>
          <InlineEditingInput className="walletName" label={intl.formatMessage(messages.name)} value={walletName} maxLength={40} onFocus={() => onStartEditing('name')} onBlur={onStopEditing} onCancel={onCancel} onSubmit={value => onFieldValueChange('name', value)} isValid={nameValidator} valueErrorMessage={intl.formatMessage(globalMessages.invalidWalletName)} readOnly={isFormBlocked} />

          {!isHardwareWallet && <ReadOnlyInput label={intl.formatMessage(messages.passwordLabel)} value={passwordMessage} isSet={isSpendingPasswordSet} withButton onClick={() => {
          this.onBlockForm();
          openDialogAction({
          ...
          {shouldDisplayRecoveryPhrase && <WalletRecoveryPhraseVerificationWidget onVerify={onVerifyRecoveryPhrase} recoveryPhraseVerificationDate={recoveryPhraseVerificationDate} recoveryPhraseVerificationStatus={recoveryPhraseVerificationStatus} recoveryPhraseVerificationStatusType={recoveryPhraseVerificationStatusType} creationDate={creationDate} locale={locale} wordCount={wordCount} isLegacy={isLegacy} />}

          {error && <p className={styles.error}>{intl.formatMessage(error)}</p>}
        </BorderedBox>

        {isDialogOpen(ChangeSpendingPasswordDialog) ? changeSpendingPasswordDialog : false}

        {IS_WALLET_PUBLIC_KEY_SHARING_ENABLED && !isLegacy && <>
            <WalletPublicKeyBox publicKey={this.props.walletPublicKey} locale={this.props.locale} onCopyWalletPublicKey={this.props.onCopyWalletPublicKey} openDialogAction={this.props.openDialogAction} />
            {isDialogOpen(WalletPublicKeyDialog) && walletPublicKeyDialogContainer}
            {isDialogOpen(WalletPublicKeyQRCodeDialog) && walletPublicKeyQRCodeDialogContainer}
          </>}

        {IS_ICO_PUBLIC_KEY_SHARING_ENABLED && !isLegacy && !isHardwareWallet && <>
            <ICOPublicKeyBox publicKey={this.props.icoPublicKey} locale={this.props.locale} onCopyICOPublicKey={this.props.onCopyICOPublicKey} openDialogAction={this.props.openDialogAction} />
            {isDialogOpen(ICOPublicKeyDialog) && icoPublicKeyDialogContainer}
            {isDialogOpen(ICOPublicKeyQRCodeDialog) && icoPublicKeyQRCodeDialogContainer}
          </>}

        {this.renderUndelegateWalletBox()}
        {isHardwareWallet ? <UnpairWallet openDialogAction={openDialogAction} isDialogOpen={isDialogOpen} unpairWalletDialogContainer={unpairWalletDialogContainer} onBlockForm={this.onBlockForm} /> : <DeleteWallet openDialogAction={openDialogAction} isDialogOpen={isDialogOpen} deleteWalletDialogContainer={deleteWalletDialogContainer} onBlockForm={this.onBlockForm} />}
      </div>;
  }
