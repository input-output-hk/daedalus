...
export const messages = defineMessages({
  ...
  recoveryPhraseStep2Subtitle: {
    ...
    description: 'Label for the recoveryPhraseStep2Subtitle on wallet settings.'
  },
...
class WalletRecoveryPhraseStep2Dialog extends Component<Props, State> {
    ...
    fields: {
      recoveryPhrase: {
        ...
        validators: ({
          ...
          validator: enteredWords => [isValidMnemonic(enteredWords.join(' '), enteredWords.length), this.context.intl.formatMessage(messages.recoveryPhraseStep2InvalidMnemonics)]
        })
      }
    }
  }, {
  ...
  render() {
    ...
    const {
      length: enteredWordCount
    } = recoveryPhraseField.value;
    const canSubmit = !recoveryPhraseField.error && !isVerifying && (Array.isArray(expectedWordCount) ? expectedWordCount.includes(enteredWordCount) : enteredWordCount === expectedWordCount);
    ...
    const maxSelections = Array.isArray(expectedWordCount) ? Math.max(...expectedWordCount) : expectedWordCount;
    return <Dialog className={styles.dialog} title={intl.formatMessage(messages.recoveryPhraseStep2Title)} subtitle={walletName} actions={actions} closeOnOverlayClick={false} onClose={onClose} closeButton={<DialogCloseButton />}>
        <div className={styles.subtitle}>
          <p>{intl.formatMessage(messages.recoveryPhraseStep2Description)}</p>
        </div>
        <Autocomplete {...recoveryPhraseField.bind()} label={intl.formatMessage(messages.recoveryPhraseStep2Subtitle)} placeholder={intl.formatMessage(messages.recoveryPhraseInputPlaceholder, {
        wordNumber: enteredWordCount + 1
      })} options={suggestedMnemonics} requiredSelections={Array.isArray(expectedWordCount) ? expectedWordCount : [expectedWordCount]} requiredSelectionsInfo={(required, actual) => Array.isArray(expectedWordCount) ? intl.formatMessage(globalMessages.unknownMnemonicWordCount, {
        actual
      }) : intl.formatMessage(globalMessages.knownMnemonicWordCount, {
        actual,
        required
      })} maxSelections={maxSelections} error={errorOrIncompleteMarker(recoveryPhraseField.error)} maxVisibleOptions={5} noResultsMessage={intl.formatMessage(messages.recoveryPhraseNoResults)} skin={AutocompleteSkin} optionHeight={50} />
      </Dialog>;
  }
