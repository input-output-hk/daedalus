...
import { TransactionTypes, TransactionStates } from "../../../domains/WalletTransaction";
type Props = {
...
export default class TransactionTypeIcon extends Component<Props> {
  ...
  renderPendingRegularIcon = () => <div className={styles.pendingTxnIconWrapper}>
      <LoadingSpinner skin={LoadingSpinnerSkin} themeOverrides={spinnerOverrides} />
    </div>;
  renderPendingWarningIcon = () => <div className={styles.pendingTxnIconWrapper}>
      <LoadingSpinner skin={LoadingSpinnerSkin} themeOverrides={spinnerOverrides} />
      <SVGInline svg={pendingIcon} className={styles.pendingTxnIcon} />
    </div>;
  renderFailedIcon = () => {
    return <SVGInline svg={pendingIcon} className={styles.transactionTypeIcon} />;
  };
  renderIcon = (icon: string) => {
    ...
    if (this.props.iconType === TransactionStates.PENDING) {
      ...
      iconType = <SVGInline svg={icon} className={styles.transactionTypeIcon} />;
    }
  ...
  render() {
    ...
    const transactionTypeIconClasses = classNames([styles.transactionTypeIconWrapper, styles[this.applyIconStyles(iconType)]]);
    let icon;
    ...
    return <div className={transactionTypeIconClasses}>{this.renderIcon(icon)}</div>;
  }
