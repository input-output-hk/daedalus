...
class VirtualTransactionList extends Component<Props> {
   ...
   */
  findIndexForTx = (tx: WalletTransaction): number => this.props.rows.findIndex(r => r instanceof TransactionInfo && r.tx.id === tx.id);
  ...
  estimateHeightOfTxExpandedRow = (row: Row, tx: WalletTransaction): number => {
    ...
    return TX_EXPANDED_ROW_BASE_HEIGHT + txAddressesHeight + txIdHeightValue + txBottomMargin;
  };
   ...
   */
  updateTxRowHeight = (tx: WalletTransaction, isExpanded: boolean, wasToggled: boolean) // Is "true" when transaction is manually expanded/collapsed
  : void => {
    const {
      rowHeights
    } = this;
    ...
    rowHeights[txIndex] = isExpanded ? this.estimateHeightOfTxExpandedRow(row, tx) : this.estimateHeightOfTxContractedRow(row);
   ...
   */
  measureTxContentHeight = (tx: WalletTransaction): number | null | undefined => {
  ...
  updateVisibleExpandedTxRowHeights = () => {
    ...
    const visibleExpandedTx = expandedTxArray.filter(tx => {
      ...
      return index >= this.overscanStartIndex && index <= this.overscanStopIndex;
    });
  ...
  onListScroll = (context: ScrollContextType, {
    scrollTop
  }: {
    scrollTop: number;
  }) => {
  ...
  render() {
    ...
    const componentStyles = classNames([styles.component, isLoadingSpinnerShown ? styles.withLoadingSpinner : null, isSyncingSpinnerShown ? styles.withSyncingSpinner : null]);
    return <WalletTransactionsListScrollContext.Consumer>
        {context => <div className={componentStyles}>
          ...
          trailing: true
            ...
            width,
            height
          }) => <List className={styles.list} ref={list => {
            this.list = list;
          }} width={width} height={height} onRowsRendered={throttle(this.onRowsRendered, 100, {
            leading: true,
            trailing: true
          })} rowCount={rows.length} rowHeight={({
            index
          }) => this.rowHeights[index] || TX_CONTRACTED_ROW_HEIGHT} rowRenderer={this.rowRenderer} style={{
            overflowY: 'scroll'
