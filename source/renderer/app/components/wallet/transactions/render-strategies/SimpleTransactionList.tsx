...
class SimpleTransactionList extends Component<Props> {
  ...
  onListScroll = (context: ScrollContextType, evt: React.SyntheticEvent<HTMLElement>) => {
    const {
      scrollTop
    } = evt.currentTarget;
  ...
  render() {
    ...
    return <WalletTransactionsListScrollContext.Consumer>
        {context => <div className={styles.component} onScroll={evt => this.onListScroll(context, evt)}>
