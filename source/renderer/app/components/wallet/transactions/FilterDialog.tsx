...
import { calculateDateRange, validateFilterForm } from "../../../utils/transaction";
import type { TransactionFilterOptionsType } from "../../../stores/TransactionsStore";
import { DateRangeTypes, emptyTransactionFilterOptions } from "../../../stores/TransactionsStore";
...
class FilterDialog extends Component<FilterDialogProps> {
  ...
  constructor(props: FilterDialogProps, context: Record<string, any>) {
    super(props);
    const {
  ...
  setFilterType = (field: "incomingChecked" | "outgoingChecked", value: boolean) => {
    this.form.select(field).set(value);

    if (value === false) {
      const otherFieldName = field === 'incomingChecked' ? 'outgoingChecked' : 'incomingChecked';
  ...
  generateDefaultFilterOptions = () => this.fillFormFields(this.props.defaultFilterOptions);
  isFormValuesEqualTo = (comparedFilterOptions: TransactionFilterOptionsType) => {
    const formFieldNames = Object.keys(this.form.fields.toJSON());
    return isEqual(this.getComposedFormValues(), pick(comparedFilterOptions, formFieldNames));
  };
  ...
  isValidToDate = (date: Record<string, any>) => {
    ...
    return date.isSameOrBefore(moment().endOf('day')) && date.isSameOrAfter(moment(fromDate).startOf('day'));
  };
  renderTypeField = () => {
    const {
      form
    ...
    return <div className={styles.type}>
        <div className={styles.body}>
          <div className={styles.typeCheckbox}>
            <TinyCheckbox {...incomingCheckboxField.bind()} onChange={isSelected => this.setFilterType('incomingChecked', isSelected)} />
          </div>
          <div className={styles.typeCheckbox}>
            <TinyCheckbox {...outgoingCheckboxField.bind()} onChange={isSelected => this.setFilterType('outgoingChecked', isSelected)} />
          </div>
        </div>
      </div>;
  };
  renderDateRangeField = () => {
    ...
    return <div className={styles.dateRange}>
        <TinySelect {...dateRangeFieldBindProps} onChange={(...args) => {
      ...
      }} placeholder={intl.formatMessage(messages.selectTimeRange)} options={this.dateRangeOptions} />
      </div>;
  };
  renderDateRangeFromToField = () => {
    ...
    const {
      invalidFields
    } = validateFilterForm(this.getComposedFormValues());
    const fromDateFieldBindProps = form.$('fromDate').bind();
    const toDateFieldBindProps = form.$('toDate').bind();
    const fromDateLocaleClassName = locale === 'ja-JP' ? styles.japaneseFromDateInput : null;
    const toDateLocaleClassName = locale === 'ja-JP' ? styles.japaneseToDateInput : null;
    const fromDateClassNames = classNames([styles.dateRangeInput, styles.fromDateInput, fromDateLocaleClassName]);
    const toDateClassNames = classNames([styles.dateRangeInput, styles.toDateInput, toDateLocaleClassName]);
    return <div className={styles.dateRangeFromTo}>
        <div className={styles.body}>
          <div className={fromDateClassNames}>
            <TinyDatePicker {...fromDateFieldBindProps} onChange={(...args) => {
            fromDateFieldBindProps.onChange(...args);
            this.form.select('dateRange').set(DateRangeTypes.CUSTOM);
          }} label={intl.formatMessage(globalMessages.rangeFrom)} pickerPanelPosition="left" closeOnSelect onReset={() => form.select('fromDate').set('')} isValidDate={this.isValidFromDate} locale={locale} dateFormat={dateFormat} />
          </div>
          <div className={toDateClassNames}>
            <TinyDatePicker {...toDateFieldBindProps} onChange={(...args) => {
            toDateFieldBindProps.onChange(...args);
            this.form.select('dateRange').set(DateRangeTypes.CUSTOM);
          }} label={intl.formatMessage(globalMessages.rangeTo)} pickerPanelPosition="right" closeOnSelect onReset={() => form.select('toDate').set('')} isValidDate={this.isValidToDate} locale={locale} dateFormat={dateFormat} error={invalidFields.toDate} />
          </div>
        </div>
      </div>;
  };
  renderAmountRangeField = () => {
    ...
    const fromAmountLocaleClassName = locale === 'ja-JP' ? styles.japaneseFromAmountInput : null;
    const toAmountLocaleClassName = locale === 'ja-JP' ? styles.japaneseToAmountInput : null;
    const fromAmountClassNames = classNames([styles.amountRangeInput, styles.fromAmountInput, fromAmountLocaleClassName]);
    const toAmountClassNames = classNames([styles.amountRangeInput, styles.toAmountInput, toAmountLocaleClassName]);
    return <div className={styles.amountRange}>
        <div className={styles.header}>
          <label>{intl.formatMessage(messages.amountRange)}</label>
        </div>
        <div className={styles.body}>
          <div className={fromAmountClassNames}>
            <TinyInput {...fromAmountField.bind()} value={fromAmountField.value} onSubmit={this.handleSubmit} label={intl.formatMessage(globalMessages.rangeFrom)} bigNumberFormat={NUMBER_FORMATS[numberFormat]} decimalPlaces={DECIMAL_PLACES_IN_ADA} allowSigns={false} />
          </div>
          <div className={toAmountClassNames}>
            <TinyInput {...toAmountField.bind()} value={toAmountField.value} onSubmit={this.handleSubmit} label={intl.formatMessage(globalMessages.rangeTo)} bigNumberFormat={NUMBER_FORMATS[numberFormat]} decimalPlaces={DECIMAL_PLACES_IN_ADA} allowSigns={false} error={invalidFields.toAmount} />
          </div>
        </div>
      </div>;
  };
  renderActionButton = () => {
    ...
    return <div className={styles.action}>
        <TinyButton label={intl.formatMessage(messages.apply)} loading={false} disabled={this.isFormValuesEqualTo(populatedFilterOptions) || !isValid} onClick={this.handleSubmit} />
      </div>;
  };
  ...
  render() {
    ...
    return <PopOver arrow={false} interactive trigger="click" appendTo={document.body} onShow={instance => {
      this.popoverTippyInstance.current = instance;
    }} duration={0} offset={[0, 10]} maxWidth={640} placement="bottom" themeVariables={{
      '--rp-pop-over-bg-color': 'var(--theme-transactions-filter-modal-bg-color)',
    ...
    }} content={<div className={styles.component}>
            <div className={styles.title}>
              <h4 className={styles.titleText}>
                {intl.formatMessage(globalMessages.filter)}
              </h4>
              <div>
                <button className={styles.titleLink} onClick={this.generateDefaultFilterOptions} disabled={this.isFormValuesEqualTo(defaultFilterOptions)}>
                  {intl.formatMessage(messages.allTransactions)}
                </button>
                <button className={styles.titleLink} onClick={this.resetForm} disabled={this.isFormValuesEqualTo(emptyTransactionFilterOptions)}>
