...
import { TransactionStates, TransactionTypes, WalletTransaction } from "../../../domains/WalletTransaction";
...
const messages = defineMessages({
  ...
  metadataDisclaimer: {
    ...
    defaultMessage: '!!!Transaction metadata is not moderated and may contain inappropriate content.',
  ...
  cancelPendingTxnNote: {
    ...
    defaultMessage: '!!!This transaction has been pending for a long time. To release the funds used by this transaction, you can try canceling it.',
  ...
  supportArticleUrl: {
    ...
    defaultMessage: '!!!https://iohk.zendesk.com/hc/en-us/articles/360038113814',
...
export default class Transaction extends Component<Props, State> {
  ...
  componentDidUpdate(prevProps: Props, prevState: State) {
    ...
    if (!prevState.showUnmoderatedMetadata && this.state.showUnmoderatedMetadata && this.props.onShowMetadata) {
  ...
  deletePendingTransaction = async () => {
    ...
    if (state !== TransactionStates.PENDING && state !== TransactionStates.FAILED) {
    ...
    if (overPendingTimeLimit || state === TransactionStates.FAILED) {
      return <Fragment>
          <div className={styles.pendingTxnNote}>
            {state === TransactionStates.PENDING ? intl.formatMessage(messages.cancelPendingTxnNote) : intl.formatMessage(messages.cancelFailedTxnNote)}
            <Link className={styles.articleLink} onClick={this.handleOpenSupportArticle} label={state === TransactionStates.PENDING ? intl.formatMessage(messages.cancelPendingTxnSupportArticle) : intl.formatMessage(messages.cancelFailedTxnSupportArticle)} underlineOnHover skin={LinkSkin} />
          </div>
          <div>
            <CancelTransactionButton state={state === TransactionStates.PENDING ? 'cancel' : 'remove'} onClick={state === TransactionStates.PENDING ? this.showConfirmationDialog : this.deletePendingTransaction} />
          </div>
        </Fragment>;
    }
  ...
  renderTxnStateTag = () => {
    ...
    const styleLabel = this.hasExceededPendingTimeLimit() ? `${state}WarningLabel` : `${state}Label`;
    return <div className={styles[styleLabel]}>
  ...
  includesUnresolvedAddresses = (addresses: Array<string | null | undefined>) => includes(addresses, null);
  addressesList = (addresses: Array<string | null | undefined>): any => {
    ...
    if (addresses && addresses.length > 0) {
      const hasUnresolvedAddresses = this.includesUnresolvedAddresses(addresses);
      return type !== TransactionTypes.EXPEND && hasUnresolvedAddresses ? <div className={styles.explorerLinkRow}>
          <Link className={styles.explorerLink} onClick={() => onOpenExternalLink(getUrlByType('tx', data.id))} label={intl.formatMessage(messages.unresolvedInputAddressesLinkLabel)} skin={LinkSkin} />
          <span>
            {intl.formatMessage(messages.unresolvedInputAddressesAdditionalLabel)}
          </span>
        </div> : addresses.map((address, addressIndex) => <div // eslint-disable-next-line react/no-array-index-key
      key={`${data.id}-from-${address || ''}-${addressIndex}`} className={styles.addressRow}>
            <Link onClick={() => onOpenExternalLink(getUrlByType('address', address))} label={<WholeSelectionText className={styles.address} text={address} />} skin={LinkSkin} />
          </div>);
    }
  ...
  render() {
    ...
    const componentStyles = classNames([styles.component, isExpanded ? 'Transaction_expanded' : null]);
    const contentStyles = classNames([styles.content, isLastInList ? styles.last : null, isExpanded ? styles.contentExpanded : null]);
    const detailsStyles = classNames([styles.details, isExpanded ? styles.detailsExpanded : styles.detailsClosed]);
    const arrowStyles = classNames([styles.arrow, isExpanded ? styles.arrowExpanded : null]);
    const transactionsType = this.hasAssets ? intl.formatMessage(messages.multipleTokens) : intl.formatMessage(globalMessages.adaUnit);
    const typeOfTransaction = this.hasAssets ? intl.formatMessage(headerStateTranslations[state]) : intl.formatMessage(globalMessages.adaUnit);
    ...
    const assetsSeparatorStyles = classNames([styles.assetsSeparator, isExpanded ? styles.expanded : null]);
    const assetsSeparatorBasicHeight = 27;
    const assetsSeparatorCalculatedHeight = this.assetsList.length ? assetsSeparatorBasicHeight * this.assetsList.length - 15 : assetsSeparatorBasicHeight;
    return <Fragment>
        <div onClick={this.toggleDetails.bind(this)} className={componentStyles} role="presentation" aria-hidden>
          <div className={styles.toggler}>
            <TransactionTypeIcon exceedsPendingTimeLimit={exceedsPendingTimeLimit} iconType={getIconType(state)} />

            <div className={styles.togglerContent}>
              <div className={styles.header}>
                <div className={styles.title}>
                  {data.type === TransactionTypes.EXPEND ? intl.formatMessage(messages.sent, {
                  transactionsType
          ...
          <div className={contentStyles}>
            <div className={detailsStyles} onClick={event => event.stopPropagation()} role="presentation" aria-hidden>
              <div>
                ...
                {data.addresses.withdrawals.length ? <>
                    ...
                    {data.addresses.withdrawals.map((address, addressIndex) => <div // eslint-disable-next-line react/no-array-index-key
                key={`${data.id}-to-${address}-${addressIndex}`} className={styles.addressRow}>
                        <Link onClick={() => onOpenExternalLink(getUrlByType('address', address))} label={<WholeSelectionText className={styles.address} text={address} />} skin={LinkSkin} />
                      </div>)}
                  </> : null}
                ...
                {this.hasAssets && <>
                    <h2>
                      {data.type === TransactionTypes.EXPEND ? intl.formatMessage(messages.tokensSent) : intl.formatMessage(messages.tokensReceived)}
                    </h2>
                    {isLoadingAssets ? <div className={styles.assetContainer}>
                        <div className={assetsSeparatorStyles} style={{
                    height: '12px'
                  }} />
                        <h3>
                      ...
                      </div> : this.assetsList.map((asset, assetIndex) => <div // eslint-disable-next-line react/no-array-index-key
                key={`${data.id}-to-${asset.policyId}-${assetIndex}`} className={styles.assetContainer}>
                          {assetIndex === 0 && <div className={assetsSeparatorStyles} style={{
                    height: `${assetsSeparatorCalculatedHeight}px`
                          ...
                          {asset.quantity && <AssetAmount amount={asset.quantity} metadata={asset.metadata} decimals={asset.decimals} className={styles.assetAmount} />}
                        </div>)}
                  </>}

                <h2>{intl.formatMessage(messages.transactionId)}</h2>
                <div className={styles.transactionIdRow}>
                  <Link onClick={() => onOpenExternalLink(getUrlByType('tx', data.id))} label={<WholeSelectionText className={styles.transactionId} text={data.id} />} skin={LinkSkin} />
                </div>
                {this.renderCancelPendingTxnContent()}

                {data.metadata != null && <div className={styles.metadata}>
                    <h2>{intl.formatMessage(messages.metadataLabel)}</h2>
                    {data.metadata && (this.state.showUnmoderatedMetadata || isShowingMetadata) ? <TransactionMetadataView data={data.metadata} /> : <>
                        <p className={styles.metadataDisclaimer}>
                          {intl.formatMessage(messages.metadataDisclaimer)}
                        </p>
                        <Link isUnderlined={false} hasIconAfter={false} underlineOnHover label={intl.formatMessage(messages.metadataConfirmationLabel)} onClick={e => {
                    e.preventDefault();
                    this.setState({
        ...
        {showConfirmationDialog && <CancelTransactionConfirmationDialog isSubmitting={isDeletingTransaction} onCancel={this.hideConfirmationDialog} onConfirm={this.deletePendingTransaction} />}
      </Fragment>;
  }
