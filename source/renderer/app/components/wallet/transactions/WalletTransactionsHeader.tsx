...
class WalletTransactionsHeader extends Component<Props> {
  ...
  render() {
    ...
    const hasAny = true;
    const componentClassnames = classnames([styles.component, isScrolling ? styles.isScrolling : null]);
    const isCsvButtonDisabled = numberOfTransactions === 0;
    const cvsButtonClassnames = classnames([styles.csvButton, isCsvButtonDisabled ? styles.csvButtonDisabled : null, 'flat']);
    return <div className={componentClassnames}>
        ...
        {hasAny && <div className={styles.actions}>
            <TinyButton label={<Fragment>
                  {intl.formatMessage(messages.exportCSVButtonLabel)}
                  <SVGInline svg={downloadIcon} className={styles.downloadIcon} />
                </Fragment>} onClick={onRequestCSVFile} containerClassName={styles.csvButtonContainer} className={cvsButtonClassnames} disabled={isCsvButtonDisabled} loading={false} />
            <FilterDialog {...filterDialogProps} triggerElement={<FilterButton disabled={isFilterDisabled} numberOfFilterDimensionsApplied={numberOfFilterDimensionsApplied} />} />
          </div>}
      </div>;
  }
