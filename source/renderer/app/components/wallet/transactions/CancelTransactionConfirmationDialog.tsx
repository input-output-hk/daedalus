...
class CancelTransactionConfirmationDialog extends Component<Props> {
  ...
  render() {
    ...
    const confirmButtonClasses = classnames(['confirmButton', 'attention', styles.confirmButton, isSubmitting ? styles.isSubmitting : null]);
    const actions = [{
    ...
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.headline)} actions={actions} closeOnOverlayClick={false} onClose={!isSubmitting && onCancel}>
