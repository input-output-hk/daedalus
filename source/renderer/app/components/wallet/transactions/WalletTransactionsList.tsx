...
class WalletTransactionsList extends Component<Props, State> {
  ...
  groupTransactionsByDay(transactions: Array<WalletTransaction>): Array<TransactionsGroup> {
    ...
    for (const transaction of transactions) {
      ...
      let group = groups.find(g => g.date.format(DATE_FORMAT) === date.format(DATE_FORMAT));
    ...
    return groups.sort((a: TransactionsGroup, b: TransactionsGroup) => b.date.valueOf() - a.date.valueOf());
  }
  ...
  isTxExpanded = (tx: WalletTransaction) => this.expandedTransactionIds.has(tx.id);
  isTxShowingMetadata = (tx: WalletTransaction) => this.transactionsShowingMetadata.has(tx.id);
  ...
  renderGroup = (data: TransactionsGroup): Node => <div className={styles.groupDate}>{this.localizedDate(data.date)}</div>;
  renderTransaction = (data: TransactionInfo): Node => {
    ...
    const txClasses = classnames([styles.transaction, isFirstInGroup ? styles.firstInGroup : null, isLastInGroup ? styles.lastInGroup : null]);
    ...
    return <div id={`tx-${tx.id}`} className={txClasses}>
        <Transaction data={tx} deletePendingTransaction={deletePendingTransaction} formattedWalletAmount={formattedWalletAmount} isExpanded={this.isTxExpanded(tx)} isShowingMetadata={this.isTxShowingMetadata(tx)} isLastInList={isLastInGroup} isRestoreActive={isRestoreActive} onDetailsToggled={() => this.toggleTransactionExpandedState(tx)} onOpenExternalLink={onOpenExternalLink} onShowMetadata={() => this.onShowMetadata(tx)} getUrlByType={getUrlByType} state={tx.state} walletId={walletId} isDeletingTransaction={isDeletingTransaction} currentTimeFormat={currentTimeFormat} assetTokens={assetTokens} hasAssetsEnabled={hasAssetsEnabled} isInternalAddress={isInternalAddress} isLoadingAssets={isLoadingAssets} onCopyAssetParam={onCopyAssetParam} />
      </div>;
  };
  renderItem = (row: Row) => {
  ...
  render() {
    ...
    const loadingSpinner = (isLoadingTransactions || hasMoreToLoad) && !isRestoreActive ? <LoadingSpinner ref={component => {
      this.loadingSpinner = component;
    }} /> : null;
    const syncingTransactionsSpinner = isRestoreActive ? <div className={styles.syncingTransactionsWrapper}>
        <LoadingSpinner big />
        <p className={styles.syncingTransactionsText}>
          {intl.formatMessage(messages.syncingTransactionsMessage)}
        </p>
      </div> : null;
    const buttonClasses = classnames(['primary', styles.showMoreTransactionsButton]);
    ...
    const showMoreTxButton = <Button className={buttonClasses} label={intl.formatMessage(messages.showMoreTransactionsButtonLabel)} // eslint-disable-next-line react/jsx-no-bind
    onClick={this.onShowMoreTransactions.bind(this, walletId)} skin={ButtonSkin} />;
    if (isPreloading) return <div className={styles.preloadingBlockWrapper}>
          <LoadingSpinner big />
        </div>;
    return <div className={styles.component}>
        {syncingTransactionsSpinner}
        {isRenderingAsVirtualList ? <VirtualTransactionList getExpandedTransactions={this.getExpandedTransactions} ref={list => {
        this.virtualList = list;
      }} renderRow={this.renderItem} rows={rows} isLoadingSpinnerShown={loadingSpinner !== null} isSyncingSpinnerShown={isRestoreActive} /> : <SimpleTransactionList ref={list => {
        this.simpleList = list;
      }} renderRow={this.renderItem} rows={rows} />}
