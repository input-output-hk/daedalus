...
class VirtualAddressesList extends Component<Props> {
   ...
   */
  estimateAddressHeight = (lines: number): number => ADDRESS_LINE_HEIGHT * lines + ADDRESS_LINE_PADDING;
  ...
  rowRenderer = ({
    ...
    return <div key={key} style={style} className={styles.address}>
  ...
  render() {
    ...
    return <div className={styles.component}>
        <AutoSizer onResize={throttle(this.onResize, 100)}>
          ...
          this.list = list;
        }} width={width} height={height} rowCount={rows.length} rowHeight={() => this.addressHeight} rowRenderer={this.rowRenderer} style={{
          overflowY: 'scroll'
        }} />}
        </AutoSizer>
      </div>;
