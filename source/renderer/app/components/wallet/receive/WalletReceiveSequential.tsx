...
class WalletReceiveSequential extends Component<Props, State> {
  ...
  debounceAddressCalculation = debounce(() => this.calculateAddressSlice(), 300);
  ...
  handleRegisterHTMLElements = (addressElement: HTMLElement, containerElement: HTMLElement) => {
    setTimeout(() => {
      if (this._isMounted) {
  ...
  calculateAddressSlice = () => {
    if (this._isMounted) {
      ...
      const addressSlice = containerWidth < addressWidth ? Math.floor(containerWidth / charWidth / 2) - 1 : 0;
      this.setState({
        addressSlice
      });
    }
  ...
  renderRow = (address: WalletAddress, index: number) => {
    ...
    return <AddressSequential address={address} onShareAddress={onShareAddress} onCopyAddress={onCopyAddress} shouldRegisterAddressElement={index === 0} onRegisterHTMLElements={this.handleRegisterHTMLElements} addressSlice={addressSlice} />;
  };
