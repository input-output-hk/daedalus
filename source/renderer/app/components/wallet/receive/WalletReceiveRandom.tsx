...
const messages = defineMessages({
  ...
  walletReceiveInstructions: {
    ...
    defaultMessage: '!!!Share this wallet address to receive payments. To protect your privacy, always use a new address when requesting funds. To generate a new address please enter your spending password and press the ‘Generate a new address’ button.',
...
class WalletReceiveRandom extends Component<Props> {
  ...
  renderRow = (address: WalletAddress, index: number) => <AddressRandom index={index} address={address} onCopyAddress={this.props.onCopyAddress} onShareAddress={this.props.onShareAddress} />;
  submit = () => {
    this.form.submit({
      onSuccess: form => {
        const {
  ...
  getFilteredAddresses = (walletAddresses: Array<WalletAddress>): Array<WalletAddress> => walletAddresses.filter((address: WalletAddress) => !address.used || this.props.showUsed);
  ...
  render() {
    ...
    const walletAddressClasses = classnames([styles.hash, isWalletAddressUsed ? styles.usedHash : null]);
    const generateAddressWrapperClasses = classnames([styles.generateAddressWrapper, isSidebarExpanded ? styles.fullWidthOnSmallScreen : null]);
    const generateAddressButtonClasses = classnames(['primary', 'generateAddressButton', walletHasPassword ? styles.submitWithPasswordButton : styles.submitButton, isSubmitting ? styles.spinning : null]);
    const passwordField = form.$('spendingPassword');
    const canSubmit = !isSubmitting && passwordField.value;
    const generateAddressForm = <div className={generateAddressWrapperClasses}>
        {walletHasPassword && <Input className={styles.spendingPassword} {...passwordField.bind()} ref={input => {
        this.passwordField = input;
      }} error={passwordField.error} skin={InputSkin} onKeyPress={this.handleSubmitOnEnter} />}

        <Button className={generateAddressButtonClasses} disabled={!canSubmit} label={intl.formatMessage(messages.generateNewAddressButtonLabel)} skin={ButtonSkin} onClick={this.submit} />
      </div>;
        ...
        <BorderedBox fullHeight>
          <div className={styles.container}>
            <div className={styles.qrCodeAndInstructions}>
              {walletAddress && <div className={styles.qrCode}>
                  <QRCode value={walletAddress} bgColor={qrCodeBackgroundColor} fgColor={qrCodeForegroundColor} size={152} />
                </div>}

              <div className={styles.instructions}>
                {walletAddress && <div className={walletAddressClasses}>
                    {walletAddress}
                    <CopyToClipboard text={walletAddress} // eslint-disable-next-line react/jsx-no-bind
                ...
                <div className={styles.generatedAddresses}>
                  <h2>
                    ...
                    <div className={styles.hideUsed}>
                      <TinySwitch label={intl.formatMessage(messages.showUsedLabel)} onChange={this.toggleUsedAddresses} checked={showUsed} />
                    </div>
                  </h2>

                  <VirtualAddressesList rows={this.getFilteredAddresses(walletAddresses)} renderRow={this.renderRow} />
                </div>
              </> : null}
          </div>
        </BorderedBox>
