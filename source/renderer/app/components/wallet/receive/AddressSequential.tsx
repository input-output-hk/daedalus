...
class AddressSequential extends Component<Props> {
  ...
  componentDidMount() {
    if (this.props.shouldRegisterAddressElement) {
      this.props.onRegisterHTMLElements(this.addressElement, this.addressContainerElement);
    }
  }
  ...
  render() {
    ...
    const addressClasses = classnames(['Address', `receiveAddress-${rawAddress}`, styles.component, address.used ? styles.usedWalletAddress : null]);
    return <div className={addressClasses} onClick={() => onShareAddress(address)} role="link" aria-hidden>
        <div className={styles.addressId} ref={ref => {
        this.addressContainerElement = ref;
      }} id={`address-${rawAddress}`}>
          {renderAddress}
          {shouldRegisterAddressElement && <span ref={ref => {
          this.addressElement = ref;
        }} className={styles.addressElement}>
              {rawAddress}
            </span>}
        </div>
        <AddressActions address={address} onShareAddress={onShareAddress} onCopyAddress={onCopyAddress} type="share" />
      </div>;
  }
