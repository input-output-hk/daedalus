...
const messages = defineMessages({
  ...
  invalidAddressConfirmationLabel: {
    ...
    defaultMessage: '!!!Yes, I am sure I have compared the address displayed in Daedalus with the address displayed on the {deviceType} device by comparing the beginning and ending of the address.',
  ...
  verificationCheckOptionInvalid: {
    ...
    defaultMessage: '!!!No, I am sure that address displayed in Daedalus is different from the address displayed on my {deviceType} device',
  ...
  addressVerificationInstructions: {
    ...
    defaultMessage: '!!!Please compare the address displayed here on the screen with the address displayed on the {deviceType} device by comparing <b>at least the first 5 characters at the start</b> of the address after the "addr" part and <b>at least 5 characters at the end</b> of the address.',
  ...
  invalidAddressWarningDescription: {
    ...
    defaultMessage: '!!!You have manually compared the address shown in Daedalus with the address shown on the hardware wallet device and reported that they are different. If this is the case, please contact support and make sure you download and attach logs.',
...
class WalletReceiveDialog extends Component<Props, State> {
  ...
  constructPaths = (address: WalletAddress) => {
    ...
    const derivationSpendingPath = hardenedPathToDerivationPath(hardenedSpendingPath);
    const spendingPath = map(derivationSpendingPath.constructed, (constructeSpendingPathChunk, index) => {
      const isChangeablePart = index >= derivationSpendingPath.constructed.length - 2;

      if (isChangeablePart) {
        return <b key={`chunk-${index}`}>/{constructeSpendingPathChunk}</b>;
      }

      return index === 0 ? constructeSpendingPathChunk : `/${constructeSpendingPathChunk}`;
    });
    const derivationStakingPath = hardenedPathToDerivationPath(HW_SHELLEY_CONFIG.DEFAULT_DERIVATION_PATH);
    const stakingPath = map(derivationStakingPath.constructed, (constructeStakingPathChunk, index) => {
      const isLastIndex = index === derivationStakingPath.constructed.length - 1;

      if (isLastIndex) {
        return <b key={`chunk-${index}`}>/{constructeStakingPathChunk}</b>;
      }

      return index === 0 ? constructeStakingPathChunk : `/${constructeStakingPathChunk}`;
    });
    return {
      stakingPath,
      spendingPath
  ...
  onChangeVerificationStatus = (status: AddressVerificationCheckStatus) => {
    this.setState({
      selectedVerificationStatus: status === AddressVerificationCheckStatuses.REVERIFY ? null : status,
      isInvalidAddressConfirmed: false,
  ...
  render() {
    ...
    const buttonClasses = classnames(['attention', isSubmitting ? styles.isSubmitting : null]);
    const supportButtonLabel = !isSubmitting ? intl.formatMessage(messages.supportRequestButtonLabel) : <LoadingSpinner />;
    const isSupportRequestButton = selectedVerificationStatus === AddressVerificationCheckStatuses.INVALID;
    let actions;
    ...
    const qrCodeBackgroundColor = document.documentElement ? document.documentElement.style.getPropertyValue('--theme-receive-qr-code-background-color') : 'transparent';
    const qrCodeForegroundColor = document.documentElement ? document.documentElement.style.getPropertyValue('--theme-receive-qr-code-foreground-color') : '#000';
    ...
    const filteredVerificationOptions = filter(verificationOptions, option => {
      const isInvalidOption = option.status === AddressVerificationCheckStatuses.INVALID;

      if (!selectedVerificationStatus && (!isInvalidOption || isInvalidOption && isReverifying) || selectedVerificationStatus && selectedVerificationStatus === option.status) {
        return option;
      }

      return null;
    });
    const showActions = !isHardwareWallet || isHardwareWallet && (selectedVerificationStatus === AddressVerificationCheckStatuses.INVALID || selectedVerificationStatus === AddressVerificationCheckStatuses.VALID);
    const isAddressConfirmed = isAddressChecked && isAddressDerived && selectedVerificationStatus !== null;
    return <Dialog title={intl.formatMessage(messages.dialogTitle)} subtitle={walletName} actions={showActions ? actions : []} closeOnOverlayClick={false} onClose={onClose} closeButton={<DialogCloseButton onClose={onClose} />}>
        <div className={styles.container}>
          <div className={styles.qrCode}>
            <QRCode value={address.id} bgColor={qrCodeBackgroundColor} fgColor={qrCodeForegroundColor} size={192} />
          </div>
          ...
          <CopyToClipboard text={address.id} onCopy={() => onCopyAddress(address.id)}>
          ...
          {isHardwareWallet && <div className={styles.hardwareWalletStatusWrapper}>
              <HardwareWalletStatus hwDeviceStatus={hwDeviceStatus} walletName={walletName} isTrezor={isTrezor} />

              <div className={styles.verificationCheckboxes}>
                <Checkbox checked={isAddressDerived} label={intl.formatMessage(messages.softwareCheckLabel)} disabled skin={CheckboxSkin} />
                <Checkbox checked={isAddressConfirmed} label={intl.formatMessage(messages.confirmationCheckLabel)} disabled skin={CheckboxSkin} />
              </div>

              <p className={styles.verificationInstructions}>
                <FormattedHTMLMessage {...messages.addressVerificationInstructions} values={{
              deviceType
            }} />
              </p>

              {isAddressDerived && isAddressChecked && <RadioSet label={<p>
                      {intl.formatMessage(messages.verificationCheckOptionsLabel)}
                    </p>} items={map(filteredVerificationOptions, option => ({
            key: option.status,
            disabled: false,
            label: option.label,
            selected: option.status === selectedVerificationStatus,
            onChange: () => this.onChangeVerificationStatus(option.status)
          }))} verticallyAligned />}
            </div>}

          {selectedVerificationStatus === AddressVerificationCheckStatuses.INVALID && <div className={styles.warningWrapper}>
              <Checkbox label={intl.formatMessage(messages.invalidAddressConfirmationLabel, {
            deviceType
          })} onChange={this.handleConfirmInvalidAddress} checked={isInvalidAddressConfirmed} skin={CheckboxSkin} />
          ...
          {(!isHardwareWallet || selectedVerificationStatus === AddressVerificationCheckStatuses.VALID) && <TextArea className={styles.noteInput} skin={TextAreaSkin} autoResize={false} rows={3} maxLength={201} {...noteInputField.bind({
