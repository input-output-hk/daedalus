...
export default class WalletCreateSteps extends Component<Props> {
  ...
  get stepsList(): Array<RestoreWalletStep> {
    return CREATE_WALLET_STEPS.map(stepId => this.context.intl.formatMessage(messages[`${stepId}Step`]));
  }
  ...
  render() {
    ...
    return <div>
        <div className={styles.stepCounter}>
          <FormattedHTMLMessage {...messages.stepsCounter} values={{
          currentStep,
          totalSteps
        }} />
        </div>
        <Stepper steps={this.stepsList} activeStep={currentStep} skin={StepperSkin} labelDisabled />
      </div>;
  }
