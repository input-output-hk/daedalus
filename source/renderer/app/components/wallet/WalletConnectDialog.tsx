...
import { isLedgerEnabled, isTrezorEnabled } from "../../config/hardwareWalletsConfig";
import { DeviceModels, DeviceTypes } from "../../../../common/types/hardware-wallets.types";
...
class WalletConnectDialog extends Component<Props> {
  ...
  render() {
    ...
    const buttonLabel = !isSubmitting ? this.context.intl.formatMessage(messages.cancelButton) : <LoadingSpinner />;
    const actions = [{
    ...
    const renderUnknownDevice = () => {
      ...
      if (isTrezorEnabled && !isLedgerEnabled) {
        unknownDeviceElement = <div className={styles.hardwareWalletTrezor}>
            <SVGInline svg={trezorIcon} className={styles.trezorIcon} />
          </div>;
      } else if (isLedgerEnabled && !isTrezorEnabled) {
        unknownDeviceElement = <div className={styles.hardwareWalletLedger}>
            <SVGInline svg={ledgerIcon} className={styles.ledgerIcon} />
          </div>;
      } else {
        unknownDeviceElement = <div className={styles.hardwareWalletUnknown}>
            <SVGInline svg={unknownDeviceIcon} className={styles.unknownDeviceIcon} />
          </div>;
      }
    ...
    const instructions = isLedgerEnabled ? messages.instructions : messages.instructionsTrezorOnly;
    const supportLink = <Link className={styles.externalLink} onClick={() => onExternalLinkClick(intl.formatMessage(messages.connectingIssueSupportLinkUrl))} label={intl.formatMessage(messages.connectingIssueSupportLink)} skin={LinkSkin} />;
    return <Dialog className={dialogClasses} title={intl.formatMessage(messages.dialogTitle)} actions={actions} closeOnOverlayClick={false} onClose={!isSubmitting ? onClose : () => {}} closeButton={<DialogCloseButton />}>
        <div className={styles.hardwareWalletWrapper}>
          {(!transportDevice || !isTrezor && !isLedger) && renderUnknownDevice()}
          {isLedger && isLedgerEnabled && <div className={styles.hardwareWalletLedger}>
              {deviceModel === DeviceModels.LEDGER_NANO_X && <SVGInline svg={ledgerXIcon} className={styles.ledgerXIcon} />}
              {deviceModel === DeviceModels.LEDGER_NANO_S && <SVGInline svg={ledgerIcon} className={styles.ledgerIcon} />}
            </div>}
          {isTrezor && isTrezorEnabled && <div className={styles.hardwareWalletTrezor}>
              <SVGInline svg={trezorIcon} className={styles.trezorIcon} />
            </div>}
          {error ? <p className={styles.error}>{intl.formatMessage(error)}</p> : <div>
              ...
              <div className={styles.hardwareWalletStatusWrapper}>
                <HardwareWalletStatus hwDeviceStatus={hwDeviceStatus} onExternalLinkClick={onExternalLinkClick} isTransactionStatus={false} isTrezor={isTrezor} />
              </div>
              <div className={styles.hardwareWalletIssueArticleWrapper}>
                <p>
                  <FormattedMessage {...messages.connectingIssueSupportLabel} values={{
                supportLink
              }} />
                </p>
              </div>
