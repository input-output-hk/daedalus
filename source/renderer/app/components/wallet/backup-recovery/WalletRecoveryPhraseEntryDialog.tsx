...
const messages = defineMessages({
  verificationInstructions: {
    ...
    defaultMessage: '!!!Please enter your {wordCount}-word wallet recovery phrase. Make sure you enter the words in the correct order.',
  ...
  recoveryPhraseInputLabel: {
    ...
    description: 'Label for the recovery phrase input on dialog for entering wallet recovery phrase.'
  },
  ...
  recoveryPhraseInputPlaceholder: {
    id: 'wallet.backup.recovery.phrase.entry.dialog.recoveryPhraseInputPlaceholder',
  ...
  termOffline: {
    ...
    defaultMessage: '!!!I understand that the simplest way to keep my wallet recovery phrase secure is to never store it digitally or online. If I decide to use an online service, such as a password manager with an encrypted database, it is my responsibility to make sure that I use it correctly.',
  ...
  termRecovery: {
    ...
    defaultMessage: '!!!I understand that the only way to recover my wallet if my computer is lost, broken, stolen, or stops working is to use my wallet recovery phrase.',
...
class WalletRecoveryPhraseEntryDialog extends Component<Props> {
      ...
      recoveryPhrase: {
        ...
        validators: ({
          ...
          return validateMnemonics({
            ...
            validator: () => [isValidMnemonic(enteredWords.join(' '), enteredWords.length), this.context.intl.formatMessage(messages.recoveryPhraseInvalidMnemonics)]
          });
        }
      }
    }
  ...
  render() {
    ...
    const buttonLabel = !isSubmitting ? intl.formatMessage(messages.buttonLabelConfirm) : <LoadingSpinner />;
    const actions = [{
      label: buttonLabel,
    ...
    return <Dialog className={dialogClasses} title={intl.formatMessage(globalMessages.recoveryPhraseDialogTitle)} actions={actions} closeOnOverlayClick={false} onClose={onCancelBackup} closeButton={<DialogCloseButton onClose={onCancelBackup} />} backButton={!isValid ? <DialogBackButton onBack={onRestartBackup} /> : null}>
        {!isValid && <>
            <WalletRecoveryInstructions instructionsText={intl.formatMessage(messages.verificationInstructions, {
          wordCount
        })} />

            <Autocomplete {...recoveryPhraseField.bind()} label={intl.formatMessage(messages.recoveryPhraseInputLabel)} placeholder={intl.formatMessage(messages.recoveryPhraseInputPlaceholder, {
          wordNumber: enteredPhrase.length + 1
        })} options={suggestedMnemonics} requiredSelections={[wordCount]} requiredSelectionsInfo={(required, actual) => intl.formatMessage(globalMessages.knownMnemonicWordCount, {
          actual,
          required
        })} maxSelections={wordCount} error={errorOrIncompleteMarker(recoveryPhraseField.error)} maxVisibleOptions={5} noResultsMessage={intl.formatMessage(messages.recoveryPhraseNoResults)} skin={AutocompleteSkin} optionHeight={50} />
          </>}
        ...
        {isValid && <>
            ...
            <div>
              <div className={styles.checkbox}>
                <Checkbox label={<FormattedHTMLMessage {...messages.termOffline} />} onChange={onAcceptTermOffline} checked={isTermOfflineAccepted} skin={CheckboxSkin} />
              </div>
              <div className={styles.checkbox}>
                <Checkbox className={styles.isBold} label={intl.formatMessage(messages.termRecovery)} onChange={onAcceptTermRecovery} checked={isTermRecoveryAccepted} skin={CheckboxSkin} />
              </div>
            </div>
          </>}
      </Dialog>;
