...
export function PoolPopOver(props: {
  ...
  return <>
      <div className={styles.triggerTarget} onMouseEnter={() => setIsHovered(true)} ref={popOverTargetRef}>
        {props.children}
      </div>
      {isHovered ? // Init the pop over only when the target is hovered
    <PopOver interactive delay={props.openWithDelay ? STAKE_POOL_TOOLTIP_HOVER_WAIT : 0} duration={0} appendTo={document.body} trigger={props.openOnHover ? 'mouseenter' : 'click'} placement="auto" onShow={() => props.onOpen && props.onOpen(poolId)} onHide={close} onClickOutside={close} themeVariables={{
      '--rp-pop-over-bg-color': 'var(--theme-staking-stake-pool-tooltip-background-color)',
    ...
    reference={popOverTargetRef} content={<TooltipPool color={props.color} containerClassName={props.containerClassName} currentTheme={props.currentTheme} numberOfRankedStakePools={props.numberOfRankedStakePools} onClose={close} onOpenExternalLink={props.onOpenExternalLink} onSelect={() => {
      close();
      if (props.onSelect) props.onSelect(poolId);
    }} showWithSelectButton={props.showWithSelectButton} stakePool={props.stakePool} isGridRewardsView={props.isGridRewardsView} />} /> : null}
    </>;
}
