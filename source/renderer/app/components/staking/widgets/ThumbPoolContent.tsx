...
class ThumbPoolContent extends Component<Props> {
  ...
  render() {
    ...
    const componentClassnames = classnames([styles.component, !IS_SATURATION_DATA_AVAILABLE ? styles.hideSaturation : null]);
    const saturationClassnames = classnames([styles.saturationBar, styles[getSaturationColor(saturation)]]);
    return <div className={componentClassnames}>
        <div className={styles.ticker}>{ticker}</div>
        {isGridRewardsView && (IS_RANKING_DATA_AVAILABLE && nonMyopicMemberRewards ? <div className={styles.rewards}>
        ...
        {!isGridRewardsView && (IS_RANKING_DATA_AVAILABLE ? <div className={styles.ranking} style={{
        color
      }}>
              {nonMyopicMemberRewards ? ranking : <>
        ...
        {IS_SATURATION_DATA_AVAILABLE && <div className={saturationClassnames}>
            <span style={{
          width: `${parseFloat(saturation).toFixed(2)}%`
        }} />
          </div>}
        {IS_RANKING_DATA_AVAILABLE ? <>
            {retiring && <div className={styles.clock}>
                <SVGInline svg={clockIcon} className={styles.clockIcon} />
              </div>}
            <div className={styles.colorBand} style={{
          background: color
        }} />
