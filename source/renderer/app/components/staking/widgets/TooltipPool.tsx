...
import { formattedWalletAmount, toFixedUserFormat } from "../../../utils/formatters";
...
const messages = defineMessages({
  ...
  relativeStakeTooltip: {
    ...
    defaultMessage: '!!!Measures the amount of stake pledged by the pool plus the amount of stake currently delegated to the pool, versus the total amount in the system.',
  ...
  profitMarginTooltip: {
    ...
    defaultMessage: "!!!The pool's profit, defined as the rewards percentage kept by the pool from the stake that was delegated to it.",
  ...
  producedBlocksTooltip: {
    ...
    defaultMessage: '!!!The total number of blocks the stake pool has produced.',
  ...
  potentialRewardsTooltip: {
    ...
    defaultMessage: "!!!An estimation of the potential rewards you will earn per epoch if you delegate the intended amount of stake. The system looks at the pool's parameters and historical performance data to calculate potential rewards, assuming that the pool reaches optimal saturation.",
  ...
  saturationTooltip: {
    ...
    defaultMessage: '!!!Saturation measures the stake in the pool and indicates the point at which rewards stop increasing with increases in stake. This capping mechanism encourages decentralization by discouraging users from delegating to oversaturated stake pools.',
  ...
  pledgeTooltip: {
    ...
    defaultMessage: '!!!The amount of stake that a pool operator contributes to a pool. Pools with higher pledge amounts earn more rewards for themselves and their delegators. Pools that do not honor their pledge earn zero rewards and accrue low ranking.',
...
class TooltipPool extends Component<Props, State> {
  ...
  renderDescriptionFields = () => {
    ...
    const alpha = 0.3;
    const saturationBarClassnames = classnames([styles.saturationBar, styles[getSaturationColor(saturation)]]);
    const fields = [{
      key: 'saturation',
      value: <div className={styles.saturationValue}>
            <span>
      ...
      key: 'ranking',
      value: <div className={styles.ranking}>
            {IS_RANKING_DATA_AVAILABLE ? <span style={{
          background: getColorFromRange(ranking, {
            ...
            numberOfItems: numberOfRankedStakePools
                ...
                {potentialRewards.isZero && !potentialRewards.isZero() ? ranking : <>
                    {numberOfRankedStakePools + 1}
                    <span className={styles.asterisk}>*</span>
                  </>}
              </span> : <div className={styles.noDataDash}>
                <SVGInline svg={noDataDashSmallImage} />
              </div>}
          </div>
    }, {
      key: 'relativeStake',
      value: <div className={styles.defaultColor}>
            <span className={styles.defaultColorContent}>{`${toFixedUserFormat(relativeStake, 2)}%`}</span>
          </div>
    }, {
      key: 'profitMargin',
      value: <div>
      ...
      key: 'costPerEpoch',
      value: <div className={styles.costValue}>
            <span style={{
          background: getColorFromRange(profitMargin, {
            darken,
            alpha
      ...
      value: <div className={styles.defaultColor}>
            {isGridRewardsView && potentialRewards.isZero && potentialRewards.isZero() ? <div className={styles.noDataDash}>
                <SVGInline svg={noDataDashSmallImage} />
              </div> : <span className={styles.defaultColorContent}>
    ...
    return <div className={styles.table}>
        ...
        const labelPart = <div className={styles[`${field.key}Label`]}>
              <div className={styles.labelContainer}>
                <div className={styles.fieldLabel}>
                  {intl.formatMessage(messages[field.key])}
                </div>
                <PopOver offset={[0, 10]} key={field.key} content={<div className={styles.tooltipWithHTMLContent}>
                      <FormattedHTMLMessage {...messages[`${field.key}Tooltip`]} />
                    </div>}>
        ...
        return <div key={field.key} className={styles.dRow}>
              {labelPart}
              {field.value}
            </div>;
      })}
  ...
  render() {
    ...
    const retirementFromNow = retiring ? moment(retiring).locale(intl.locale).fromNow(true) : '';
    const idCopyIcon = idCopyFeedback ? copyCheckmarkIcon : copyIcon;
    const hoverContentClassnames = classnames([styles.hoverContent, idCopyFeedback ? styles.checkIcon : styles.copyIcon]);
    const colorBandClassnames = classnames([styles.colorBand, this.isGreyColor ? styles.greyColorBand : null]);
    const colorBandStyle = this.isGreyColor ? {} : {
      background: this.props.color
    };
    return <div className={styles.component} style={componentStyle} ref={this.rootRef}>
        ...
        <div className={styles.container}>
          ...
          {retiring && <div className={styles.retirement}>
              <FormattedMessage {...messages.retirement} values={{
            retirementFromNow
          }} />
            </div>}
          <PopOver key="id" content={<div className={styles.tooltipWithHTMLContent}>
                {intl.formatMessage(messages.copyIdTooltipLabel)}
              </div>}>
            <div className={styles.id} onMouseOut={this.onIdMouseOut} onBlur={() => {}}>
          ...
          <Link onClick={() => onOpenExternalLink(homepage)} className={styles.homepage} label={homepage} skin={LinkSkin} />
          {this.renderDescriptionFields()}
        </div>
        {onSelect && showWithSelectButton && <Button label={intl.formatMessage(messages.delegateButton)} onClick={() => onSelect && onSelect()} skin={ButtonSkin} />}
      </div>;
  }
