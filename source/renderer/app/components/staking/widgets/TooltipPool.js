...
export default class TooltipPool extends Component<Props, State> {
  ...
  getTopBottomPosition = (left: number) => {
    ...
    const componentLeft = !fromStakePool ?
      ...
      THUMBNAIL_OFFSET_WIDTH +
      paddingOffset : -((TOOLTIP_WIDTH * left) / this.containerWidth) +
      THUMBNAIL_OFFSET_WIDTH + (left - THUMBNAIL_OFFSET_WIDTH - THUMBNAIL_WIDTH);
  ...
  getLeftRightPosition = (top: number, isTopHalf: boolean, left: number) => {
    ...
    const componentLeft = fromStakePool ? -((TOOLTIP_WIDTH * left) / this.containerWidth) +
      THUMBNAIL_OFFSET_WIDTH + (left - THUMBNAIL_OFFSET_WIDTH + ARROW_HEIGHT) : THUMBNAIL_HEIGHT;
