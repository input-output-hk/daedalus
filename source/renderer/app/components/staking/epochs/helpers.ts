...
export const noDataExisting = (isLoading, data) => !isLoading && (data && !data.length || !data);
export const hasDataExisting = (isLoading, data) => data && data.length > 0 && !isLoading;
...
export const humanizeDurationToShort = (currentLocale, dateTime) => {
  ...
  return humanizeDuration(Math.max(0, new Date(dateTime).getTime() - new Date().getTime()), {
    round: true,
