...
class StakingEpochs extends Component<Props, State> {
  ...
  onSelectedEpochChange = (selectedEpoch: string) => this.setState({
    selectedEpoch
  });
  ...
  render() {
    ...
    const epochSelectOptions = [{
      label: `${currentEpochName} (${intl.formatMessage(messages.currentEpochHeading)})`,
      value: CURRENT_EPOCH
    }, {
      label: `${previousEpochName} (${intl.formatMessage(messages.previousEpochHeading)})`,
    ...
    return <div className={styles.component}>
        <BorderedBox>
          <div className={styles.headerWrapper}>
            <Select className={styles.epochSelector} options={epochSelectOptions} value={selectedEpoch} onChange={this.onSelectedEpochChange} skin={SelectSkin} selectionRenderer={option => <div className={styles.customSelectValue}>{option.label}</div>} optionHeight={50} />
          </div>
          {selectedEpoch === CURRENT_EPOCH && <div>
              <div className={styles.currentEpochProgressBar}>
                <div className={styles.progressBarContainer}>
                  <div className={styles.progress} style={{
                width: `${currentEpochProgress}%`
              }}>
                    <div className={styles.overlapProgressLabel} style={{
              ...
              <StakingEpochsCurrentEpochData currentEpochData={currentEpochData} isLoading={isLoading} />
            </div>}
          {selectedEpoch === PREVIOUS_EPOCH && <StakingEpochsPreviousEpochData previousEpochData={previousEpochData} isLoading={isLoading} />}
