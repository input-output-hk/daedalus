...
import { getTableHeadersForPreviousEpoch, noDataExisting, hasDataExisting, sortData } from "./helpers";
...
class StakingEpochsPreviousEpochData extends Component<Props, State> {
  ...
  render() {
    ...
    if (hasData) {
      sortedData = sortData(previousEpochData, previousEpochDataOrder, previousEpochDataSortBy);
    }
    ...
    const tableBody = <tbody>
        ...
        return <tr key={key}>
              ...
              <td>
                ...
                <span>{` ${intl.formatMessage(messages.tableBodySlots)} - `}</span>
                <span className={styles.mediumText}>{`${slotsElected[1]}%`}</span>
              </td>
              <td>
                <span>{`${performance[0]} ${intl.formatMessage(messages.tableBodyOf)} ${performance[1]} - `}</span>
                <span className={styles.mediumText}>{`${performance[2]}%`}</span>
              </td>
              <td>
                <span className={styles.mediumText}>{sharedRewards[0]}</span>
                <span className={styles.uppercaseText}>{` ${intl.formatMessage(globalMessages.adaUnit)} `}</span>
    ...
    return <StakingEpochsDataTable tableHeaders={tableHeaders} tableBody={tableBody} order={previousEpochDataOrder} sortBy={previousEpochDataSortBy} handleDataSort={this.handleDataSort} />;
  }
