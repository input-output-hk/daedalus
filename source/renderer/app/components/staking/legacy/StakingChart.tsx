...
class StakingChart extends Component<Props, State> {
  ...
  render() {
    ...
    if (isHovered && hoveredBarData && hoveredBarData.numberOfTransactions > 0) {
      tooltip = <div className={styles.toolTip} style={tooltipPos}>
          <StakingChartTooltip {...hoveredBarData} />
        </div>;
    }

    return <div className={styles.component} onMouseMove={this.onMouseMove.bind(this)}>
        <BarChart width={width} height={height} data={data.slice()} barSize={10} barCategoryGap={2} barGap={2}>
          <XAxis hide dataKey="slot" />
          <YAxis axisLine={false} tickLine={false} allowDecimals={false} ticks={ticks.slice()} domain={['dataMin', 'dataMax + 10']} />
          <CustomReferenceLine x={refLineSlot} stroke="#5e6066" />
          <Bar dataKey="numberOfTransactions" onMouseEnter={barData => this.setState({
          isHovered: true,
          ...
          hoveredBarData: null
            ...
            return <Cell cursor={cursor} fill={fillColor} // eslint-disable-next-line react/no-array-index-key
            key={`cell-${index}`} />;
          })}
          </Bar>
        </BarChart>
