...
const messages = defineMessages({
  ...
  description: {
    ...
    defaultMessage: '!!!You have successfully redeemed <b>{redeemedRewards}</b> to your <b>{walletName}</b> wallet. This transaction incurred <b>{transactionFees}</b> in transaction fees',
...
class Step3SuccessDialog extends Component<Props> {
  ...
  render() {
    ...
    if (onPDFDownload) actions.push({
      primary: true,
      label: intl.formatMessage(messages.downloadPDFButtonLabel),
      onClick: onPDFDownload
    });
    const closeButton = <DialogCloseButton onClose={onClose} />;
    return <Dialog onClose={onClose} actions={actions} closeButton={closeButton} closeOnOverlayClick={false} fullSize>
