...
const messages = defineMessages({
  ...
  spendingPasswordLabel: {
    ...
    defaultMessage: '!!!Wallet spending password <em>(<b>{walletName}</b> wallet)</em>',
    description: 'spendingPasswordLabel for Redeem Incentivized Testnet - Step 2'
  },
  spendingPasswordPlaceholder: {
    id: 'staking.redeemItnRewards.step2.spendingPasswordPlaceholder',
    defaultMessage: '!!!Password',
    description: 'spendingPasswordPlaceholder for Redeem Incentivized Testnet - Step 2'
  },
  continueButtonLabel: {
...
class Step2ConfirmationDialog extends Component<Props> {
  ...
  form = new ReactToolboxMobxForm({
    fields: {
      spendingPassword: {
        type: 'password',
        label: <FormattedHTMLMessage {...messages.spendingPasswordLabel} values={{
  ...
  render() {
    ...
    const calculatedTransactionFees = differenceBetweenAmountAndFee.isLessThan(minRewardsReceiverBalance) ? amount : transactionFees;
    const {
      name: walletName
    } = wallet;
    ...
    return <Dialog title={intl.formatMessage(messages.title)} actions={actions} onContinue={onContinue} onClose={onClose} closeButton={closeButton} onBack={onBack} closeOnOverlayClick={false} fullSize>
        <div className={styles.to}>
          <div>{intl.formatMessage(messages.walletToLabel)}</div>
          <div>
            <FormattedHTMLMessage {...messages.walletToName} values={{
            walletName
          }} />
          </div>
        </div>
        ...
        <Input className={styles.spendingPassword} {...spendingPasswordField.bind()} skin={InputSkin} error={spendingPasswordField.error} onKeyPress={this.handleSubmitOnEnter} />
