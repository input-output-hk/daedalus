...
import type { NextEpoch, TipInfo, FutureEpoch } from "../../../api/network/types";
...
const messages = defineMessages({
  ...
  description: {
    ...
    defaultMessage: '!!!Changes to delegation preferences will take effect after both the current and next Cardano epochs have completed. Epochs on the Incentivized Testnet last one day. Any changes made now will take effect in {timeUntilFutureEpoch}.',
...
class DelegationCenterHeader extends Component<Props, State> {
  ...
  configureUpdateTimer = () => {
    ...
    this.intervalHandler = setInterval(() => this.updateTimeUntilFutureEpoch(), EPOCH_COUNTDOWN_INTERVAL);
  };
  updateTimeUntilFutureEpoch = () => {
    ...
    if (epochStart) {
      const timeUntilFutureEpoch = Math.max(0, new Date(epochStart).getTime() - new Date().getTime());
      this.setState({
  ...
  generateCurrentEpochPanels = (epoch: number, slots: number, totalSlots: number) => {
    const {
      intl
    } = this.context;
    ...
    const labels: Array<string> = [epochLabel, currentSlotLabel, totalSlotsLabel];
    const values = [epoch, slots, totalSlots];
    const keys = ['epoch', 'slots', 'totalSlots'];
    return labels.map<any>((label: string, index: number) => <Fragment key={keys[index]}>
  ...
  render() {
    ...
    const timeUntilFutureEpoch = humanizeDurationByLocale(this.state.timeUntilFutureEpoch, currentLocale);
    ...
    const fieldPanels = this.generateCurrentEpochPanels(epoch, slot, epochLength);
    ...
    return <div className={styles.component}>
        <div className={styles.mainContent}>
          <div className={styles.mainContainer}>
            ...
            {showNextEpochCountdown && <div className={styles.countdownContainer}>
                <div className={styles.heading}>{headingSecond}</div>
                <CountdownWidget startDateTime={nextEpochStart} format="DD-HH-mm-ss" />
              </div>}
          </div>
