...
class DelegationCenterBody extends Component<Props> {
  ...
  render() {
    ...
    return <div className={componentClasses}>
        {isLoading ? <div className={styles.loadinBlockWrapper}>
            <p>{intl.formatMessage(messages.loadingStakePoolsMessage)}</p>
            {loadingSpinner}
          </div> : <div>
            <div className={styles.bodyTitle}>
              <div className={styles.leftBodyTitle}>{title}</div>
              {nextEpochNumber && futureEpochNumber && <div className={styles.rightBodyTitle}>
            ...
            <div className={styles.mainContent}>
              {wallets.map((wallet: Wallet) => <WalletRow key={wallet.id} wallet={wallet} numberOfStakePools={numberOfStakePools} numberOfRankedStakePools={numberOfRankedStakePools} onDelegate={() => onDelegate(wallet.id)} onUndelegate={() => onUndelegate(wallet.id)} delegatedStakePool={getStakePoolById(wallet.delegatedStakePoolId)} getStakePoolById={getStakePoolById} nextEpochNumber={nextEpochNumber} futureEpochNumber={futureEpochNumber} currentTheme={currentTheme} isListActive={isListActive} listName={listName} onOpenExternalLink={onOpenExternalLink} containerClassName={containerClassName} setListActive={setListActive} />)}
            </div>
          </div>}
      </div>;
  }
