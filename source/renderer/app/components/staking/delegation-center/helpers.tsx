...
export const generateFieldPanel = (labels: any, values: any, index: number) => {
  ...
  const includeDotsDelimeter = !includeSlashDelimeter && index !== values.length - 1;
  const labelStr = labels[index];
  const valueStr = value.toString();
  let zeroValues = '';

  if (index === 1 && valueStr.length < values[index + 1].toString().length || index === 0 && valueStr.length < EPOCH_MAX_LENGTH) {
    const zerosToAdd = index === 1 ? parseInt(values[index + 1].toString().length, 10) - parseInt(valueStr.length, 10) : parseInt(EPOCH_MAX_LENGTH, 10) - parseInt(valueStr.length, 10);
  ...
  return <div className={styles.fieldPanel}>
      ...
      {includeDotsDelimeter && <div className={styles.right}>
          <SVGInline svg={delimeterIcon} className={styles.delimeterIcon} />
        </div>}
      {includeSlashDelimeter && <div className={styles.right}>
          <SVGInline svg={delimeterSlashIcon} className={styles.delimeterSlashIcon} />
        </div>}
    </div>;
};