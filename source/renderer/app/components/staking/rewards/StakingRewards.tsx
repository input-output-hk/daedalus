...
import { bigNumberComparator, stringComparator } from "../../../utils/sortComparators";
...
class StakingRewards extends Component<Props, State> {
  ...
  handleExportCsv = (availableTableHeaders: Array<any>, sortedRewards: Array<Reward>) => {
    const {
      onExportCsv
    } = this.props;
    const {
    ...
    const exportedHeader = [...availableTableHeaders.map(header => header.title), intl.formatMessage(messages.tableHeaderDate)];
    ...
    const exportedBody = sortedRewards.map(reward => {
      ...
      const rewardAmount = get(reward, REWARD_FIELDS.REWARD).toFormat(DECIMAL_PLACES_IN_ADA);
      const rewardsAddress = get(reward, REWARD_FIELDS.REWARDS_ADDRESS);
      return [rewardWallet, rewardsAddress, isRestoring ? '-' : rewardAmount, date];
    });
  ...
  getSortedRewards = (): Array<Reward> => {
    ...
    return rewards.slice().sort((rewardA: Reward, rewardB: Reward) => {
      const rewardCompareResult = bigNumberComparator(rewardA.reward, rewardB.reward, rewardsOrder === REWARD_ORDERS.ASCENDING);
      const walletNameCompareResult = stringComparator(rewardA.wallet, rewardB.wallet, rewardsOrder === REWARD_ORDERS.ASCENDING);
      const walletAddressCompareResult = stringComparator(rewardA.rewardsAddress, rewardB.rewardsAddress, rewardsOrder === REWARD_ORDERS.ASCENDING);
  ...
  render() {
    ...
    const availableTableHeaders = [{
      ...
      title: `${intl.formatMessage(messages.tableHeaderReward)} (${intl.formatMessage(globalMessages.adaUnit)})`
    }];
    const exportCsvButtonLabel = isExporting ? <div className={styles.exportingSpinnerWrapper}>
        <LoadingSpinner />
      </div> : <>
    ...
    const headerWrapperClasses = classNames([styles.headerWrapper, contentScrollTop > 10 ? styles.headerWrapperWithShadow : null]);
    return <div className={styles.component}>
        <div className={headerWrapperClasses}>
          <div className={styles.title}>
            {intl.formatMessage(messages.title)}
          </div>
          {!noRewards && <Button className={exportCsvButtonClasses} label={exportCsvButtonLabel} onClick={() => this.handleExportCsv(availableTableHeaders, sortedRewards)} skin={ButtonSkin} />}
        </div>
        <div className={styles.contentWrapper} onScroll={this.handleContentScroll}>
          <BorderedBox>
            {noRewards && <div className={styles.noRewardsLabel}>
                {intl.formatMessage(messages.noRewards)}
              </div>}

            {sortedRewards.length > 0 && <table>
                <thead>
                  <tr>
                    {map(availableTableHeaders, tableHeader => {
                  const isSorted = tableHeader.name === rewardsSortBy;
                  const sortIconClasses = classNames([styles.sortIcon, isSorted ? styles.sorted : null, isSorted && rewardsOrder === 'asc' ? styles.ascending : null]);
                  return <th key={tableHeader.name} onClick={() => this.handleRewardsSort(tableHeader.name)}>
                          {tableHeader.title}
                ...
                const rewardWallet = get(reward, REWARD_FIELDS.WALLET_NAME);
                const isRestoring = get(reward, REWARD_FIELDS.IS_RESTORING);
                const syncingProgress = get(reward, REWARD_FIELDS.SYNCING_PROGRESS);
                const rewardAmount = get(reward, REWARD_FIELDS.REWARD).toFormat(DECIMAL_PLACES_IN_ADA);
                const rewardsAddress = get(reward, REWARD_FIELDS.REWARDS_ADDRESS);
                return <tr key={key}>
                        ...
                        <td className={styles.rewardsAddress}>
                          {rewardsAddress && <div>
                              <CopyToClipboard text={rewardsAddress} onCopy={() => onCopyAddress(rewardsAddress)}>
                              ...
                              {IS_EXPLORER_LINK_BUTTON_ENABLED && <ButtonLink className={explorerButtonClasses} onClick={() => onOpenExternalLink(rewardsAddress)} skin={ButtonSkin} label={intl.formatMessage(messages.actionViewInExplorer)} linkProps={{
                        ...
                        <td className={styles.rewardAmount}>
                          ...
                          {isRestoring && <div className={styles.syncingProgress}>
                              <PopOver content={intl.formatMessage(messages.syncingTooltipLabel, {
                        syncingProgress
                      })}>
            ...
            {isLoading && <div className={styles.loadingSpinnerWrapper}>
                <LoadingSpinner />
              </div>}
          </BorderedBox>
