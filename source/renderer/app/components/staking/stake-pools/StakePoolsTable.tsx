...
const messages = defineMessages({
  ...
  tableHeaderPotentialRewards: {
    ...
    description: 'Table header "Potential rewards" label on stake pools list view page'
  },
...
class StakePoolsTable extends Component<Props, State> {
  ...
  componentWillUnmount() {
    ...
    this.scrollableDomElement = document.querySelector(`.${this.props.containerClassName}`);
  }
  ...
  render() {
    ...
    if (stakePoolsList.length > PRELOADER_THRESHOLD && isPreloading) return <div className={styles.preloadingBlockWrapper}>
          <LoadingSpinner big />
        </div>;
    const sortedStakePoolList = orderBy(stakePoolsList.map(stakePool => {
    ...
    }), ['formattedTicker', 'calculatedPledge', 'calculatedCost', stakePoolsSortBy], [stakePoolsOrder, stakePoolsOrder, stakePoolsOrder, stakePoolsOrder]);
    const availableTableHeaders = [{
      name: 'ranking',
      title: <PopOver key="ranking" placement="bottom" content={<div className={styles.tooltipWithHTMLContent}>
      ...
      name: 'saturation',
      title: <PopOver key="saturation" placement="bottom" content={intl.formatMessage(messages.tableHeaderSaturationTooltip)}>
            {intl.formatMessage(messages.tableHeaderSaturation)}
          </PopOver>
    }, {
      name: 'cost',
      title: <PopOver key="cost" placement="bottom" content={intl.formatMessage(messages.tableHeaderCostTooltip)}>
            {intl.formatMessage(messages.tableHeaderCost)}
          </PopOver>
    }, {
      name: 'profitMargin',
      title: <PopOver key="profitMargin" placement="bottom" content={intl.formatMessage(messages.tableHeaderMarginTooltip)}>
            {intl.formatMessage(messages.tableHeaderMargin)}
          </PopOver>
    }, {
      name: 'producedBlocks',
      title: <PopOver key="producedBlocks" placement="bottom" content={intl.formatMessage(messages.tableHeaderProducedBlocksTooltip)}>
            {intl.formatMessage(messages.tableHeaderProducedBlocks)}
          </PopOver>
    }, {
      name: 'nonMyopicMemberRewards',
      title: <PopOver key="nonMyopicMemberRewards" placement="bottom" content={intl.formatMessage(messages.tableHeaderPotentialRewardsTooltip)}>
            {intl.formatMessage(messages.tableHeaderPotentialRewards)}
          </PopOver>
    }, {
      name: 'pledge',
      title: <PopOver key="pledge" placement="bottom" content={intl.formatMessage(messages.tableHeaderPledgeTooltip)}>
    ...
    return <div>
        <div className={componentClasses}>
          {sortedStakePoolList.length > 0 && <BorderedBox>
              <table>
                <thead onMouseEnter={onTableHeaderMouseEnter} onMouseLeave={onTableHeaderMouseLeave}>
                  <tr>
                    <StakePoolsTableHeader availableTableHeaders={availableTableHeaders} stakePoolsSortBy={stakePoolsSortBy} stakePoolsOrder={stakePoolsOrder} onHandleSort={this.handleSort} />
                  </tr>
                </thead>
                <tbody className={currentLocale === 'ja-JP' ? styles.japaneseHeader : null}>
                  <StakePoolsTableBody sortedStakePoolList={sortedStakePoolList} numberOfRankedStakePools={numberOfRankedStakePools} currentTheme={currentTheme} onOpenExternalLink={onOpenExternalLink} showWithSelectButton={showWithSelectButton} containerClassName={containerClassName} onSelect={onSelect} selectedPoolId={selectedPoolId} listName={listName} stakePoolsList={stakePoolsList} stakePoolsSortBy={stakePoolsSortBy} stakePoolsOrder={stakePoolsOrder} />
                </tbody>
              </table>
            </BorderedBox>}
        </div>
