...
export const StakePoolsList = observer((props: StakePoolsListProps) => {
  ...
  useEffect(() => {
    ...
    const scrollContainer = props.scrollElementRef ? props.scrollElementRef.current : null;
  ...
  if (props.stakePoolsList.length > PRELOADER_THRESHOLD && isLoading) {
    return <div className={styles.preloadingBlockWrapper}>
        <LoadingSpinner big />
      </div>;
  }
  ...
  function rowRenderer(itemsPerRow, {
    index,
    key,
    style
  }) {
    ...
    return <div key={key} style={style}>
        <div className={styles.tiles}>
          {stakePools.map(stakePool => <ThumbPool key={stakePool.id} stakePool={stakePool} onOpenExternalLink={props.onOpenExternalLink} highlightOnHover={props.highlightOnHover} highlightWithDelay={props.highlightWithDelay} showWithSelectButton={props.showWithSelectButton} onSelect={props.onSelect} selectOnClick={props.selectOnClick} isSelected={props.selectedPoolId === stakePool.id} currentTheme={props.currentTheme} containerClassName={props.containerClassName} numberOfRankedStakePools={props.numberOfRankedStakePools} disabledStakePoolId={props.disabledStakePoolId} isGridRewardsView={props.isGridRewardsView} />)}
          {numberOfMissingRowItems > 0 ? times(numberOfMissingRowItems, i => <div key={`${key}-${i}`} className={styles.rowFillerItem} />) : null}
        </div>
      </div>;
  }

  return <WindowScroller scrollElement={props.scrollElementRef && props.scrollElementRef.current ? props.scrollElementRef.current : window}>
      {({
      height,
      scrollTop,
      registerChild
        ...
        const itemsPerRow = Math.floor(width / (POOL_THUMB_SIZE + POOL_THUMB_GRID_GAP));
        const rowCount = Math.ceil(stakePoolsCount / itemsPerRow);
        return <div ref={el => registerChild(el)}>
                <List autoHeight className={styles.list} width={width} height={height} scrollTop={scrollTop} rowHeight={POOL_THUMB_SIZE} rowCount={rowCount} rowRenderer={args => rowRenderer(itemsPerRow, args)} overscanRowCount={3} />
              </div>;
      }}
        </AutoSizer>}
    </WindowScroller>;
