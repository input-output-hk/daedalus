...
import { shortNumber, generateThousands, formattedWalletAmount, toFixedUserFormat } from "../../../utils/formatters";
import { getFilteredWallets, getAllAmounts } from "../../../utils/walletsForStakePoolsRanking";
import Wallet from "../../../domains/Wallet";
import { RANKING_SLIDER_RATIO, MIN_DELEGATION_FUNDS_LOG, MIN_DELEGATION_FUNDS, INITIAL_DELEGATION_FUNDS_LOG, INITIAL_DELEGATION_FUNDS, CIRCULATING_SUPPLY, ALL_WALLETS_SELECTION_ID, IS_RANKING_DATA_AVAILABLE } from "../../../config/stakingConfig";
...
const messages = defineMessages({
  ...
  rankingAllWalletsStart: {
    ...
    defaultMessage: '!!!Stake pools are currently ranked based on the combined amount in',
    description: 'All wallets description before dropdown.'
  },
  rankingDescription: {
    id: 'staking.stakePools.rankingDescription',
    defaultMessage: '!!!Use the slider to rank the stake pools and check the potential rewards <strong>based on the amount of stake you intend to delegate</strong>.',
  ...
  rankingOneWalletStart: {
    ...
    defaultMessage: '!!!Stake pools are currently ranked based on the amount in',
...
class StakePoolsRanking extends Component<Props, State> {
  ...
  state = {
    sliderValue: Math.round(INITIAL_DELEGATION_FUNDS_LOG * RANKING_SLIDER_RATIO),
  ...
  componentDidMount() {
    ...
    if (stake) {
      ...
      const displayValue = hasDecimal ? formattedWalletAmount(new BigNumber(stake), false) : toFixedUserFormat(stake, 0);
      this.setState({
  ...
  componentDidUpdate(prevProps: Props) {
    const {
      selectedDelegationWalletId: prevWalletId
    } = prevProps;
    const {
      selectedDelegationWalletId: currentWalletId
    } = this.props;
  ...
  onSelectedWalletChange = (selectedWalletId: string) => {
    ...
    const wasSelectedWalletChanged = selectedWalletId !== selectedDelegationWalletId;
    // Prevent ranking stake pools if we don't have data for the selected wallet ready
    if (wasSelectedWalletChanged && !isAllWalletsSelected && !hasSelectedWallet) return;
    let amountValue = 0;
    let sliderValue = 0;

    if (selectedWalletId === ALL_WALLETS_SELECTION_ID) {
      amountValue = Math.min(getAllAmounts(wallets).toNumber(), maxDelegationFunds);
    ...
    const displayValue = formattedWalletAmount(new BigNumber(amountValue), false);
    this.setState({
      sliderValue,
      displayValue
    });
  ...
  onSliderChange = (sliderValue: number) => {
    ...
    if (sliderValue === Math.round(MIN_DELEGATION_FUNDS_LOG * RANKING_SLIDER_RATIO)) {
      amountValue = MIN_DELEGATION_FUNDS;
    } else if (sliderValue === Math.round(maxDelegationFundsLog * RANKING_SLIDER_RATIO)) {
      amountValue = maxDelegationFunds;
    } else {
      amountValue = generateThousands(Math.exp(sliderValue / RANKING_SLIDER_RATIO));
    }
  ...
  generateInfo = () => {
    ...
    const walletSelectorClasses = classnames([styles.walletSelector, selectedDelegationWalletId === null ? 'noValueSelected' : null]);
    const walletSelectorContainerClasses = classnames([styles.walletSelectorContainer, styles.col]);
    ...
    if (selectedDelegationWalletId === null) {
      walletSelectionStart = intl.formatMessage(messages.rankingSelectWalletStart);
      ...
      walletSelectionStart = intl.formatMessage(messages.rankingAllWalletsStart);
    ...
    return <div className={styles.component}>
        <div className={styles.upper}>
          <div className={styles.selectWallet}>
            ...
            {getFilteredWallets(wallets).length > 0 ? <div className={styles.row}>
                <div className={styles.col}>{walletSelectionStart}</div>
                <div className={walletSelectorContainerClasses}>
                  <WalletsDropdown className={walletSelectorClasses} placeholder={intl.formatMessage(messages.rankingSelectWallet)} wallets={walletSelectorWallets} onChange={this.onSelectedWalletChange} disabled={isLoading || isRanking} value={selectedDelegationWalletId || '0'} selectionRenderer={option => <button className="customValue" onClick={() => {
                const selectionInput = document.querySelector('.StakePoolsRanking_walletSelectorContainer input');
                ...
                if (selectionInput) {
                  selectionInput.click();
                        ...
                        {option.label}
                      </button>} numberOfStakePools={numberOfStakePools} getStakePoolById={getStakePoolById} />
                </div>
                <div className={styles.col}>{walletSelectionEnd}</div>
              </div> : null}
          </div>
          {false ? // eslint-disable-line
        <ButtonLink className={learnMoreButtonClasses} onClick={() => onOpenExternalLink(learnMoreUrl)} skin={ButtonSkin} label={intl.formatMessage(messages.actionLearnMore)} linkProps={{
        ...
        <div className={styles.lower}>
          <div className={styles.row}>
            ...
            <div className={styles.slider}>
              <Slider min={Math.round(MIN_DELEGATION_FUNDS_LOG * RANKING_SLIDER_RATIO)} minDisplayValue={MIN_DELEGATION_FUNDS} max={Math.round(maxDelegationFundsLog * RANKING_SLIDER_RATIO)} maxDisplayValue={maxDelegationFunds} value={sliderValue} displayValue={displayValue} showRawValue onChange={this.onSliderChange} onAfterChange={rankStakePools} disabled={isLoading || isRanking} showTooltip minTooltip={intl.formatMessage(messages.rankingMinTooltip)} maxTooltip={intl.formatMessage(messages.rankingMaxTooltip)} />
            </div>
            <div className={styles.col}>
              <div className={styles.outOfRangeMaxAmount}>
                <PopOver content={intl.formatMessage(messages.rankingExtraTooltip)}>
