...
class StakePoolsTableHeader extends Component<TableHeaderProps> {
  render() {
    ...
    return map(availableTableHeaders, tableHeader => {
      const isSorted = tableHeader.name === stakePoolsSortBy || tableHeader.name === 'ticker' && stakePoolsSortBy === 'ticker';
      const defaultOrdering = defaultTableOrdering[tableHeader.name];
      const sortIconClasses = classNames([styles.sortIcon, isSorted ? styles.sorted : null, isSorted && stakePoolsOrder === 'asc' ? styles.ascending : null, isSorted && styles[`${stakePoolsOrder}CurrentOrdering`], styles[`${defaultOrdering}DefaultOrdering`]]);
      return <th key={tableHeader.name} onClick={() => onHandleSort(tableHeader.name)}>
          {tableHeader.title}
