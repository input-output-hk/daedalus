...
export class StakePoolsSearch extends Component<Props> {
  ...
  autoSelectOnFocus = () => this.searchInput ? this.searchInput.inputElement.current.select() : false;
  ...
  render() {
    ...
    const gridButtonClasses = classnames([styles.gridView, isGridView ? styles.selected : null]);
    const gridRewardsButtonClasses = classnames([styles.gridRewardsView, isGridRewardsView ? styles.selected : null]);
    const listButtonClasses = classnames([styles.listView, isListView ? styles.selected : null]);
    const isBigSearchComponent = isListView || isGridView || isGridRewardsView;
    const searchInputClases = classnames([styles.searchInput, isBigSearchComponent ? styles.inputExtrasSearch : null, IS_GRID_REWARDS_VIEW_AVAILABLE ? styles.withGridRewardsView : null]);
    const clearSearchClasses = classnames([styles.inputExtras, isBigSearchComponent ? styles.inputExtrasSearch : null, IS_GRID_REWARDS_VIEW_AVAILABLE ? styles.withGridRewardsView : null]);
    return <div className={styles.component}>
        <div className={styles.container}>
          <SVGInline svg={searchIcon} className={styles.searchIcon} />
          <Input autoFocus label={label || null} className={searchInputClases} onChange={onSearch} ref={input => {
          this.searchInput = input;
        }} placeholder={placeholder || intl.formatMessage(messages.searchInputPlaceholder)} skin={InputSkin} value={search} maxLength={150} onFocus={this.autoSelectOnFocus} />
          {this.hasSearchClearButton && <div className={clearSearchClasses}>
              <PopOver content={intl.formatMessage(messages.clearTooltip)}>
                <button onClick={this.handleClearSearch} className={styles.clearSearchButton}>
          ...
          {isBigSearchComponent && <div className={styles.viewButtons}>
              ...
              {IS_GRID_REWARDS_VIEW_AVAILABLE && <PopOver content={intl.formatMessage(messages.gridRewardsIconTooltip)}>
