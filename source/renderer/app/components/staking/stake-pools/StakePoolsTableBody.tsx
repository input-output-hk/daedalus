...
class StakePoolsTableBody extends Component<TableBodyProps> {
  ...
  render() {
    ...
    return map(sortedStakePoolList, (stakePool, key) => {
      ...
      const memberRewards = new BigNumber(get(stakePool, 'potentialRewards', ''));
      const potentialRewards = formattedWalletAmount(memberRewards);
      const retirement = retiring && moment(retiring).locale(intl.locale).fromNow(true);
      const pledgeValue = formattedWalletAmount(pledge, false, false);
      const costValue = formattedWalletAmount(cost, false, false);
      const progressBarContentClassnames = classNames([styles.progressBarContent, styles[getSaturationColor(saturation)]]);
      ...
      return <tr key={key}>
          ...
          <td>
            <PoolPopOver color={color} currentTheme={currentTheme} onOpenExternalLink={onOpenExternalLink} onSelect={onSelect} isSelected={selectedPoolId === stakePool.id} stakePool={stakePool} containerClassName={containerClassName} numberOfRankedStakePools={numberOfRankedStakePools} showWithSelectButton={showWithSelectButton}>
            ...
            <div className={styles.saturation}>
              <div className={styles.progressBar}>
                <div className={styles.progressBarContainer}>
                  <div className={progressBarContentClassnames} style={{
          ...
          <td>{pledgeValue}</td>
          <td>
            {retirement ? <span className={styles.retiring}>{retirement}</span> : '-'}
          </td>
        </tr>;
    });
  }
