...
const messages = defineMessages({
  title: {
    ...
    description: 'Title "Confirm Delegation" on the delegation setup "confirmation" step dialog.'
  },
  stepIndicatorLabel: {
    id: 'staking.delegationSetup.confirmation.step.dialog.stepIndicatorLabel',
    defaultMessage: '!!!STEP {currentStep} OF {totalSteps}',
    description: 'Step indicator label on the delegation setup "confirmation" step dialog.'
  },
  description: {
    id: 'staking.delegationSetup.confirmation.step.dialog.description',
    defaultMessage: '!!!Confirm your delegation choice to <span>[{selectedPoolTicker}]</span> stake pool for your <span>{selectedWalletName}</span> wallet.',
  ...
  depositLabel: {
    ...
    description: 'Deposit label on the delegation setup "confirmation" step dialog.'
  },
...
class DelegationStepsConfirmationDialog extends Component<Props> {
  ...
  render() {
    ...
    const buttonLabel = !isSubmitting ? intl.formatMessage(messages.confirmButtonLabel) : <LoadingSpinner />;
    const actions = [{
      ...
      onClick: this.submit,
      primary: true,
      disabled: !isHardwareWallet && !spendingPasswordField.isValid || isHardwareWallet && hwDeviceStatus !== HwDeviceStatuses.VERIFYING_TRANSACTION_SUCCEEDED || isSubmitting || !transactionFee
    }];
    const dialogClassName = classNames([commonStyles.delegationSteps, styles.delegationStepsConfirmationDialogWrapper]);
    const contentClassName = classNames([commonStyles.content, styles.content]);
    const stepsIndicatorLabel = <FormattedMessage {...messages.stepIndicatorLabel} values={{
      currentStep: 3,
      totalSteps: stepsList.length
    }} />;
    return <Dialog title={intl.formatMessage(messages.title)} subtitle={stepsIndicatorLabel} actions={actions} closeOnOverlayClick onClose={!isSubmitting ? onClose : () => {}} className={dialogClassName} closeButton={<DialogCloseButton onClose={onClose} />} backButton={<DialogBackButton onBack={!isSubmitting ? onBack : () => {}} />}>
        <div className={commonStyles.delegationStepsIndicatorWrapper}>
          <Stepper steps={stepsList} activeStep={3} skin={StepperSkin} labelDisabled />
        </div>
        ...
        <div className={contentClassName}>
          ...
          <div className={styles.feesRow}>
            <div className={styles.feesWrapper}>
              ...
              <p className={styles.feesAmount}>
                {!transactionFee ? <span className={styles.calculatingFeesLabel}>
            ...
            {transactionFee && transactionFee.deposits.isZero && !transactionFee.deposits.isZero() && <>
          ...
          {isHardwareWallet ? <div className={styles.hardwareWalletStatusWrapper}>
              <HardwareWalletStatus hwDeviceStatus={hwDeviceStatus} walletName={selectedWalletName} isTrezor={isTrezor} onExternalLinkClick={onExternalLinkClick} />
            </div> : <Input className={styles.spendingPassword} {...spendingPasswordField.bind()} skin={InputSkin} error={spendingPasswordField.error} onKeyPress={this.handleSubmitOnEnter} />}
        </div>
