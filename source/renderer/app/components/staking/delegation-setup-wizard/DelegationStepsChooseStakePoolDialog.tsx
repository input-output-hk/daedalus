...
const messages = defineMessages({
  title: {
    ...
    description: 'Title "Choose a stake pool" on the delegation setup "choose stake pool" dialog.'
  },
  description: {
  ...
  selectStakePoolLabel: {
    ...
    defaultMessage: '!!!Select a stake pool to receive your delegated funds in the <span>{selectedWalletName}<span> wallet.',
    description: 'Select / Selected pool section label on the delegation setup "choose stake pool" dialog.'
  },
  selectedStakePoolLabel: {
    ...
    description: '"Selected Pools" Selected pool label on the delegation setup "choose stake pool" dialog.'
  },
  selectedStakePoolLabelRetiring: {
    id: 'staking.delegationSetup.chooseStakePool.step.dialog.selectedStakePoolLabelRetiring',
    defaultMessage: '!!!The [{selectedPoolTicker}] stake pool which you have selected to delegate your <span>{selectedWalletName}</span> wallet funds is about to retire.',
    description: '"Selected Pools" Selected pool label on the delegation setup "choose stake pool" dialog.'
  },
  ...
  stepIndicatorLabel: {
    ...
    description: 'Step indicator label on the delegation setup "choose wallet" step dialog.'
  },
  retiringPoolFooter: {
    id: 'staking.delegationSetup.chooseStakePool.step.dialog.retiringPoolFooter',
    defaultMessage: '!!!The stake pool you have selected is about to be retired. If you continue the delegation process, you will need to delegate your stake to another pool at least one complete epoch before the current pool’s retirement date to avoid losing rewards.',
    description: 'Retiring Pool Footer label on the delegation setup "choose wallet" step dialog.'
  }
});
type Props = {
...
export default class DelegationStepsChooseStakePoolDialog extends Component<Props, State> {
  ...
  stakePoolsScrollElementRef = createRef<any>();
  state = {
    searchValue: '',
    selectedPoolId: get(this.props, ['selectedPool', 'id'], null)
  };
  handleSearch = (searchValue: string) => this.setState({
    searchValue
  });
  ...
  render() {
    ...
    const selectedPool = find(stakePoolsList, stakePool => stakePool.id === selectedPoolId);
    const lastDelegatedStakePoolId = get(selectedWallet, 'lastDelegatedStakePoolId', null);
    const delegatedStakePoolId = get(selectedWallet, 'delegatedStakePoolId', null);
    ...
    const footer = selectedPool && selectedPool.retiring ? <div className={styles.retiringPoolFooter}>
          {intl.formatMessage(messages.retiringPoolFooter)}
        </div> : null;
    const dialogClassName = classNames([commonStyles.delegationSteps, styles.delegationStepsChooseStakePoolDialogWrapper]);
    ...
    const selectionPoolLabel = () => {
      ...
      if (selectedPoolId && activeStakePoolId === delegatedStakePoolId && delegatedStakePoolId === selectedPoolId) {
        label = <FormattedHTMLMessage {...messages.delegatedStakePoolLabel} values={{
          selectedWalletName,
          selectedPoolTicker
        }} />;
      } else if (selectedPoolId && activeStakePoolId === lastDelegatedStakePoolId && lastDelegatedStakePoolId === selectedPoolId) {
        label = <FormattedHTMLMessage {...messages.delegatedStakePoolNextLabel} values={{
          selectedWalletName,
          selectedPoolTicker
        }} />;
      } else if (selectedPoolId) {
        // Stake pool selected and selected wallet are not delegated to it
        const message = !selectedPool.retiring ? messages.selectedStakePoolLabel : messages.selectedStakePoolLabelRetiring;
        label = <FormattedHTMLMessage {...message} values={{
          selectedWalletName,
          selectedPoolTicker
        }} />;
      } else {
        // Stake pool not selected.
        label = <FormattedHTMLMessage {...messages.selectStakePoolLabel} values={{
          selectedWalletName
        }} />;
      }
    ...
    const stepsIndicatorLabel = <FormattedMessage {...messages.stepIndicatorLabel} values={{
      currentStep: 2,
      totalSteps: stepsList.length
    }} />;
    const filteredStakePoolsList: Array<StakePool> = getFilteredStakePoolsList(stakePoolsList, searchValue);
    const numberOfRankedStakePools: number = stakePoolsList.filter(stakePool => IS_RANKING_DATA_AVAILABLE && stakePool.nonMyopicMemberRewards).length;
    return <Dialog title={intl.formatMessage(messages.title)} subtitle={stepsIndicatorLabel} actions={actions} footer={footer} closeOnOverlayClick onClose={onClose} className={dialogClassName} closeButton={<DialogCloseButton onClose={onClose} />} backButton={<DialogBackButton onBack={onBack} />} scrollWrapperRef={this.stakePoolsScrollElementRef}>
        <BackToTopButton scrollableElementClassName="Dialog_contentWrapper" buttonTopPosition={100} scrollTopToActivate={100} />

        <div className={commonStyles.delegationStepsIndicatorWrapper}>
          <Stepper steps={stepsList} activeStep={2} skin={StepperSkin} labelDisabled />
        </div>
        ...
        <div className={contentClassName}>
          ...
          <div className={styles.selectStakePoolWrapper}>
            <ThumbSelectedPool stakePool={selectedPool} numberOfRankedStakePools={numberOfRankedStakePools} alreadyDelegated={selectedPool && !canSubmit} />
          ...
          <div className={styles.recentStakePoolsWrapper}>
            {recentStakePools.length > 0 && <p className={styles.recentStakePoolsLabel}>
                <FormattedMessage {...messages.recentPoolsLabel} values={{}} />
              </p>}
            <StakePoolsList listName="recentStakePools" stakePoolsList={recentStakePools} onOpenExternalLink={onOpenExternalLink} currentTheme={currentTheme} containerClassName="Dialog_content" onSelect={this.handleSelect} selectedPoolId={selectedPoolId} numberOfRankedStakePools={numberOfRankedStakePools} disabledStakePoolId={activeStakePoolId} highlightOnHover highlightWithDelay selectOnClick scrollElementRef={this.stakePoolsScrollElementRef} />
          </div>

          <div className={styles.searchStakePoolsWrapper}>
            <StakePoolsSearch search={searchValue} label={intl.formatMessage(messages.searchInputLabel)} placeholder={intl.formatMessage(messages.searchInputPlaceholder)} onSearch={this.handleSearch} onClearSearch={this.handleClearSearch} scrollableElementClassName="Dialog_content" disabledStakePoolId={activeStakePoolId} />
          </div>

          <div className={styles.stakePoolsListWrapper}>
            <StakePoolsList listName="selectedIndexList" stakePoolsList={filteredStakePoolsList} onOpenExternalLink={onOpenExternalLink} currentTheme={currentTheme} onSelect={this.handleSelect} selectedPoolId={selectedPoolId} containerClassName="Dialog_content" numberOfRankedStakePools={numberOfRankedStakePools} disabledStakePoolId={activeStakePoolId} highlightOnHover highlightWithDelay selectOnClick scrollElementRef={this.stakePoolsScrollElementRef} />
          </div>
        </div>
      </Dialog>;
  }
