...
class Sidebar extends Component<Props> {
  ...
  render() {
    ...
    if (menus && menus.wallets && menus.wallets.items && activeSidebarCategory === walletsCategoryRoute) {
      subMenu = <SidebarWalletsMenu wallets={menus.wallets ? menus.wallets.items : []} onAddWallet={onAddWallet} onWalletItemClick={menus.wallets && menus.wallets.actions ? menus.wallets.actions.onWalletItemClick : null} isActiveWallet={id => id === (menus.wallets ? menus.wallets.activeWalletId : null)} isAddWalletButtonActive={pathname === ROUTES.WALLETS.ADD} isShelleyActivated={isShelleyActivated} visible={isShowingSubMenus} />;
    }

    const sidebarStyles = classNames([styles.component, !isShowingSubMenus || subMenu == null ? styles.minimized : null]);
    return <div className={sidebarStyles}>
        <div className={styles.minimized}>
          ...
          return <SidebarCategory key={category.name} category={category} isActive={isActive} onClick={onActivateCategory} content={content} />;
        })}
        </div>
        {subMenu}
      </div>;
