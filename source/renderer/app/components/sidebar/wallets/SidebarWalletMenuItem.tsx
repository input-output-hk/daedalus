...
class SidebarWalletMenuItem extends Component<Props> {
  render() {
    ...
    const componentStyles = classNames([styles.component, active ? styles.active : null, showLegacyBadge ? styles.legacyItem : null, className, hasNotification ? styles.notification : null, isNotResponding ? styles.notResponding : null]);
    const hwIconStyles = classNames([styles.hardwareWalletsIcon, isHardwareWallet && isHardwareWalletDisconnected && isHardwareWalletIndicatorEnabled ? styles.disconnected : styles.connected]);
    return <button className={componentStyles} onClick={onClick}>
        <div className={styles.meta}>
          <div className={styles.topContainer}>
            <div className={styles.title}>{title}</div>
            {isHardwareWallet && <div className={styles.hardwareWalletsIconWrapper}>
          ...
          {showLegacyBadge && <LegacyBadge mode={LEGACY_BADGE_MODES.FLOATING} />}
        </div>
      </button>;
  }
