...
const messages = defineMessages({
  ...
  subtitle: {
    ...
    defaultMessage: '!!!You are currently running Daedalus version {currentAppVersion}.<br />Daedalus version {availableAppVersion} is now available to download.',
    description: '"subtitle" for the App Update Overlay'
  },
  checkboxLabel: {
    id: 'appUpdate.overlay.checkboxLabel',
    defaultMessage: '!!!I understand that I need to complete the installation before starting Daedalus.',
  ...
  manualUpdateDescriptionError: {
    ...
    defaultMessage: '!!!We were unable to launch the update installer automatically.',
  ...
  manualUpdateDescriptionErrorLinux: {
    ...
    description: '"manualUpdateDescriptionErrorLinux" for the App Update Overlay'
  },
  ...
  manualUpdateButtonUrlForMainnet: {
    ...
    description: '"manualUpdateButtonUrl" for the App Update Overlay on Mainnet'
  },
  ...
  manualUpdateButtonUrlForTestnet: {
    ...
    defaultMessage: '!!!https://developers.cardano.org/en/testnets/cardano/get-started/wallet/',
    description: '"manualUpdateButtonUrl" for the App Update Overlay on Testnet'
  }
});
type Props = {
  update: News.News;
...
class AppUpdateOverlay extends Component<Props, State> {
  ...
  progressActions = () => {
    ...
    return <div className={styles.progressBar}>
        <ProgressBarLarge leftLabel={intl.formatMessage(messages.downloadProgressLabel)} rightLabel1={intl.formatMessage(messages.downloadTimeLeft, {
        downloadTimeLeft
      })} rightLabel2={intl.formatMessage(messages.downloadProgressData, {
        totalDownloaded,
        totalDownloadSize
      })} progress={downloadProgress} isDarkMode />
      </div>;
  };
  openInstallerAction = () => {
    const {
    ...
    const checkboxStyles = classnames([styles.checkbox, isCheckboxDisabled ? styles.disabled : null]);
    const isButtonDisabled = !areTermsOfUseAccepted || isWaitingToQuitDaedalus;
    const buttonStyles = classnames([styles.button, isButtonDisabled ? styles.disabled : null, isWaitingToQuitDaedalus ? styles.installing : null]);
    const buttonLabel = isLinux ? messages.buttonInstallUpdateLabel : messages.buttonLaunchInstallerLabel;
    const postponeLinkStyles = classnames([styles.postponeLink, !isLinux && isWaitingToQuitDaedalus ? styles.disabled : null, isLinux && isWaitingToQuitDaedalus ? styles.noLink : null]);
    const postponeLabel = isLinux && isWaitingToQuitDaedalus ? messages.installingUpdateLabel : messages.postponeInstallLinkLabel;
    const postponeAction = !isWaitingToQuitDaedalus ? onPostponeUpdate : () => {};
    const actionsStyles = classnames([!isLinux || !isWaitingToQuitDaedalus ? styles.actions : null]);
    return <div className={actionsStyles}>
        {!isLinux && <Checkbox label={intl.formatMessage(messages.checkboxLabel)} onChange={this.toggleAcceptance} className={checkboxStyles} checked={areTermsOfUseAccepted || isWaitingToQuitDaedalus} skin={CheckboxSkin} themeOverrides={styles.checkbox} disabled={isCheckboxDisabled} />}
        {isLinux && isWaitingToQuitDaedalus ? <div className={styles.progressBar}>
            <ProgressBarLarge progress={installationProgress} labelLeft={intl.formatMessage(messages.installingUpdateLabel)} isDarkMode />
          </div> : <>
            <Button className={buttonStyles} onClick={onInstallUpdate} skin={ButtonSpinnerSkin} loading={isWaitingToQuitDaedalus} label={intl.formatMessage(buttonLabel)} disabled={isButtonDisabled} />
            <Link className={postponeLinkStyles} onClick={postponeAction} label={intl.formatMessage(postponeLabel)} skin={LinkSkin} hasIconAfter={false} />
          </>}
      </div>;
  };
  manualUpdateAction = () => {
    ...
    const errorMessage = isLinux ? messages.manualUpdateDescriptionErrorLinux : messages.manualUpdateDescriptionError;
    let manualUpdateButtonUrl = intl.formatMessage(messages.manualUpdateButtonUrlForMainnet);

    if (isTestnet) {
      manualUpdateButtonUrl = intl.formatMessage(messages.manualUpdateButtonUrlForTestnet);
    }

    if (isFlight) {
      manualUpdateButtonUrl = intl.formatMessage(messages.manualUpdateButtonUrlForFlight);
    }
    ...
    return <div className={styles.actions}>
        ...
        <Button className={styles.button} onClick={() => onExternalLinkClick(manualUpdateButtonUrl)} skin={ButtonSkin} label={<span>
              <SVGInline svg={externalLinkIcon} className={styles.externalLinkIcon} />
              {intl.formatMessage(messages.manualUpdateButtonLabel)}
            </span>} />
        <Link className={styles.postponeLink} onClick={onPostponeUpdate} label={intl.formatMessage(messages.postponeInstallLinkLabel)} skin={LinkSkin} hasIconAfter={false} />
      </div>;
  };
  ...
  render() {
    ...
    let actions;
    if (isAutomaticUpdateFailed) actions = this.manualUpdateAction();else if (!isUpdateDownloaded) actions = this.progressActions();else actions = this.openInstallerAction();
    return <div className={styles.component} role="presentation" onClick={!isUpdateDownloaded ? onClose : () => {}}>
        {!isUpdateDownloaded && !isAutomaticUpdateFailed && <DialogCloseButton onClose={onClose} className={styles.closeButton} />}
        <h1 className={styles.title}>{intl.formatMessage(messages.title)}</h1>
        <span className={styles.subtitle}>
          <FormattedHTMLMessage {...messages.subtitle} values={{
          availableAppVersion,
          currentAppVersion
        }} />
        </span>
