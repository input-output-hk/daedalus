...
const messages = defineMessages({
  ...
  checkboxLabelWithDisclaimer: {
    ...
    description: 'Label for the "I agree with terms of service" checkbox when terms of use are not translated.'
  },
  submitLabel: {
...
class TermsOfUseForm extends Component<Props, State> {
  ...
  render() {
    ...
    const buttonClasses = classnames(['primary', isSubmitting ? styles.submitButtonSpinning : styles.submitButton]);
    return <div className={styles.component}>
        <div className={styles.centeredBox}>
          <TermsOfUseText localizedTermsOfUse={localizedTermsOfUse} onOpenExternalLink={onOpenExternalLink} />

          <div className={styles.checkbox}>
            <Checkbox label={intl.formatMessage(messages.checkboxLabel)} onChange={this.toggleAcceptance} checked={areTermsOfUseAccepted} skin={CheckboxSkin} />
          </div>

          {error && <p className={styles.error}>{intl.formatMessage(error)}</p>}

          <Button className={buttonClasses} label={intl.formatMessage(messages.submitLabel)} onClick={this.submit} disabled={!areTermsOfUseAccepted} skin={ButtonSkin} />
        </div>
      </div>;
  }
