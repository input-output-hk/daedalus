...
class VotingRegistrationDialog extends Component<Props> {
  ...
  render() {
    ...
    const stepsIndicatorLabel = <FormattedMessage {...messages.subtitle} values={{
      step: activeStep,
      stepCount: stepsList.length
    }} />;
    return <Dialog className={styles.component} title={intl.formatMessage(messages.dialogTitle)} subtitle={!hideSteps && stepsIndicatorLabel} onClose={onClose} closeOnOverlayClick={false} closeButton={hideCloseButton ? null : <DialogCloseButton />} backButton={onBack && <DialogBackButton onBack={onBack} />} actions={actions}>
        {!hideSteps && <div className={styles.votingRegistrationStepsIndicatorWrapper}>
            <Stepper steps={stepsList} activeStep={activeStep} skin={StepperSkin} labelDisabled />
          </div>}
