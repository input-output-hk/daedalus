...
const messages = defineMessages({
  description: {
    ...
    defaultMessage: '!!!Please sign the voting registration transaction. This transaction links your wallet balance with your Fund6 voting registration, as a proof of your voting power. Funds will not leave your wallet, but registration requires paying transaction fees, as displayed on-screen.',
  ...
  continueButtonLabel: {
    ...
    description: 'Label for continue button on the voting registration "sign" step.'
  },
  feesLabel: {
...
class VotingRegistrationStepsRegister extends Component<Props> {
  ...
  render() {
    ...
    return <VotingRegistrationDialog onClose={!isSubmitting ? onClose : () => {}} stepsList={stepsList} activeStep={activeStep} actions={actions} onBack={onBack} containerClassName={styles.component}>
        <p className={styles.description}>
          <FormattedHTMLMessage {...messages.description} />
        </p>

        <div className={styles.learnMoreWrapper}>
          <Link className={styles.externalLink} onClick={event => onExternalLinkClick(learnMoreLinkUrl, event)} label={intl.formatMessage(messages.learnMoreLink)} skin={LinkSkin} />
        </div>
        ...
        {isHardwareWallet ? <div className={styles.hardwareWalletStatusWrapper}>
          ...
          </div> : <Input {...spendingPasswordField.bind()} autoFocus skin={InputSkin} error={spendingPasswordField.error} onKeyPress={this.handleSubmitOnEnter} />}

        {transactionFeeError ? <div className={styles.errorMessage}>
            <p>{transactionFeeError}</p>
          </div> : null}

        {transactionError ? <div className={styles.errorMessage}>
