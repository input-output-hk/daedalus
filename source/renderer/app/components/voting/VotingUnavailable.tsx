...
const messages = defineMessages({
  ...
  paragraph1: {
    ...
    defaultMessage: '!!!Project Catalyst Fund5 has now ended. Fund6 is currently in preparation, voting registration is not available yet.',
...
class VotingUnavailable extends Component<Props> {
  static contextTypes = {
  ...
  render() {
    ...
    const link1 = <Link className={styles.link} label={intl.formatMessage(messages.link1Text)} onClick={() => onExternalLinkClick(intl.formatMessage(messages.link1Url))} />;
    const link2 = <Link className={styles.link} label={intl.formatMessage(messages.link2Text)} onClick={() => onExternalLinkClick(intl.formatMessage(messages.link2Url))} />;
    return <div className={styles.component}>
        {isVotingRegistrationAvailable ? <>
            ...
            <h1>{heading}</h1>
            <p>{paragraph1}</p>
            <FormattedMessage {...messages.paragraph2} values={{
          link1,
          link2
        }} tagName="p" />
          </>}
