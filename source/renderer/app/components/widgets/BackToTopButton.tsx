...
export default class BackToTopButton extends Component<Props, State> {
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (this._isMounted) {
        this.scrollableDomElement = document.querySelector(`.${this.props.scrollableElementClassName}`);
        if (!this.scrollableDomElement) return false;
        return this.scrollableDomElement.addEventListener('scroll', throttle(this.getIsBackToTopActive, 300, {
          leading: false,
          trailing: true
        }));
      }
  ...
  componentWillUnmount() {
    if (this._isMounted) {
      ...
      this.scrollableDomElement = document.querySelector(`.${this.props.scrollableElementClassName}`);
      if (!this.scrollableDomElement) return false;
      return this.scrollableDomElement.removeEventListener('scroll', this.getIsBackToTopActive);
    }
  ...
  render() {
    ...
    return <button style={{
      top
    }} className={componentStyles} onClick={this.backToTop}>
