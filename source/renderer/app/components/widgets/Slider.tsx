...
class Slider extends Component<Props> {
  ...
  render() {
    ...
    const valueMarkLeftPosition = max === min ? `0` : `${(value - min) / (max - min) * 100}%`;
    const valueMarkStyle = {
      left: valueMarkLeftPosition
    };
    const formattedValue = showRawValue ? displayValue || value : new BigNumber(value).toFormat(0);
    return <div className={styles.component}>
        <div className={styles.upperMarks}>
          <div className={styles.minMark}>
            {showTooltip ? <PopOver content={minTooltip}>
                {shortNumber(minDisplayValue || min)}
              </PopOver> : shortNumber(minDisplayValue || min)}
          </div>
          <div className={styles.maxMark}>
            {showTooltip ? <PopOver content={maxTooltip}>
                {shortNumber(maxDisplayValue || max)}
              </PopOver> : shortNumber(maxDisplayValue || max)}
          </div>
        </div>
        <RcSlider {...rest} />
