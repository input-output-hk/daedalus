...
export default class Dialog extends Component<Props> {
  render() {
    ...
    const {
      items,
      direction
    } = Array.isArray(actions) ? { ...defaultActionOptions,
      items: actions
    } : { ...defaultActionOptions,
      ...actions
    };
    let themeOverrides;
    if (defaultThemeOverrides) themeOverrides = dialogOverrides;else if (fullSize) themeOverrides = dialogFullSizeOverride;
    const classActionsClasses = classnames([styles.actions, styles[`${direction}Direction`]]);
    return <Modal isOpen triggerCloseOnOverlayClick={closeOnOverlayClick} onClose={onClose} skin={ModalSkin} themeOverrides={themeOverrides}>
        <div className={classnames([styles.dialogWrapper, className])}>
          ...
          {children && <div className={styles.contentWrapper} ref={ref => {
          ...
          {items && <div className={classActionsClasses}>
              {map(items, (action, key) => {
            const buttonClasses = classnames([action.className ? action.className : null, action.primary ? 'primary' : 'flat']);
            return !action.isLink ? <Button key={key} className={buttonClasses} label={action.label} onClick={action.onClick} disabled={action.disabled} skin={ButtonSkin} autoFocus={action.primary ? primaryButtonAutoFocus : false} /> : <Link key={key} className={action.className} onClick={action.onClick} label={action.label} skin={LinkSkin} hasIconAfter={action.hasIconAfter} hasIconBefore={action.hasIconBefore} />;
          })}
            </div>}
