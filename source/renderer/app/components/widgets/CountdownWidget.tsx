...
type Format = "YYYY-MM-DD-HH-mm-ss" | "MM-DD-HH-mm-ss" | "DD-HH-mm-ss" | "HH-mm-ss" | "mm-ss" | "YYYY" | "MM" | "DD" | "HH" | "mm" | "ss";
type Props = {
...
class CountdownWidget extends Component<Props, State> {
  ...
  startTimer = () => {
    this.updateTimeLeft();
    this.intervalHandler = setInterval(() => this.updateTimeLeft(), TIME_LEFT_INTERVAL);
  };
  stopTimer = () => {
  ...
  updateTimeLeft = () => {
    ...
    if (startDateTime) {
      const timeLeft = Math.max(0, new Date(startDateTime).getTime() - new Date().getTime());
      this.setState({
        timeLeft
      });
  ...
  generateFieldPanel = (labels: any, values: any, index: number) => {
    ...
    const shouldBeHidden = values.slice(0, index).reduce((acc, val) => acc + val, 0) === 0 && value === 0 && !format;
    ...
    return <div className={styles.fieldPanel}>
        ...
        {includeDelimeter && <div className={styles.right}>
  ...
  generateCountdownPanels = () => {
    ...
    const labels: Array<string> = format ? format.split('-').map(item => intl.formatMessage(globalMessages[COLUMNS[item]])) : [yearsLabel, monthsLabel, daysLabel, hoursLabel, minutesLabel, secondsLabel];
    ...
    const values = format ? format.split('-').map(item => duration[COLUMNS[item]]()) : [years, months, days, hours, minutes, seconds];
    const keys = format ? format.split('-').map(item => COLUMNS[item]) : ['years', 'months', 'days', 'hours', 'minutes', 'seconds'];
  ...
  render() {
    ...
    return <div className={styles.timeLeftContainer}>
        <div className={styles.timeLeft}>
          {showSpinner ? <SVGInline svg={spinnerIcon} className={styles.spinnerIcon} /> : fieldPanels}
        </div>
      </div>;
  }
