...
class InlineEditingInput extends Component<Props, State> {
    ...
    fields: {
      inputField: {
        ...
        validators: [({
          ...
          const errorMessage = typeof valueErrorMessage === 'function' ? valueErrorMessage(value) : valueErrorMessage;
  ...
  render() {
    ...
    let error;
    if (inputField.error) error = inputField.error;else if (!hasChanged) error = !!errorMessage;
    const showEditButton = !isActive && !isLoading && !hasChanged && label.length && !readOnly;
    const showFocusButtons = !isLoading && !disabled && !readOnly && (isActive || hasChanged);
    const showLoadingButton = isLoading;
    const componentStyles = classnames([className, styles.component, isActive ? null : styles.inactive, readOnly ? styles.readOnly : null, isLoading ? styles.isLoading : null, showEditButton || showLoadingButton ? styles.twoButtons : null, showFocusButtons ? styles.twoButtons : null]);
    const inputStyles = classnames([successfullyUpdated ? 'input_animateSuccess' : null, isActive ? null : 'input_cursorPointer']);
    const buttonsWrapperStyles = classnames([styles.buttonsWrapper, readOnly ? styles.readOnly : null]);
    ...
    const submittingButtonStyles = classnames([styles.button, styles.submittingButton]);
    return <div className={componentStyles}>
        <Input {...inputField.bind()} className={inputStyles} placeholder={placeholder || ''} themeOverrides={styles} type="text" maxLength={maxLength} label={label} onFocus={this.onFocus} onBlur={this.onBlur} onChange={this.onChange} onKeyDown={event => this.handleInputKeyDown(event)} error={isActive ? error : !!error} disabled={disabled} readOnly={readOnly} ref={input => {
        if (!this.inputElement) {
          this.inputElement = get(input, 'inputElement.current');
        }
      }} />

        <div className={buttonsWrapperStyles} onMouseDown={this.preventDefaultHelper} onMouseUp={this.preventDefaultHelper}>
          {showEditButton && <Button className={editButtonStyles} onMouseUp={this.setInputFocus} label={<SVGInline svg={penIcon} className={styles.icon} />} />}
          {showFocusButtons && <Button className={cancelButtonStyles} onClick={this.onCancel} label={<SVGInline svg={crossIcon} className={styles.icon} />} />}
          {showFocusButtons && <Button className={okButtonStyles} onMouseUp={this.submit} label={<SVGInline svg={arrowIcon} className={styles.icon} />} />}
          {showLoadingButton && <Button className={submittingButtonStyles} onMouseUp={() => {}} label={<SVGInline svg={spinningIcon} className={styles.icon} />} />}
        </div>

        {successfullyUpdated && <div className={styles.savingResultLabel}>
            {intl.formatMessage(messages.changesSaved)}
          </div>}

        {errorMessage && !hasChanged && <div className={styles.errorMessage}>{errorMessage}</div>}
      </div>;
  }
