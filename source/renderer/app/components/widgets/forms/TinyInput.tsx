...
export default class TinyInput extends Component<Props, State> {
  ...
  setEditMode = (isEditMode: boolean) => this.setState({
    isEditMode
  });
  ...
  render() {
    ...
    return <div className={styles.component} onFocus={() => this.setEditMode(true)} onBlur={() => this.setEditMode(false)} role="button" tabIndex={-1}>
        {useReadMode && !isEditMode && <div className={styles.contentInReadMode}>
        ...
        {(!useReadMode || isEditMode) && <>
            {type === 'number' ? <NumericInput themeId={IDENTIFIERS.INPUT} skin={InputSkin} {...restProps} autoFocus={autoFocus} onKeyPress={this.onKeyPress} /> : <Input themeId={IDENTIFIERS.INPUT} skin={InputSkin} {...restProps} autoFocus={useReadMode ? true : autoFocus} onKeyPress={this.onKeyPress} type={type} />}
          </>}
      </div>;
  }
