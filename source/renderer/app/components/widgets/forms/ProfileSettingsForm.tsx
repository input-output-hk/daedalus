...
import { LANGUAGE_OPTIONS, NUMBER_OPTIONS, DATE_ENGLISH_OPTIONS, DATE_JAPANESE_OPTIONS, TIME_OPTIONS, PROFILE_SETTINGS } from "../../../config/profileConfig";
...
class ProfileSettingsForm extends Component<ProfileSettingsFormProps> {
  ...
  get dateFormat() {
    ...
    return {
      options: currentLocale === 'en-US' ? DATE_ENGLISH_OPTIONS : DATE_JAPANESE_OPTIONS,
  ...
  getSelect = (id: string) => {
    ...
    return <Select label={formatMessage(messages[`${id}SelectLabel`])} value={value} options={options} onChange={(v: string) => onChangeItem(id, v)} skin={SelectSkin} className={styles.select} key={id} optionHeight={50} />;
  };
  ...
  render() {
    ...
    return <div className={componentClassNames}>
        ...
        {onSubmit && <Button className={classNames(['primary', styles.submitButton])} label={formatMessage(messages.submitLabel)} skin={ButtonSpinnerSkin} loading={isSubmitting} onClick={onSubmit} />}
      </div>;
  }
