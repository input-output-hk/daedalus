...
const messages = defineMessages({
  ...
  moveFundsDescriptionLine1: {
    ...
    defaultMessage: '!!!"{activeWalletName}"" is a Byron legacy wallet that does not support Shelley delegation features. To earn ada from delegating your stake, please move all funds from this wallet to a new wallet that is Shelley-compatible.',
  ...
  moveFundsDescriptionLine2: {
    ...
    defaultMessage: '!!!You can create a {moveFundsLink} or move funds to one of your existing wallets.',
    description: 'Legacy notification description.'
  },
  moveFundsLinkLabel: {
  ...
  addWalletDescriptionLine2: {
    ...
    defaultMessage: '!!!Since all of your wallets are Byron legacy wallets you will first need to create a new Shelley wallet.',
    description: 'Legacy notification description.'
  },
  actionLearnMore: {
...
class LegacyNotification extends Component<Props> {
  ...
  getValue = (messageHasRewardsWallets: string, messageNoRewardsWallets: string, _values?: Record<string, any>) => {
    const {
      hasRewardsWallets,
      activeWalletName
    } = this.props;
    const message = hasRewardsWallets ? messageHasRewardsWallets : messageNoRewardsWallets;
    const values = {
      activeWalletName,
      ..._values
    };
  ...
  render() {
    ...
    const moveFundsLink = <Link className={styles.descriptionLink} onClick={onWalletAdd} label={intl.formatMessage(messages.moveFundsLinkLabel)} skin={LinkSkin} hasIconAfter={false} />;
    const title = getValue(messages.moveFundsTitle, messages.addWalletTitle);
    const description1 = getValue(messages.moveFundsDescriptionLine1, messages.addWalletDescriptionLine1);
    const description2 = getValue(messages.moveFundsDescriptionLine2, messages.addWalletDescriptionLine2, {
      moveFundsLink
    });
    ...
    return <div className={styles.component}>
        ...
        <div className={styles.actions}>
          {showLearnMoreButton && <ButtonLink className={styles.actionLearnMore} onClick={onLearnMore} skin={ButtonSkin} label={intl.formatMessage(messages.actionLearnMore)} linkProps={{
          className: styles.externalLink,
          hasIconBefore: false,
          hasIconAfter: true
        }} />}
          <Button className={styles.actionMove} label={buttonLabel} onClick={buttonAction} skin={ButtonSkin} />
        </div>
      </div>;
  }
