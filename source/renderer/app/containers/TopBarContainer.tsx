...
class TopBarContainer extends Component<Props> {
  ...
  render() {
    ...
    const testnetLabel = !isMainnet ? <WalletTestEnvironmentLabel network={network} /> : null;
    ...
    const onTransferFunds = (sourceWalletId: string) => actions.wallets.transferFundsSetSourceWalletId.trigger({
      sourceWalletId
    });
    ...
    return <TopBar leftIcon={leftIcon} onLeftIconClick={actions.sidebar.toggleSubMenus.trigger} activeWallet={activeWallet} onTransferFunds={onTransferFunds} hasRewardsWallets={hasRewardsWallets} onWalletAdd={onWalletAdd} onLearnMore={openExternalLink} isShelleyActivated={isShelleyActivated}>
        {testnetLabel}
        <NodeSyncStatusIcon isSynced={isSynced} syncPercentage={syncPercentage} hasTadaIcon={shouldShowTadaIcon} />
        {shouldShowTadaIcon && <TadaButton onClick={onClickTadaButton} shouldAnimate={shouldShowTadaIconAnimation} />}
        <NewsFeedIcon onNewsFeedIconClick={actions.app.toggleNewsFeed.trigger} hasNotification={hasUnreadNews} hasUpdate={displayAppUpdateNewsItem} />
      </TopBar>;
  }
