...
class Staking extends Component<Props> {
  ...
  render() {
    ...
    const isDelegationWizardOpen = uiDialogs.isOpen(DelegationSetupWizardDialog);
    const isUndelegationWizardOpen = uiDialogs.isOpen(UndelegateWalletConfirmationDialog);
    ...
    return <MainLayout>
        {isStakingDelegationCountdown ? children : <StakingWithNavigation isActiveNavItem={this.isActiveNavItem} onNavItemClick={this.handleNavItemClick} activeItem={app.currentPage} showInfoTab={shouldShowInfoTab}>
            {children}
