...
import { MIN_DELEGATION_FUNDS, RECENT_STAKE_POOLS_COUNT, DELEGATION_ACTIONS } from "../../../config/stakingConfig";
...
class DelegationSetupWizardDialogContainer extends Component<Props, State> {
  ...
  handleIsWalletAcceptable = (walletAmount?: BigNumber, walletReward: BigNumber = 0) => walletAmount && walletAmount.gte(new BigNumber(MIN_DELEGATION_FUNDS)) && !walletAmount.isEqualTo(walletReward);

  get selectedWalletId() {
    return get(this.props, ['stores', 'uiDialogs', 'dataForActiveDialog', 'walletId'], null);
  }

  get selectedPoolId() {
    return get(this.props, ['stores', 'uiDialogs', 'dataForActiveDialog', 'poolId'], null);
  }
  ...
  STEPS_LIST = [this.context.intl.formatMessage(messages.delegationSetupStep1Label), this.context.intl.formatMessage(messages.delegationSetupStep2Label), this.context.intl.formatMessage(messages.delegationSetupStep3Label)];
  handleDialogClose = () => {
    const {
      stores,
      actions
  ...
  handleConfirm = (spendingPassword: string | null | undefined, isHardwareWallet: boolean) => {
    const {
      selectedPoolId,
      selectedWalletId
    } = this.state;
  ...
  render() {
    ...
    const acceptableWallets = find(wallets.allWallets, ({
      amount,
      reward
    ...
    return <DelegationSetupWizardDialog wallets={wallets.allWallets} stepsList={this.STEPS_LIST} activeStep={activeStep} minDelegationFunds={MIN_DELEGATION_FUNDS} isDisabled={activeStep === 1 && !acceptableWallets} isWalletAcceptable={this.handleIsWalletAcceptable} selectedWallet={selectedWallet} selectedPool={selectedPool || null} stakePoolsList={stakePools} recentStakePools={take(recentStakePools, RECENT_STAKE_POOLS_COUNT)} stakePoolJoinFee={stakePoolJoinFee} futureEpochStartTime={futureEpochStartTime} currentLocale={currentLocale} onOpenExternalLink={app.openExternalLink} currentTheme={currentTheme} onClose={this.handleDialogClose} onContinue={this.handleContinue} onSelectWallet={this.handleSelectWallet} onSelectPool={this.handleSelectPool} onBack={this.onBack} onLearnMoreClick={this.handleLearnMoreClick} onConfirm={this.handleConfirm} getStakePoolById={getStakePoolById} isSubmitting={joinStakePoolRequest.isExecuting || sendMoneyRequest.isExecuting || isDelegationTransactionPending} error={joinStakePoolRequest.error || sendMoneyRequest.error || selectCoinsRequest.error} hwDeviceStatus={hwDeviceStatus} isTrezor={isTrezor} />;
  }
  ...
  async _handleCalculateTransactionFee(poolId: string) {
    ...
    if (this._isMounted && isOpen(DelegationSetupWizardDialog) && stakePoolJoinFee) {
      this.setState({
        stakePoolJoinFee
      });
    }
