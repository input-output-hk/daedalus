...
class RedeemItnRewardsContainer extends Component<Props> {
  ...
  render() {
    ...
    if (!isSynced && redeemStep === REDEEM_ITN_REWARDS_STEPS.CONFIGURATION) return <RedemptionUnavailableContainer onClose={closeRedeemDialog.trigger} />;
    if (!allWallets.length) return <NoWalletsContainer onClose={closeRedeemDialog.trigger} />;
    const CurrentContainer = this.containers[redeemStep];
    return <>
        {(isSubmittingReedem || isCalculatingReedemFees) && <LoadingOverlay />}
        <CurrentContainer onBack={onRedeemStart.trigger} onClose={closeRedeemDialog.trigger} />
      </>;
  }
