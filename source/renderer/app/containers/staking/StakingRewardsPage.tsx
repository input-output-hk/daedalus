...
class StakingRewardsPage extends Component<Props> {
  ...
  onOpenExternalLink = (rewardsAddress: string) => {
    ...
    const cardanoExplorerLink = `${getNetworkExplorerUrl(network)}/address/${rewardsAddress}`;
  ...
  render() {
    ...
    return <StakingRewards rewards={rewards} isLoading={false} isExporting={wallets.generatingRewardsCsvInProgress} onLearnMoreClick={this.handleLearnMoreClick} onExportCsv={requestCSVFile.trigger} onCopyAddress={this.handleCopyAddress} onOpenExternalLink={this.onOpenExternalLink} />;
  }
