...
class StakePoolsListPage extends Component<Props> {
  ...
  render() {
    ...
    const isRanking = !isLoading && staking.isRanking && stakePoolsRequest.isExecuting;
    return <Fragment>
        <StakePools wallets={all} currentLocale={currentLocale} stakePoolsList={stakePools} stakePoolsDelegatingList={recentStakePools} onOpenExternalLink={app.openExternalLink} currentTheme={currentTheme} updateDelegatingStake={this.onUpdateDelegatingStake} rankStakePools={this.onRankStakePools} selectedDelegationWalletId={selectedDelegationWalletId} stake={stake} onDelegate={this.handleDelegate} isLoading={isLoading} isFetching={isFetchingStakePools} isRanking={isRanking} getStakePoolById={getStakePoolById} smashServerUrl={smashServerUrl} onSmashSettingsClick={this.handleSmashSettingsClick} maxDelegationFunds={maxDelegationFunds} />
