...
class WalletReceivePage extends Component<Props, State> {
  ...
  getAddressAndFilepath = async (fileExtension: string = 'pdf') => {
    ...
    const prefix = `${intl.formatMessage(messages.address)}-${activeWallet.name}`;
    ...
    const defaultPath = path.join(desktopDirectoryPath, `${name}.${fileExtension}`);
    const params = {
      defaultPath,
      filters: [{
        name,
  ...
  handleDownloadPDF = async (note: string) => {
    const {
      address,
      filePath
    } = await this.getAddressAndFilepath();
  ...
  handleSaveQRCodeImage = async () => {
    const {
      address,
      filePath
    } = await this.getAddressAndFilepath('png');
    // if cancel button is clicked or path is empty
    if (!filePath || !address) return;
    this.handleCloseShareAddress();
  ...
  handleSupportRequestClick = async (supportRequestLinkUrl: string) => {
    ...
    const supportUrl = generateSupportRequestLink(supportRequestLinkUrl, environment, profile.currentLocale);
  ...
  render() {
    ...
    if (!activeWallet) throw new Error('Active wallet required for WalletReceivePage.');
    const {
      hasPassword,
      isRandom
    } = activeWallet;
    ...
    const isByronWalletAddressUsed = addresses.active ? addresses.active.used : false;
    const isTrezor = checkIsTrezorByWalletId(activeWallet.id);
    return <Fragment>
        <VerticalFlexContainer>
          {isRandom ? <WalletReceiveRandom walletAddress={byronWalletAddress} isWalletAddressUsed={isByronWalletAddressUsed} walletAddresses={walletAddresses} onGenerateAddress={this.handleGenerateAddress} onShareAddress={this.handleShareAddress} onCopyAddress={this.handleCopyAddress} isSidebarExpanded={sidebar.isShowingSubMenus} walletHasPassword={hasPassword} isSubmitting={addresses.createByronWalletAddressRequest.isExecuting} error={addresses.error} showUsed={showUsedAddresses} onToggleUsedAddresses={this.handleToggleUsedAddresses} /> : <WalletReceiveSequential walletAddresses={walletAddresses} isAddressValid={this.handleIsAddressValid} onShareAddress={this.handleShareAddress} onCopyAddress={this.handleCopyAddress} onToggleSubMenus={toggleSubMenus} showUsed={showUsedAddresses} onToggleUsedAddresses={this.handleToggleUsedAddresses} />}
        </VerticalFlexContainer>

        {uiDialogs.isOpen(WalletReceiveDialog) && addressToShare && <WalletReceiveDialog address={addressToShare} walletName={activeWallet.name} onCopyAddress={this.handleCopyAddress} onDownloadPDF={this.handleDownloadPDF} onSaveQRCodeImage={this.handleSaveQRCodeImage} onClose={this.handleCloseShareAddress} isHardwareWallet={activeWallet.isHardwareWallet} hwDeviceStatus={hwDeviceStatus} transportDevice={transportDevice} isAddressDerived={isAddressDerived} isAddressChecked={isAddressChecked} onChangeVerificationStatus={setAddressVerificationCheckStatus} onSupportRequestClick={this.handleSupportRequestClick} isTrezor={isTrezor} />}
      </Fragment>;
  }
