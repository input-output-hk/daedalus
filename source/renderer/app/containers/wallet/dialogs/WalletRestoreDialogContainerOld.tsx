...
class WalletRestoreDialogContainer extends Component<Props> {
  ...
  render() {
    ...
    const error = restoreDaedalusRequest.error || restoreLegacyRequest.error || getWalletRecoveryPhraseFromCertificateRequest.error;
    const isExecuting = restoreDaedalusRequest.isExecuting || restoreLegacyRequest.isExecuting || getWalletRecoveryPhraseFromCertificateRequest.isExecuting;
    return <WalletRestoreDialog mnemonicValidator={isValidMnemonic} suggestedMnemonics={validWords} isSubmitting={isExecuting} onSubmit={this.onSubmit} onCancel={this.onCancel} onChoiceChange={this.resetRequests} error={error} />;
  }
