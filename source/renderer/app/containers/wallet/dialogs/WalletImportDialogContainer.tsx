...
class WalletImportDialogContainer extends Component<Props> {
  ...
  render() {
    ...
    const {
      nextStep,
      resetMigration
    } = actions.walletMigration;
    const CurrentContainer = walletMigrationStep ? this.containers[walletMigrationStep] : WalletFileImportStepContainer;
    return <CurrentContainer onContinue={() => nextStep.trigger()} onClose={() => resetMigration.trigger()} />;
  }
