...
class VerificationDialogContainer extends Component<Props> {
  ...
  render() {
    ...
    if (!walletCertificateRecoveryPhrase || !additionalMnemonicWords) {
      throw new Error('Props walletCertificateRecoveryPhrase and additionalMnemonicWords are required');
    }

    return <VerificationDialog suggestedMnemonics={validWords} additionalMnemonicWords={additionalMnemonicWords} walletCertificateRecoveryPhrase={walletCertificateRecoveryPhrase} onContinue={this.onContinue} onClose={this.props.onClose} />;
  }
