...
const messages: WalletSettingRemoveMessages = defineMessages({
  ...
  confirmButtonLabel: {
    ...
    description: 'Label for the "Unpair (x)" button in the unpair wallet dialog.'
  },
  confirmationQuestion: {
    id: 'wallet.settings.unpair.dialog.confirmationQuestion',
    defaultMessage: '!!!Do you really want to unpair <strong>{walletName}</strong> wallet?',
...
class UnpairWalletDialogContainer extends Component<Props> {
  ...
  render() {
    ...
    return <WalletSettingsActionConfirmationDialog isTest={isTest} isUnpair walletName={activeWallet.name} hasWalletFunds={activeWallet.hasFunds} countdownFn={uiDialogs.countdownSinceDialogOpened} isBackupNoticeAccepted={dialogData.isBackupNoticeAccepted} messages={Object.freeze(messages)} onAcceptBackupNotice={() => updateDataForActiveDialog.trigger({
      data: {
    ...
    }} confirmationValue={dialogData.confirmationValue} onConfirmationValueChange={confirmationValue => updateDataForActiveDialog.trigger({
      data: {
        confirmationValue
      }
