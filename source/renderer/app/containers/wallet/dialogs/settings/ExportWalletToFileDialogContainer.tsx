...
class ExportWalletToFileDialogContainer extends Component<Props> {
  ...
  onSubmit = async (params: OnSubmitParams) => {
    ...
    const {
      filePath
    } = await showSaveDialogChannel.send(fileParams);
    const {
      stores,
      actions
    } = this.props;
  ...
  render() {
    ...
    return <ExportWalletToFileDialog walletName={activeWallet.name} isSubmitting={exportWalletToFileRequest.isExecuting} onSubmit={this.onSubmit} onClose={this.onCancel} error={exportWalletToFileRequest.error} />;
  }
