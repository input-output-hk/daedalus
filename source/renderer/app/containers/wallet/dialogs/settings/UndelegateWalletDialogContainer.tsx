...
import { DELEGATION_ACTIONS, DELEGATION_DEPOSIT } from "../../../../config/stakingConfig";
...
class UndelegateWalletDialogContainer extends Component<Props, State> {
  ...
  async _handleCalculateTransactionFee() {
    ...
    const selectedWallet = find(wallets.allWallets, wallet => wallet.id === this.selectedWalletId);
    const {
  ...
  render() {
    ...
    if ((!stakePoolId || !isDelegationTransactionPending) && undelegateWalletSubmissionSuccess && !quitStakePoolRequest.error) {
      return <UndelegateWalletSuccessDialog walletName={walletName} futureEpochStartTime={futureEpochStartTime} currentLocale={currentLocale} onClose={() => {
    ...
    return <UndelegateWalletConfirmationDialog selectedWallet={walletToBeUndelegated} stakePoolName={stakePoolName} stakePoolTicker={stakePoolTicker} onConfirm={(passphrase: string, isHardwareWallet: boolean) => {
    ...
    }} onExternalLinkClick={onExternalLinkClick} isSubmitting={quitStakePoolRequest.isExecuting || sendMoneyRequest.isExecuting || isDelegationTransactionPending} error={quitStakePoolRequest.error || sendMoneyRequest.error || selectCoinsRequest.error} fees={stakePoolQuitFee} hwDeviceStatus={hwDeviceStatus} isTrezor={isTrezor} />;
  }
