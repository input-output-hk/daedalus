...
import { RECOVERY_PHRASE_VERIFICATION_WORD_COUNT, LEGACY_WALLET_RECOVERY_PHRASE_WORD_COUNT } from "../../../../config/cryptoConfig";
...
class WalletRecoveryPhraseContainer extends Component<Props> {
  ...
  render() {
    ...
    const onContinue = this.step === 2 ? recoveryPhraseVerificationCheck.trigger : recoveryPhraseVerificationContinue.trigger;
    const onClose = recoveryPhraseVerificationClose.trigger;
    const expectedWordCount = activeWallet.isRandom ? LEGACY_WALLET_RECOVERY_PHRASE_WORD_COUNT : RECOVERY_PHRASE_VERIFICATION_WORD_COUNT;
    const WalletRecoveryPhraseDialog = this.recoveryPhraseComponent;
    return WalletRecoveryPhraseDialog && <WalletRecoveryPhraseDialog onContinue={onContinue} onClose={onClose} expectedWordCount={expectedWordCount} openExternalLink={openExternalLink} walletName={activeWallet.name} />;
  }
