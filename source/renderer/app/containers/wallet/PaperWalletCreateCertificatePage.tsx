...
class PaperWalletCreateCertificatePage extends Component<Props, State> {
  ...
  componentDidUpdate() {
    const stepChanged = this.props.stores.wallets.certificateStep !== this.state.currentStep;
  ...
  CREATE_CERTIFICATE_DIALOGS = ['instructions', 'print', 'securingPassword', 'verification', 'completion'];
  state = {
    currentStep: 0,
  ...
  render() {
    ...
    if (uiDialogs.isOpen(InstructionsDialog)) {
      activeDialog = <InstructionsDialogContainer onContinue={this.onContinue} onClose={this.onClose} />;
    }

    if (uiDialogs.isOpen(PrintDialog)) {
      activeDialog = <PrintDialogContainer onContinue={this.onContinue} onClose={this.showConfirmationDialog} />;
    }

    if (uiDialogs.isOpen(SecuringPasswordDialog)) {
      activeDialog = <SecuringPasswordDialogContainer onContinue={this.onContinue} onClose={this.showConfirmationDialog} />;
    }

    if (uiDialogs.isOpen(VerificationDialog)) {
      activeDialog = <VerificationDialogContainer onContinue={this.onContinue} onClose={this.showConfirmationDialog} />;
    }
    ...
    return <div>
        {activeDialog}
        {showConfirmationDialog && <ConfirmationDialog network={network} onCancel={this.hideConfirmationDialog} onConfirm={this.onClose} />}
      </div>;
  }
