...
class WalletAddPage extends Component<Props> {
  ...
  render() {
    ...
    const onCreateWallet = createWalletUseNewProcess ? () => actions.wallets.createWalletBegin.trigger() : // TODO: Remove once the new wallet creation process is ready
    () => actions.dialogs.open.trigger({
      dialog: WalletCreateDialog
    });
    const onRestoreWallet = restoreWalletUseNewProcess ? () => actions.wallets.restoreWalletBegin.trigger() : // TODO: Remove once the new wallet restoration process is ready
    () => actions.dialogs.open.trigger({
      dialog: WalletRestoreDialog
    });

    const onImportWallet = () => actions.walletMigration.initiateMigration.trigger();
    ...
    return <Layout>
        <WalletAdd onCreate={onCreateWallet} onRestore={onRestoreWallet} onImport={onImportWallet} onConnect={onConnectWallet} isMaxNumberOfWalletsReached={wallets.hasMaxWallets} isMainnet={isMainnet} isTestnet={isTestnet} isProduction={isProduction} />
        {activeDialog}
      </Layout>;
  }
