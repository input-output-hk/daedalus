...
class WalletSendPage extends Component<Props> {
  ...
  openDialog = (dialog: (...args: Array<any>) => any, isHardwareWallet: boolean, walletId: string) => {
    const {
      isFlight
    } = global;
  ...
  render() {
    ...
    const selectedAsset = activeAsset ? this.getAssetByUniqueId(activeAsset, allAssets) : null;
    ...
    return <WalletSendForm currencyMaxIntegerDigits={MAX_INTEGER_PLACES_IN_ADA} currencyMaxFractionalDigits={DECIMAL_PLACES_IN_ADA} currentNumberFormat={profile.currentNumberFormat} calculateTransactionFee={(address: string, amount: number, selectedAssets: ApiTokens) => this.calculateTransactionFee({
      walletId: wallet.id,
      address,
      amount,
      isHardwareWallet,
      selectedAssets
    })} walletAmount={wallet.amount} validateAmount={validateAmount} validateAssetAmount={validateAssetAmount} addressValidator={isValidAddress} assets={assetTokens} hasAssets={hasAssetsEnabled && hasRawAssets} selectedAsset={selectedAsset} isLoadingAssets={isLoadingAssets} isDialogOpen={uiDialogs.isOpen} isRestoreActive={wallet.isRestoring} isHardwareWallet={isHardwareWallet} hwDeviceStatus={hwDeviceStatus} onOpenDialogAction={params => this.openDialog(params.dialog, isHardwareWallet, wallet.id)} onUnsetActiveAsset={unsetActiveAsset.trigger} onExternalLinkClick={app.openExternalLink} isAddressFromSameWallet={isAddressFromSameWallet} />;
  }
