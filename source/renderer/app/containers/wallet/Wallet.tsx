...
class Wallet extends Component<Props> {
  ...
  isActiveScreen = (page: string, item: NavDropdownProps) => {
    ...
    if (options && options.length) {
      options.forEach(option => {
        if (app.currentRoute && app.currentRoute.includes(option.value.toString())) {
  ...
  render() {
    ...
    return <MainLayout>
        {isRestoring ? <RestoreNotification restoreProgress={activeWallet.restorationProgress} /> : null}

        <WalletWithNavigation activeItem={app.currentPage} hasNotification={hasNotification} hasPassword={hasPassword} isActiveScreen={this.isActiveScreen} isLegacy={isLegacy} isNotResponding={isNotResponding} isHardwareWallet={isHardwareWallet} isSetWalletPasswordDialogOpen={isDialogOpen(ChangeSpendingPasswordDialog)} onOpenExternalLink={(url: string) => stores.app.openExternalLink(url)} onRestartNode={() => restartNode.trigger()} onSetWalletPassword={() => {
