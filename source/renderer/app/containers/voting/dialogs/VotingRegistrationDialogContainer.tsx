...
import { VOTING_REGISTRATION_MIN_WALLET_FUNDS, VOTING_REGISTRATION_FEE_CALCULATION_AMOUNT } from "../../../config/votingConfig";
...
class VotingRegistrationDialogContainer extends Component<Props, State> {
  ...
  handleIsWalletAcceptable = (isLegacy?: boolean, isRestoring?: boolean, walletAmount?: BigNumber, walletReward: BigNumber = 0) => !isLegacy && !isRestoring && walletAmount && walletAmount.gte(new BigNumber(VOTING_REGISTRATION_MIN_WALLET_FUNDS)) && !walletAmount.isEqualTo(walletReward);
  ...
  STEPS_LIST = [this.context.intl.formatMessage(messages.votingRegistrationStep1Label), this.context.intl.formatMessage(messages.votingRegistrationStep2Label), this.context.intl.formatMessage(messages.votingRegistrationStep3Label), this.context.intl.formatMessage(messages.votingRegistrationStep4Label), this.context.intl.formatMessage(messages.votingRegistrationStep5Label)];
  ...
  handleSendTransaction = (spendingPassword: string | null | undefined) => {
    const amount = formattedAmountToLovelace(`${VOTING_REGISTRATION_FEE_CALCULATION_AMOUNT}`);
  ...
  render() {
    ...
    const selectedWallet = find(all, wallet => wallet.id === selectedWalletId);
    let isTrezor = false;
    ...
    return <>
        <VotingRegistrationDialogWizard onClose={this.handleClose} stepsList={this.STEPS_LIST} activeStep={registrationStep} stakePoolsList={stakePools} wallets={all} minVotingRegistrationFunds={VOTING_REGISTRATION_MIN_WALLET_FUNDS} isWalletAcceptable={this.handleIsWalletAcceptable} selectedWallet={selectedWallet} getStakePoolById={getStakePoolById} onContinue={this.handleContinue} onSelectWallet={this.handleSelectWallet} onSetPinCode={this.handleSetPinCode} onSubmit={this.handleSendTransaction} onRestart={this.handleRestart} onBack={this.handleBack} transactionFee={transactionFee} transactionFeeError={transactionFeeError} qrCode={qrCode} onDownloadPDF={saveAsPDF.trigger} isTransactionPending={isTransactionPending || isHardwareWallet && isHwTransactionPending} isTransactionConfirmed={isTransactionConfirmed} transactionConfirmations={transactionConfirmations} hwDeviceStatus={hwDeviceStatus} transactionError={getWalletPublicKeyRequest.error || createVotingRegistrationTransactionRequest.error || signMetadataRequest.error || isHardwareWallet && sendMoneyRequest.error} onExternalLinkClick={openExternalLink} isTrezor={isTrezor} isHardwareWallet={isHardwareWallet} />
        {isConfirmationDialogOpen && <ConfirmationDialog onConfirm={closeConfirmationDialog.trigger} onCancel={() => {
        this.props.actions.dialogs.closeActiveDialog.trigger();
      }} />}
      </>;
  }
  ...
  async _handleCalculateTransactionFee() {
    ...
    try {
      ...
      const transactionFeeError = errorHasLink ? <FormattedHTMLMessageWithLink message={error} onExternalLinkClick={app.openExternalLink} /> : this.context.intl.formatMessage(error);
