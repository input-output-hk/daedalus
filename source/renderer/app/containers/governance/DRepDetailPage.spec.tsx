...
describe('DRepDetailPage', () => {
  ...
  it('labels the on-chain section with the on-chain source label', async () => {
    await renderPage();
  ...
  it('renders the anchor presence with the on-chain anchor reference label', async () => {
    await renderPage();
  ...
  it('renders the category badge in the detail header (snapshot)', async () => {
    await renderPage();

    // baseEntry: verifiedName null, drepActivity 34 -> Non-metadata.
    expect(
  ...
  it('renders the verified name with the off-chain label and anchor host tooltip', async () => {
    ...
    expect(label.getAttribute('title')).toEqual(
      expect.stringContaining('governance-preview.example.org')
    );
  });
  ...
  it('renders every verified profile field with a verified off-chain label', async () => {
    await renderPage({
      governanceOverrides: {
        fetchDRep: jest.fn().mockResolvedValue({
          ...baseDetail,
  ...
  it('renders an identity reference under the claim caption and never as a plain link', async () => {
    await renderPage({
      governanceOverrides: {
        fetchDRep: jest.fn().mockResolvedValue({
          ...baseDetail,
