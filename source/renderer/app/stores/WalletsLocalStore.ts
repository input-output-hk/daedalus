...
export default class WalletsLocalStore extends Store {
  @observable
  localWalletsRequest: Request<WalletsLocalData> = new Request(this.api.localStorage.getWalletsLocalData);
  @observable
  setWalletLocalDataRequest: Request<any> = new Request(this.api.localStorage.setWalletLocalData);
  @observable
  unsetWalletLocalDataRequest: Request<any> = new Request(this.api.localStorage.unsetWalletLocalData);

  setup() {
    const {
      walletsLocal: actions
    } = this.actions;
  ...
  get all(): WalletsLocalData {
    return this.localWalletsRequest.result ? this.localWalletsRequest.result : {};
  }
  ...
  _refreshWalletsLocalData = async () => {
    ...
    await asyncForEach(wallets, async ({
      id: walletId
    }) => {
