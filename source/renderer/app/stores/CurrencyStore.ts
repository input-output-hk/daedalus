...
import { CURRENCY_REQUEST_RATE_INTERVAL, getLocalizedCurrency, getLocalizedCurrenciesList, getCurrencyFromCode } from "../config/currencyConfig";
...
export default class CurrencyStore extends Store {
  ...
  _setupCurrency = async () => {
    ...
    this._getCurrencyRateInterval = setInterval(this.getCurrencyRate, CURRENCY_REQUEST_RATE_INTERVAL);
    // Fetch the currency list and rate
    this.getCurrencyList();
    this.getCurrencyRate();
  };
  @action
  _setCurrencySelected = async ({
    code
  }: {
    code: string;
  }) => {
