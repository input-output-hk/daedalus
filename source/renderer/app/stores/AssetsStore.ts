...
export default class AssetsStore extends Store {
  ...
  // REQUESTS
  @observable
  favoritesRequest: Request<Record<string, any>> = new Request(this.api.localStorage.getWalletTokenFavorites);
  @observable
  ...
  // REQUESTS
  @observable
  getAssetSettingsDialogWasOpenedRequest: Request<void> = new Request(this.api.localStorage.getAssetSettingsDialogWasOpened);
  ...
  getAsset = (policyId: string, assetName: string): Asset | null | undefined => this.details[`${policyId}${assetName}`];
  ...
  @action
  _createWalletTokensRequest = (walletId: string): Request<GetAssetsResponse> => {
  ...
  _retrieveAssetsRequest = (walletId: string): Request<GetAssetsResponse> => this.assetsRequests[walletId] || this._createWalletTokensRequest(walletId);
}