...
import { ALLOWED_TIME_DIFFERENCE, NETWORK_STATUS_POLL_INTERVAL, NETWORK_CLOCK_POLL_INTERVAL, MAX_ALLOWED_STALL_DURATION, DECENTRALIZATION_LEVEL_POLLING_INTERVAL } from "../config/timingConfig";
import { INITIAL_DESIRED_POOLS_NUMBER } from "../config/stakingConfig";
import { logger } from "../utils/logging";
import { cardanoStateChangeChannel, cardanoTlsConfigChannel, restartCardanoNodeChannel, getCachedCardanoStatusChannel, setCachedCardanoStatusChannel } from "../ipc/cardano.ipc";
...
import type { GetNetworkInfoResponse, GetNetworkClockResponse, GetNetworkParametersResponse, NextEpoch, FutureEpoch, TipInfo } from "../api/network/types";
import type { CardanoNodeState, CardanoStatus, TlsConfig } from "../../../common/types/cardano-node.types";
...
const NODE_STOPPING_STATES = [CardanoNodeStates.EXITING, CardanoNodeStates.STOPPING, CardanoNodeStates.UPDATING];
const NODE_STOPPED_STATES = [CardanoNodeStates.CRASHED, CardanoNodeStates.ERRORED, CardanoNodeStates.STOPPED, CardanoNodeStates.UPDATED, CardanoNodeStates.UNRECOVERABLE];
// END CONSTANTS ----------------------------
const {
  isFlight
} = global;
export default class NetworkStatusStore extends Store {
  ...
  @observable
  getNetworkInfoRequest: Request<GetNetworkInfoResponse> = new Request(this.api.ada.getNetworkInfo);
  @observable
  getNetworkClockRequest: Request<GetNetworkClockResponse> = new Request(this.api.ada.getNetworkClock);
  @observable
  getNetworkParametersRequest: Request<GetNetworkParametersResponse> = new Request(this.api.ada.getNetworkParameters);
  @observable
  ...
  setup() {
    ...
    networkStatusActions.forceCheckNetworkClock.listen(this._forceCheckNetworkClock);
    ...
    // ========== MOBX REACTIONS =========== //
    this.registerReactions([this._updateNetworkStatusWhenConnected, this._updateNetworkStatusWhenDisconnected, this._updateNodeStatus]);
  ...
  _getStateDirectoryPath = async () => {
    this._onReceiveStateDirectoryPath(await getStateDirectoryPathChannel.request());
  };
  ...
  _updateTlsConfig = (config: TlsConfig | null | undefined): Promise<void> => {
    if (config == null || isEqual(config, this.tlsConfig)) return Promise.resolve();
  ...
  _extractNodeStatus = (from: Record<string, any> & CardanoStatus): CardanoStatus => {
    const {
      isNodeResponding,
      isNodeSyncing,
      isNodeInSync,
  ...
  _setNetworkStatusPollingInterval = () => {
    this._networkStatusPollingInterval = setInterval(this._updateNetworkStatus, NETWORK_STATUS_POLL_INTERVAL);
  };
  @action
  _setNetworkClockPollingInterval = () => {
    this._networkClockPollingInterval = setInterval(this._updateNetworkClock, NETWORK_CLOCK_POLL_INTERVAL);
  };
  @action
  _setNetworkParametersPollingInterval = () => {
    this._networkParametersPollingInterval = setInterval(this._getNetworkParameters, DECENTRALIZATION_LEVEL_POLLING_INTERVAL);
  };
  @action
  ...
  _updateNetworkClock = async (isForceCheck: boolean = false) => {
    ...
    if (!this.isNodeResponding || this.isSystemTimeIgnored && !isForceCheck) return;
    ...
    try {
      ...
      runInAction('update localTimeDifference and isNodeTimeCorrect', () => {
        ...
        if (networkClock.status !== 'pending') {
          ...
          this.isNodeTimeCorrect = this.localTimeDifference != null && // If we receive 'null' it means NTP check failed
  ...
  _updateNetworkStatus = async () => {
    ...
    try {
      const networkStatus: GetNetworkInfoResponse = await this.getNetworkInfoRequest.execute().promise;
      ...
      if (nextEpoch && this.epochLength && this.slotLength) {
        ...
        futureEpoch = {
          ...
          epochStart: nextEpoch.epochStart ? moment(nextEpoch.epochStart).add(startDelta, 'seconds').toISOString() : ''
        };
      }
      ...
      runInAction('update localTip, networkTip, nextEpoch and futureEpoch', () => {
      ...
      runInAction('update isSyncProgressStalling', () => {
        ...
        if (this.isNodeInSync || // Update last sync progress change timestamp if node is in sync
        ...
        this.lastSyncProgressChangeTimestamp > Date.now() || !this.isNodeTimeCorrect && !this.isSystemTimeIgnored)) {
          this.lastSyncProgressChangeTimestamp = Date.now(); // Record last sync progress change timestamp
        }

        const lastSyncProgressChangeStall = moment(Date.now()).diff(moment(this.lastSyncProgressChangeTimestamp));
        this.isSyncProgressStalling = lastSyncProgressChangeStall > MAX_ALLOWED_STALL_DURATION;
      });
  ...
  _getNetworkParameters = async () => {
    ...
    try {
      const networkParameters: GetNetworkParametersResponse = await this.getNetworkParametersRequest.execute().promise;
      let {
        isShelleyActivated,
        isShelleyPending,
      ...
      if (eras) {
        ...
        if (shelleyActivationTime !== '') {
          const shelleyActivationTimeStamp = new Date(shelleyActivationTime).getTime();
        ...
        if (alonzoActivationTime !== '') {
          const alonzoActivationTimeStamp = new Date(alonzoActivationTime).getTime();
      ...
      runInAction('Update Desired Pool Number', () => {
        this.desiredPoolNumber = desiredPoolNumber || INITIAL_DESIRED_POOLS_NUMBER;
      });
  ...
  @action
  _onCheckVerificationProgress = (verificationProgress: number): Promise<void> => {
  ...
  get isEpochsInfoAvailable(): boolean {
    ...
    return get(nextEpoch, 'epochNumber', null) !== null && get(nextEpoch, 'epochStart', null) !== null && get(networkTip, 'epoch', null) !== null && get(networkTip, 'slot', null) !== null;
  }
