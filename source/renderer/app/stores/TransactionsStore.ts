...
import type { GetTransactionFeeRequest, DeleteTransactionRequest, GetTransactionsResponse, CreateExternalTransactionRequest, GetWithdrawalsResponse } from "../api/transactions/types";
import { isValidAmountInLovelaces, isValidAssetAmountInNaturalUnits } from "../utils/validations";
import transactionsCsvGenerator from "../utils/transactionsCsvGenerator";
import { i18nContext } from "../utils/i18nContext";
import { generateFilterOptions, isTransactionInFilterRange } from "../utils/transaction";
...
export type DateRangeType = "" | "last7Days" | "last30Days" | "last90Days" | "thisYear" | "custom";
...
export default class TransactionsStore extends Store {
  ...
  @observable
  deleteTransactionRequest: Request<DeleteTransactionRequest> = new Request(this.api.ada.deleteTransaction);
  @observable
  createExternalTransactionRequest: Request<CreateExternalTransactionRequest> = new Request(this.api.ada.createExternalTransaction);
  @observable
  _filterOptionsForWallets = {};
  @observable
  calculateTransactionFeeRequest: Request<GetTransactionFeeRequest> = new Request(this.api.ada.calculateTransactionFee);
  ...
  get withdrawals(): Record<string, BigNumber> {
    ...
    for (const wallet of wallets) {
      ...
      withdrawals[walletId] = get(request, 'result.withdrawals') || new BigNumber(0);
    }
  ...
  get allFiltered(): Array<WalletTransaction> {
    ...
    const allFiltered = this.all.filter(transaction => isTransactionInFilterRange(this.filterOptions, transaction));
    // Straight away show recent filtered transactions if all filtered ones are not loaded yet
    return !allFiltered.length && recentFiltered.length ? recentFiltered : allFiltered;
  }
  ...
  get recentFiltered(): Array<WalletTransaction> {
    return this.recent.filter(transaction => isTransactionInFilterRange(this.filterOptions, transaction));
  }
  ...
  calculateTransactionFee = async (transactionFeeRequest: TransactionFeeRequest) => {
    ...
    return this.calculateTransactionFeeRequest.execute({ ...transactionFeeRequest,
  ...
  deletePendingTransaction = async ({
    ...
    if (!wallet) {
      throw new Error('Active wallet required before deleting a pending transaction.');
    }
  ...
  validateAmount = (amountInLovelaces: string): Promise<boolean> => Promise.resolve(isValidAmountInLovelaces(amountInLovelaces));
  validateAssetAmount = (amountInNaturalUnits: string): Promise<boolean> => Promise.resolve(isValidAssetAmountInNaturalUnits(amountInNaturalUnits));
  // ======================= PRIVATE ========================== //
  @action
  _updateFilterOptions = (filterOptions: TransactionFilterOptionsType) => {
    ...
    this._filterOptionsForWallets[wallet.id] = { ...currentFilterOptions,
      ...filterOptions
    };
    return true;
  };
  ...
  _clearFilterOptions = () => {
    const wallet = this.stores.wallets.active;
    if (!wallet) return false;
    this._filterOptionsForWallets[wallet.id] = { ...emptyTransactionFilterOptions
    };
    return true;
  };
  @action
  ...
  _getWithdrawalsRequest = (walletId: string): Request<GetWithdrawalsResponse> => {
    const foundRequest = find(this.transactionsRequests, {
      walletId
    });
    if (foundRequest && foundRequest.withdrawalsRequest) return foundRequest.withdrawalsRequest;
