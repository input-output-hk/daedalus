...
export default class WalletSettingsStore extends Store {
  ...
  @observable
  updateSpendingPasswordRequest: Request<boolean> = new Request(this.api.ada.updateSpendingPassword);
  @observable
  exportWalletToFileRequest: Request<Promise<[]>> = new Request(this.api.ada.exportWalletToFile);
  @observable
  getWalletUtxosRequest: Request<WalletUtxos> = new Request(this.api.ada.getWalletUtxos);
  @observable
  ...
  setup() {
    ...
    walletSettingsActions.startEditingWalletField.listen(this._startEditingWalletField);
    walletSettingsActions.stopEditingWalletField.listen(this._stopEditingWalletField);
    walletSettingsActions.cancelEditingWalletField.listen(this._cancelEditingWalletField);
    walletSettingsActions.updateWalletField.listen(this._updateWalletField);
    walletSettingsActions.updateSpendingPassword.listen(this._updateSpendingPassword);
    walletSettingsActions.exportToFile.listen(this._exportToFile);
    walletSettingsActions.startWalletUtxoPolling.listen(this._startWalletUtxoPolling);
    walletSettingsActions.stopWalletUtxoPolling.listen(this._stopWalletUtxoPolling);
    walletSettingsActions.recoveryPhraseVerificationContinue.listen(this._recoveryPhraseVerificationContinue);
    walletSettingsActions.recoveryPhraseVerificationCheck.listen(this._recoveryPhraseVerificationCheck);
    walletSettingsActions.recoveryPhraseVerificationClose.listen(this._recoveryPhraseVerificationClose);
    walletSettingsActions.toggleShowUsedAddresses.listen(this._toggleShowUsedAddressesStatuses);
    sidebarActions.walletSelected.listen(this._onWalletSelected);
  }

  // =================== PUBLIC API ==================== //
  // GETTERS
  getWalletsRecoveryPhraseVerificationData = (walletId: string) => this.walletsRecoveryPhraseVerificationData[walletId] || {};
  getLocalWalletDataById = (id: any): WalletLocalData | null | undefined => {
    const {
      all: walletsLocalData
    } = this.stores.walletsLocal;
  ...
  get walletsRecoveryPhraseVerificationData() {
    ...
    return Object.keys(walletsLocalData).map(key => walletsLocalData[key]).reduce((obj, {
      id,
      ...
      const hasNotification = recoveryPhraseVerificationStatus === RECOVERY_PHRASE_VERIFICATION_STATUSES.NOTIFICATION;
      obj[id] = {
        id,
        recoveryPhraseVerificationDate,
        creationDate,
  ...
  _startWalletUtxoPolling = () => {
    ...
    this.pollingApiInterval = setInterval(this._getWalletUtxoApiData, WALLET_UTXO_API_REQUEST_INTERVAL);
  };
  @action
  ...
  _recoveryPhraseVerificationContinue = async () => {
    ...
    if (step === 4) this.recoveryPhraseStep = 2;else this.recoveryPhraseStep = step + 1;
  };
  @action
  ...
  _recoveryPhraseVerificationCheck = async ({
    ...
    if (!walletId) throw new Error('It was not possible to retrieve the walletId.');
    const activeWallet = this.stores.wallets.active;
    if (!activeWallet) throw new Error('Active wallet required before checking recovery phrase.');
    ...
    runInAction('AdaWalletBackupStore::_recoveryPhraseVerificationCheck', () => {
  ...
  _toggleShowUsedAddressesStatuses = async () => {
    const activeWallet = this.stores.wallets.active;
    if (!activeWallet) throw new Error('Active wallet required before checking show used addresses statuses.');
    const localWalletData: WalletLocalData | null | undefined = this.getLocalWalletDataById(activeWallet ? activeWallet.id : '');
    const {
      showUsedAddresses
    } = localWalletData || {};
    await this.actions.walletsLocal.setWalletLocalData.trigger({
