...
export default class WalletMigrationStore extends Store {
  ...
  constructor(
    ...
    makeObservable(this, {
      walletMigrationStep: observable,
      isExportRunning: observable,
      exportedWallets: observable,
      exportErrors: observable,
      exportSourcePath: observable,
      defaultExportSourcePath: observable,
      isTestMigrationEnabled: observable,
      isRestorationRunning: observable,
      restoredWallets: observable,
      restorationErrors: observable,
      getWalletMigrationStatusRequest: observable,
      setWalletMigrationStatusRequest: observable,
      restoreExportedWalletRequest: observable,
      _initiateMigration: action,
      _selectExportSourcePath: action,
      _resetExportSourcePath: action,
      _nextStep: action,
      _toggleWalletImportSelection: action,
      _updateWalletImportStatus: action,
      _updateWalletName: action,
      _exportWallets: action,
      _restoreWallets: action,
      _restoreWallet: action,
      _generateMigrationReport: action,
      _startMigration: action,
      _resetExportData: action,
      _resetRestorationData: action,
      _resetMigration: action,
      _setFakedImportPath: action,
      _enableTestWalletMigration: action,
      _finishMigration: action,
      pendingImportWallets: computed,
      pendingImportWalletsCount: computed,
      exportedWalletsData: computed,
      exportedWalletsCount: computed,
      restoredWalletsData: computed,
      restoredWalletsCount: computed,
    });
  }
  ...
  _restoreWallet = async (exportedWallet: ExportedByronWallet) => {
    ...
    try {
      const restoredWallet =
        await this.restoreExportedWalletRequest.execute(exportedWallet).promise;
      if (!restoredWallet)
