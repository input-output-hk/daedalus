...
import type { WalletMigrationReportData, ExportedWalletData, RestoredWalletData } from "../../../common/types/logging.types";
import type { ExportedByronWallet, WalletImportStatus, ImportFromOption } from "../types/walletExportTypes";
import { WalletImportStatuses, ImportFromOptions } from "../types/walletExportTypes";
...
export type WalletMigrationStatus = "unstarted" | "running" | "completed" | "skipped" | "errored";
...
export default class WalletMigrationStore extends Store {
  ...
  @observable
  getWalletMigrationStatusRequest: Request<WalletMigrationStatus> = new Request(this.api.localStorage.getWalletMigrationStatus);
  @observable
  setWalletMigrationStatusRequest: Request<void> = new Request(this.api.localStorage.setWalletMigrationStatus);
  @observable
  restoreExportedWalletRequest: Request<Wallet> = new Request(this.api.ada.restoreExportedByronWallet);
  ...
  setup() {
    ...
    walletMigration.toggleWalletImportSelection.listen(this._toggleWalletImportSelection);
  ...
  getExportedWalletById = (id: string): ExportedByronWallet | null | undefined => this.exportedWallets.find(w => w.id === id);
  getExportedWalletDuplicatesById = (id: string, index: number): Array<ExportedByronWallet> => this.exportedWallets.filter(w => w.id === id && w.index !== index);
  getExportedWalletByIndex = (index: number): ExportedByronWallet | null | undefined => this.exportedWallets.find(w => w.index === index);
  @action
  _initiateMigration = () => {
  ...
  _selectExportSourcePath = async ({
    ...
    const params = importFrom === ImportFromOptions.STATE_DIR ? {
      defaultPath: this.defaultExportSourcePath,
      properties: ['openDirectory']
    } : {
      defaultPath: path.join(this.stores.profile.desktopDirectoryPath, 'secret.key'),
      properties: ['openFile'],
      filters: [{
        name: 'secret',
        extensions: ['key']
    ...
    const {
      filePaths
    } = await showOpenDialogChannel.send(params);
  ...
  _toggleWalletImportSelection = ({
    ...
    if (wallet) {
      ...
      this._updateWalletImportStatus(index, isPending ? WalletImportStatuses.UNSTARTED : WalletImportStatuses.PENDING);

      const walletDuplicates = this.getExportedWalletDuplicatesById(wallet.id, index);
  ...
  @action
  _updateWalletImportStatus = (index: number, status: WalletImportStatus, error?: string) => {
    const wallet = this.getExportedWalletByIndex(index);
  ...
  _exportWallets = async () => {
    ...
    runInAction('update exportedWallets and exportErrors', () => {
      this.exportedWallets = orderBy(wallets.map(wallet => {
        const hasName = wallet.name !== null;
        const importedWallet = this.stores.wallets.getWalletById(`legacy_${wallet.id}`);
        const isImported = typeof importedWallet !== 'undefined';
        if (isImported && importedWallet) wallet.name = importedWallet.name;
        const status = isImported ? WalletImportStatuses.EXISTS : WalletImportStatuses.UNSTARTED;
        return { ...wallet,
          hasName,
          import: {
            status,
      ...
      }), ['hasName', 'id', 'name', 'isEmptyPassphrase'], ['desc', 'asc', 'asc', 'asc']);
      // Guard against duplicated wallet ids
      this.exportedWallets.forEach((wallet, index) => {
        wallet.index = index + 1;
      });
      this.exportErrors = errors || !this.exportedWalletsCount ? 'No wallets found' : '';
    });
    logger.debug(`WalletMigrationStore: Exported ${this.exportedWalletsCount} wallets`, {
  ...
  _restoreWallets = async () => {
    ...
    logger.debug(`WalletMigrationStore: Restoring ${this.pendingImportWalletsCount} selected wallets...`);
    this.isRestorationRunning = true;
    await Promise.all(this.pendingImportWallets.map((wallet, index) => {
      logger.debug(`WalletMigrationStore: Restoring ${index + 1}. wallet...`, {
        id: wallet.id,
        name: wallet.name,
    ...
    logger.debug(`WalletMigrationStore: Restored ${this.restoredWalletsCount} of ${this.pendingImportWalletsCount} selected wallets`);
  ...
  _restoreWallet = async (exportedWallet: ExportedByronWallet) => {
    ...
    try {
      const restoredWallet = await this.restoreExportedWalletRequest.execute(exportedWallet).promise;
      ...
      runInAction('update restoredWallets', () => {
        ...
        const walletDuplicates = this.getExportedWalletDuplicatesById(id, index);
      ...
      const errorStr = error.defaultMessage || error.message || error.toString();
      runInAction('update restorationErrors', () => {
        ...
        this._updateWalletImportStatus(index, WalletImportStatuses.ERRORED, errorStr);
  ...
  _generateMigrationReport = async () => {
    const finalMigrationStatus = await this.getWalletMigrationStatusRequest.execute().promise;
    ...
    logger.debug('WalletMigrationStore: Generating wallet migration report...', {
      walletMigrationReportData
    });

    try {
      await generateWalletMigrationReportChannel.send(toJS(walletMigrationReportData));
      logger.debug('WalletMigrationStore: Generated wallet migration report');
    } catch (error) {
      logger.error('WalletMigrationStore: Wallet migration report generation failed', {
        error
      });
    }
  };
  ...
  _startMigration = async () => {
    ...
    if (isMainnet || isTestnet || isTest && this.isTestMigrationEnabled) {
      ...
      const walletMigrationStatus = await this.getWalletMigrationStatusRequest.execute().promise;
      ...
      if (walletMigrationStatus === WalletMigrationStatuses.UNSTARTED) {
        ...
        await this.setWalletMigrationStatusRequest.execute(WalletMigrationStatuses.RUNNING);
      ...
      await this.setWalletMigrationStatusRequest.execute(WalletMigrationStatuses.SKIPPED);
    }
  };
  @action
  _resetExportData = () => {
  ...
  _enableTestWalletMigration = async () => {
    if (this.environment.isTest) {
      ...
      await this.setWalletMigrationStatusRequest.execute(WalletMigrationStatuses.UNSTARTED);
  ...
  _finishMigration = async () => {
    ...
    if (walletMigrationStatus === WalletMigrationStatuses.RUNNING) {
      ...
      if (this.exportErrors === '' && !this.restorationErrors.length) {
        ...
        await this.setWalletMigrationStatusRequest.execute(WalletMigrationStatuses.COMPLETED);
      } else {
        logger.debug('WalletMigrationStore: Wallet migration failed', {
          exportErrors: this.exportErrors,
          restorationErrors: this.restorationErrors
        });
        await this.setWalletMigrationStatusRequest.execute(WalletMigrationStatuses.ERRORED);
      }
  ...
  get pendingImportWallets(): Array<ExportedByronWallet> {
    ...
    }: ExportedByronWallet) => status === WalletImportStatuses.PENDING);
  }
