...
export default class UiDialogsStore extends Store {
  ...
  isOpen = (dialog: (...args: Array<any>) => any): boolean => this.activeDialog === dialog;
  countdownSinceDialogOpened = (countDownTo: number) => Math.max(countDownTo - this.secondsSinceActiveDialogIsOpen, 0);
  @action
  _onOpen = ({
    dialog
  }: {
