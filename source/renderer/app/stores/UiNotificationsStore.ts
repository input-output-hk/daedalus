...
export default class UiNotificationsStore extends Store {
  @observable
  activeNotifications: Record<NotificationId, {
    labelValues?: {};
    index: number;
  }> = {};
  ...
  setup() {
    this.actions.notifications.registerNotification.listen(this._registerNotification);
  ...
  _registerNotification = (notificationConfig: NotificationConfig) => {
    ...
    actionToListenAndOpen.listen((labelValues?: Record<string, any>) => this._openNotification(notificationConfig, labelValues));

    if (actionToListenAndClose) {
      actionToListenAndClose.listen(() => this._onClose({
        id
      }));
    }
  };
  @action
  _openNotification = (notificationConfig: NotificationConfig, labelValues?: Record<string, any>) => {
    const {
      id,
    ...
    this.activeNotificationsTimeouts[id] = setTimeout(() => this._onClose({
      id
    }), duration);
  };
  @action
