...
import { TransactionStates, WalletTransaction } from "../domains/WalletTransaction";
import { formattedArrayBufferToHexString } from "../utils/formatters";
import walletUtils from "../utils/walletUtils";
import { VOTING_REGISTRATION_TRANSACTION_POLLING_INTERVAL, VOTING_REGISTRATION_MIN_TRANSACTION_CONFIRMATIONS, VOTING_FUND_NUMBER, VOTING_REGISTRATION_END_DATE, VOTING_REGISTRATION_END_CHECK_INTERVAL } from "../config/votingConfig";
...
export default class VotingStore extends Store {
  ...
  @observable
  isRegistrationEnded: boolean = false;
  transactionPollingInterval: IntervalID | null | undefined = null;
  registrationEndCheckInterval: IntervalID | null | undefined = null;
  ...
  setup() {
    ...
    votingActions.previousRegistrationStep.listen(this._previousRegistrationStep);
  ...
  // REQUESTS
  @observable
  getWalletPublicKeyRequest: Request<string> = new Request(this.api.ada.getWalletPublicKey);
  @observable
  createVotingRegistrationTransactionRequest: Request<WalletTransaction> = new Request(this.api.ada.createVotingRegistrationTransaction);
  @observable
  signMetadataRequest: Request<Buffer> = new Request(this.api.ada.createWalletSignature);
  @observable
  getTransactionRequest: Request<GetTransactionRequest> = new Request(this.api.ada.getTransaction);
  // ACTIONS
  @action
  ...
  _resetRegistration = () => {
    ...
    if (this.transactionPollingInterval) clearInterval(this.transactionPollingInterval);
    if (this.registrationEndCheckInterval) clearInterval(this.registrationEndCheckInterval);
  };
  @action
  ...
  @action
  _initializeRegistrationEndCheckInterval = () => {
    if (this.registrationEndCheckInterval) clearInterval(this.registrationEndCheckInterval);
  ...
  prepareVotingData = async ({
    ...
    try {
      ...
      if (!this.votingRegistrationKey) throw new Error('Failed to generate voting registration key.');
      const votingKey = formattedArrayBufferToHexString(this.votingRegistrationKey.public().bytes());
  ...
  _sendTransaction = async ({
    ...
    if (!walletId) throw new Error('Selected wallet required before send voting registration.');
  ...
  _generateQrCode = async (pinCode: number) => {
    const {
      symmetric_encrypt: symmetricEncrypt
    } = await walletUtils;
    const password = new Uint8Array(4);
    pinCode.toString().split('').forEach((value: string, index: number) => {
      password[index] = parseInt(value, 10);
    });
    if (!this.votingRegistrationKey) throw new Error('Failed to generate QR code due to missing voting registration key.');
    const encrypt = symmetricEncrypt(password, this.votingRegistrationKey.bytes());
  ...
  _checkVotingRegistrationTransaction = async () => {
    ...
    if (!this.isTransactionConfirmed && confirmations >= VOTING_REGISTRATION_MIN_TRANSACTION_CONFIRMATIONS) {
