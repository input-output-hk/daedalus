...
import { requestDownloadChannel, requestResumeDownloadChannel, deleteDownloadedFile, getDownloadLocalDataChannel, clearDownloadLocalDataChannel, checkFileExistsChannel } from "../ipc/downloadManagerChannel";
import { manageAppUpdateChannel } from "../ipc/manageAppUpdateChannel";
import type { DownloadMainResponse, DownloadLocalDataMainResponse, CheckFileExistsMainResponse, ManageAppUpdateMainResponse } from "../../../common/ipc/api";
import { APP_UPDATE_DOWNLOAD_ID, UPDATE_INSTALLATION_STATUSES as statuses } from "../../../common/config/appUpdateConfig";
import { formattedDownloadData } from "../utils/formatters";
import { DOWNLOAD_EVENT_TYPES, DOWNLOAD_STATES } from "../../../common/config/downloadManagerConfig";
import type { SoftwareUpdateInfo } from "../api/news/types";
import type { DownloadInfo, DownloadData } from "../../../common/types/downloadManager.types";
...
export default class AppUpdateStore extends Store {
  ...
  @observable
  getAppAutomaticUpdateFailedRequest: Request<Promise<boolean>> = new Request(this.api.localStorage.getAppAutomaticUpdateFailed);
  @observable
  setAppAutomaticUpdateFailedRequest: Request<Promise<void>> = new Request(this.api.localStorage.setAppAutomaticUpdateFailed);
  @observable
  unsetAppAutomaticUpdateFailedRequest: Request<Promise<void>> = new Request(this.api.localStorage.unsetAppAutomaticUpdateFailed);
  @observable
  getAppUpdateCompletedRequest: Request<Promise<boolean>> = new Request(this.api.localStorage.getAppUpdateCompleted);
  @observable
  setAppUpdateCompletedRequest: Request<Promise<void>> = new Request(this.api.localStorage.setAppUpdateCompleted);
  @observable
  unsetAppUpdateCompletedRequest: Request<Promise<void>> = new Request(this.api.localStorage.unsetAppUpdateCompleted);
  ...
  _watchForNewsfeedUpdates = () => {
    const {
      update
    } = this.stores.newsFeed.newsFeedData;
  ...
  get displayAppUpdateOverlay(): boolean {
    return !!this.availableUpdate && !this.isUpdatePostponed && (this.isUpdateProgressOpen || this.isUpdateDownloaded || this.isAutomaticUpdateFailed);
  }
  ...
  get formattedDownloadData(): FormattedDownloadData {
    return formattedDownloadData(this.downloadData, this.stores.profile.currentLocale);
  }
    ...
    if (info && data) {
      ...
      if (data.state === DOWNLOAD_STATES.FINISHED && data.progress === 100) {
        ...
        if (!installerFileStillExists) {
          logger.error('AppUpdateStore:_setAppAutomaticUpdateFailed: Failed to find the installer file');
  ...
  _getUpdateDownloadLocalData = async (): Promise<DownloadLocalDataMainResponse> => getDownloadLocalDataChannel.request({
    id: APP_UPDATE_DOWNLOAD_ID
  });
  _checkFileExists = async (): Promise<CheckFileExistsMainResponse> => checkFileExistsChannel.request({
    id: APP_UPDATE_DOWNLOAD_ID
  });
  _manageUpdateResponse = ({
    ...
    runInAction('updates the download information', () => {
      ...
      if (eventType === DOWNLOAD_EVENT_TYPES.ERROR) {
        logger.error('AppUpdateStore:_setAppAutomaticUpdateFailed: Received an error event from the main process', {
          error
        });
      ...
      if (eventType === DOWNLOAD_EVENT_TYPES.END || eventType === DOWNLOAD_EVENT_TYPES.PAUSE || eventType === DOWNLOAD_EVENT_TYPES.ERROR) {
  ...
  _installUpdate = async () => {
    if (!this.availableUpdate || this.isUpdateDownloading || !this.isUpdateDownloaded || !this.downloadInfo) {
      logger.error('AppUpdateStore:_setAppAutomaticUpdateFailed: Unable to install the update', {
    ...
    const {
      hash
    } = this.getUpdateInfo(this.availableUpdate);
