...
export default class SidebarStore extends Store {
  ...
  setup() {
    ...
    sidebarActions.activateSidebarCategory.listen(this._onActivateSidebarCategory);
    sidebarActions.walletSelected.listen(this._onWalletSelected);
    this.registerReactions([this._syncSidebarRouteWithRouter, this._syncSidebarItemsWithShelleyActivation]);
  ...
  get wallets(): Array<SidebarWalletType> {
    ...
    return wallets.all.map(wallet => {
      const isHardwareWalletDisconnected = get(hardwareWalletsConnectionData, [wallet.id, 'disconnected'], true);
      const {
        hasNotification
  ...
  _configureCategories = () => {
    ...
    const categoryValidation: Record<string, boolean | ((...args: Array<any>) => any)> = {
