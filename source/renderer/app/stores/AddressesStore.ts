...
export default class AddressesStore extends Store {
  ...
  // REQUESTS
  @observable
  createByronWalletAddressRequest: Request<Address> = new Request(this.api.ada.createAddress);
  @observable
  inspectAddressRequest: Request<InspectAddressResponse> = new Request(this.api.ada.inspectAddress);
  ...
  get active(): WalletAddress | null | undefined {
    ...
    if (this.lastGeneratedAddress && !this.lastGeneratedAddress.used) return this.lastGeneratedAddress;
  ...
  _getStakeAddress = async (walletId: string, isLegacy: boolean) => {
    ...
    if (!hasStakeAddress) {
      if (isLegacy) {
        ...
        const getWalletStakeKeyRequest = new Request(this.api.ada.getWalletPublicKey);
  ...
  isInternalAddress = (address: string): boolean => {
    return findIndex(this.all, {
      id: address
    }) > -1;
  };
  getAddressIndex = (address: string): number => {
    return this.all.length - findIndex(this.all, {
      id: address
    }) - 1;
  };
  getAccountIndexByWalletId = async (walletId: string): Promise<number | null | undefined> => {
    // $FlowFixMe
  ...
  getAddressesByWalletId = async (walletId: string): Promise<Array<WalletAddress>> => {
    const addresses = await this._getAddressesAllRequest(walletId);
    return addresses || [];
  };
  _getAddressesAllRequest = (walletId: string): CachedRequest<Array<WalletAddress>> => {
