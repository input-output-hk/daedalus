...
import { DEFAULT_NUMBER_FORMAT, NUMBER_FORMATS } from "../../../common/types/number.types";
import { hasLoadedRequest, isRequestSet, requestGetter, requestGetterLocale, getRequestKeys } from "../utils/storesUtils";
import { NUMBER_OPTIONS, DATE_ENGLISH_OPTIONS, DATE_JAPANESE_OPTIONS, TIME_OPTIONS, PROFILE_SETTINGS } from "../config/profileConfig";
export default class ProfileStore extends Store {
  ...
  @observable
  getProfileLocaleRequest: Request<string> = new Request(this.api.localStorage.getUserLocale);
  @observable
  setProfileLocaleRequest: Request<string> = new Request(this.api.localStorage.setUserLocale);
  @observable
  getProfileNumberFormatRequest: Request<string> = new Request(this.api.localStorage.getUserNumberFormat);
  @observable
  setProfileNumberFormatRequest: Request<string> = new Request(this.api.localStorage.setUserNumberFormat);
  @observable
  getProfileDateFormatEnglishRequest: Request<string> = new Request(this.api.localStorage.getUserDateFormatEnglish);
  @observable
  setProfileDateFormatEnglishRequest: Request<string> = new Request(this.api.localStorage.setUserDateFormatEnglish);
  @observable
  getProfileDateFormatJapaneseRequest: Request<string> = new Request(this.api.localStorage.getUserDateFormatJapanese);
  @observable
  setProfileDateFormatJapaneseRequest: Request<string> = new Request(this.api.localStorage.setUserDateFormatJapanese);
  @observable
  getProfileTimeFormatRequest: Request<string> = new Request(this.api.localStorage.getUserTimeFormat);
  @observable
  setProfileTimeFormatRequest: Request<string> = new Request(this.api.localStorage.setUserTimeFormat);
  @observable
  getTermsOfUseAcceptanceRequest: Request<string> = new Request(this.api.localStorage.getTermsOfUseAcceptance);
  @observable
  setTermsOfUseAcceptanceRequest: Request<string> = new Request(this.api.localStorage.setTermsOfUseAcceptance);
  @observable
  getDataLayerMigrationAcceptanceRequest: Request<string> = new Request(this.api.localStorage.getDataLayerMigrationAcceptance);
  @observable
  setDataLayerMigrationAcceptanceRequest: Request<string> = new Request(this.api.localStorage.setDataLayerMigrationAcceptance);
  @observable
  getThemeRequest: Request<string> = new Request(this.api.localStorage.getUserTheme);
  @observable
  setThemeRequest: Request<string> = new Request(this.api.localStorage.setUserTheme);
  @observable
  ...
  setup() {
    ...
    profileActions.finishInitialScreenSettings.listen(this._finishInitialScreenSettings);
    ...
    profileActions.acceptDataLayerMigration.listen(this._acceptDataLayerMigration);
    ...
    this.registerReactions([this._updateBigNumberFormat, this._redirectToInitialSettingsIfNoLocaleSet, this._redirectToTermsOfUseScreenIfTermsNotAccepted, // this._redirectToDataLayerMigrationScreenIfMigrationHasNotAccepted,
    this._redirectToMainUiAfterTermsAreAccepted, this._redirectToMainUiAfterDataLayerMigrationIsAccepted]);
  ...
  get currentTheme(): string {
    ...
    if (global.isFlight) {
      ...
      systemValue = this.environment.isMainnet ? THEMES.DARK_CARDANO : THEMES.LIGHT_BLUE;
    }
  ...
  get currentNumberFormat(): string {
    return requestGetter(this.getProfileNumberFormatRequest, this.systemNumberFormat);
  }

  @computed
  get currentDateFormat(): string {
    return this.currentLocale === 'en-US' ? this.currentDateEnglishFormat : this.currentDateJapaneseFormat;
  }

  @computed
  get currentDateEnglishFormat(): string {
    return requestGetter(this.getProfileDateFormatEnglishRequest, this.systemDateFormatEnglish);
  }

  @computed
  get currentDateJapaneseFormat(): string {
    return requestGetter(this.getProfileDateFormatJapaneseRequest, this.systemDateFormatJapanese);
  }

  @computed
  get currentTimeFormat(): string {
    return requestGetter(this.getProfileTimeFormatRequest, this.systemTimeFormat);
  }
  ...
  get hasLoadedTermsOfUseAcceptance(): boolean {
    return this.getTermsOfUseAcceptanceRequest.wasExecuted && this.getTermsOfUseAcceptanceRequest.result !== null;
  }
  ...
  get hasLoadedDataLayerMigrationAcceptance(): boolean {
    return this.getDataLayerMigrationAcceptanceRequest.wasExecuted && this.getDataLayerMigrationAcceptanceRequest.result !== null;
  }
  ...
  _updateUserLocalSetting = async ({
    ...
    const consolidatedValue = value || (this as any)[camelCase(['current', param])];
    const {
      set,
      get
    } = getRequestKeys(param, this.currentLocale);
  ...
  _getDesktopDirectoryPath = async () => {
    this._onReceiveDesktopDirectoryPath(await getDesktopDirectoryPathChannel.request());
  };
  ...
  _redirectToInitialSettingsIfNoLocaleSet = () => {
    if (this.hasLoadedCurrentLocale && !this.isCurrentLocaleSet || this.isInitialScreen) {
  ...
  _redirectToTermsOfUseScreenIfTermsNotAccepted = () => {
    ...
    if (!this.isInitialScreen && this.isCurrentLocaleSet && termsOfUseNotAccepted) {
      this.actions.router.goToRoute.trigger({
  ...
  _redirectToDataLayerMigrationScreenIfMigrationHasNotAccepted = () => {
    ...
    const dataLayerMigrationNotAccepted = this.hasLoadedDataLayerMigrationAcceptance && !this.isDataLayerMigrationAccepted;

    if (isConnected && this.isCurrentLocaleSet && this.areTermsOfUseAccepted && this.stores.wallets.hasLoadedWallets && dataLayerMigrationNotAccepted) {
  ...
  _isOnDataLayerMigrationPage = () => this.stores.app.currentRoute === ROUTES.PROFILE.DATA_LAYER_MIGRATION;
  _redirectToRoot = () => {
    this.actions.router.goToRoute.trigger({
      route: ROUTES.ROOT
    });
  ...
  _toggleDisableDownloadLogs = action(async ({
    isDownloadNotificationVisible
  }) => {
    this.actions.app.setIsDownloadingLogs.trigger(isDownloadNotificationVisible);
  });
  @action
