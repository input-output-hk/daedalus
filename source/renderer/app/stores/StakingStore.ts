...
import { STAKE_POOL_TRANSACTION_CHECK_INTERVAL, STAKE_POOL_TRANSACTION_CHECKER_TIMEOUT, STAKE_POOLS_INTERVAL, STAKE_POOLS_FAST_INTERVAL, STAKE_POOLS_FETCH_TRACKER_INTERVAL, STAKE_POOLS_FETCH_TRACKER_CYCLES, REDEEM_ITN_REWARDS_STEPS as steps, INITIAL_DELEGATION_FUNDS, SMASH_SERVERS_LIST, SMASH_SERVER_TYPES, SMASH_SERVER_INVALID_TYPES, CIRCULATING_SUPPLY } from "../config/stakingConfig";
import type { Reward, JoinStakePoolRequest, GetDelegationFeeRequest, DelegationCalculateFeeResponse, QuitStakePoolRequest, PoolMetadataSource } from "../api/staking/types";
...
export default class StakingStore extends Store {
  ...
  setup() {
    ...
    this.refreshPolling = setInterval(this.getStakePoolsData, STAKE_POOLS_FAST_INTERVAL);
    // Redeem ITN Rewards actions
    stakingActions.onRedeemStart.listen(this._onRedeemStart);
    stakingActions.onConfigurationContinue.listen(this._onConfigurationContinue);
    ...
    stakingActions.goToStakingDelegationCenterPage.listen(this._goToStakingDelegationCenterPage);
  ...
  // REQUESTS
  @observable
  joinStakePoolRequest: Request<JoinStakePoolRequest> = new Request(this.api.ada.joinStakePool);
  @observable
  quitStakePoolRequest: Request<QuitStakePoolRequest> = new Request(this.api.ada.quitStakePool);
  @observable
  stakePoolsRequest: Request<Array<StakePool>> = new Request(this.api.ada.getStakePools);
  @observable
  calculateDelegationFeeRequest: Request<DelegationCalculateFeeResponse> = new Request(this.api.ada.calculateDelegationFee);
  // @REDEEM TODO: Proper type it when the API endpoint is implemented.
  @observable
  getRedeemItnRewardsFeeRequest: Request<any> = new Request(this.api.ada.getRedeemItnRewardsFee);
  @observable
  requestRedeemItnRewardsRequest: Request<any> = new Request(this.api.ada.requestRedeemItnRewards);
  @observable
  getSmashSettingsRequest: Request<any> = new Request(this.api.ada.getSmashSettings);
  @observable
  updateSmashSettingsRequest: Request<PoolMetadataSource> = new Request(this.api.ada.updateSmashSettings);
  // =================== PUBLIC API ==================== //
  @action
  _getSmashSettingsRequest = async () => {
    ...
    if (!smashServerUrl || smashServerUrl === SMASH_SERVER_INVALID_TYPES.NONE || smashServerUrl === SMASH_SERVER_TYPES.DIRECT && localSmashServer !== SMASH_SERVER_TYPES.DIRECT) {
      smashServerUrl = this.environment.isSelfnode ? SMASH_SERVERS_LIST.direct.url : SMASH_SERVERS_LIST.iohk.url;
  ...
  _startStakePoolsFetchTracker = () => {
    ...
    this.stakePoolsFetchTrackerInterval = setInterval(this._stakePoolsFetchTracker, STAKE_POOLS_FETCH_TRACKER_INTERVAL);
  ...
  _stakePoolsFetchTracker = () => {
    ...
    if (lastNumberOfStakePoolsFetched === this.numberOfStakePoolsFetched && this.numberOfStakePoolsFetched > 0) {
      this.cyclesWithoutIncreasingStakePools++;
    } else {
      this.cyclesWithoutIncreasingStakePools = 0;
    }

    if (this.cyclesWithoutIncreasingStakePools >= STAKE_POOLS_FETCH_TRACKER_CYCLES) {
      this._stopStakePoolsFetchTracker();
    }
  };
  @action
  ...
  _joinStakePool = async (request: JoinStakePoolRequest) => {
    ...
    try {
      ...
      this.delegationCheckTimeInterval = setInterval(this.checkDelegationTransaction, STAKE_POOL_TRANSACTION_CHECK_INTERVAL, {
  ...
  checkDelegationTransaction = (request: {
    ...
    const stakePoolTransaction = find(recentTransactions, transaction => transaction.id === transactionId && transaction.state === TransactionStates.OK);
  ...
  _requestCSVFile = async ({
    ...
    const {
      filePath
    } = await showSaveDialogChannel.send(params);
  ...
  calculateDelegationFee = async (delegationFeeRequest: GetDelegationFeeRequest): Promise<DelegationCalculateFeeResponse | null | undefined> => {
    const {
      walletId
    ...
    if (!wallet) {
      throw new Error('Active wallet required before calculating transaction fees.');
    }
    ...
    try {
      const delegationFee: DelegationCalculateFeeResponse = await this.calculateDelegationFeeRequest.execute({ ...delegationFeeRequest
      }).promise;
  ...
  get recentStakePools(): Array<StakePool> {
    ...
    map(this.stores.wallets.all, wallet => {
      const hasPendingDelegations = wallet.pendingDelegations && wallet.pendingDelegations.length > 0;
      ...
      if (lastDelegatedStakePoolId) {
        const delegatingStakePoolExistInList = find(delegatedStakePools, delegatedStakePool => delegatedStakePool.id === lastDelegatedStakePoolId);

        if (!delegatingStakePoolExistInList) {
          const delegatingStakePool = find(this.stakePools, stakePool => stakePool.id === lastDelegatedStakePoolId);
          if (delegatingStakePool) delegatedStakePools.push(delegatingStakePool);
        }
      }
    });
  ...
  getStakePoolsData = async (isSmash?: boolean) => {
    ...
    try {
      ...
      const stakeInLovelace = parseInt(stakeInBigNumber.times(LOVELACES_PER_ADA), 10);
  ...
  _resetPolling = (type?: "regular" | "failed" | "kill" | "smash") => {
    if (type === 'kill') {
      ...
      const interval = isSmash ? STAKE_POOLS_FETCH_TRACKER_INTERVAL : STAKE_POOLS_INTERVAL;
      this.pollingStakePoolsInterval = setInterval(() => this.getStakePoolsData(isSmash), interval);
    }
  };
  @action
  _resetIsRanking = () => {
  ...
  _setFakedStakePools = () => {
    if (this.environment.isDev) {
      ...
      const newStakePools = [this.stakePoolsRequest.result[1], this.stakePoolsRequest.result[2]];
      this.stakePoolsRequest.reset();
  ...
  _onCalculateRedeemWalletFees = async ({
    ...
    try {
      const [address] = await this.stores.addresses.getAddressesByWalletId(walletId);
  ...
  _onConfirmationContinue = async ({
    ...
    const {
      redeemRecoveryPhrase: recoveryPhrase,
      redeemWallet
    } = this;
  ...
  getStakePoolById = (stakePoolId: string) => this.stakePools.find(({
    id
  }: StakePool) => id === stakePoolId);
}