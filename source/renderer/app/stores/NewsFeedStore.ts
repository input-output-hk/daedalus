...
import { NEWS_POLL_INTERVAL, NEWS_POLL_INTERVAL_ON_ERROR, NEWS_POLL_INTERVAL_ON_INCIDENT } from "../config/timingConfig";
import News, { NewsTypes, IncidentColors } from "../domains/News";
import type { GetNewsResponse, GetReadNewsResponse, NewsItem, MarkNewsAsReadResponse } from "../api/news/types";
const {
  isTest,
  isDev
} = global.environment;
const AVAILABLE_NEWSFEED_EVENT_ACTIONS = ['DOWNLOAD_LOGS', 'OPEN_DIAGNOSTIC_DIALOG'];
export default class NewsFeedStore extends Store {
  ...
  @observable
  getReadNewsRequest: Request<GetReadNewsResponse> = new Request(this.api.localStorage.getReadNews);
  @observable
  markNewsAsReadRequest: Request<MarkNewsAsReadResponse> = new Request(this.api.localStorage.markNewsAsRead);
  @observable
  markNewsAsUnreadRequest: Request<MarkNewsAsReadResponse> = new Request(this.api.localStorage.markNewsAsUnread);
  @observable
  ...
  setup() {
    ...
    if (!isTest) {
      ...
      this.pollingNewsIntervalId = setInterval(this.getNews, NEWS_POLL_INTERVAL);
    }
  }
  ...
  getNews = async (params?: {
    ...
    try {
      ...
      const hasIncident = find(rawNews.items, news => news.type === NewsTypes.INCIDENT);

      // Check for "Alerts" with repeatable state and set as unread
      if (params && params.isInit && rawNews) {
        const repeatableNews = find(rawNews.items, news => news.type === NewsTypes.ALERT && news.repeatOnStartup);
      ...
      if (hasIncident && !this.pollingNewsOnIncidentIntervalId) {
        ...
        this.pollingNewsOnIncidentIntervalId = setInterval(this.getNews, NEWS_POLL_INTERVAL_ON_INCIDENT);
      }
      ...
      if (!isTest && !this.rawNews) {
        ...
        if (!this.pollingNewsOnErrorIntervalId) {
          this.pollingNewsOnErrorIntervalId = setInterval(this.getNews, NEWS_POLL_INTERVAL_ON_ERROR);
        }
      }
  ...
  openAlert = (newsId: number) => {
    if (this.getNewsRequest.wasExecuted) {
      const alertToOpen = this.newsFeedData.alerts.all.find(newsItem => newsItem.id === newsId);
  ...
  proceedNewsAction = (newsItem: News.News, e: MouseEvent) => {
    ...
    } else if (route && newsItem.type !== NewsTypes.INCIDENT && newsItem.type !== NewsTypes.ALERT) {
