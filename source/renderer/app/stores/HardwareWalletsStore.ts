...
import { TransactionSigningMode, AddressType } from "@cardano-foundation/ledgerjs-hw-app-cardano";
import Store from "./lib/Store";
import Request from "./lib/LocalizedRequest";
import { HwDeviceStatuses } from "../domains/Wallet";
import WalletAddress from "../domains/WalletAddress";
import { toJS } from "../../../common/utils/helper";
import { HW_SHELLEY_CONFIG, SHELLEY_PURPOSE_INDEX, ADA_COIN_TYPE, MINIMAL_TREZOR_FIRMWARE_VERSION, MINIMAL_LEDGER_FIRMWARE_VERSION, MINIMAL_CARDANO_APP_VERSION, isHardwareWalletSupportEnabled, isTrezorEnabled, isLedgerEnabled } from "../config/hardwareWalletsConfig";
import { TIME_TO_LIVE } from "../config/txnsConfig";
import { getHardwareWalletTransportChannel, getExtendedPublicKeyChannel, getCardanoAdaAppChannel, getHardwareWalletConnectionChannel, signTransactionLedgerChannel, signTransactionTrezorChannel, handleInitTrezorConnectChannel, handleInitLedgerConnectChannel, resetTrezorActionChannel, deriveAddressChannel, showAddressChannel } from "../ipc/getHardwareWalletChannel";
import { prepareLedgerInput, prepareLedgerOutput, prepareTxAux, prepareBody, prepareLedgerCertificate, prepareLedgerWithdrawal, CachedDeriveXpubFactory, ShelleyTxWitnessShelley, ShelleyTxInputFromUtxo, ShelleyTxOutput, ShelleyTxCert, ShelleyTxWithdrawal, cborizeTxAuxiliaryVotingData, prepareLedgerAuxiliaryData, CATALYST_VOTING_REGISTRATION_TYPE } from "../utils/shelleyLedger";
import { prepareTrezorInput, prepareTrezorOutput, prepareTrezorCertificate, prepareTrezorWithdrawal, prepareTrezorAuxiliaryData, TrezorTransactionSigningMode } from "../utils/shelleyTrezor";
import { DeviceModels, DeviceTypes, DeviceEvents } from "../../../common/types/hardware-wallets.types";
import { formattedAmountToLovelace } from "../utils/formatters";
import { TransactionStates } from "../domains/WalletTransaction";
import { CERTIFICATE_TYPE, getParamsFromPath } from "../utils/hardwareWalletUtils";
import type { HwDeviceStatus } from "../domains/Wallet";
import type { CoinSelectionsPaymentRequestType, CoinSelectionsDelegationRequestType, CreateExternalTransactionResponse, CoinSelectionsResponse, VotingDataType } from "../api/transactions/types";
import type { HardwareWalletLocalData, HardwareWalletsLocalData, HardwareWalletDevicesType, SetHardwareWalletLocalDataRequestType, SetHardwareWalletDeviceRequestType } from "../api/utils/localStorage";
import type { TransportDevice, LedgerModel, TrezorModel, HardwareWalletExtendedPublicKeyResponse, HardwareWalletConnectionRequest, Witness, TrezorWitness } from "../../../common/types/hardware-wallets.types";
...
const useCardanoAppInterval = (getCardanoAdaApp: any, interval: number, path: string | null | undefined, address: string | null | undefined, addressVerification: WalletAddress | null | undefined) => setInterval((devicePath, txWalletId, verificationAddress): any => {
  try {
...
}, interval, path, address, addressVerification);

export default class HardwareWalletsStore extends Store {
  @observable
  selectCoinsRequest: Request<CoinSelectionsResponse> = new Request(this.api.ada.selectCoins);
  @observable
  sendMoneyRequest: Request<CreateExternalTransactionResponse> = new Request(this.api.ada.createExternalTransaction);
  // @TODO - improve types
  getPublicKeyRequest: Request<any> = new Request(this.api.ada.getPublicKey);
  // @TODO - improve types
  constructAddressRequest: Request<any> = new Request(this.api.ada.constructAddress);
  @observable
  ...
  @observable
  setHardwareWalletLocalDataRequest: Request<HardwareWalletLocalData> = new Request(this.api.localStorage.setHardwareWalletLocalData);
  @observable
  unsetHardwareWalletLocalDataRequest: Request<void> = new Request(this.api.localStorage.unsetHardwareWalletLocalData);
  @observable
  hardwareWalletDevicesRequest: Request<HardwareWalletsLocalData> = new Request(this.api.localStorage.getHardwareWalletDevices);
  @observable
  setHardwareWalletDeviceRequest: Request<HardwareWalletLocalData> = new Request(this.api.localStorage.setHardwareWalletDevice);
  @observable
  overrideHardwareWalletDevicesRequest: Request<HardwareWalletDevicesType> = new Request(this.api.localStorage.overrideHardwareWalletDevices);
  @observable
  unsetHardwareWalletDeviceRequest: Request<HardwareWalletLocalData> = new Request(this.api.localStorage.unsetHardwareWalletDevice);
  @observable
  unsetHardwareWalletDevicesAllRequest: Request<void> = new Request(this.api.localStorage.unsetHardwareWalletDevicesAll);
  @observable
  unsetHardwareWalletLocalDataAllRequest: Request<HardwareWalletLocalData> = new Request(this.api.localStorage.unsetHardwareWalletLocalDataAll);
  @observable
  hwDeviceStatus: HwDeviceStatus = HwDeviceStatuses.CONNECTING;
  @observable
  extendedPublicKey: HardwareWalletExtendedPublicKeyResponse | null | undefined = null;
  @observable
  ...
  setup() {
    ...
    hardwareWalletsActions.refreshHardwareWalletsLocalData.listen(this._refreshHardwareWalletsLocalData);
    getHardwareWalletConnectionChannel.onReceive(this._changeHardwareWalletConnectionStatus);
    this.initTrezor();
    this.initLedger();
  ...
  initLedger = async () => {
    logger.debug(`[HW-DEBUG] HWStore - initLedger() | isHardwareWalletSupportEnabled=${isHardwareWalletSupportEnabled} isLedgerEnabled=${isLedgerEnabled}`);
  ...
  getAvailableDevices = async (params: {
    ...
    map(this.hardwareWalletDevices, async device => {
      ...
      if (!isTrezor && device.deviceType !== DeviceTypes.LEDGER || isTrezor && (device.deviceType !== DeviceTypes.TREZOR || device.deviceType === DeviceTypes.TREZOR && !this.isTrezorBridgeInstalled)) {
        return;
      }
  ...
  _sendMoney = async (params?: {
    ...
    try {
      ...
      if (!isDelegationTransaction) {
        ...
        this.checkTransactionTimeInterval = setInterval(this.checkTransaction, 1000, {
  ...
  checkTransaction = (request: {
    ...
    const recentTransactionsResponse = this.stores.transactions._getTransactionsRecentRequest(walletId).result;

    const recentTransactions = recentTransactionsResponse ? recentTransactionsResponse.transactions : [];
    let targetTransaction;

    if (isVotingRegistrationTransaction) {
      // Return transaction when state is not "PENDING"
      targetTransaction = find(recentTransactions, transaction => transaction.id === transactionId);
      ...
      // Return transaction when state is not "PENDING"
      targetTransaction = find(recentTransactions, transaction => transaction.id === transactionId && transaction.state === TransactionStates.OK);
  ...
  selectCoins = async (params: CoinSelectionsPaymentRequestType) => {
    ...
    if (!wallet) throw new Error('Active wallet required before coins selections.');
    const {
      amount: totalAmount,
      availableAmount,
      reward
  ...
  establishHardwareWalletConnection = async () => {
    ...
    try {
      ...
      if ((this.activeDelegationWalletId || this.activeVotingWalletId) && this.isTransactionInitiated) {
      ...
      if (activeWalletId) {
        ...
        recognizedPairedHardwareWallet = find(hardwareWalletDevices, recognizedDevice => recognizedDevice.paired === activeWalletId);
        relatedConnectionData = find(hardwareWalletsConnectionData, connection => connection.id === activeWalletId);
      }

      const lastUnpairedDevice = findLast(this.hardwareWalletDevices, hardwareWalletDevice => !hardwareWalletDevice.paired && !hardwareWalletDevice.disconnected);
      logger.debug('[HW-DEBUG] HWStore - establishHardwareWalletConnection:: START');
      // Tx Special cases!
      ...
      if (this.isTransactionInitiated || this.isAddressVerificationInitiated) {
        logger.debug('[HW-DEBUG] HWStore - Establish connection:: New Transaction / Address verification initiated - check device');
        ...
        if (recognizedPairedHardwareWallet && !recognizedPairedHardwareWallet.disconnected) {
          logger.debug('[HW-DEBUG] HWStore - Establish connection:: New Transaction / Address verification initiated - Recognized device found');
          logger.debug('[HW-DEBUG] HWStore - Set transport device 1', {
            recognizedPairedHardwareWallet: toJS(recognizedPairedHardwareWallet)
          });
          ...
          if (this.isExportKeyAborted) {
            if (isTrezor) {
              await this._getExtendedPublicKey(recognizedPairedHardwareWallet.path, activeWalletId, this.unfinishedWalletAddressVerification);
            } else {
              this.cardanoAdaAppPollingInterval = useCardanoAppInterval(this.getCardanoAdaApp, CARDANO_ADA_APP_POLLING_INTERVAL, recognizedPairedHardwareWallet.path, activeWalletId, this.unfinishedWalletAddressVerification);
            }
          }
        ...
        if (relatedConnectionDataDeviceType) {
          logger.debug('[HW-DEBUG] HWStore - Connect - New Transaction / Address verification initiated - return last device');
          // $FlowFixMe
          ...
          if (this.isExportKeyAborted) {
            if (isTrezor) {
              await this._getExtendedPublicKey(lastDeviceTransport.path, activeWalletId, this.unfinishedWalletAddressVerification);
            } else {
              this.cardanoAdaAppPollingInterval = useCardanoAppInterval(this.getCardanoAdaApp, CARDANO_ADA_APP_POLLING_INTERVAL, lastDeviceTransport.path, activeWalletId, this.unfinishedWalletAddressVerification);
            }
          } // End of special case
      ...
      if (lastUnpairedDevice) {
        logger.debug('[HW-DEBUG] HWStore - establishHardwareWalletConnection:: Start process with last UNPAIRED device');
        ...
        logger.debug('[HW-DEBUG] HWStore - establishHardwareWalletConnection:: Listening for device');
        ...
        logger.debug('[HW-DEBUG] HWStore - establishHardwareWalletConnection:: Set device listener');
      ...
      logger.debug('[HW-DEBUG] HWStore - establishHardwareWalletConnection:: start process with known transport');
      ...
      if (transportDevice) {
        ...
        if (deviceType === DeviceTypes.TREZOR) {
          ...
          logger.debug('[HW-DEBUG] HWStore - getCardanoAdaApp - from  establishHardwareWalletConnection');
          this.stopCardanoAdaAppFetchPoller();
          this.cardanoAdaAppPollingInterval = useCardanoAppInterval(this.getCardanoAdaApp, CARDANO_ADA_APP_POLLING_INTERVAL, devicePath);
        }
      } else {
  ...
  getCardanoAdaApp = async (params: {
    ...
    logger.debug('[HW-DEBUG] HWStore - START FUNCTION getCardanoAdaApp PARAMS: ', {
      walletId,
      path,
      address
    });
    ...
    try {
      ...
      logger.debug('[HW-DEBUG] HWStore - cardanoAdaApp RESPONSE: ', toJS(cardanoAdaApp));
      ...
      if (cardanoAdaApp) {
        ...
        const isValidAppVersion = semver.gte(cardanoAppVersion, MINIMAL_CARDANO_APP_VERSION);
      ...
      if (isDeviceBusy) {
        ...
        runInAction('HardwareWalletsStore:: set HW device CONNECTING FAILED', () => {
          this.hwDeviceStatus = HwDeviceStatuses.CONNECTING_FAILED;
          this.activeDevicePath = null;
          this.unfinishedWalletTxSigning = this.isTransactionInitiated ? walletId : null;
          this.unfinishedWalletAddressVerification = this.isAddressVerificationInitiated ? address : null;
        });
      }
      ...
      if (error.code === 'DEVICE_NOT_CONNECTED') {
        ...
        // Stop poller and re-initiate connecting state / don't kill devices listener
        this.stopCardanoAdaAppFetchPoller();
        const pairedDevice = find(this.hardwareWalletDevices, recognizedDevice => recognizedDevice.path === path);
        ...
        if (walletId) {
          ...
          if (hardwareWalletConnectionData) {
            ...
            await this._setHardwareWalletLocalData({
              walletId,
              data: { ...hardwareWalletConnectionData,
                path: error.path,
                device: { ...hardwareWalletConnectionData.device,
                  path: error.path
                }
              }
            });
        ...
        if (this.isTransactionInitiated || this.isAddressVerificationInitiated) {
          logger.debug('[HW-DEBUG] Update connected wallet data with new path - Set to LC');
          runInAction('HardwareWalletsStore:: Change active device path for Transaction send', () => {
            this.activeDevicePath = error.path;
          });
        }

        this.cardanoAdaAppPollingInterval = setInterval((devicePath, txWalletId, verificationAddress) => this.getCardanoAdaApp({
          path: devicePath,
          walletId: txWalletId,
          address: verificationAddress
        }), CARDANO_ADA_APP_POLLING_INTERVAL, error.path, walletId, address);
      }
  ...
  isAddressVerificationEnabled = (walletId: string) => {
    const hardwareWalletConnectionData = get(this.hardwareWalletsConnectionData, walletId, {});
    const deviceType = get(hardwareWalletConnectionData, ['device', 'deviceType']);
    return deviceType === DeviceTypes.LEDGER;
  };
  initiateAddressVerification = async (address: WalletAddress, path: string | null | undefined) => {
    ...
    if (!hardwareWalletConnectionData) throw new Error('Wallet not paired or Device not connected');
    const {
      disconnected,
      device
    ...
    let devicePath = path || hardwareWalletConnectionData.path || hardwareWalletConnectionData.device.path;
    logger.debug('[HW-DEBUG] HWStore - Verify address - check is device connected: ', {
      disconnected,
      deviceType,
      devicePath
    });
    ...
    if (disconnected) {
      ...
      try {
        ...
        if (transportDevice) {
          ...
          runInAction('HardwareWalletsStore:: Set transport device fomr tx init', () => {
            this.transportDevice = transportDevice;
          });
        }
      } catch (e) {
    ...
    if (deviceType === DeviceTypes.TREZOR) {
      ...
      runInAction('HardwareWalletsStore:: Set transport device from tx init', () => {
        this.transportDevice = transportDevice;
      });
      const newConnectionData = get(this.hardwareWalletsConnectionData, walletId);
      const activeDevice = find(this.hardwareWalletDevices, hardwareWalletDevice => hardwareWalletDevice.paired === walletId) || {};
      ...
      this.stopCardanoAdaAppFetchPoller();
      this.cardanoAdaAppPollingInterval = setInterval((verificationDevicePath, addressToVerify) => this.getCardanoAdaApp({
        path: verificationDevicePath,
        walletId,
        address: addressToVerify
      }), CARDANO_ADA_APP_POLLING_INTERVAL, devicePath, address);
    }
  };
  @action
  verifyAddress = async (params: {
    ...
    try {
      const derivedAddress = await deriveAddressChannel.request({
        ...
        networkId: isMainnet ? HW_SHELLEY_CONFIG.NETWORK.MAINNET.networkId : HW_SHELLEY_CONFIG.NETWORK.TESTNET.networkId,
        protocolMagic: isMainnet ? HW_SHELLEY_CONFIG.NETWORK.MAINNET.protocolMagic : HW_SHELLEY_CONFIG.NETWORK.TESTNET.protocolMagic
      });
      ...
      if (derivedAddress === address.id) {
        ...
        if (isTrezor) {
          runInAction('HardwareWalletsStore:: Address Verified and is correct - Trezor', () => {
          ...
          runInAction('HardwareWalletsStore:: Address Verified and is correct - Ledger', () => {
            this.isAddressDerived = true;
          });
          this.showAddress(params);
        }
      } else {
        runInAction('HardwareWalletsStore:: Address Verified but not correct', () => {
      ...
      if (isCancelled || isAborted) {
        logger.debug('[HW-DEBUG] HWStore - verifyAddress:: WAIT FOR ANOTHER DEVICE');
        // Special case. E.g. device unplugged before cardano app is opened
        // Stop poller and re-initiate connecting state / don't kill devices listener
        this.stopCardanoAdaAppFetchPoller();
        runInAction('HardwareWalletsStore:: Re-run initiated connection', () => {
  ...
  @action
  setAddressVerificationCheckStatus = (checkStatus: AddressVerificationCheckStatus) => {
    // Yes / No - Reverify / No - Invalid
    if (checkStatus === AddressVerificationCheckStatuses.VALID) {
      runInAction('HardwareWalletsStore:: Set address verification status CORRECT', () => {
  ...
  @action
  _getExtendedPublicKey = async (forcedPath: string | null | undefined, walletId?: string, address?: WalletAddress | null | undefined) => {
    ...
    try {
      ...
      const recognizedStoredWallet = find(this.hardwareWalletsConnectionData, hardwareWalletData => extendedPublicKey.chainCodeHex === hardwareWalletData.extendedPublicKey.chainCodeHex && extendedPublicKey.publicKeyHex === hardwareWalletData.extendedPublicKey.publicKeyHex);
      const recognizedWallet = recognizedStoredWallet ? this.stores.wallets.getWalletById(recognizedStoredWallet.id) : null;
      ...
      if (recognizedWallet) {
        ...
        const recognizedDevice = find(this.hardwareWalletDevices, device => device.path === forcedPath);

        if (recognizedDevice) {
          logger.debug('[HW-DEBUG] HWStore - _getExtendedPublicKey - UNSET Device with path: ', {
        ...
        if (this.isTransactionInitiated) {
          logger.debug('[HW-DEBUG] HWStore - Re-initiate tx from _getExtendedPublicKey: ', {
            walletId,
          ...
          if (!walletId || recognizedWallet.id !== walletId) {
            ...
            logger.debug('[HW-DEBUG] HWStore - Transaction Initiated - Close: ', walletId);
        ...
        if (this.isAddressVerificationInitiated && address) {
          logger.debug('[HW-DEBUG] HWStore - Re-initiate Address verification from _getExtendedPublicKey: ', {
            address: toJS(address),
            devicePath,
            walletId,
            recognizedWalletId: recognizedWallet.id,
          ...
          if (!walletId || recognizedWallet.id !== walletId) {
            logger.debug('[HW-DEBUG] HWStore - Device not belongs to this wallet');
            // Show message to reconnect proper software wallet device pair
            logger.debug('[HW-DEBUG] unfinishedWalletAddressVerification SET: ', walletId);
            runInAction('HardwareWalletsStore:: set HW device CONNECTING FAILED', () => {
            ...
            logger.debug('[HW-DEBUG] HWStore - Address Verification - Close: ', walletId);
            logger.debug('[HW-DEBUG] unfinishedWalletTxSigning UNSET');
            runInAction('HardwareWalletsStore:: Initiate transaction', () => {
              this.isAddressVerificationInitiated = false;
      ...
      logger.debug('[HW-DEBUG] HWStore - I don not have recognized wallet - create new one or reject TX: ', {
        deviceId
      });
      ...
      if (recognizedPendingDevice && recognizedPendingDevice.isPending) {
        logger.debug('[HW-DEBUG] HWStore - Export key - UNSET Device with path: ', {
          path,
  ...
  // Trezor - Shelley only
  @action
  _signTransactionTrezor = async (walletId: string, deviceId?: string | null | undefined) => {
    const {
      coinSelection
    } = this.txSignRequest;
    ...
    // Guard against potential null value
    if (!hardwareWalletConnectionData) throw new Error('Wallet not paired or Device not connected');
    const publicKeyHex = get(hardwareWalletConnectionData, ['extendedPublicKey', 'publicKeyHex']);
    const chainCodeHex = get(hardwareWalletConnectionData, ['extendedPublicKey', 'chainCodeHex']);
    ...
    const _withdrawalsData = map(withdrawals, async (withdrawal) => prepareTrezorWithdrawal(withdrawal));
    ...
    const recognizedDevice = find(this.hardwareWalletDevices, hardwareWalletDevice => hardwareWalletDevice.paired === walletId);
    ...
    if (this.isTransactionInitiated) {
      // Check if sender wallet match transaction initialization
      if (!recognizedDevice || recognizedDevice && deviceId && recognizedDevice.id !== deviceId) {
        logger.debug('[HW-DEBUG] HWStore - Device not belongs to this wallet');
        // Keep isTransactionInitiated active & Set new device listener by initiating transaction
        // Show message to reconnect proper software wallet device pair
        logger.debug('[HW-DEBUG] unfinishedWalletTxSigning SET: ', walletId);
      ...
      logger.debug('[HW-DEBUG] HWStore - Transaction Initiated - RESET: ', walletId);
      runInAction('HardwareWalletsStore:: Initiate transaction', () => {
        this.isTransactionInitiated = false;
        this.unfinishedWalletTxSigning = null;
      });
    ...
    try {
      const signedTransaction = await signTransactionTrezorChannel.request({
        ...
        networkId: isMainnet ? HW_SHELLEY_CONFIG.NETWORK.MAINNET.networkId : HW_SHELLEY_CONFIG.NETWORK.TESTNET.networkId,
        protocolMagic: isMainnet ? HW_SHELLEY_CONFIG.NETWORK.MAINNET.trezorProtocolMagic : HW_SHELLEY_CONFIG.NETWORK.TESTNET.trezorProtocolMagic,
      ...
      if (serializedTx) {
        runInAction('HardwareWalletsStore:: transaction successfully signed', () => {
      ...
      const unsignedTxWithdrawals = withdrawals.length > 0 ? ShelleyTxWithdrawal(withdrawals) : null;
      ...
      const auxiliaryDataSupplement = get(signedTransaction, ['payload', 'auxiliaryDataSupplement']);
      ...
      if (unsignedTxAuxiliaryData && auxiliaryDataSupplement) {
        ...
        txAuxiliaryData = cborizeTxAuxiliaryVotingData(unsignedTxAuxiliaryData, auxiliaryDataSupplement.catalystSignature);
      }
  ...
  _signWitnesses = async (witnesses: Array<TrezorWitness | Witness>, xpubHex: string) => {
  ...
  ShelleyWitness = async (witness: TrezorWitness | Witness, xpubHex: string) => {
    let publicKey;
  ...
  @action
  _signTransactionLedger = async (walletId: string, devicePath: string | null | undefined) => {
    runInAction('HardwareWalletsStore:: set Transaction verifying', () => {
      this.hwDeviceStatus = HwDeviceStatuses.VERIFYING_TRANSACTION;
    });
    const {
    ...
    // Construct Withdrawals
    const _withdrawalsData = map(withdrawals, async (withdrawal) => prepareLedgerWithdrawal(withdrawal));
    ...
    const auxiliaryData = unsignedTxAuxiliaryData ? prepareLedgerAuxiliaryData(unsignedTxAuxiliaryData) : null;
    ...
    try {
      ...
      if (unsignedTxAuxiliaryData && signedTransaction && signedTransaction.auxiliaryDataSupplement) {
        txAuxData = { ...txAuxData,
          txAuxiliaryData: unsignedTxAuxiliaryData,
          txAuxiliaryDataHash: signedTransaction.auxiliaryDataSupplement.auxiliaryDataHashHex
        };
        txAuxiliaryData = cborizeTxAuxiliaryVotingData(unsignedTxAuxiliaryData, signedTransaction.auxiliaryDataSupplement.catalystRegistrationSignatureHex);
      }
  ...
  initiateTransaction = async (params: {
    ...
    if (disconnected) {
      ...
      try {
        ...
        if (hardwareWalletConnectionData.device.deviceType === DeviceTypes.TREZOR) {
        ...
        logger.debug('[HW-DEBUG] HWStore - initiateTransaction - DISCONNECTED - ERROR');
        runInAction('HardwareWalletsStore:: Initiate transaction', () => {
          this.isTransactionInitiated = false;
          this.hwDeviceStatus = HwDeviceStatuses.VERIFYING_TRANSACTION_FAILED;
        });
    ...
    if (deviceType === DeviceTypes.TREZOR && walletId) {
      ...
      logger.debug('[HW-DEBUG] HWStore - getCardanoAdaApp - from  initiateTransaction', {
        devicePath
      });

      if (walletId) {
        this.stopCardanoAdaAppFetchPoller();
        this.cardanoAdaAppPollingInterval = setInterval((path, wid) => this.getCardanoAdaApp({
          path,
          walletId: wid
        }), CARDANO_ADA_APP_POLLING_INTERVAL, devicePath, walletId);
      }
    }
  };
  ...
  @action
  _changeHardwareWalletConnectionStatus = async (params: HardwareWalletConnectionRequest) => {
    const {
      disconnected,
      deviceType,
      deviceId,
    ...
    if (error && deviceType === DeviceTypes.TREZOR) {
      if (error.payload && error.payload && error.payload.code === 'ECONNREFUSED') {
        runInAction('HardwareWalletsStore:: Mark Trezor Bridge as not installed', () => {
    ...
    if (deviceType === DeviceTypes.TREZOR && !this.isTrezorBridgeInstalled) {
      runInAction('HardwareWalletsStore:: Mark Trezor Bridge as installed', () => {
    ...
    const recognizedPairedHardwareWallet = find(hardwareWalletsConnectionData, connection => // We can not be sure that Ledger is right Wallet device because we don't have device ID at this point
    ...
    if (disconnected && deviceType === DeviceTypes.LEDGER) {
      ...
      const recognizedLedgerDevice = find(hardwareWalletDevices, hardwareWalletDevice => hardwareWalletDevice.path === path);
      ...
      const recognizedLedgerWallet = find(hardwareWalletsConnectionData, connection => // We can not be sure that Ledger is right Wallet device because we don't have device ID at this point
    ...
    if (recognizedPairedHardwareWallet && recognizedPairedHardwareWallet.device.deviceType === DeviceTypes.TREZOR) {
      ...
      logger.debug('[HW-DEBUG] HWStore - set Hardware Wallet local data: ', recognizedPairedHardwareWallet.id);
      await this._setHardwareWalletLocalData({
    ...
    if (this.isListeningForDevice && !disconnected && (!eventType || eventType === DeviceEvents.CONNECT)) {
      ...
      if (deviceType === DeviceTypes.LEDGER) {
        ...
        logger.debug('[HW-DEBUG] HWStore - CALL establish connection with pendingID: ', pendingId);
    ...
    if (this.unfinishedWalletTxSigning && !disconnected && eventType === DeviceEvents.CONNECT) {
      logger.debug('[HW-DEBUG] CHANGE STATUS to: ', HwDeviceStatuses.CONNECTING);
      runInAction('HardwareWalletsStore:: Change status to Connecting', () => {
        this.hwDeviceStatus = HwDeviceStatuses.CONNECTING;
      });
      logger.debug('[HW-DEBUG] HWStore - Reinitialize TX signing: ', this.unfinishedWalletTxSigning);
    ...
    if (this.unfinishedWalletAddressVerification && !disconnected && (!eventType || eventType === DeviceEvents.CONNECT)) {
      logger.debug('[HW-DEBUG] CHANGE STATUS to: ', HwDeviceStatuses.CONNECTING);
      runInAction('HardwareWalletsStore:: Change status to Connecting', () => {
        this.hwDeviceStatus = HwDeviceStatuses.CONNECTING;
      });
      logger.debug('[HW-DEBUG] HWStore - Reinitialize Address Verification process: ', toJS(this.unfinishedWalletAddressVerification));
      // It is not possible to pass null value that FLOW marks as error (FlowFixMe used)
      this.initiateAddressVerification( // $FlowFixMe
      this.unfinishedWalletAddressVerification, path);
    }
  };
  @action
  ...
  checkIsTrezorByWalletId = (walletId: string): boolean => {
    const hardwareWalletConnectionData = find(this.hardwareWalletsConnectionData, connectionData => connectionData.id === walletId);
    return hardwareWalletConnectionData && hardwareWalletConnectionData.device.deviceType === DeviceTypes.TREZOR;
  };
  ...
  _getHardwareWalletDeviceInfoByWalletId = (walletId: string): HardwareWalletLocalData => {
    return find(this.hardwareWalletsConnectionData, connectionData => connectionData.id === walletId);
  };
  _setHardwareWalletLocalData = async ({
    walletId,
    data
  }: SetHardwareWalletLocalDataRequestType) => {
    logger.debug('[HW-DEBUG] HWStore - CALL SET - _setHardwareWalletLocalData METHOD: ', walletId);
  ...
  _unsetHardwareWalletLocalData = async ({
    ...
    const pairedDevice = find(this.hardwareWalletDevices, recognizedDevice => recognizedDevice.paired === walletId);
  ...
  _resetHardwareWallets = async () => {
    if (global.environment.isDev) {
      await Promise.all(this.stores.wallets.all.map(async wallet => {
