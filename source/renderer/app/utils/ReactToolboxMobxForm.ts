...
export const handleFormErrors = async (querySelector: string, options: HandleFormErrorsOptions = {}) => {
  const {
    focusElement
  } = options;
  ...
  if (focusElement && firstErrorLabel && firstErrorLabel.parentNode instanceof HTMLElement) {
    const input = firstErrorLabel.parentNode.querySelector('div:not(.SimpleAutocomplete_selectedWords) > input');
