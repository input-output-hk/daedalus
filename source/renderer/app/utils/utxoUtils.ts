...
export const getUtxoChartData = (histogram: Histogram): UtxoChartData => Object.entries(histogram).sort().reduce((data, [lovelaceWalletAmount, walletUtxosAmount]) => {
  const walletAmount = formattedLovelaceToAmount(parseInt(lovelaceWalletAmount, 10));

  if (walletAmount > 100000) {
    let lastItem: UtxoChartItem = last(data);
    const {
      walletUtxosAmount: lastWalletUtxosAmount
    } = lastItem;
    lastItem = { ...lastItem,
      walletUtxosAmount: parseInt(walletUtxosAmount, 10) + parseInt(lastWalletUtxosAmount, 10)
    };
...
export const getWalletUtxosTotalAmount = (histogram: Histogram): number => {
  ...
  const walletUtxosAmount = histogramArr.length ? histogramArr.reduce((amount, value) => parseInt(amount, 10) + parseInt(value, 10)) : 0;
