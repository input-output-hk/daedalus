 ...
 */
export const filterAssets = (assets: Array<any>, transactionType: TransactionType, isInternalAddress: (...args: Array<any>) => any): Array<any> => assets.filter(({
  address
}) => transactionType === TransactionTypes.INCOME && isInternalAddress(address) || transactionType === TransactionTypes.EXPEND && !isInternalAddress(address));
 ...
 * See Asset/Token differences at the begining of this doc
 */
export const getAssetTokens = (assets: Array<Asset>, tokens: Tokens): Array<AssetToken> => assets.map(asset => getAssetToken(asset, getToken(asset, tokens))).filter(token => !!token.uniqueId) // @TOKEN TODO - Remove this filter once we can list zero tokens
 ...
 * See Asset/Token differences at the begining of this doc
 */
export const getAssetTokenFromToken = (asset: Token, getAsset: (...args: Array<any>) => any): AssetToken => {
  const {
    policyId,
    assetName,
    quantity,
...
export const getNonZeroAssetTokens = (tokens: Tokens, getAsset: (...args: Array<any>) => any): Array<AssetToken> => tokens.map(token => getAssetTokenFromToken(token, getAsset)).filter(token => !!token.uniqueId).sort(sortAssets('fingerprint', 'asc'));
 ...
 */
export const sortAssets = (sortBy: SortBy, sortDirection: SortDirection) => (asset1: AssetToken, asset2: AssetToken) => {
  const {
 ...
 * @returns {boolean}
 */
export const hasTokensLeftAfterTransaction = (allAvailableTokens: AssetToken[], initialSelectedAssets: AssetToken[], selectedAssets?: string[]): boolean => {
  if (!!selectedAssets && selectedAssets.length && selectedAssets.length > 0 && !!initialSelectedAssets && initialSelectedAssets?.length && initialSelectedAssets?.length > 0) {
    ...
    if (initialSelectedAssets.length < allAvailableTokens.length || selectedAssets.length < initialSelectedAssets.length) {
      return true;
    }

    return !!find(selectedAssets, (selectedAsset, index) => !initialSelectedAssets[index]?.quantity?.isEqualTo(selectedAsset));
  }
 ...
 */
export const searchAssets = (rawSearchValue: string, assets: Array<AssetToken>) => {
...
export const isTokenMissingInWallet = (wallet?: Wallet | null | undefined, token?: Token) => {
...
export const tokenHasBalance = (token: Token, amount: BigNumber) => token.quantity.isGreaterThanOrEqualTo(amount);
