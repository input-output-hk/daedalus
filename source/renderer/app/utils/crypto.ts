...
export const scramblePaperWalletMnemonic = (passphrase: string, input: string) => {
  let iv;
...
export const unscramblePaperWalletMnemonic = (passphrase: string, scrambledInput: string) => {
  const input = CardanoCrypto.PaperWallet.unscrambleStrings(passphrase, scrambledInput);
  return input.split(' ');
};
export const mnemonicToSeedHex = (mnemonic: string, password: string | null | undefined) => {
...
export const decodeBech32 = (data: string): Buffer => Buffer.from(bech32.fromWords(bech32.decode(data).words));
export const encodeBech32 = (prefix: string, data: Buffer): string => bech32.encode(prefix, bech32.toWords(data));
export const getStakeAddressFromStakeKey = (stakeKey: string): string => {
  ...
  const stakeAddress = encodeBech32(addressPrefix, Buffer.from([...networkPrefix, ...stakeKeyHash]));
  return stakeAddress;
};