import { utils, TxOutputDestinationType, AddressType, TxAuxiliaryDataType // CHECK THIS
} from "@cardano-foundation/ledgerjs-hw-app-cardano";
import { encode } from "borc";
import blakejs from "blakejs";
import _ from "lodash";
import { derivationPathToLedgerPath, CERTIFICATE_TYPE, groupTokensByPolicyId } from "./hardwareWalletUtils";
import { deriveXpubChannel } from "../ipc/getHardwareWalletChannel";
import { AddressStyles } from "../domains/WalletAddress";
// Types
import type { CoinSelectionInput, CoinSelectionOutput, CoinSelectionCertificate, CoinSelectionWithdrawal, CoinSelectionAssetsType } from "../api/transactions/types";
...
// Constructors
export const ShelleyTxWitnessShelley = (publicKey: Buffer, signature: Buffer) => {
...
export const ShelleyTxOutput = (output: CoinSelectionOutput, addressStyle: AddressStyle) => {
  const {
    address,
    amount,
    derivationPath,
  ...
  const coins = assets && assets.length > 0 ? [adaCoinQuantity, ShelleyTxOutputAssets(assets)] : adaCoinQuantity;

  function encodeCBOR(encoder: any) {
    const addressBuff = addressStyle === AddressStyles.ADDRESS_SHELLEY ? utils.bech32_decodeAddress(address) : utils.base58_decode(address);
...
export const ShelleyTxWithdrawal = (withdrawals: Array<CoinSelectionWithdrawal>) => {
  function encodeCBOR(encoder: any) {
...
export const prepareLedgerWithdrawal = (withdrawal: CoinSelectionWithdrawal) => {
  return {
...
export const ShelleyTxAux = (inputs: Array<ShelleyTxInputType>, outputs: Array<ShelleyTxOutputType>, fee: ShelleyFeeType, ttl: ShelleyTtlType, certs: Array<Certificate | null | undefined>, withdrawals: ShelleyTxWithdrawalsType | null | undefined, auxiliaryData: TxAuxiliaryData | null | undefined, auxiliaryDataHash: string | null | undefined) => {
  const blake2b = data => blakejs.blake2b(data, null, 32);

  function getId() {
    return blake2b(encode(ShelleyTxAux(inputs, outputs, fee, ttl, certs, withdrawals, auxiliaryData, auxiliaryDataHash)) // 32
    ).toString('hex');
  }
...
export const ShelleySignedTransactionStructured = (txAux: ShelleyTxAuxType, witnesses: Map<number, ShelleyTxWitnessType>, txAuxiliaryData: CborizedVotingRegistrationMetadata | null | undefined) => {
  function getId() {
    return txAux.getId();
  }
...
export const CachedDeriveXpubFactory = (deriveXpubHardenedFn: (...args: Array<any>) => any) => {
  const derivedXpubs = {};
  let xpubMemo;

  const deriveXpub = async (absDerivationPath: Array<number>, xpubHex: string | null | undefined) => {
    ...
    if (!derivedXpubsMemo) {
      ...
      derivedXpubsMemo = deriveHardened ? await deriveXpubHardenedFn(xpubMemo) : await deriveXpubNonhardenedFn(absDerivationPath);
    }
...
export const prepareLedgerOutput = (output: CoinSelectionOutput, addressStyle: AddressStyle) => {
  ...
  return {
    destination: {
      ...
      params: {
        addressHex: addressStyle === AddressStyles.ADDRESS_SHELLEY ? utils.buf_to_hex(utils.bech32_decodeAddress(output.address)) : utils.buf_to_hex(utils.base58_decode(output.address))
      }
    },
    amount: output.amount.quantity.toString(),
    tokenBundle
...
export const cborizeTxVotingRegistration = ({
  ...
  return [61284, new Map<number, Buffer | number>([[1, Buffer.from(votingPubKey, 'hex')], [2, Buffer.from(stakePubKey, 'hex')], [3, utils.bech32_decodeAddress(rewardDestinationAddress.address)], [4, Number(nonce)]])];
};
export const cborizeTxAuxiliaryVotingData = (txAuxiliaryData: TxAuxiliaryData, signatureHex: string) => [new Map<number, Map<number, Buffer | number>>([cborizeTxVotingRegistration(txAuxiliaryData), [61285, new Map<number, Buffer | number>([[1, Buffer.from(signatureHex, 'hex')]])]]), []];
export const prepareTxAux = ({
  ...
  return ShelleyTxAux(txInputs, txOutputs, txFee, txTtl, txCerts, txWithdrawals, txAuxiliaryData, txAuxiliaryDataHash);
};
export const prepareBody = (unsignedTx: ShelleyTxAuxType, txWitnesses: any, txAuxiliaryData: CborizedVotingRegistrationMetadata | null | undefined) => {
  const signedTransactionStructure = ShelleySignedTransactionStructured(unsignedTx, txWitnesses, txAuxiliaryData);
