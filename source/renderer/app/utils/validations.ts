 ...
 */
export const containsDecimalNumber = (s: string) => /\p{Decimal_Number}/u.test(s);

/**
 * Checks if a string contains a lower case letter
 */
export const containsLowerCaseLetter = (s: string) => /\p{Lowercase_Letter}/u.test(s);

/**
 * Checks if a string contains a upper case letter
 */
export const containsUpperCaseLetter = (s: string) => /\p{Uppercase_Letter}/u.test(s);
 ...
 */
export const isCaselessString = (s: string) => s.toLowerCase() === s && s.toUpperCase() === s;

/**
 * Test if a whole string is in unicase letters (or digits)
 */
export const isUnicaseString = (password: string) => // We require at least one unicase letter
...
export const isValidRepeatPassword = (spendingPassword: string, repeatPassword: string) => spendingPassword === repeatPassword;
...
export function validateMnemonics(params: ValidateMnemonicsParams) {
  ...
  const isPhraseComplete = Array.isArray(requiredWords) ? requiredWords.includes(providedWordsCount) : providedWordsCount === requiredWords;
...
export const isValidRepeatPinCode = (pinCode: string, repeatPinCode: string) => pinCode === repeatPinCode;