import { MAINNET_EXPLORER_URL, STAGING_EXPLORER_URL, TESTNET_EXPLORER_URL, MAINNET_NEWS_URL, TESTNET_NEWS_URL, STAGING_NEWS_URL, DEVELOPMENT_NEWS_URL, MAINNET_NEWS_HASH_URL, STAGING_NEWS_HASH_URL, TESTNET_NEWS_HASH_URL, DEVELOPMENT_NEWS_HASH_URL } from "../config/urlsConfig";
import { MAINNET, STAGING, TESTNET, DEVELOPMENT } from "../../../common/types/environment.types";
...
export const getNetworkExplorerUrl = (network: string): string => {
  const protocol = network === MAINNET || network === TESTNET || network === DEVELOPMENT ? 'https://' : 'http://';
  const uri = getNetworkExplorerUri(network);
  return `${protocol}${uri}`;
};
export const getNetworkExplorerUrlByType = (type: "tx" | "address", param: string, network: string, currentLocale: string): string => {
  ...
  return `${getNetworkExplorerUrl(network)}${localePrefix}/${typeValue}${queryStringPrefix}${param}`;
};
