...
import type { FormatMessageContextParams, Logger, LoggingLevel } from "../../../common/types/logging.types";
const {
  environment,
  electronLog
} = global;
...
const logToLevel = (level: LoggingLevel) => (message: string, data: Record<string, any> | null | undefined) => {
  const args = [formatContext({ ...messageContext,
    level
  }), {
    message,
    data,
