...
import { formattedWalletAmount, formattedTokenWalletAmount } from "./formatters";
...
const transactionsCsvGenerator = async ({
  ...
  const columns = [intl.formatMessage(messages.columnID), intl.formatMessage(messages.columnType), intl.formatMessage(messages.columnTotal), intl.formatMessage(messages.columnSentAmount), intl.formatMessage(messages.columnDepositAmount), intl.formatMessage(messages.columnFee), intl.formatMessage(messages.columnDateTime), intl.formatMessage(messages.columnStatus), intl.formatMessage(messages.columnAddressesFrom), intl.formatMessage(messages.columnAddressesTo), intl.formatMessage(messages.columnWithdrawals)];
  ...
  transactions.forEach(({
    ...
    const valueType = type === TransactionTypes.EXPEND ? intl.formatMessage(messages.valueTypeSent) : intl.formatMessage(messages.valueTypeReceived);
    ...
    const valueTokens = filterAssets(assets, type, isInternalAddress).map(({
      policyId,
      assetName,
      quantity
    }) => {
      ...
      const formattedAmount = formattedTokenWalletAmount(quantity, metadata, 0);
      return `${formattedAmount} (${fingerprint})`;
    }).join(', ');
    const valueDateTime = date ? date.toISOString() : '';
    const valueStatus = state === 'pending' ? intl.formatMessage(messages.valueStatusPending) : intl.formatMessage(messages.valueStatusConfirmed);
    const valueAddressesFrom = !includes(addresses.from, null) ? addresses.from.join(', ') : ' ';
    const valueAddressesTo = addresses.to.join(', ');
    const valueWithdrawals = addresses.withdrawals.join(', ');
    const txValues = [id, valueType, valueTotal, valueSentAmount, valueDepositAmount, valueTransactionFee, `${valueDateTime}`, valueStatus, valueAddressesFrom, valueAddressesTo, valueWithdrawals];
