...
export const derivationPathToLedgerPath = (derivationPath: Array<string>) => {
  const transformedPath = _.map(derivationPath, chunk => chunk.replace('H', "'"));
...
export const bech32EncodePublicKey = (data: Buffer): string => {
  ...
  return bech32.encode(KEY_PREFIXES.PUBLIC_KEY_WITH_CHAIN_CODE, data5bit, MAX_HUMAN_ADDRESS_LENGTH);
};
export const bech32DecodePublicKey = (data: string): Buffer => {
  const {
    words
  } = bech32.decode(data, 1000);
  return Buffer.from(bech32.fromWords(words));
};
export const groupTokensByPolicyId = (assets: CoinSelectionAssetsType) => {
  const compareStringsCanonically = (string1: string, string2: string) => string1.length - string2.length || string1.localeCompare(string2);

  const groupedAssets = {};

  _(assets).orderBy(['policyId', 'assetName'], ['asc', 'asc']).groupBy(({
    policyId
  }) => policyId).mapValues(tokens => tokens.map(({
    assetName,
    quantity,
    policyId
  }) => ({
  ...
  })).sort((token1, token2) => compareStringsCanonically(token1.policyId, token2.policyId)).value().map(sortedAssetsGroup => {
