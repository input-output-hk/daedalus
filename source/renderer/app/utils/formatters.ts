...
export const formattedWalletAmount = (amount: BigNumber, withCurrency: boolean = true, long: boolean = true): string => {
  let formattedAmount = long ? new BigNumber(amount).toFormat(DECIMAL_PLACES_IN_ADA) : shortNumber(amount);
  const {
    decimalSeparator
...
export const formattedWalletCurrencyAmount = (amount: BigNumber, currencyRate: number, decimalDigits?: number | null | undefined, currencyCode?: string | null | undefined): string => `${amount ? amount.times(currencyRate).toFormat(decimalDigits || 2) : 0} ${currencyCode || ''}`;
export const formattedTokenWalletAmount = (amount: BigNumber, metadata?: AssetMetadata | null | undefined, decimals: number | null | undefined, isShort?: boolean): string => {
  const {
    ticker
  } = metadata || {};
...
export const formattedTokenDecimals = (amount: BigNumber, decimals: number | null | undefined): BigNumber => {
  const decimalPrecision = decimals || DEFAULT_DECIMAL_PRECISION;
  const divider = parseInt(getMultiplierFromDecimalPlaces(decimalPrecision), 10);
...
export const shortNumber = (value: number | BigNumber): string => {
  ...
  if (amount.isZero()) {
    ...
    formattedAmount = `${amount.decimalPlaces(DECIMAL_PLACES_IN_ADA, BigNumber.ROUND_DOWN)}`;
  } else if (amount.isLessThan(1000000)) {
    formattedAmount = `${amount.dividedBy(1000).decimalPlaces(1, BigNumber.ROUND_DOWN)}K`;
  } else if (amount.isLessThan(1000000000)) {
    formattedAmount = `${amount.dividedBy(1000000).decimalPlaces(1, BigNumber.ROUND_DOWN)}M`;
  } else if (amount.isLessThan(1000000000000)) {
    formattedAmount = `${amount.dividedBy(1000000000).decimalPlaces(1, BigNumber.ROUND_DOWN)}B`;
  } else if (amount.isLessThan(1000000000000000)) {
    formattedAmount = `${amount.dividedBy(1000000000000).decimalPlaces(1, BigNumber.ROUND_DOWN)}T`;
  } else {
    formattedAmount = `${amount.dividedBy(1000000000000000).decimalPlaces(1, BigNumber.ROUND_DOWN)}Q`;
  }

  return formattedAmount;
};
export const formattedAmountToNaturalUnits = (amount: string | null | undefined): string => {
  if (!amount) {
    return '0';
  }

  const cleanedAmount = amount.replace(/\./g, '') // removes all the dot separators
...
export const formattedAmountToLovelace = (amount: string): number => parseInt(formattedAmountToBigNumber(amount).times(LOVELACES_PER_ADA), 10);
export const formattedLovelaceToAmount = (lovelace: number): number => formattedAmountToBigNumber(String(lovelace)).dividedBy(LOVELACES_PER_ADA).toNumber();
...
export const formattedDownloadData = (downloadData?: DownloadData | null | undefined, userLocale: Locale): FormattedDownloadData => {
  let timeLeft = '';
  let downloaded = '';
  let total = '';
  let progress = 0;
...
export const formattedArrayBufferToHexString = (arrayBuffer: Uint8Array): string => {
...
export const toFixedWithoutRounding = (num: number | string, fixed: number): string => {
  const re = new RegExp(`^-?\\d+(?:.\\d{0,${fixed || -1}})?`);
  const results = num.toString().match(re);
  return results && results.length ? results[0].toString() : '0';
};
export const formattedNumber = (value: number | string, dp?: number): string => new BigNumber(value).toFormat(dp);
export const formattedCpuModel = (model: string): string => {
  ...
  const formattedSpeedSection = speedSection.replace(/[\d,.]+/, formattedSpeedNumber);
  const formattedModel = `${model.substring(0, atCharPosition)}${formattedSpeedSection}`;
