...
const copy = async () => {
  ...
  const step1 = async () => {
    ...
    if (!foundProperties.length) {
      warn(`I couldn't find any property with the prefix "${cyan(fromPrefix)}". Please, try again.`);
      return step1();
    }
    ...
    if (foundProperties.length > MAX_RESULTS_BEFORE_WARNING) {
      const shouldProceed = await prompt({
        type: 'confirm',
        message: red(`I've found ${foundProperties.length} properties. Are you sure you want to proceed?`)
      });
  ...
  const step3 = async () => {
    ...
    const conflictingProperties = existingProperties.filter(existingProperty => {
      const selectedProperty = replaceSingleProperty(existingProperty, toPrefix, fromPrefix);
      return !!selectedProperties.filter(property => property === selectedProperty).length;
    });
    ...
    if (conflictingProperties.length) {
      const confirmReplace = await prompt({
        type: 'checkbox',
        message: 'The following properties already exist. Select which ones should be replaced.\n',
      ...
      selectedProperties = selectedProperties.filter(selectedProperty => {
        const newProperty = replaceSingleProperty(selectedProperty, fromPrefix, toPrefix);
        return !conflictingProperties.includes(newProperty) || confirmReplace.includes(newProperty);
      });
      const removed = originalSelectedPropertiesLength - selectedProperties.length;
  ...
  const step4 = async () => {
    ...
    if (!toCategory) {
      ...
      if (isNewCategory) {
        toCategory = await prompt({
          type: 'input',
          message: `What is the category name? (e.g. ${orange('newFeatureName')})'\n--->`
        });
      } else {
        toCategory = await prompt({
          type: 'list',
    ...
    const newProperties = selectedProperties.map(selectedProperty => replaceSingleProperty(selectedProperty, fromPrefix, toPrefix));
    const confirmMessage = `Great, I'll copy ${selectedProperties.length} properties from "${cyan(fromPrefix)}" to "${cyan(toPrefix)}" into ${orange(toCategory)}.
Here's an example of how they will look like:
"{
  ...
  ${orange(toCategory)}: {${!isNewCategory ? `
    ...` : ''}
    ${newProperties.map(newProperty => `${cyan(newProperty)}: ${magenta('...')},`).join(`\n    `)}
  },
  ...
}"
Should I proceed?
`;
   ...
   */
  const step5 = async () => {
    const pendingUpdates = EXISTING_THEME_OUTPUTS.reduce((pending, [themeName, theme]) => {
      const fromProperties: CompletePropertiesList = toPairs(theme[fromCategory]).filter(([propertyKey]) => selectedProperties.includes(propertyKey));
      const toProperties = fromProperties.map(([fromPropertyKey, fromPropertyValue]) => [replaceSingleProperty(fromPropertyKey, fromPrefix, toPrefix), fromPropertyValue]);
      pending[themeName] = {};
      pending[themeName][toCategory] = toProperties.reduce((obj, [propertyKey, propertyValue]) => {
    ...
    info(`\nGreat! I have finished adding the new properties and am running Prettier.\nMeanwhile you need to update the '${cyan('CreateTheme.js')}' file, because I can't do it automatically.\n`);
  };
...
// Helpers
const getCategoriesChoices = () => categories.map(category => ({
  value: category,
  short: `\nOk, I'll use the existing ${orange(category)} category`,
  name: category
}));

const getChoicesFromProperties = properties => properties.map(propertyName => ({
  value: propertyName,
  short: `\n✔ ${propertyName}`,
...
const findPropertiesFromPrefix = (themeObj: Record<string, any>, prefix: string): {
  category: string;
  items: Array<string>;
} => toPairs(themeObj).reduce((response, [categoryName, categoryObj]) => {
  const existingProperties = keys(categoryObj).filter(property => property.indexOf(prefix) > -1);
...
const replaceSingleProperty = (propertyName: string, fromPrefix: string, toPrefix) => {
