...
export const findUpdates = (createThemeOutputs: Array<[string, Record<string, any>]>): PendingThemesUpdates => {
  const pendingThemesUpdates = createThemeOutputs.reduce((defsToAdd: Record<string, any>, themeOutput: [string, Record<string, any>]) => {
    const [fileName, themeObj] = themeOutput;
    const missingDefs = { ...findMissingDefinitions(themeObj, EXISTING_THEME_OUTPUTS_OBJ[fileName]),
  ...
  if (isEmpty(pendingThemesUpdates)) {
    console.log(chalk.hex('#2cbb69')(`\n${chalk.bold.inverse('*** All Daedalus themes are up to date with the createTheme object! ***')}`));
  }
