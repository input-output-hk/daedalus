...
export const runUpdateThemesCLI = (pendingUpdates: PendingThemesUpdates) => {
  ...
  rl.on('line', line => {
    switch (line.trim()) {
      case 'yes':
        {
          ...
          logMsg(chalk`{cyan Prettier is now formatting the updated theme files.}`);
          logMsg(chalk`{cyan Once Prettier finshes, please check the diff and commit the changes if all is correct.}`);
      ...
      default:
        {
          logMsg(chalk.cyan(`\nCommand not recognized.\n\nEnter ( ${YES_OR_NO} ) to continue, or ${logInputOption('ctrl + c')} to exit.`));
          break;
        }
    }
  }).on('close', () => {
