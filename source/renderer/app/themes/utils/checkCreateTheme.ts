...
export const checkCreateTheme = (createThemeObj: Record<string, any>) => {
  const missingDefinitions = EXISTING_THEME_OUTPUTS.reduce((defsToAdd: Record<string, any>, themeOutput: [string, Record<string, any>]) => {
    const [fileName, themeObj] = themeOutput;
    const missingDefs = { ...findMissingDefinitions(themeObj, createThemeObj),
  ...
  if (isEmpty(missingDefinitions)) {
    console.log(chalk.hex('#2cbb69')(`\n${chalk.bold.inverse('*** createTheme.js is up to date with all the Daedalus themes! ***')}`));
  }
};
export const findMissingDefinitions = (basis: Record<string, any>, target: Record<string, any>): Record<string, any> => {
...
export const findMissingCSSVars = (basis: Record<string, any>, target: Record<string, any>): Record<string, any> => {
    ...
    if (basisEntry && has(target, basisEntry)) {
      for (const basisCSSVar in basis[basisEntry]) {
        if (basisCSSVar && !has(target[basisEntry], basisCSSVar)) {
          missingCSSVariables[basisEntry] = { ...missingCSSVariables[basisEntry],
