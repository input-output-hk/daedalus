...
import type { LocalizedCurrency, CurrencyApiConfig } from "../types/currencyTypes";
import type { GetCurrencyListResponse, GetCurrencyRateResponse } from "../api/wallets/types";
...
const responses = {
  list: (apiResponse: Array<Record<string, any>>): GetCurrencyListResponse => {
    try {
      ...
      const list = vsCurrencies.map(code => currenciesList[code] || completeList.find(currency => currency.symbol === code)).filter(item => !!item);
