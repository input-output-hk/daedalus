...
function parseNewsUrl(value: string | undefined, fallback: string): URL {
  try {
    return new URL(value || fallback);
  } catch {
    console.error(
      `[urlsConfig] Invalid URL "${value}", falling back to "${fallback}"`
    );
    return new URL(fallback);
  }
}
const _newsUrl = parseNewsUrl(
  process.env.NEWS_URL,
  'https://newsfeed.daedalus.io'
);
const _newsHashUrl = parseNewsUrl(
  process.env.NEWS_HASH_URL,
  'https://newsfeed.daedaluswallet.io'
);
...
// Pathname prefix from NEWS_URL (e.g. '/daedalus/7.2'); empty string for root URLs.
