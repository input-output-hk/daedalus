...
import type { RequestName, Currency, LocalizedCurrency } from "../types/currencyTypes";
...
export const genericCurrencyRequest = (requestName: RequestName): (...args: Array<any>) => any => async (payload?: any): any => {
...
export const getLocalizedCurrenciesList = (rawCurrencyList: Array<Currency>, currentLocale: Locale) => map(rawCurrencyList, rawCurrency => getLocalizedCurrency(rawCurrency, currentLocale));
export const getLocalizedCurrency = (rawCurrency: Currency, currentLocale: Locale): LocalizedCurrency => ({ ...omit(rawCurrency, ['name']),
  name: rawCurrency.name[currentLocale] || rawCurrency.name[LOCALES.english]
});
export const getCurrencyFromCode = (code: string): Currency => currenciesList[code];