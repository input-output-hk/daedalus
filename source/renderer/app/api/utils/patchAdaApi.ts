...
import type { GetNetworkInfoResponse, NetworkInfoResponse } from "../network/types";
...
export default ((api: AdaApi) => {
  api.getNetworkInfo = async (): Promise<GetNetworkInfoResponse> => {
    ...
    try {
      ...
      const syncProgress = get(sync_progress, 'status') === 'ready' ? 100 : get(sync_progress, 'quantity', 0);
  ...
  api.setTestingNewsFeed = (testingNewsFeedData: GetNewsResponse | null | undefined) => {
    const {
    ...
    TESTING_NEWSFEED_JSON = { ...testingNewsFeedData,
      items: newsFeedItems
    };
  };
  ...
  api.setTestingWallet = (testingWalletData: Record<string, any>, walletIndex: number = 0): void => {
    TESTING_WALLETS_DATA[walletIndex] = testingWalletData;
  };

  api.setTestingWallets = (testingWalletsData: Array<Record<string, any>>): void => {
  ...
  const getModifiedWallet = action((wallet: Record<string, any>) => {
    ...
    if (typeof availableAmount !== 'object') availableAmount = new BigNumber(availableAmount);
  ...
  api.getWallets = async (): Promise<Array<Wallet>> => {
    ...
    const modifiedWallets = originalWallets.map((originalWallet: Wallet, index: number) => {
      const testingWallet = TESTING_WALLETS_DATA[index] || {};
      const modifiedWallet = { ...originalWallet,
        ...testingWallet
      };
  ...
  api.setTestingStakePools = (testingStakePoolsData: Array<Record<string, any>>): void => {
    api.getStakePools = (): Array<StakePool> => testingStakePoolsData.map((stakePool: Record<string, any>) => new StakePool(stakePool));
  };
