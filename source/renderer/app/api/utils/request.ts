...
const logSocketStats = (state: string, {
  sockets,
  freeSockets
}) => {
...
function typedRequest<Response>(httpOptions: RequestOptions, queryParams?: {}, rawBodyParams?: any, requestOptions?: {
  returnMeta?: boolean;
  isOctetStreamRequest?: boolean;
  isOctetStreamResponse?: boolean;
}): Promise<Response> {
  return new Promise((resolve, reject) => {
    const options: RequestOptions = Object.assign({}, httpsOptions, httpOptions);
    const {
    ...
    if (rawBodyParams) {
      ...
      options.headers = { ...options.headers,
        Accept: isOctetStreamResponse ? 'application/octet-stream' : 'application/json; charset=utf-8'
      };
    }

    const httpOnlyOptions = omit(options, ['agent', 'ca', 'cert', 'key', 'ciphers', 'minVersion', 'maxVersion']);
    const httpsRequest = isSelfnode ? global.http.request(httpOnlyOptions) : global.https.request(options);
    ...
    httpsRequest.on('response', response => {
      ...
      response.on('end', () => {
        try {
          ...
          const isSuccessResponse = statusCode >= 200 && statusCode <= 206 || statusCode === 404 && includes(ALLOWED_ERROR_EXCEPTION_PATHS, options.path);

          if (isSuccessResponse) {
            if (isOctetStreamResponse) {
              resolve(stream);
            } else {
              const data = statusCode === 404 ? 'null' : `"statusCode: ${statusCode} -- statusMessage: ${statusMessage}"`;
