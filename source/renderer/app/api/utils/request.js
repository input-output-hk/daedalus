...
const logSocketStats = (state, { sockets, freeSockets }) => {
  ...
  const used = (0, lodash_1.flatten)((0, lodash_1.values)(sockets)).length;
  const free = (0, lodash_1.flatten)((0, lodash_1.values)(freeSockets)).length;
...
function typedRequest(httpOptions, queryParams, rawBodyParams, requestOptions) {
  return new Promise((resolve, reject) => {
    const options = Object.assign({}, httpsOptions, httpOptions);
    ...
    if (queryParams && (0, lodash_1.size)(queryParams) > 0) {
      options.path += `?${querystring_1.default.stringify(queryParams)}`;
    }
    ...
    if (rawBodyParams) {
      ...
      if (isOctetStreamRequest) {
        ...
        requestBody = json_bigint_1.default.stringify(rawBodyParams);
        options.headers = {
    ...
    const httpOnlyOptions = (0, lodash_1.omit)(options, [
      'agent',
      'ca',
      'cert',
      'key',
        ...
        try {
          ...
          if (isSuccessResponse) {
            if (isOctetStreamResponse) {
              ...
              resolve(json_bigint_1.default.parse(body));
            }
          } else if (stream) {
            // Error response with a stream
            const parsedStream = json_bigint_1.default.parse(stream.toString());
            reject(parsedStream);
          } else if (body) {
            // Error response with a body
            const parsedBody = json_bigint_1.default.parse(body);
