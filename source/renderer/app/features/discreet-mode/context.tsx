...
export function DiscreetModeFeatureProvider({ children, analyticsClient }: Props) {
  const localStorageFeature = useLocalStorageFeature();
  const [discreetModeFeature] = useState<DiscreetMode>(() => {
    const feature = new DiscreetMode(new DiscreetModeApi(localStorageFeature), analyticsClient);
