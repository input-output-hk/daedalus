...
export class DiscreetMode extends Feature {
  ...
  @observable
  getDiscreetModeSettingsTooltipRequest: Request<Promise<boolean>> =
    // @ts-ignore ts-migrate(2729) FIXME: Property 'api' is used before its initialization.
    new Request(this.api.getDiscreetModeSettingsTooltip);
  @observable
  setDiscreetModeSettingsTooltipRequest: Request<Promise<boolean>> =
    // @ts-ignore ts-migrate(2729) FIXME: Property 'api' is used before its initialization.
  ...
  _setupDiscreetMode = async () => {
    ...
    const isSettingsTooltipEnabled =
      this.getDiscreetModeSettingsTooltipRequest.result;
    const isNotificationEnabled =
      this.getDiscreetModeNotificationRequest.result;
