...
module.exports = {
  ...
  module: {
    rules: [{
      ...
      include: /source/,
      exclude: /source\/main/,
      use: (isCi ? [] : ['cache-loader', 'thread-loader']).concat(['babel-loader'])
    }, {
      test: /\.scss/,
  ...
  plugins: [new MiniCssExtractPlugin({
    ...
    entry: {
      vendor: ['aes-js', 'bignumber.js', 'bip39', 'blakejs', 'bs58', 'classnames', 'es6-error', 'history', 'humanize-duration', 'lodash', 'mobx', 'mobx-react', 'mobx-react-form', 'mobx-react-router', 'moment', 'pbkdf2', 'qrcode.react', 'react', 'react-copy-to-clipboard', 'react-datetime', 'react-dom', 'react-router', 'react-router-dom', 'react-svg-inline', 'recharts', 'route-parser', 'safe-buffer', 'unorm', 'validator']
    }
  })].filter(Boolean)
};