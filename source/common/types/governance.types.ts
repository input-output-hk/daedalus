...
/**
 * A value from a field no standard Daedalus knows defines.
 *
 * JSON-LD carries structure, and a DRep's own vocabulary uses it: a multi-sig
 * DRep listing its members gives each one a name, a title and an affiliation.
 * Flattening that to a string would lose the shape its author wrote, and
 * dropping it would lose the members. Neither is necessary. Every leaf is text
 * that renders as text, and nothing here becomes clickable, so a nested list
 * costs nothing a flat one does not already cost.
 */
export type DRepAdditionalValue =
  | { kind: 'text'; text: string }
  | { kind: 'list'; items: DRepAdditionalValue[] }
  | { kind: 'group'; fields: DRepAdditionalField[] };

/** One field the document carried that no standard Daedalus knows defines. */
export interface DRepAdditionalField {
  /** The author's own key, shown literally: nobody can translate it. */
  key: string;
...
export interface VerifiedDRepAnchorContent {
  ...
  doNotList: boolean;
  /**
   * Everything else the document carried, in the order it was written.
   *
   * JSON-LD is extensible, and the community extends it: nineteen terms outside
   * the canonical set appear across the mainnet sample. Dropping them silently
   * would freeze the vocabulary at this release, since a convention nobody can
   * see is a convention nobody adopts.
   */
