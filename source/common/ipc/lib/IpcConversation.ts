...
export type IpcReceiver = {
  on: (channel: string, arg1: (event: IpcEvent, conversationId: string, ...args: Array<any>) => Promise<any> | void) => void;
  removeListener: (channel: string, listener: (...args: Array<any>) => void) => void;
};
...
export class IpcConversation<Incoming, Outgoing> {
  ...
  async request(message: Outgoing, sender: IpcSender, receiver: IpcReceiver): Promise<Incoming> {
    return new Promise((resolve, reject) => {
      ...
      const handler = (event, messageId: string, isOk: boolean, response: Incoming) => {
   ...
   * from the other side.
   */
  onRequest(handler: (arg0: Incoming) => Promise<Outgoing>, receiver: IpcReceiver): void {
    receiver.on(this._channelName, async (event: IpcEvent, conversationId: string, message: Incoming) => {
      try {
