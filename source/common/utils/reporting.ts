...
export const generateSupportRequestLink = (baseUrl: string, environmentData: Environment, locale: string): string => {
  const {
    version,
    apiVersion,
    network,
  ...
  return `${baseUrl}?${Object.entries(supportRequestData).map(([key, val]: [string, any]) => `${encodeURIComponent(key)}=${encodeURIComponent(val)}`).join('&')}`;
};