...
import { ALONZO_PURPLE, DEVELOPMENT, LINUX, MAC_OS, MAINNET, MAINNET_FLIGHT, PRODUCTION, SELFNODE, STAGING, TEST, TESTNET, WINDOWS, networkPrettyNames } from "../types/environment.types";
...
export const getBuildLabel = (build: string, network: string, currentNodeEnv: string, isFlight: boolean, version: string) => {
  const networkLabel = isFlight ? 'Flight' : networkPrettyNames[network];
  let buildLabel = `Daedalus ${networkLabel} (${version}#${build})`;
  if (!checkIsProduction(currentNodeEnv)) buildLabel += ` ${upperFirst(currentNodeEnv)}`;
  return buildLabel;
};
export const checkIsDev = (currentNodeEnv: string) => currentNodeEnv === DEVELOPMENT;
export const checkIsTest = (currentNodeEnv: string) => currentNodeEnv === TEST;
export const checkIsProduction = (currentNodeEnv: string) => currentNodeEnv === PRODUCTION;
export const checkIsMainnet = (network: string) => network === MAINNET;
export const checkIsTestnet = (network: string) => network === TESTNET;
export const checkIsAlonzoPurple = (network: string) => network === ALONZO_PURPLE;
