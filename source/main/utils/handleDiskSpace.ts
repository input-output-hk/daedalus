...
import { DISK_SPACE_REQUIRED, DISK_SPACE_REQUIRED_MARGIN_PERCENTAGE, DISK_SPACE_CHECK_LONG_INTERVAL, DISK_SPACE_CHECK_MEDIUM_INTERVAL, DISK_SPACE_CHECK_SHORT_INTERVAL, DISK_SPACE_RECOMMENDED_PERCENTAGE, stateDirectoryPath } from "../config";
export const handleDiskSpace = (mainWindow: BrowserWindow, onCheckDiskSpace?: (...args: Array<any>) => any) => {
  ...
  const handleCheckDiskSpace = async (forceDiskSpaceRequired?: number) => {
    ...
    try {
      ...
      const diskSpaceRecommended = diskTotalSpace * DISK_SPACE_RECOMMENDED_PERCENTAGE / 100;
      const diskSpaceRequiredMargin = diskSpaceRequired - diskSpaceRequired * DISK_SPACE_REQUIRED_MARGIN_PERCENTAGE / 100;
      ...
      if (diskSpaceAvailable <= diskSpaceRequiredMargin) {
        ...
        const newDiskSpaceCheckIntervalLength = diskSpaceAvailable >= diskSpaceRequired * 2 ? DISK_SPACE_CHECK_LONG_INTERVAL : DISK_SPACE_CHECK_MEDIUM_INTERVAL;
      ...
      if (isNotEnoughDiskSpace) logger.info('Not enough disk space', {
        response
      });
