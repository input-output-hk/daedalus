...
import { STORAGE_TYPES, STORAGE_KEYS } from "../../common/config/electron-store.config";
import type { DownloadInfo, DownloadData, DownloadDataUpdate, DownloadLocalDataResponse } from "../../common/types/downloadManager.types";
import { DOWNLOAD_DATA_DEFAULT, DOWNLOAD_STATES } from "../../common/config/downloadManagerConfig";
...
export const downloadManagerLocalStorage = {
  ...
  setData: async (newData: DownloadDataUpdate, id: string): Promise<DownloadData> => {
    const {
      data: oldData,
      info
    } = await downloadManagerLocalStorage.get(id);
    const data = mergeWith(oldData, newData, (o, n, key) => {
      if (typeof n === 'number' && key !== 'remainingSize') return Math.max(o, n);
      return n;
    });
    await requestElectronStore({
      type: STORAGE_TYPES.SET,
