...
import { ALLOWED_DOWNLOAD_DIRECTORIES, DOWNLOAD_DATA_DEFAULT, DOWNLOAD_EVENT_TYPES as types, DOWNLOAD_STATES as states, ERROR_TIME_AFTER_NO_END_EVENT } from "../../common/config/downloadManagerConfig";
...
import type { DownloadRendererRequest, DownloadMainResponse } from "../../common/ipc/api";
import type { AllowedDownloadDirectories, DownloadInfoInit, DownloadInfoProgress, DownloadInfoEnd, DownloadInfoError, DownloadInfo, DownloadDataUpdate } from "../../common/types/downloadManager.types";
import { stateDirectoryPath } from "../config";
export const downloads = {};
export const getIdFromFileName = (fileName: string): string => fileName.replace(/\./g, '-');
export const getPathFromDirectoryName = (directoryName: AllowedDownloadDirectories) => {
...
export const getOriginalFilename = ({
  ...
  let name = '';
  if (resumeDownload) name = resumeDownload.originalFilename;else if (options && typeof options.fileName === 'string') name = options.fileName;else name = extractFileNameFromPath(fileUrl);
  return name;
};

const getPath = (info: DownloadInfo): {
...
export const getEventActions = async (info: DownloadInfo, window: BrowserWindow, requestDownloadChannel: MainIpcChannel<DownloadRendererRequest, DownloadMainResponse>): Promise<Record<string, any>> => {
  const {
    downloadId
  } = info;
