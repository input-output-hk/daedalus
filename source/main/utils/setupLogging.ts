...
import type { ConstructMessageBodyParams, MessageBody, LogSystemInfoParams, StateSnapshotLogParams, WalletMigrationReportData } from "../../common/types/logging.types";
...
export const setupLogging = () => {
  ...
  log.transports.console.format = (message: Record<string, any>): string => formatMessage(message);
  ...
  log.transports.rendererConsole.format = (message: Record<string, any>): string => {
  ...
  fs.readdir(appLogsFolderPath, (err, files) => {
    files.filter(isFileNameWithTimestamp()).forEach(fileName => {
      ...
      try {
        ...
        console.error(`Compressed log file "${filePath}" deletion failed: ${error}`);
      }
    });
  });
};
...
export const logStateSnapshot = (props: StateSnapshotLogParams): MessageBody => {
  const { ...data
  } = props;
  const {
    currentTime: at,
...
export const generateWalletMigrationReport = (data: WalletMigrationReportData) => {
  const walletMigrationrReportFilePath = path.join(pubLogsFolderPath, 'Wallet-migration-report.json');
  const generatedAt = new Date().toISOString();
  fs.writeFileSync(walletMigrationrReportFilePath, JSON.stringify({ ...data,
    generatedAt
  }));
};