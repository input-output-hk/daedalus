...
if (!isStartedByLauncher) {
  ...
  if (isProduction) {
    dialogMessage = isWindows ? 'Please start Daedalus using the icon in the Windows start menu or using Daedalus icon on your desktop.' : 'Daedalus was launched without needed configuration. Please start Daedalus using the shortcut provided by the installer.';
  } else {
...
export const buildLabel = getBuildLabel(build, network, current, isFlight, version);
// Logging config
export const ALLOWED_LOGS = ['Daedalus.json', 'System-info.json', 'Daedalus-versions.json', 'State-snapshot.json', 'Wallet-migration-report.json', 'cardano-wallet.log', 'node.log'];
...
export const FALLBACK_TOKEN_METADATA_SERVER_URL = 'https://metadata.cardano-testnet.iohkdev.io';
// Used by mock-token-metadata-server
export const MOCK_TOKEN_METADATA_SERVER_URL = 'http://localhost';
export const MOCK_TOKEN_METADATA_SERVER_PORT = process.env.MOCK_TOKEN_METADATA_SERVER_PORT || 0;