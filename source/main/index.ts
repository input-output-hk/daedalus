...
import { setupLogging, logSystemInfo, logStateSnapshot, generateWalletMigrationReport } from "./utils/setupLogging";
...
import { launcherConfig, pubLogsFolderPath, stateDirectoryPath } from "./config";
...
import type { GenerateWalletMigrationReportRendererRequest, SetStateSnapshotLogMainResponse } from "../common/ipc/api";
...
import { restoreSavedWindowBounds, saveWindowBoundsOnSizeAndPositionChange } from "./windows/windowBounds";

/* eslint-disable consistent-return */
// Global references to windows to prevent them from being garbage collected
let mainWindow: BrowserWindow;
let cardanoNode: CardanoNode | null | undefined;
const {
  isDev,
  isTest,
  isWatchMode,
...
const onAppReady = async () => {
  setupLogging();
  logUsedVersion(environment.version, path.join(pubLogsFolderPath, 'Daedalus-versions.json'));
  const cpu = os.cpus();
  ...
  process.env.PATH = [process.env.PATH, process.env.DAEDALUS_INSTALL_DIRECTORY].join(path.delimiter);
  ...
  mainWindow = createMainWindow(locale, restoreSavedWindowBounds(screen, requestElectronStore));
  ...
  const onCheckDiskSpace = ({
    ...
    if (cardanoNode) {
      if (isNotEnoughDiskSpace) {
        if (cardanoNode.state !== CardanoNodeStates.STOPPING && cardanoNode.state !== CardanoNodeStates.STOPPED) {
          try {
            cardanoNode.stop();
          } catch (e) {} // eslint-disable-line

        }
      } else if (cardanoNode.state !== CardanoNodeStates.STARTING && cardanoNode.state !== CardanoNodeStates.RUNNING) {
        cardanoNode.restart();
      }
    }
  };
  ...
  setStateSnapshotLogChannel.onReceive((data: SetStateSnapshotLogMainResponse) => {
    return Promise.resolve(logStateSnapshot(data));
  });
  generateWalletMigrationReportChannel.onReceive((data: GenerateWalletMigrationReportRendererRequest) => {
    return Promise.resolve(generateWalletMigrationReport(data));
  });
  getStateDirectoryPathChannel.onRequest(() => Promise.resolve(stateDirectoryPath));
  getDesktopDirectoryPathChannel.onRequest(() => Promise.resolve(app.getPath('desktop')));
  ...
  await enableApplicationMenuNavigationChannel.onReceive(() => new Promise(resolve => {
    buildAppMenus(mainWindow, cardanoNode, locale, {
      isNavigationEnabled: true
    });
    resolve();
  }));
  await rebuildApplicationMenu.onReceive(data => new Promise(resolve => {
  ...
  app.on('before-quit', async event => {
    ...
    if (isSelfnode) {
      if (keepLocalClusterRunning || isTest) {
        logger.info('ipcMain: Keeping the local cluster running while exiting Daedalus', {
          keepLocalClusterRunning
        });
        return safeExitWithCode(0);
      }
      ...
      const {
        response
      } = await dialog.showMessageBox(mainWindow, exitSelfnodeDialogOptions);
