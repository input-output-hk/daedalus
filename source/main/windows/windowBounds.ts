...
import { STORAGE_KEYS, STORAGE_TYPES } from "../../common/config/electron-store.config";
...
function maybeRectangle(data: any): Rectangle | null | undefined {
  return data != null && typeof data.x === 'number' && typeof data.y === 'number' && typeof data.width === 'number' && typeof data.height === 'number' ? (data as Rectangle) : null;
}
...
function isWithinBounds(rect: Rectangle, bounds: Rectangle): boolean {
  return rect.x >= bounds.x && rect.y >= bounds.y && getRightEdge(rect) <= getRightEdge(bounds) && getBottomEdge(rect) <= getBottomEdge(bounds);
}
...
function getCenteredRectInBounds(rect: Rectangle, bounds: Rectangle): Rectangle {
  return {
    width: rect.width,
    height: rect.height,
 ...
 */
export function restoreSavedWindowBounds(screen: Screen, sendStoreRequest: SendStoreRequest): Rectangle | null | undefined {
  const savedBounds = maybeRectangle(sendStoreRequest({
...
export function saveWindowBoundsOnSizeAndPositionChange(window: Window, sendStoreRequest: SendStoreRequest, debounceWait: number = 1000) {
  const saveWindowBoundsSoon = debounce(() => sendStoreRequest({
