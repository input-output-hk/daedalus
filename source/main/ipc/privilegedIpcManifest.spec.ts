...
for (const symbol of apiModule ? checker.getExportsOfModule(apiModule) : []) {
  if (privilegedIpcManifest.some(({ contract }) => contract === symbol.name))
...
describe('privileged IPC manifest', () => {
  it('matches every live constructor, transport, and renderer adapter exactly once', () => {
    ...
    expect(privilegedIpcManifest).toHaveLength(77);
    expect(mainConstructions).toHaveLength(77);
    expect(rendererConstructions).toHaveLength(77);
