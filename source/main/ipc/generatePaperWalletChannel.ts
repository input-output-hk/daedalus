...
import type { GeneratePaperWalletMainResponse, GeneratePaperWalletRendererRequest } from "../../common/ipc/api";
...
MainIpcChannel<GeneratePaperWalletRendererRequest, GeneratePaperWalletMainResponse> = new MainIpcChannel(GENERATE_PAPER_WALLET_CHANNEL);
export const handlePaperWalletRequests = () => {
  generatePaperWalletChannel.onReceive((request: GeneratePaperWalletRendererRequest) => new Promise((resolve, reject) => {
    // Prepare params
    const {
      address,
      mnemonics,
    ...
    try {
      ...
      // first page
      const page1Uri = readAssetSync(isMainnet ? paperWalletPage1Path : paperWalletPage1PathTestnet);
      ...
      // second page
      doc.addPage();
      const page2Uri = readAssetSync(isMainnet ? paperWalletPage2Path : paperWalletPage2PathTestnet);
