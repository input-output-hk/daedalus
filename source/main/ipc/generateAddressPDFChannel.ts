...
import type { GenerateAddressPDFRendererRequest, GenerateAddressPDFMainResponse } from "../../common/ipc/api";
...
MainIpcChannel<GenerateAddressPDFRendererRequest, GenerateAddressPDFMainResponse> = new MainIpcChannel(GENERATE_ADDRESS_PDF_CHANNEL);
export const handleAddressPDFRequests = () => {
  generateAddressPDFChannel.onReceive((request: GenerateAddressPDFRendererRequest) => new Promise((resolve, reject) => {
    // Prepare params
    const {
      address,
      filePath,
    ...
    try {
      ...
      // Creation date
      doc.font(fontBufferRegular).fontSize(12).text(creationDate.toUpperCase(), {
        align: 'center',
      ...
      // Footer
      if (!isMainnet) {
        doc.fontSize(12).font(fontBufferMedium).fillColor(textColorRed).text(`${networkLabel} ${networkName}`, {
          align: 'right'
        });
      }
