...
import { getOriginalFilename, getPathFromDirectoryName, getEventActions, getIdFromFileName, downloads } from "../utils/downloadManager";
import { REQUEST_DOWNLOAD, RESUME_DOWNLOAD, DELETE_DOWNLOADED_FILE, GET_DOWNLOAD_LOCAL_DATA, GET_DOWNLOADS_LOCAL_DATA, CLEAR_DOWNLOAD_LOCAL_DATA, CHECK_FILE_EXISTS } from "../../common/ipc/api";
import { DEFAULT_DIRECTORY_NAME, TEMPORARY_FILENAME, DOWNLOAD_STATES } from "../../common/config/downloadManagerConfig";
import { generateFileNameWithTimestamp } from "../../common/utils/files";
import { downloadManagerLocalStorage as localStorage } from "../utils/mainLocalStorage";
import type { DownloadRendererRequest, DownloadMainResponse, ResumeDownloadRendererRequest, ResumeDownloadMainResponse, DeleteDownloadedFileRendererRequest, DeleteDownloadedFileMainResponse, DownloadLocalDataRendererRequest, DownloadLocalDataMainResponse, DownloadsLocalDataRendererRequest, DownloadsLocalDataMainResponse, ClearDownloadLocalDataRendererRequest, ClearDownloadLocalDataMainResponse, CheckFileExistsMainResponse, CheckFileExistsRendererRequest } from "../../common/ipc/api";
localStorage.setAllPaused();

const requestDownload = async (downloadRequestPayload: DownloadRendererRequest, window: BrowserWindow): Promise<any> => {
  const {
    fileUrl,
  ...
  const temporaryFilename = resumeDownload ? resumeDownload.temporaryFilename : generateFileNameWithTimestamp(TEMPORARY_FILENAME);
  ...
  if (downloads[downloadId]) {
    logger.info(`DownloadManager: Preventing download "${downloadId}" duplicity`, {
      downloadId
    });
    return false;
  }

  const eventActions = await getEventActions(info, window, requestDownloadChannel);
  ...
  if (resumeDownload) {
    const {
      total: downloadSize
    } = await download.getTotalSize(); // get the total size from the server
  ...
  const progressType = options.progressIsThrottled === false ? 'progress' : 'progress.throttled';
...
const requestResumeDownload = async (resumeDownloadRequestPayload: ResumeDownloadRendererRequest, window: BrowserWindow): Promise<any> => {
  const downloadLocalData = await getDownloadLocalData(resumeDownloadRequestPayload);
  const {
    temporaryFilename,
    originalFilename,
    downloadId: id,
  ...
  if (fs.existsSync(filePath)) {
    requestDownloadPayload = { ...requestDownloadPayload,
      resumeDownload: {
        temporaryFilename,
        originalFilename
      }
  ...
  return requestDownload({ ...requestDownloadPayload,
    override: true
  }, window);
};
...
const getDownloadsLocalData = async (): Promise<DownloadsLocalDataMainResponse> => localStorage.getAll();
...
const checkFileExists = async ({
  ...
  const fileName = state === DOWNLOAD_STATES.FINISHED ? originalFilename : temporaryFilename;
  const filePath = `${destinationPath}/${fileName}`;
  return fs.existsSync(filePath);
};

const requestDownloadChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<DownloadRendererRequest, DownloadMainResponse> = new MainIpcChannel(REQUEST_DOWNLOAD);
const requestResumeDownloadChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<ResumeDownloadRendererRequest, ResumeDownloadMainResponse> = new MainIpcChannel(RESUME_DOWNLOAD);
const deleteDownloadedFileChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<DeleteDownloadedFileRendererRequest, DeleteDownloadedFileMainResponse> = new MainIpcChannel(DELETE_DOWNLOADED_FILE);
const getDownloadLocalDataChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<DownloadLocalDataRendererRequest, DownloadLocalDataMainResponse> = new MainIpcChannel(GET_DOWNLOAD_LOCAL_DATA);
const getDownloadsLocalDataChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<DownloadsLocalDataRendererRequest, DownloadsLocalDataMainResponse> = new MainIpcChannel(GET_DOWNLOADS_LOCAL_DATA);
const clearDownloadLocalDataChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<ClearDownloadLocalDataRendererRequest, ClearDownloadLocalDataMainResponse> = new MainIpcChannel(CLEAR_DOWNLOAD_LOCAL_DATA);
const checkFileExistsChannel: // IpcChannel<Incoming, Outgoing>
MainIpcChannel<CheckFileExistsRendererRequest, CheckFileExistsMainResponse> = new MainIpcChannel(CHECK_FILE_EXISTS);
export const downloadManagerChannel = (window: BrowserWindow) => {
  requestDownloadChannel.onRequest((downloadRequestPayload: DownloadRendererRequest) => requestDownload(downloadRequestPayload, window));
  requestResumeDownloadChannel.onRequest((resumeDownloadRequestPayload: ResumeDownloadRendererRequest) => requestResumeDownload(resumeDownloadRequestPayload, window));
...
export const pauseActiveDownloads = () => {
  forEach(downloads, (download, downloadId) => {
    try {
      if (download && download.state === DOWNLOAD_STATES.DOWNLOADING) download.pause();
      logger.info(`DownloadManager:PauseDownloads download "${downloadId}" was paused`, {
        downloadId
      });
    } catch (error) {
      logger.error(`DownloadManager:PauseDownloads download "${downloadId}" could not be paused`, {
        error
      });
    }
  });
