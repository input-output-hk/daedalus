...
import type { ManageAppUpdateRendererRequest as Request, ManageAppUpdateMainResponse as Response } from "../../common/ipc/api";
...
const manageAppUpdateChannel: MainIpcChannel<Request, Response> = new MainIpcChannel(MANAGE_APP_UPDATE);
...
export const handleManageAppUpdateRequests = (window: BrowserWindow) => {
  const response = (success: boolean | null | undefined, functionPrefix: string, messageText: string = '', _data?: Record<string, any>): Response => {
    let status = statuses.PROGRESS;
    if (success === true) status = statuses.SUCCESS;else if (success === false) status = statuses.ERROR;
    ...
    return new Promise((resolve, reject) => {
      ...
      updater.stdout.on('data', progressData => {
        ...
        const progress = info.reduce((prog, infoItem) => {
          ...
          if (progressStr) {
            ...
            return parseInt(parseInt(item, 10) * 100 / parseInt(total, 10), 10);
          }
      ...
      updater.on('close', code => {
        if (code !== 0) {
          ...
          reject(response(false, functionPrefix, `updater closed with code ${code}`, {
            code
          }));
        }
      ...
      updater.on('error', error => {
        ...
        reject(response(false, functionPrefix, 'installation failed', {
          error
        }));
      });
      updater.on('exit', code => {
        if (code !== 0) {
          ...
          reject(response(false, functionPrefix, `updater exited with code ${code}`, {
            code
          }));
        }
  ...
  manageAppUpdateChannel.onRequest(async ({
    filePath,
    hash: expectedHash
  }) => {
    const functionPrefix = 'onRequest';
    const fileExists = fs.existsSync(filePath);
    if (!fileExists) return response(false, functionPrefix, 'Installer not found:', {
      info: {
        filePath
      }
    });
