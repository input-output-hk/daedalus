...
import { deviceDetection, waitForDevice } from './ledger/deviceDetection';
...
import { Device } from './ledger/deviceDetection/types';
import { DeviceDetectionPayload } from './ledger/deviceDetection/deviceDetection';
import {
  initTrezorConnect,
  reinitTrezorConnect,
} from '../../trezor/connection';
...
class EventObserver {
  constructor(
    private sendHardwareWalletConnectionMessage: (message: any) => Promise<void>
  ) {}
        ...
        if (event.type === 'add') {
          if (!devicesMemo[device.path]) {
            ...
            try {
              ...
              this.sendHardwareWalletConnectionMessage({
                disconnected: false,
                deviceType: 'ledger',
                deviceId: null,
...
interface ResetTrezorListenersArgs {
  sendHardwareWalletConnectionMessage: (message: any) => Promise<void>;
}

const resetTrezorListeners = ({
  sendHardwareWalletConnectionMessage,
}: ResetTrezorListenersArgs) => {
...
export const onWaitForLedgerDevicesToConnectChannelRequest = async () => {
...
export const onGetHardwareWalletTransportChannelRequest = async (
  request: HardwareWalletTransportDeviceRequest
  ...
  try {
    ...
    const openTransportLayer = async (pathToOpen: string, device: Device) => {
...
export const onHandleInitTrezorConnectChannelRequest = async ({
  sendHardwareWalletConnectionMessage,
}: ResetTrezorListenersArgs) => {
  logger.info('[HW-DEBUG] INIT TREZOR');
  resetTrezorListeners({ sendHardwareWalletConnectionMessage });
  TrezorConnect.manifest(manifest);
  await initTrezorConnect();
};

export const onHandleInitLedgerConnectChannelRequest = async ({
  sendHardwareWalletConnectionMessage,
}: ResetTrezorListenersArgs) => {
  logger.info('[HW-DEBUG] INIT LEDGER');
  observer = new EventObserver(sendHardwareWalletConnectionMessage);
...
export const onDderiveXpubChannelRequest = async (params) => {
...
export const onDeriveAddressChannelRequest = async (params) => {
  const {
    addressType,
    spendingPathStr,
    stakingPathStr,
...
export const onShowAddressChannelRequest = async (params) => {
  const {
    addressType,
    spendingPathStr,
    stakingPathStr,
...
export const onGetCardanoAdaAppChannelRequest = async (request) => {
...
export const onGetExtendedPublicKeyChannelRequest = async (params) => {
  // Params example:
...
export const onSignTransactionLedgerChannelRequest = async (params) => {
  const {
    inputs,
    outputs,
    protocolMagic,
...
export const onSignTransactionTrezorChannelRequest = async (params) => {
  const {
    inputs,
    outputs,
    protocolMagic,
...
export const onResetTrezorActionChannelRequest = async () => {
  logger.info('[TREZOR-CONNECT] Called TrezorConnect.cancel()');
  TrezorConnect.cancel('Method_Cancel');
};

export const isLedgerListenerActive = () =>
  ledgerStatus.listening && !!ledgerStatus.Listener;

export const closeLedgerListener = () => {
  if (!isLedgerListenerActive()) return;
