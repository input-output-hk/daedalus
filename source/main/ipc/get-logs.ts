...
import { pubLogsFolderPath, ALLOWED_LOGS, ALLOWED_NODE_LOGS, ALLOWED_WALLET_LOGS, ALLOWED_LAUNCHER_LOGS, MAX_NODE_LOGS_ALLOWED, MAX_WALLET_LOGS_ALLOWED, MAX_LAUNCHER_LOGS_ALLOWED } from "../config";
import { MainIpcChannel } from "./lib/MainIpcChannel";
import { GET_LOGS_CHANNEL } from "../../common/ipc/api";
import type { GetLogsRendererRequest, GetLogsMainResponse } from "../../common/ipc/api";
import type { LogFiles } from "../../renderer/app/types/LogTypes";
// IpcChannel<Incoming, Outgoing>
export const getLogsChannel: MainIpcChannel<GetLogsRendererRequest, GetLogsMainResponse> = new MainIpcChannel(GET_LOGS_CHANNEL);
...
const isFileAllowed = (fileName: string) => includes(ALLOWED_LOGS, fileName) || isOldDaedalusLog(fileName);

const isFileNodeLog = (fileName: string, nodeLogsIncluded: number) => ALLOWED_NODE_LOGS.test(fileName) && nodeLogsIncluded < MAX_NODE_LOGS_ALLOWED;

const isFileWalletLog = (fileName: string, walletLogsIncluded: number) => ALLOWED_WALLET_LOGS.test(fileName) && walletLogsIncluded < MAX_WALLET_LOGS_ALLOWED;

const isFileLauncherLog = (fileName: string, nodeLogsIncluded: number) => ALLOWED_LAUNCHER_LOGS.test(fileName) && nodeLogsIncluded < MAX_LAUNCHER_LOGS_ALLOWED;
