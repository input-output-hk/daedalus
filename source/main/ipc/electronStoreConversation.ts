...
export const electronStoreConversation: MainIpcConversation<ElectronStoreMessage, ElectronStoreMessage> = new MainIpcConversation(ELECTRON_STORE_CHANNEL);

const getNetworkKey = (key: string) => `${environment.network}-${key}`;

const unset = async (key: StorageKey) => requestElectronStore({
  type: types.DELETE,
  key
});
