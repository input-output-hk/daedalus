...
import type { CardanoNodeImplementations, CardanoNodeState, CardanoStatus, FaultInjection, FaultInjectionIpcRequest, FaultInjectionIpcResponse, TlsConfig } from "../../common/types/cardano-node.types";
...
type Actions = {
  ...
  createWriteStream: (path: string, options?: Record<string, any>) => WriteStream;
...
} = deriveProcessNames(platform, nodeImplementation, isSelfnode);
...
export class CardanoNode {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      if (isSelfnode) {
        try {
          ...
          reject(new Error('CardanoNode#start: Unable to initialize cardano-launcher'));
        }
      } else {
        try {
          ...
          node.start().then(api => {
            ...
            _log.info(`CardanoNode#start: cardano-node child process spawned with PID ${processes.node.pid}`, {
              pid: processes.node.pid
            });

            _log.info(`CardanoNode#start: cardano-wallet child process spawned with PID ${processes.wallet.pid}`, {
            ...
            reject(new Error('CardanoNode#start: Error while spawning cardano-node'));
          });
        } catch (error) {
          _log.error('CardanoNode#start: Unable to initialize cardano-launcher', {
            error
  ...
  async expectNodeUpdate(): Promise<void> {
    ...
    try {
      await promisedCondition(() => this._state === CardanoNodeStates.UPDATED, _config.updateTimeout);
  ...
  setFault = async (request: FaultInjectionIpcRequest): Promise<void> => {
    ...
    try {
      ...
      return Promise.reject(new Error(`cardano-node did not inject the fault "${fault}" correctly.`));
    }
  };
   ...
   * @returns {boolean}
   */
  _isDead = async (): Promise<boolean> => !this._isConnected() && this._isNodeProcessNotRunningAnymore();
  ...
  _ensureProcessIsNotRunning = async (pid: number, name: string) => {
    ...
    if (await this._isProcessRunning(pid, name)) {
      ...
      try {
        ...
        _log.error(`CardanoNode: could not kill ${name} process (PID: ${pid})`, {
          name,
          pid,
          error
        });
  ...
  _ensurePreviousCardanoNodeIsNotRunning = async (): Promise<void> => {
    ...
    const previousPID: number | null | undefined = await this._retrieveData(PREVIOUS_CARDANO_PID);
  ...
  _killProcessWithName = async (pid: number, name: string): Promise<void> => {
    ...
    try {
      ...
      await promisedCondition(async () => (await this._isProcessRunning(pid, name)) === false, _config.killTimeout);

      this._log.info(`CardanoNode: successfully killed ${name} process (PID: ${pid})`, {
        name,
        pid
      });
      ...
      this._log.error(`CardanoNode: _killProcessWithName returned an error attempting to kill ${name} process (PID: ${pid})`, {
        processName: name,
        pid,
        error
      });
  ...
  _storeData = (identifier: string, data: number): Promise<void> => new Promise((resolve, reject) => {
    try {
  ...
  _retrieveData = (identifier: string): Promise<number | null | undefined> => new Promise((resolve, reject) => {
    try {
  ...
  _isNodeProcessStillRunning = async (): Promise<boolean> => this._node != null && this._isProcessRunning(this._node.pid, CARDANO_PROCESS_NAME);
  _isNodeProcessNotRunningAnymore = async () => (await this._isNodeProcessStillRunning()) === false;
  _waitForNodeProcessToExit = async (timeout: number) => promisedCondition(this._isNodeProcessNotRunningAnymore, timeout);
  ...
  _isUnrecoverable = (config: CardanoNodeConfig) => this._startupTries >= config.startupMaxRetries;
}