...
import { FALLBACK_TOKEN_METADATA_SERVER_URL, MOCK_TOKEN_METADATA_SERVER_URL, MOCK_TOKEN_METADATA_SERVER_PORT } from "../config";
import { MAINNET, STAGING, TESTNET, SELFNODE } from "../../common/types/environment.types";
...
export async function CardanoWalletLauncher(walletOptions: WalletOptions): Launcher {
  const {
    nodeImplementation,
    nodeConfig,
  ...
  if (!isProduction && nodeImplementation === CardanoNodeImplementationOptions.CARDANO) {
  ...
  switch (nodeImplementation) {
    case CardanoNodeImplementationOptions.CARDANO:
      if (cluster === SELFNODE) {
        ...
        } = await createSelfnodeConfig(configFile, genesisFile, stateDir, cliBin);
