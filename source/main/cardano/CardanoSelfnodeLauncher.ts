...
const TOKEN_METADATA_SERVER_PROCESS_NAME = platform === 'win32' ? 'mock-token-metadata-server.exe' : 'mock-token-metadata-server';
export async function CardanoSelfnodeLauncher(selfnodeOptions: SelfnodeOptions): Promise<{
  node: Selfnode;
  replyPort: number;
}> {
  return new Promise(async (resolve, reject) => {
    ...
    const isSelfnodeRunning = processList.length && processList[0].name === processName;
    ...
    if (isSelfnodeRunning) {
      ...
      tcpPortUsed.waitUntilUsed(CARDANO_WALLET_PORT, CARDANO_WALLET_START_CHECK_INTERVAL, CARDANO_WALLET_START_TIMEOUT).then(() => {
        node.connected = true;
        resolve({
          node,
          replyPort: CARDANO_WALLET_PORT
...
const setupSelfnode = ({
  ...
  onStop: (...args: Array<any>) => any;
  connected: boolean;
}): Selfnode => Object.assign({}, nodeProcess, {
  wpid: nodeProcess.pid,
  stop: async () => {
    if (mockTokenMetadataServer !== null) {
      await onStop(mockTokenMetadataServer.pid, TOKEN_METADATA_SERVER_PROCESS_NAME);
    }
...
const setupMockTokenMetadataServer = async (mockTokenMetadataServerBin: string) => {
  const processList: Array<Process> = await find('port', TOKEN_METADATA_SERVER_PORT);
  const isMockTokenMetadataServerRunning = processList.length && processList[0].name === TOKEN_METADATA_SERVER_PROCESS_NAME;
  ...
  if (isMockTokenMetadataServerRunning) {
    ...
    const mockTokenMetadataServerProcess = spawn(mockTokenMetadataServerBin, ['--port', TOKEN_METADATA_SERVER_PORT.toString(), TOKEN_METADATA_REGISTRY], {
      env: { ...process.env
      },
      detached: true,
      stdio: 'ignore'
