...
import { NODE_KILL_TIMEOUT, NODE_SHUTDOWN_TIMEOUT, NODE_STARTUP_MAX_RETRIES, NODE_STARTUP_TIMEOUT, NODE_UPDATE_TIMEOUT } from "../config";
import { logger } from "../utils/logging";
import type { LauncherConfig } from "../config";
import type { CardanoNodeState, CardanoStatus, TlsConfig } from "../../common/types/cardano-node.types";
import type { ExportWalletsRendererRequest, CardanoFaultInjectionRendererRequest } from "../../common/ipc/api";
import { cardanoAwaitUpdateChannel, cardanoFaultInjectionChannel, cardanoRestartChannel, cardanoStateChangeChannel, getCachedCardanoStatusChannel, cardanoTlsConfigChannel, setCachedCardanoStatusChannel, exportWalletsChannel } from "../ipc/cardano.ipc";
import { safeExitWithCode } from "../utils/safeExitWithCode";

const startCardanoNode = (node: CardanoNode, launcherConfig: LauncherConfig) => {
  const {
    logsPrefix,
    nodeImplementation,
    nodeConfig,
 ...
 * @param mainWindow
 */
export const setupCardanoNode = (launcherConfig: LauncherConfig, mainWindow: BrowserWindow): CardanoNode => {
  const cardanoNode = new CardanoNode(logger, {
    ...
    readFileSync,
    createWriteStream,
    broadcastTlsConfig: (config: TlsConfig | null | undefined) => {
  ...
  setCachedCardanoStatusChannel.onReceive((status: CardanoStatus | null | undefined) => {
  ...
  cardanoFaultInjectionChannel.onReceive((fault: CardanoFaultInjectionRendererRequest) => {
    logger.info('ipcMain: Received request to inject a fault into cardano node', {
      fault
    });
  ...
  exportWalletsChannel.onRequest(({
    ...
    return Promise.resolve(exportWallets(exportSourcePath, launcherConfig, mainWindow, locale));
  });
  return cardanoNode;
};