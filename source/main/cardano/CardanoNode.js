...
export class CardanoNode {
  ...
  start = async (
    ...
    const logFileNameGenerator = time => {
      if (!time) {
        this._cardanoLogFile = config.logFilePath;
    ...
    const rotateFileStream = async () => {
      ...
      rfs(logFileNameGenerator(), {
        size: '5M',
        maxFiles: 4,
      });
      return this._cardanoLogFile;
    };

    return new Promise(async (resolve, reject) => {
      const logFilename = await rotateFileStream();
      if (logFilename) {
  ...
  _reset = () => {
    if (this._cardanoLogFile) this._cardanoLogFile = '';
