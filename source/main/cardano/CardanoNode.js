...
export class CardanoNode {
   ...
   * @private
   */
  _cardanoProcessName: CardanoNodeProcessNames;
  ...
  constructor(
    ...
    this._cardanoProcessName = CARDANO_PROCESS_NAME;
  }
  ...
  _handleCardanoNodeExit = async (code: number, signal: string) => {
    const { _log, _config, _node, _cardanoProcessName } = this;
    ...
    try {
      ...
      try {
        if (_node)
          await this._ensureProcessIsNotRunning(_node.pid, _cardanoProcessName);
      } catch (e) {
  ...
  _ensureCurrentCardanoNodeIsNotRunning = async (): Promise<void> => {
    const { _log, _node, _cardanoProcessName } = this;
    _log.info(
    ...
    return this._ensureProcessIsNotRunning(_node.pid, _cardanoProcessName);
  };

  _ensurePreviousCardanoNodeIsNotRunning = async (): Promise<void> => {
    const { _log, _cardanoProcessName } = this;
    ...
    return this._ensureProcessIsNotRunning(previousPID, _cardanoProcessName);
  };
  ...
  _isNodeProcessStillRunning = async (): Promise<boolean> =>
    this._node != null &&
    this._isProcessRunning(this._node.pid, this._cardanoProcessName);
