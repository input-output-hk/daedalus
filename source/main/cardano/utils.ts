...
import type { CardanoNodeStorageKeys, CardanoNodeImplementations, NetworkNames, PlatformNames, ProcessNames } from "../../common/types/cardano-node.types";
import { CardanoProcessNameOptions, CardanoNodeImplementationOptions, NetworkNameOptions } from "../../common/types/cardano-node.types";
export type Process = {
  pid: number;
  name: string;
  cmd: string;
...
const checkCondition = async (condition: () => boolean, resolve: (...args: Array<any>) => any, reject: (...args: Array<any>) => any, timeout: number, retryEvery: number, timeWaited: number = 0): Promise<void> => {
  ...
  if (result) {
    ...
    setTimeout(() => checkCondition(condition, resolve, reject, timeout, retryEvery, timeWaited + retryEvery), retryEvery);
  }
};

export const promisedCondition = (cond: (...args: Array<any>) => any, timeout: number = 5000, retryEvery: number = 1000): Promise<void> => new Promise((resolve, reject) => {
  checkCondition(cond, resolve, reject, timeout, retryEvery);
});

const getNetworkName = (network: NetworkNames): string => NetworkNameOptions[network] || NetworkNameOptions.development;
...
export const deriveProcessNames = (platform: PlatformNames, nodeImplementation: CardanoNodeImplementations, isSelfnode: boolean): ProcessNames => ({
  CARDANO_PROCESS_NAME: CardanoProcessNameOptions[isSelfnode ? CardanoNodeImplementationOptions.SELFNODE : nodeImplementation][platform] || 'cardano-node'
});
export const createSelfnodeConfig = async (configFilePath: string, genesisFilePath: string, stateDir: string, cliBin: string): Promise<{
  configPath: string;
  genesisPath: string;
  genesisHash: string;
}> => {
  ...
  const genesisFile = JSON.stringify({ ...genesisFileContent,
    startTime
  });
  ...
  const {
    stdout: genesisHashBuffer
  } = spawnSync(cliBin, ['print-genesis-hash', '--genesis-json', genesisPath]);
  const genesisHash = genesisHashBuffer.toString().replace('\r', '').replace('\n', '');
  ...
  const configFile = JSON.stringify({ ...JSON.parse(configFileContent),
...
export const exportWallets = async (exportSourcePath: string, launcherConfig: LauncherConfig, mainWindow: BrowserWindow, locale: string): Promise<ExportWalletsMainResponse> => {
  const {
    legacySecretKey,
    legacyWalletDB,
    stateDir,
  ...
  try {
    const response = await prepareMigrationData(mainWindow, stateDir, legacySecretKeyPath, legacyWalletDBPath, locale);
  ...
  const legacyWalletDBPathExists = await fs.pathExists(`${legacyWalletDBPath}-acid`);
...
const prepareMigrationData = async (mainWindow: BrowserWindow, stateDir: string, legacySecretKey: string, legacyWalletDB: string, locale: string): Promise<{
  ...
  try {
    ...
    const legacyWalletDBPathExists = await fs.pathExists(legacyWalletDBFullPath);

    if (legacyWalletDBPathExists) {
      logger.info('ipcMain: Copying wallet db directory...', {
        legacyWalletDBFullPath
      });
      legacyWalletDBPath = path.join(stateDir, 'migration-data/wallet-db-acid');
    ...
    if (code === 'EBUSY') {
      ...
      const {
        response
      } = await showExportWalletsWarning(mainWindow, locale);

      if (response === 0) {
        // User confirmed migration retry
        logger.info('ipcMain: User confirmed wallet migration retry');
        resolve(prepareMigrationData(mainWindow, stateDir, legacySecretKey, legacyWalletDB, locale));
      } else {
...
const showExportWalletsWarning = (mainWindow: BrowserWindow, locale: string): Promise<{
  response: number;
}> => {
  ...
  const exportWalletsDialogOptions = {
    buttons: [translation('exportWalletsWarning.confirm'), translation('exportWalletsWarning.cancel')],
    type: 'warning',
