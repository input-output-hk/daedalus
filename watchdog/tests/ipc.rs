...
impl Watchdog {
    /// Drain events until one with `event == name` arrives; panic after timeout.
...
fn lifecycle_stop() {
    ...
    wdog.expect("watchdog_started");
    wdog.expect("chain_status");

    let ev = wdog.expect("node_started");
    assert!(ev["pid"].as_u64().unwrap() > 0);
    assert!(ev["started_at_unix_ms"].as_u64().unwrap() > 0);

    let ev = wdog.expect("wallet_started");
    assert!(ev["pid"].as_u64().unwrap() > 0);
    assert!(ev["started_at_unix_ms"].as_u64().unwrap() > 0);

    let ev = wdog.expect("wallet_ready");
    ...
    wdog.wait();
    // dir is cleaned up by TempDir Drop
}
...
fn wallet_restarts_on_crash() {
    ...
    wdog.expect("wallet_exited");

    let ev = wdog.expect("wallet_restarting");
