...
check_clean_worktree() {
  ...
  if ! worktree_status=$(git status --short --untracked-files=all); then
    exit_with_summary 1 'Failed to read git working tree status.'
  fi

  section 'Git Worktree Check'

  if [[ -z "$worktree_status" ]]; then
    printf 'GIT_WORKTREE_CHECK=CONTINUE\n'
    success 'Working tree is clean.'
    return 0
  fi

  printf 'GIT_WORKTREE_CHECK=STOP\n'
  warn 'Working tree is not clean. Resolve or commit existing changes before running Ralph.'
  printf '%s\n' "$worktree_status"
  exit_with_summary 1 'Stopped before launching opencode because the git working tree is dirty.'
}
...
for ((iteration = 1; iteration <= max_iterations; iteration++)); do
  ...
  info "Launching: opencode run --model ${orchestrator_model} --variant ${orchestrator_variant} < ${prompt_file}"

  opencode_output_file=$(mktemp)

  set +e
  opencode run --model "$orchestrator_model" --variant "$orchestrator_variant" < "$prompt_file" 2>&1 | tee "$opencode_output_file"
  ...
  if [[ -n "$stop_reason" ]]; then
    resume_hint="opencode --continue --model ${orchestrator_model} --variant ${orchestrator_variant}"
